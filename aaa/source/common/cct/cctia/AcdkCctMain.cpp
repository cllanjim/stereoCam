...
*
********************************************************************************/
MINT32 AcdkCctMain::takePicture(
                MUINT32 const mode,
                MUINT32 const imgType,
                Func_CB const capCb,
                MUINT32 const width,
                ...
                MINT32  const isSaveImg)
{
    ...
    SensorParam rSensorParam(mSensorIndex,        // device ID
                             ...
                             rawType == 0 ? ECamShot_RAWTYPE_PURE : ECamShot_RAWTYPE_PREPROC);          // RAW type : 0-pure raw, 1-processed raw
    ...
    for(MUINT32 i = 0; i < captureLoop; ++i)
    {
        ...
        mGetShutTime = m_pAcdkCctMhalObj->acdkCctMhalGetShutTime();
        ...
        if(mOperaMode == ACDK_OPT_FACTORY_MODE && mSetShutTime != 0)
        {
            ...
            m_pAcdkCctMhalObj->acdkCctMhalSetShutTime(mSetShutTime);
 ...
 *
 ******************************************************************************/
MBOOL AcdkCctMain::camShotDataCB(MVOID* user, CamShotDataInfo const msg)
{
    ACDK_LOGD("msg.msgType(%u)",msg.msgType);

    AcdkCctMain * const _This = reinterpret_cast<AcdkCctMain *>(user);
    ...
    {
        ACDK_LOGE("NULL AcdkCctMain pointer");
        return MFALSE;
    }
