...
public:
    ...
    {
        MY_LOGD_IF(1 ,PROC_TAG("received event[%d] arg1[%d] arg2[%d] arg3[%d]"),
...
class SwnrProcedure
{
    ...
    public:
        static sp<Processor>            createProcessor(InitParams& params);
                                        ...
                                        : mbEnableLog(params.enableLog)
                                        {
                                            mpSwnr = MAKE_SwNR(params.openId);
...
config(ConfigParams const& rParams)
{
    ...
    //
    {
        ...
        mEnableVencStream       = rParams.enableVencStream;
        mFps                    = rParams.fps;
#if FORCE_VENC_STREAM_ON
        mEnableVencStream       = MTRUE;
        mFps                    = 30;
#endif
#if 0
        // HAL1 cannot get buffer size at config phase
        if (mEnableVencStream) {
            for (size_t i = 0; i < rParams.vOutImage.size(); i++) {
                if (rParams.vOutImage[i]->getUsageForAllocator() &
                    ...
                    MY_LOGD("start the direct link of video encorder. fps:%d, width:%d, hight:%d",
                        rParams.fps, size.w, size.h);
                    mpP2Processor->notify(P2Procedure::P2_START_VENC_STREAM, rParams.fps, size.w, size.h);
                    break;
                }
            }
        }
...
)
{
    ...
    for( size_t run_idx = 0 ; run_idx < imageIOMapSet.size(); run_idx++ )
    {
        ...
        {
            MY_LOGD("get input buffer failed");
            return BAD_VALUE;
        }
        //
        {
...
#if FORCE_BURST_ON
        pRequest->context.burst_num = pRequest->context.resized ? 2 : 0;
#else
        ...
        for( size_t i = 0; i < imageIOMap.vOut.size(); i++ )
        {
            ...
            if (mEnableVencStream && !mInitVencStream) {
                if (pBufferHandle.get() && imageIOMap.vOut[i]->getUsageForAllocator() &
                    GRALLOC_USAGE_HW_VIDEO_ENCODER) {

                    if (OK != pBufferHandle->waitState(BufferHandle::STATE_READABLE))
                        MY_LOGW("get video buffer failed");

                    if (pBufferHandle->getBuffer()) {
                        MSize size = pBufferHandle->getBuffer()->getImgSize();
                        MY_LOGD("start the direct link of video encorder. fps:%d, width:%d, hight:%d",
                            mFps, size.w, size.h);
                        mpP2Processor->notify(P2Procedure::P2_START_VENC_STREAM, mFps, size.w, size.h);
    ...
    if( mpPipe ) {
        if (mEnableVencStream) {
            if (mpPipe->sendCommand(ESDCmd_RELEASE_VENC_DIRLK)) {
                MY_LOGE("release venc stream failed");
            }
        }
    ...
    MINTPTR const arg3)
{
    switch (event)
    {
        case P2_START_VENC_STREAM:
            if (mpPipe->sendCommand(ESDCmd_CONFIG_VENC_DIRLK,
                    arg1, arg2, arg3))
                mEnableVencStream = MTRUE;
            else
                MY_LOGE("Config venc stream failed");
            break;
    }
    return;
}
