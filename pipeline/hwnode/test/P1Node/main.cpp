...
namespace {
                   ...
                   , public virtual BaseNode
    {
        ...
        public:     ////                    Operations.
             ...
             *
             */
            virtual MERROR                  init(InitParams const& rParams) { return OK; }
    ...
    class ImitationPipelineNodeCallback
        : public IPipelineNodeCallback
    {
        public:     ////                    Operations.
            virtual MVOID onDispatchFrame(android::sp<IPipelineFrame> const& pFrame,
                            IPipelineNode::NodeId_T nodeId)
                            {
                                ...
                                if  ( ! err && ! nextNodes.empty() )
                                {
                                    for (size_t i = 0; i < nextNodes.size(); i++) {
                                        MY_LOGD("NextNode:%d - FrameNo:%d", i, pFrame->getFrameNo());
    ...
    //
    android::sp<ImitationPipelineNodeCallback> mpImitationPipelineNodeCallback = NULL;
    //
...
void clear_global_var()
{
    ...
    mpvHalImageRaw.clear();
    mpHalImageResizeRaw = NULL;
...
void createFullImageStream(MSize size)
{
    ...
    MUINT32 pixelMode = ONE_PIXEL_MODE;
    NSIoPipe::NSCamIOPipe::NormalPipe_QueryInfo queryRst;
    NSIoPipe::NSCamIOPipe::NormalPipe_QueryIn input;
    input.width = size.w;
    input.pixMode = (pixelMode == 0)? (NSIoPipe::NSCamIOPipe::_1_PIX_MODE) : NSIoPipe::NSCamIOPipe::_2_PIX_MODE;
    NSIoPipe::NSCamIOPipe::INormalPipe::query(
            NSIoPipe::PORT_IMGO.index,
            NSIoPipe::NSCamIOPipe::ENPipeQueryCmd_X_PIX|
            NSIoPipe::NSCamIOPipe::ENPipeQueryCmd_STRIDE_PIX|
            NSIoPipe::NSCamIOPipe::ENPipeQueryCmd_STRIDE_BYTE,
            (EImageFormat)format,
            input,
            queryRst);

    bufPlane.rowStrideInBytes = queryRst.stride_byte;
    bufPlane.sizeInBytes = bufPlane.rowStrideInBytes * size.h ;
    MY_LOGI("Stream Full Size query (%dx%d) stride[%d] size[%d]", size.w, size.h, bufPlane.rowStrideInBytes, bufPlane.sizeInBytes);
...
{
    MINT const format = eImgFmt_FG_BAYER10;
    MUINT const usage = eBUFFER_USAGE_SW_READ_OFTEN |
                        eBUFFER_USAGE_HW_CAMERA_READWRITE
                        ;
    IImageStreamInfo::BufPlanes_t bufPlanes;
    ...
    bufPlane.rowStrideInBytes = queryRst.stride_byte;
    bufPlane.sizeInBytes = bufPlane.rowStrideInBytes * size.h ;
    MY_LOGI("Stream Resizer Size query (%dx%d) stride[%d] size[%d]", size.w, size.h, bufPlane.rowStrideInBytes, bufPlane.sizeInBytes);
...
void prepareConfig(MUINT sensor_mode)
{
    ...
    if (test_resize) {
        ...
        createResizerImageStream(MSize(VAL_ALIGN((mSensorParams.size.w*7/10), 8), mSensorParams.size.h));
    }
...
int main(int argc, char** argv)
{
    ...
    #if 0//SUPPORT_3A//[EP_TEMP]
    IHal3A* mp3A = MAKE_Hal3A(gSensorId, "haha");
    #endif
    ...
    //
    mpImitationPipelineNodeCallback = new ImitationPipelineNodeCallback();
    ...
    //config
    {
       ...
       mpNode1->setCallback(mpImitationPipelineNodeCallback);
    ...
    /* FIXME: for sleep before flush, need to remove it */
    printf("after preview running, sleep before flush...\n");
    usleep(100000);
    ...
    /* FIXME: for sleep before flush, need to remove it */
    printf("after capture running, sleep before flush...\n");
    usleep(100000);
