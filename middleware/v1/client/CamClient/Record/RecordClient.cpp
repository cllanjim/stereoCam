...
startPreview()
{
    ...
    {
        MY_LOGW("Preview has been started");
        ret = true;
        goto lbExit;
    }
    //
    ...
    //
    mpParamsMgr->set(MtkCameraParameters::KEY_VR_BUFFER_COUNT, mRecBufNum);
    //
    if(miIonDevFd < 0)
    {
        MY_LOGD("mt_ion_open");
        miIonDevFd = mt_ion_open(LOG_TAG);
        if(miIonDevFd < 0)
        {
            MY_LOGE("ion_open fail");
            goto lbExit;
        }
    }
    //
...
stopPreview()
{
    ...
    if(::android_atomic_release_load(&mIsPrvStarted) == 0)
    {
        MY_LOGW("Preview has been stopped");
        ret = true;
        goto lbExit;
    }
    //
...
startRecording()
{
    ...
    mLastTimeStamp = 0;
    //
    if(mpParamsMgr->getVideoStabilization() && (mpParamsMgr->getInt(MtkCameraParameters::KEY_EIS25_MODE) == 1))
    {
        if ((mi4RecWidth >= 3840) && (mi4RecHeight >= 2160))
        {
            mRecBufNum = 8 + (mpParamsMgr->getInt(MtkCameraParameters::KEY_EIS_SUPPORTED_FRAMES));
        }else
        {
            mRecBufNum = 10 + (mpParamsMgr->getInt(MtkCameraParameters::KEY_EIS_SUPPORTED_FRAMES));
        }
        MY_LOGD("+ record: WxH=%dx%d, EIS VR: %d", mi4RecWidth, mi4RecHeight, mRecBufNum);
    }
    //
    ret = onStateChanged();
    //
...
stopRecording()
{
    ...
    //
    while(::android_atomic_release_load(&mIsWaitBufBack) == 1)
    {
        if(count >= mRecBufNum)
        {
            MY_LOGD("Force to stop due to timeout");
            break;
        }
        count++;
        MY_LOGD("Wait for cancel wait buffer back count(%d)",count);
        usleep(30*1000);
    }
    //
    ret = onStateChanged();
...
sendCommand(int32_t cmd, int32_t arg1, int32_t arg2)
{
    ...
    switch(cmd)
    {
        ...
        default:
        {
            MY_LOGD("No implement cmd(%d)",cmd);
            break;
        }
    }
    #endif
