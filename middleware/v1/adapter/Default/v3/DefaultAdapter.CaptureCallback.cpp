...
)
{
    ...
    uint32_t  u4PostviewSize_plane = 0;
    for ( size_t i = 0; i < pPostviewBuf->getPlaneCount(); ++i ) {
...
)
{
    ...
    if  ( mIsRaw16CBEnable )//CAMERA_CMD_ENABLE_RAW16_CALLBACK
    {
        ...
        MyDngopImgInfo.BIT_NUM = getRawBitDepth();//rSensorParam.u4Bitdepth , buffer bit num
        ...
        //debug dump raw buffer
        {
            ...
            property_get("debug.dngraw.dump", value, "0");
...
)
{
    ...
    if  ( mIsRaw16CBEnable )//CAMERA_CMD_ENABLE_RAW16_CALLBACK
    {
        ...
        //static
            ...
            entryCount += 2;
            dataCount += ::calculate_camera_metadata_entry_data_size(TYPE_BYTE, 2);//
...
)
{
    ...
    if  ( msgTypeEnabled(CAMERA_MSG_RAW_IMAGE_NOTIFY) )
    {
        ...
        doNotifyCallback(CAMERA_MSG_RAW_IMAGE_NOTIFY, 0, 0);
        return  true;
    }
    //
...
)
{
    ...
    {
        image = mpCamMsgCbInfo->mRequestMemory(-1, u4BitstreamSize, 1, NULL);
        if  ( image )
        {
            pImage = reinterpret_cast<uint8_t*>(image->data);
        }
