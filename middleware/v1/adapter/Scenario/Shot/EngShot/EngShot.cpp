...
//
#include <mtkcam/utils/exif/IDbgInfoContainer.h>
...
onCmd_capture()
{
    ...
    {
        pSingleShot = NSCamShot::ISingleShot::createInstance(static_cast<EShotMode>(mu4ShotMode), "EngShot", ePipelineMode_Feature);
        CAM_LOGD("ISingleShot::createInstance - ePipelineMode_Feature");
    }
    else
    {
    ...
    updatePostNRSetting(nrtype, isMfll, &mShotParam.mHalSetting);
    //
...
)
{
    ...
    if  ( (u4RawImgSize > 0) && (puRawImgBuf != 0) && (s8RawFilePath != ""))
    {
        ...
        if (mpEngParam.mi4CamShotType == EngParam::ENG_CAMSHOT_TYPE_MFLL)
        {
            ...
            sprintf(mpszSuffix, "__%dx%d_%d_%d-%02d", mpEngParam.u4SensorWidth, mpEngParam.u4SensorHeight, mpEngParam.u4Bitdepth, mpEngParam.u4RawPixelID, mpEngParam.mi4MFLLnum);
        }
        else
        {
