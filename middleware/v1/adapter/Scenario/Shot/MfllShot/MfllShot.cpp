...
{
    // AeMgr should fix bug of manual flare offset, or says that AeMgr needs
    // to provide a manual mode AE + offset method for users, or enalbe flare
    // offset in manual mode will cause some information wrong.
#if AEMGR_BUGFIX_FLARE_OFFSET
    ...
    return std::move(hal3a_ask_manual_flare_offset);
#else
    return NULL;
#endif
}
...
{
    mfllLogD("streamInfo(%#08x) name(%s) format(%#08x) size(%dx%d) min/max(%zu/%zu)",
    ...
    , mSequence(0)
{
    ...
    // debug property setting
    {
        ...
        // log level
        property_get("mediatek.mfll.log", value, "0");
        ...
        IMetadata  const result)
{
    ...
    switch (streamId)
    {
        ...
        default:
            mfllLogW("unsupported streamID(%#x)", streamId);
    }
}
    ...
    switch (streamId)
    {
        ...
        case eSTREAMID_IMAGE_PIPE_YUV_00:
            ...
            if (pBuffer.get() && (errorBuffer == MFALSE))
            {
                ...
                int dumpPostview = property_get_int32("mediatek.mfll.dump.postview", -1);
                if (dumpPostview > 0)
                {
                    char filename[64];
...
bool MfllShot::zsdCapture()
{
        ...
        do {
            ...
            job_update_jpeg_orientation = [](IMetadata *ap, IMetadata *t) -> void {
                ...
                if (requestOrientation != targetOrientation) {
                    updateEntry<MINT32>(t, TAG, requestOrientation);
                }
            };
            //}}}
    ...
    { ///{{{
        ...
        for (size_t i = 0; i < vCaptureRequest.size(); i++)
        {
            ...
            if (err)
            {
                mfllLogW("get selected result failed idx(%d), failed times(%d), " \
            ...
            {
                mfllLogE("continuous failed %d times, don't wait anymore", BREAK_TIMES);
        ...
        {
            mfllLogW("send dummy request %d", i);
            ...
            updateEntry<MUINT8>(&halSetting, MTK_HAL_REQUEST_ERROR_FRAME, 1);
...
status_t MfllShot::createPipeline(const sp<StreamBufferProvider>& provider)
{
    ...
    {
        mfllLogE("shot mode is not MfllShot, it's %#d", shotMode);
        return BAD_VALUE;
    }
        ...
        IMetadata& halSetting)
{
    ...
    {
        mfllLogE("Selector get input raw buffer failed! bufferSetSize(%d)", rvBufferSet.size());
        return UNKNOWN_ERROR;
    }
    ...
    for (size_t i = 0; i < rvBufferSet.size(); i++)
    {
        ...
        if (buffer == NULL)
        {
            mfllLogE("result buffer %d is NULL", i);
            return BAD_VALUE;
        }
        ...
        switch (rvBufferSet[i].id)
        {
            ...
            case eSTREAMID_IMAGE_PIPE_RAW_LCSO:
                ...
                break;
            default:
                mfllLogW("unsupported image buffer got, id = 0x%x", rvBufferSet[i].id);
        }
    }
