...
)
{
#if SUPPORT_3A
    {
        ...
        if (mpParamsManagerV3 != NULL)
        {
            ...
            //
            mpParamsManagerV3->getParamsMgr()->set(MtkCameraParameters::KEY_ENG_EV_CALBRATION_OFFSET_VALUE, iAECurrentEV + AENVRAM.rCCTConfig.i4BVOffset);
...
setParameters()
{
    ...
    if(::strcmp(mpParamsManagerV3->getParamsMgr()->getStr(CameraParameters::KEY_FOCUS_MODE), "fullscan") == 0)
    {
...
)
{
    ...
    if (mPipelineMode == PipelineMode_PREVIEW && mLPBConfigParams.enableEIS &&
        (u4VHDmode == SENSOR_VHDR_MODE_NONE))
    {
