...
protected:  ////                    Data Members.
    ...
    //
    android::Vector<NS3Av3::CaptureParam_T>
                                    mvHdrCaptureParams;
...
submitRequest(AppRequest& request)
{
    ...
    const size_t captureFrames = mvHdrCaptureParams.size();

    // the number of delayed frames
    const size_t delayedFrames = vOrigCaptureParams.size();
    ...
    if ( MTRUE == mPass1Resource.getReuseFlag() )
    {
        ...
        mvHdrCaptureParams.appendVector(vOrigCaptureParams);
    }
    //
    MERROR err = OK;
    for (size_t frm_idx = 0; frm_idx < mvHdrCaptureParams.size(); frm_idx++ ) {
        //
        ...
        //
        MY_LOGD("hdr req:%d frm:%d", mvHdrCaptureParams.size(), frm_idx);
        ...
        if (frm_idx==0)
        {
            ...
            for ( size_t i = 0; i < evaluateResult.vUpdatedImageInfos.size(); i++ )
            {
                ...
                const sp<IImageStreamInfo>& streamInfo(
                        evaluateResult.vUpdatedImageInfos.valueAt(i));
                MY_LOGD("replace streaminfo id(%#" PRIxPTR ") name(%s)",
            ...
            {
                MY_LOGD("hdrnode output streamid: 0x%x", request.vOImageBuffers.keyAt(i));
            ...
            for ( size_t i=0; i<request.vOImageBuffers.size(); i++)
            {
                MY_LOGD("jpegnode output streamid: 0x%x", request.vOImageBuffers.keyAt(i));
                switch( request.vOImageBuffers[i]->getStreamInfo()->getImgFormat() )
                {
                    case eImgFmt_BLOB:
...
#undef setStreamBuffers
        ...
        //
        MY_LOGD("construct hdr req:%d frm:%d", mvHdrCaptureParams.size(), frm_idx);
        //
        sp<IPipelineFrame> pFrame = builder
                .updateFrameCallback((frm_idx==0)? this : NULL)
                .build(request.requestNo, mpPipelineContext);
        MY_LOGD("construct hdr done req:%d frm:%d", mvHdrCaptureParams.size(), frm_idx);
        if( ! pFrame.get() ) {
    ...
    //
    {
        ...
        if( pBuf.get() )
        {
            ...
            // update AE capture parameters
            {
                ...
                memcpy(capParams.editArray(), &mvHdrCaptureParams[frameIndex],
