...
#if defined(HAVE_COMMAND_test_dac)
//
 ...
 *
 ******************************************************************************/
namespace NSCmd_test_dac {
char const
gPromptText[] = {
    "\n"
    "\n test_dac <action> <optional arguments...>"
    "\n "
    "\n where <action> may be one of the following:"
    "\n  <-h>                       help"
    "\n  <AM>                 auto moving dac"
    "\n  <MM>               manually set dac"
    "\n  <SD>               manually set dac value"
    "\n  <stop>                     stop test"
    "\n "
};
...
protected:  ////                Implementation.
    ...
    virtual bool                onAutoMove(sp<Argument> pArgument);
    virtual bool                onManualMove(sp<Argument> pArgument);
    virtual bool                onSetDac(sp<Argument> pArgument);
    virtual bool                onStop();
 ...
 *
 ******************************************************************************/
bool CmdImp::isInstantiate = CmdMap::inst().addCommand(HAVE_COMMAND_test_dac, new CmdImp(HAVE_COMMAND_test_dac));
};  // NSCmd_test_dac
using namespace NSCmd_test_dac;
...
onParseArgumentCommand(Vector<String8>& rvCmd, sp<Argument> pArgument)
{
    ...
    pArgument->mOpenId = 0;
    pArgument->mPreviewSize = Size(1920, 1080);
...
//        MY_LOGD("<key/val>=<%s/%s>", key.string(), val.string());
        ...
        if  ( key == "-value") {
            pArgument->mDacValue= ::atoi(val);
            MY_LOGD("dac: %d", pArgument->mDacValue);
        }
        //
        if  ( key == "-awb") {
...
onParseActionCommand(Vector<String8>& rvCmd, sp<Argument> pArgument)
{
    ...
    for (size_t i = 1; i < rvCmd.size(); i++)
    {
        ...
        //
        if  ( s8Cmd == "AM" ) {
            return  onAutoMove(pArgument);
        }
        //
        if  ( s8Cmd == "MM" ) {
            return  onManualMove(pArgument);
        }
        if  ( s8Cmd == "SD" ) {
            return  onSetDac(pArgument);
        }
        if  ( s8Cmd == "stop" ) {
            return  onStop();
        }
...
bool
CmdImp::
onAutoMove(sp<Argument> pArgument)
{
    ...
    isAuto = true;
    return  (OK == run("CmdImp::onAutoMove"));
}
...
bool
CmdImp::
onManualMove(sp<Argument> pArgument)
{
    if  ( 1 == ::android_atomic_release_load(&mThraedRunning) ) {
        MY_LOGW("the test has been starting before...skip this command.");
        return  false;
    ...
    mpArgument = pArgument;
    isAuto = false;
    return  (OK == run("CmdImp::onManualMove"));
}
...
*******************************************************************************/
bool
CmdImp::onSetDac(sp<Argument> pArgument)
{
    if (mpCamera != 0)
    {
        ...
        //
        if (mpArgument->mDacValue != pArgument->mDacValue)
        {
            params.set(MtkCameraParameters::KEY_FOCUS_ENG_STEP, pArgument->mDacValue);
            CAM_LOGD("onUpdateParameter: DacValue(%d)", pArgument->mDacValue);
        }
...
threadLoop()
{
    ...
    mpCamera->startPreview();
    {
        ...
        {
            nsecs_t nsTimeoutToWait = 5LL*1000LL*1000LL*1000LL;//wait 5 sec.
        if(isAuto == false)
            nsTimeoutToWait = 30LL*1000LL*1000LL*1000LL;//wait 30 sec.
...
setupParameters(sp<Camera> spCamera)
{
    ...
    //
    params.set(CameraParameters::KEY_FOCUS_MODE, "manual");
    if(isAuto) {
        static int i = 0;
        params.set(MtkCameraParameters::KEY_FOCUS_ENG_STEP, i);
        if(i < 1025)
            i= i+10;
    }
