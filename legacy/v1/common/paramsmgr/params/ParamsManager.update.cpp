...
updatePreviewFpsParams(NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    ...
    String8 s8FrameRateList("10,15,20");        // add 15 for video night mode, because sensor may can't support fixed 15fps
...
updateExposureCompensationParams(NSCameraFeature::FeatureInfo const& rFeatureInfo)
{
    ...
    MY_LOGD("camera mode(%d)",i4CamMode);
    if(i4CamMode <= 0 &&    //non-mtk camera
       NULL != mParameters.get(CameraParameters::KEY_EXPOSURE_COMPENSATION))
    {
...
updateDefaultParams1_ByDefault()
{
    ...
    //  Video Hdr
    mParameters.set(MtkCameraParameters::KEY_VIDEO_HDR, MtkCameraParameters::OFF);
...
evaluateHalAppModeUtility(String8& mode) const
{
    ...
    MY_LOGD("KEY_CAMERA_MODE:%d, HAL Force Mode = [%s]", i4CamMode, halForceMode);
