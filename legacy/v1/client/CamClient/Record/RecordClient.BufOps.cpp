...
{
    MY_LOGD("+");
    uint32_t u4ImgStrides[3];
    //
    bool ret = false;
    uint32_t recBufNum = mRecBufNum;
    //
    //  (1) Lock
    ...
    {
        u4ImgStrides[0]= getImgStrideUnit(0);
        u4ImgStrides[1]= getImgStrideUnit(1);
        u4ImgStrides[2]= getImgStrideUnit(2);
        mpImgBufMgr = RecBufManager::alloc(
                        mbMetaMode,
                        getImgImgFormat(),
                        0,
...
prepareAllTodoBuffers(sp<IImgBufQueue>const& rpBufQueue, sp<RecBufManager>const& rpBufMgr)
{
    ...
    for(i = 0; i < mvRecBufInfo.size(); i++)
    {
        if(mvRecBufInfo[i].Sta == REC_BUF_STA_EMPTY)
        {
            ...
            MY_LOGD("EQ:Idx(%d),Addr(%p/%p),CbVA(%p)",
                    i,
                    (MUINTPTR)(pCameraImgBuf->getVirAddr()),
                    (MUINTPTR)(pCameraImgBuf->getPhyAddr()),
...
performRecordCallback(int32_t bufIdx, sp<RecImgBuf>const& pCameraImgBuf, int32_t const msgType)
{
    ...
    if  ( pCameraImgBuf != 0 )
    {
        ...
        if(mbMetaMode)
        {
           ...
           //* |  buffer type  |          payload                           |
           //* --------------------------------------------------------------
           *((uint32_t*)pCameraImgBuf->get_camera_memory()->data) = kMetadataBufferTypeNativeHandleSource;
           *((buffer_handle_t*)((char*)pCameraImgBuf->get_camera_memory()->data + sizeof(uint32_t))) = pCameraImgBuf->getGrallocBufferHandle();
        }
        //
        MY_LOGD("CB:FC(%02d),Idx(%d),Addr(%p/%p),CbVA(0x%08X),GH(0x%08X),TS(%d.%06d)",
                mFrameCount,
                bufIdx,
                pCameraImgBuf->getVirAddr(),
                pCameraImgBuf->getPhyAddr(),
...
{
    bool bQuaryIonInfo = false, bFindTarget = false;
    uint32_t i,j,size;
    struct ion_sys_data sys_data;
    VideoNativeHandleMetadata *pMetadata = NULL;
    //
    ...
    if(mbMetaMode)
    {
        ...
        pMetadata = (VideoNativeHandleMetadata*)opaque;
        gralloc_extra_query(pMetadata->pHandle, GRALLOC_EXTRA_GET_ION_FD, &bufIonFd);
        //
        if(ion_import(miIonDevFd, bufIonFd, &IonHandle))
        {
            MY_LOGE("ion_import fail, bufIonFd(0x%x)", bufIonFd);
            goto close_handle;
        }
        //
        sys_data.sys_cmd = ION_SYS_GET_PHYS;
        sys_data.get_phys_param.handle = IonHandle;
        if (ion_custom_ioctl(miIonDevFd, ION_CMD_SYSTEM, &sys_data))
        {
            MY_LOGE("IOCTL[ION_IOC_CUSTOM] Get Phys failed!\n");
            goto close_handle;
        }
        //
        if(ion_free(miIonDevFd, IonHandle))
        {
        ...
        //
        close_handle:
        native_handle_close(pMetadata->pHandle);
        native_handle_delete(pMetadata->pHandle);
    }
    //
    for(i=0; i<size; i++)
    {
        ...
        {
            if( bQuaryIonInfo &&
                mvRecBufInfo[i].PhyAddr == (MINTPTR)(sys_data.get_phys_param.phy_addr) &&
                mvRecBufInfo[i].Sta  == REC_BUF_STA_CB)
            {
                if(mvRecBufInfo[i].CbVirAddr == opaque)
                {
                    ...
                    bFindTarget = true;
                    //
                    MY_LOGD("Same:Idx(%d),Addr(%p/%p)",
                            i,
                            mvRecBufInfo[i].CbVirAddr,
                            mvRecBufInfo[i].PhyAddr);
                }
                else
                {
                    for(j=0; j<size; j++)
                    {
                        if( i != j &&
                            mvRecBufInfo[j].CbVirAddr == opaque &&
                            mvRecBufInfo[j].Sta  == REC_BUF_STA_CB)
                        {
                            REC_BUF_INFO_STRUCT recBufInfo;
                            sp<GraphicBuffer> spGraphicBuffer = NULL;
                            //
                            spGraphicBuffer                     = mpImgBufMgr->getBuf(j)->getGrallocBuffer();
                            recBufInfo.BufIonFd                 = mpImgBufMgr->getBuf(j)->getIonFd();
                            recBufInfo.PhyAddr                  = (MINTPTR)(mpImgBufMgr->getBuf(j)->getPhyAddr());
                            //
                            mpImgBufMgr->getBuf(j)->setGrallocBuffer(mpImgBufMgr->getBuf(i)->getGrallocBuffer());
                            mpImgBufMgr->getBuf(j)->setIonFd(mpImgBufMgr->getBuf(i)->getIonFd());
                            mpImgBufMgr->getBuf(j)->setPhyAddr(mpImgBufMgr->getBuf(i)->getPhyAddr());
                            mvRecBufInfo.editItemAt(j).BufIonFd = mpImgBufMgr->getBuf(i)->getIonFd();
                            mvRecBufInfo.editItemAt(j).PhyAddr  = (MINTPTR)(mpImgBufMgr->getBuf(i)->getPhyAddr());
                            mvRecBufInfo.editItemAt(j).Sta      = REC_BUF_STA_EMPTY;
                            //
                            mpImgBufMgr->getBuf(i)->setGrallocBuffer(spGraphicBuffer);
                            mpImgBufMgr->getBuf(i)->setIonFd(recBufInfo.BufIonFd);
                            mpImgBufMgr->getBuf(i)->setPhyAddr((void*)(recBufInfo.PhyAddr));
                            mvRecBufInfo.editItemAt(i).BufIonFd = recBufInfo.BufIonFd;
                            mvRecBufInfo.editItemAt(i).PhyAddr  = recBufInfo.PhyAddr;
                            //
                            bFindTarget = true;
                            //
                            MY_LOGD("Replace:Idx(%d -> %d),VA(%p),PA(%p -> %p)",
                                    i,
                                    j,
                                    mvRecBufInfo[j].CbVirAddr,
                                    mvRecBufInfo[i].PhyAddr,
                                    mvRecBufInfo[j].PhyAddr);
                            //
                            break;
                        }
                    }
    ...
    if(!bFindTarget)
    {
        if(mbMetaMode)
        {
            if(pMetadata != NULL)
            {
                MY_LOGE("Can't find PA(%p)",sys_data.get_phys_param.phy_addr);
                for(i=0; i<size; i++)
                {
                    MY_LOGE("Idx(%d),Addr(%p/%p)",
                            i,
                            mvRecBufInfo[i].CbVirAddr,
                            mvRecBufInfo[i].PhyAddr);
                }
        ...
        {
            MY_LOGE("Can't find VA(%p)",opaque);
        }
    }
}
