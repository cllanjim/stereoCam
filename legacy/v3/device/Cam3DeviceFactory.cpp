...
#include <mtkcam/device/IExternalDevice.h>
//
...
)
{
    ...
    //
    {
        String8 const s8CamDeviceInstFactory = String8::format("createCam3Device_External");
        void* pCreateInstance = ::dlsym(handle, s8CamDeviceInstFactory.string());
        MY_LOGF_IF(0==pCreateInstance, "Not exist: %s for %s", s8CamDeviceInstFactory.string(), s8ClientAppMode.string());
        pdev = reinterpret_cast<NSCam::Cam3Device* (*)(String8 const&, int32_t const, NSCam::IExternalDevice*, NSCam::CameraInfo*)>
                    (pCreateInstance)(s8ClientAppMode, i4OpenId, aExternal, aInfo)
                    ;
    }
    //
    MY_LOGI("- %p", pdev);
lbExit:
    ...
    return  pdev;
#else
    MY_LOGE("Not support external device.");
    return NULL;
#endif
}
