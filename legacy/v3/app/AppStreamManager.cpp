...
#define LOG_TAG "MtkCam/AppStreamManager"
//
#include <mmutils/IGrallocHelper.h>
#include <mtkcam/metadata/mtk_metadata_types.h>
#include <mtkcam/metadata/client/TagMap.h>
#include "MyUtils.h"
#include "AppStreamManager.h"
...
)
{
    ...
    ::property_get("debug.camera.AppstreamMgr.meta", cLogLevel, "1");
    if(strcmp(cLogLevel, "0") == 0) {
        return new AppStreamManager(openId, callback_ops, pMetadataProvider);
    } else {
        return new AppStreamMgr(openId, callback_ops, pMetadataProvider);
    }
}
...
)
{
    ...
    //
    {
        status_t status = run("AppStreamManager::configureStreams");
        if  ( OK != status ) {
 ...
 *
 ******************************************************************************/
AppStreamManager::AppImageStreamInfo*
AppStreamManager::
 ...
 *
 ******************************************************************************/
AppStreamManager::AppMetaStreamInfo*
AppStreamManager::
    ...
    //  vInputMetaBuffers <- camera3_capture_request_t::settings
    {
        ...
        if (!isRepeating) {
            ...
            if (OK == find_camera_metadata_ro_entry(request->settings,
                ...
                MY_LOGD_IF(
                    mLogLevel >= 1,
                    "frameNo:%u, AF_state: %d",
                    request->frame_number, *e1.data.u8
                );
                CAM_TRACE_END();
            }
            ...
            if (OK == find_camera_metadata_ro_entry(request->settings,
                ...
                MY_LOGD_IF(
                    mLogLevel >= 1,
                    "frameNo:%u, ae precapture trigger: %d", request->frame_number, *e2.data.u8
                );
                CAM_TRACE_END();
            }
 ...
 *
 ******************************************************************************/
AppStreamManager::AppImageStreamBuffer*
AppStreamManager::
 ...
 *
 ******************************************************************************/
AppStreamManager::AppMetaStreamBuffer*
AppStreamManager::
...
)
{
    ...
    //
    ssize_t const index = mResultQueue.indexOf(frameNo);
...
performCallback(CallbackParcel const& cbParcel)
{
    ...
    if  ( ! cbParcel.vOutputImageItem.isEmpty() || ! cbParcel.vInputImageItem.empty() )
    {
        ...
        for (size_t i = 0; i < cbParcel.vOutputImageItem.size(); i++) {
                ...
                )
            {
                ...
                if  ( pImageBufferHeap->lockBuf(LOG_TAG, GRALLOC_USAGE_SW_WRITE_OFTEN|GRALLOC_USAGE_SW_READ_OFTEN) ) {
                    ...
                    if ( ! pImageBufferHeap->unlockBuf(LOG_TAG) )
                        MY_LOGE("Fail to unlock jpeg");
                    else
        ...
        camera3_capture_result const result =
        {
            frame_number        : frame_number,
            result              : NULL,
            num_output_buffers  : static_cast<uint32_t>(vOutBuffers.size()),
