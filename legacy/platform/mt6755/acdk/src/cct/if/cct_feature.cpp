...
)
{
    ...
    case ACDK_CCT_OP_AWB_SAVE_FLASH_AWB_PARA:
        ...
        break;
    case ACDK_CCTIA_OP_AWB_GET_PARA:
        err = NS3A::IAwbMgr::getInstance().CCTOPAWBGetAWBParam(mSensorDev, (AWB_PARAM_T*)puParaOut);
        break;
    ...
    case ACDK_CCT_OP_STROBE_WRITE_NVRAM:
        ...
        ret = NS3A::FlashMgr::getInstance().cctWriteNvram(mSensorDev);
        if(ret==1)
            err=0;
        else
            err=-1;
    ...
    case ACDK_CCT_OP_STROBE_SET_NVDATA_META:
        ...
        ret = NS3A::FlashMgr::getInstance().cctSetNvdataMeta(mSensorDev, puParaIn, u4ParaInLen);
        if(ret==1)
            err=0;
        else
            err=-1;
    ...
    case ACDK_CCT_OP_STROBE_READ_NVRAM_TO_PC_META:
        ...
        ret = NS3A::FlashMgr::getInstance().cctReadNvramToPcMeta(mSensorDev, puParaOut, pu4RealParaOutLen);
        if(ret==1)
            err=0;
        else
            err=-1;
    ...
    case ACDK_CCT_OP_FLASH_CONTROL:
        ...
        ret = NS3A::FlashMgr::getInstance().cctFlashLightTest(mSensorDev,(VOID *)puParaIn);
        if(ret==1)
            err=0;
        else
            err=-1;
    ...
    case ACDK_CCT_OP_FLASH_ENABLE:
        ...
        ret = NS3A::FlashMgr::getInstance().cctFlashEnable(mSensorDev, 1); //YosenFlash
        if(ret==1)
            err=0;
        else
            err=-1;
    ...
    case ACDK_CCT_OP_FLASH_DISABLE:
        ...
        ret = NS3A::FlashMgr::getInstance().cctFlashEnable(mSensorDev, 0); //YosenFlash
        if(ret==1)
            err=0;
        else
            err=-1;
    ...
    case ACDK_CCT_OP_FLASH_GET_INFO:
        ...
        ret = NS3A::FlashMgr::getInstance().cctGetFlashInfo(mSensorDev, (int*)puParaOut);
        if(ret==1)
            err=0;
        else
            err=-1;
    ...
    case ACDK_CCT_OP_FLASH_SET_MANUAL_FLASH:
        ...
        ret = NS3A::FlashMgr::getInstance().setManualFlash(mSensorDev, i32In[0], i32In[1]);
        if(ret==1)
            err=0;
        else
            err=-1;
    ...
    case ACDK_CCT_OP_FLASH_CLEAR_MANUAL_FLASH:
        ...
        ret = NS3A::FlashMgr::getInstance().clearManualFlash(mSensorDev);
        if(ret==1)
            err=0;
        else
            err=-1;
        ...
        MY_LOG("ACDK_CCT_OP_FLASH_ENABLE line=%d\n",__LINE__);
        ret = NS3A::FlashMgr::getInstance().cctFlashEnable(mSensorDev, 1);
        if(ret==1)
            err=0;
        else
            err=-1;
...
#define DO_CCT_CTRL(ctl_cocde)  \
    ...
    DO_CCT_CTRL( ACDK_CCT_V2_OP_GET_ISP_CCM_RATIO );
    DO_CCT_CTRL( ACDK_CCT_V2_OP_SET_ISP_CCM_RATIO );
#endif
