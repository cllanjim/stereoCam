    ...
    ,mFBBufferIndex(0)
{
    ...
    memset((void*)mLayerInfo, 0, sizeof(mLayerInfo));
    memset((void*)&mFinfo, 0, sizeof(mFinfo));
...
MINT32 surfaceView::getBootMode(MUINT32 &mode)
{
    ...
    char bootMode[4];
    MINT32 nread = 0;
    nread = ::read(fd, (MVOID*)bootMode, 4);
    if(nread == 0)
    {
...
MINT32 surfaceView::init()
{
    ...
    if(g_overlayFlag == 0)
    {
        ...
        */
        ...
        //for( int i = 0; i < VIDOE_LAYER_NO; i++ )
        for( int i = 0; i < 4; i++ ) // the maxium of Video layer id is 4 on this platform
        {
            mLayerInfo[0].layer_id = i; //just for disable usage. mLayerInfo[0] disable layer0 and mLayerInfo[1] disable layer1

            if (ioctl(mFBfd, MTKFB_SET_VIDEO_LAYERS, &mLayerInfo[0]) < 0)
            {
                ACDK_LOGE("MTKFB_SET_VIDEO_LAYERS failed: id=%d", i);
                return ACDK_RETURN_API_FAIL;
            }
        }
...
MINT32 surfaceView::uninit()
{
    ...
    if (mFBfd > 0 )
    {
        ...
        {
            ACDK_LOGE("MTKFB_SET_OVERLAY_LAYER failed");
            return ACDK_RETURN_API_FAIL;
        }
        ...
        MUINT32 const vFlip)
{
    ...
    if (bltParam.srcFormat == ACDK_SURFACE_FORMAT_ABGR_8888)
    {
        ...
        if (NULL == g_fp)
        {
            ACDK_LOGE("[setOverlayBuf] Can not open file /sdcard/prv.raw");
        }
...
MINT32 surfaceView::refresh()
{
    ...
    if(g_overlayFlag == 0)
    {
        ...
        ACDK_LOGD_DYN(g_overlayDebug,"src VA(0x%x), PA(0x%x)",(MUINTPTR)mLayerInfo[0].src_base_addr,(MUINTPTR)mLayerInfo[0].src_phy_addr );

        #if 0
        mLayerInfo[1].layer_id = 0; //just for disable usage. mLayerInfo[0] disable layer0 and mLayerInfo[1] disable layer1

        if (ioctl(mFBfd, MTKFB_SET_VIDEO_LAYERS, &mLayerInfo[1]) < 0)
        {
            ACDK_LOGE("MTKFB_SET_VIDEO_LAYERS failed");
            return ACDK_RETURN_API_FAIL;
        }
        ...
        mLayerInfo[1].layer_id = 3; //just for disable usage. mLayerInfo[0] disable layer0 and mLayerInfo[1] disable layer1
        ...
        ACDK_LOGD("src VA(0x%x), PA(0x%x)",(MUINTPTR)mLayerInfo[0].src_base_addr,(MUINTPTR)mLayerInfo[0].src_phy_addr );
        if (ioctl(mFBfd, MTKFB_SET_OVERLAY_LAYER, &mLayerInfo[0]) < 0)
        {
            ACDK_LOGE("MTKFB_SET_OVERLAY_LAYER failed");
            return ACDK_RETURN_API_FAIL;
    ...
    else
    {
        ...
        ACDK_LOGD_DYN(g_overlayDebug,"use mmap");
        ACDK_LOGD_DYN(g_overlayDebug,"src VA(0x%x), size(%u)",(MUINTPTR)mLayerInfo[0].src_base_addr,mLayerInfo[0].src_width * mLayerInfo[0].src_height * 2);
        ...
        for(MUINT32 i = 0; i < mLayerInfo[0].src_height; ++i)
        {
            memcpy((m_pFrameBuf + (mFBBufferIndex * mFBBufferSize))+ mLayerInfo[0].tgt_offset_x + (mVinfo.xres_virtual * mLayerInfo[0].tgt_offset_y * bytePrePixel) + (i * mVinfo.xres_virtual * bytePrePixel),
                    (MUINT8 *)mLayerInfo[0].src_base_addr + (i * mLayerInfo[0].src_width * 2), mLayerInfo[0].src_width * 2);
        }
