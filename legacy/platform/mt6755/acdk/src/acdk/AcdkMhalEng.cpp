                      ...
                      void *user)
{
    ...
    uint32_t uMsg_type = static_cast<uint32_t>(msg_type);
    switch  (uMsg_type & ~CAMERA_MSG_PREVIEW_METADATA)
    {
...
AcdkMhalEng::AcdkMhalEng()
{
    ...
    memset(&mAcdkMhalCapParam,0,sizeof(mAcdkMhalCapParam));
...
MINT32 AcdkMhalEng::acdkMhalPreviewStart(MVOID *a_pBuffIn)
{
    ...
    else if(mAcdkMhalPrvParam.sensorIndex == 1)
        AcdkMhalOpenDevice(mModule, "1");
    else
        AcdkMhalOpenDevice(mModule, "2");
