    ...
    ,mIsISPEng(MFALSE)
{
    ...
    memset((void *)sensorStaticInfo/*array*/, 0, sizeof(sensorStaticInfo));
    memset((void *)sensorDynamicInfo/*array*/, 0, sizeof(sensorDynamicInfo));
...
MINT32 AcdkMain::init()
{
    ...
    //if((mOperationMode == 1 || mOperationMode == 0) && m_pSensorHalObj != NULL)
...
MINT32 AcdkMain::startPreview(Func_CB prvCb)
{
    ...
    if (!m_pSensorHalObj) {
        ACDK_LOGE("create sensor fail index %d", mSensorIndex);
        return ACDK_RETURN_API_FAIL;
    }
                ...
                MINT32  const isSaveImg)
{
   ...
   setScenario(mode, sensorScenarioId);
    ...
    setDimension(mode,acdkCapInfo.RAWImgBufInfo.imgWidth,acdkCapInfo.RAWImgBufInfo.imgHeight);
                ...
                acdkCapInfo.RAWImgBufInfo.imgSize   = mCapIMemInfo.size;//acdkCapInfo.RAWImgBufInfo.imgWidth*acdkCapInfo.RAWImgBufInfo.imgHeight*10/8;
                ...
                MINT32  const isSaveImg)
{
  ...
  setDimension(mode, mCapWidth, mCapHeight);
    ...
    if(m_pSingleShot != NULL)
    {
        ...
        retval = m_pSingleShot->uninit();
        if (retval == MFALSE) {
            ACDK_LOGE("SingleShot uninit return false");
        }
