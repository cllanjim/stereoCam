...
static const struct connect_notify_rule gNotifyConnectRule_ZSD[] =
{
    ...
    {  CONTROL_SHUTTER                         , CTRLNODE     , CALLBACKNODE } ,
    {  CONTROL_P2DONE                          , CAPPASS2NODE , CALLBACKNODE } ,
    {  CONTROL_STOP_PASS1                      , CTRLNODE     , PASS1NODE    } ,
    { 0, 0, 0 },
};
...
startOne(SensorParam const & rSensorParam)
{
    ...
    if( (dataInBit & DATA_MAINYUV) && mbGetP2outBuf )
    {
        mpAllocBufHandler->updateRequest( mapToNodeData(DATA_MAINYUV), mShotParam.u4PictureTransform, NSCam::NSIoPipe::EPortCapbility_Cap);
    }
...
sendCommand(MINT32 cmd, MINT32 arg1, MINT32 arg2, MINT32 arg3)
{
    ...
    switch( cmd )
    {
        ...
        case ECamShot_CMD_SET_RWB_PROC_TYPE:
            ...
            break;
        case ECamShot_CMD_SET_P2OUT_BUF:
            mbGetP2outBuf = MTRUE;
            MY_LOGD("set getP2outBuf %d", arg1);
            break;
        default:
            break;
    }
    ...
    if( pSingleShot )
    {
        switch( msg.msgType )
        {
            case NSCamNode::ENode_NOTIFY_MSG_SHUTTER:
                ...
                break;
            case NSCamNode::ENode_NOTIFY_MSG_P2DONE:
                pSingleShot->handleNotifyCallback(ECamShot_NOTIFY_MSG_P2DONE, 0, 0);
                break;
            default:
                break;
        }
...
createNodes(MUINT32 const NodeDataSet, MUINT32 const NodeCbSet, IImageBuffer const *pImgBuffer)
{
    ...
    if( isNotifyMsgEnabled(ECamShot_NOTIFY_MSG_P2DONE) )
        mpCallback->enableNotifyMsg(ENode_NOTIFY_MSG_P2DONE);
