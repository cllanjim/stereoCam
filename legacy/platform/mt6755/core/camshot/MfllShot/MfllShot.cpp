...
// ImpShot
#include <v1/adapter/inc/Scenario/Shot/IShot.h>
#include <v1/adapter/Scenario/Shot/inc/ImpShot.h>
...
/**
 *  Define this option to dump RAW as an unpakced RAW
 *
 *  #define DUMP_UNPACKED_RAW
 */
 ...
 *  Performance Service
 *
 *  Due to CPU core usage is controlled by system, sometimes MFLL can not be
 *  processed with the maximum cores, thereforce we use PerformanceService to
 *  enable all cores during MFLL processing time.
 */
#define USING_MTK_PERFSERVICE_MFLL
#ifdef USING_MTK_PERFSERVICE_MFLL
...
#define PERF_LIB_FULLNAME        "/vendor/lib/libperfservicenative.so"
...
#define SCENARIO_SIZE   (1)
/* value is 0 means DON'T CARE */
static camera_scenario gScenTable[SCENARIO_SIZE] =
{
    {
        .scen = ResMgrDrv::SCEN_SW_CAM_CAP,
        .bigcore = 4,
        .bigfreq = 2000000, // 2.0 GHz
        .littlecore = 4,
        .littlefreq = 1150000,
...
/**
 *  Usually, bits 0~3 represents little cores, bits 4~7 represents big cores.
 *  This bit map indicates that the CPU core that MFLL threads will be processed in.
 */
const unsigned int gCpuEnableCore = 0xF0;

/**
 *  PerformanceService is not synchronized call, and no callback to indicate
 *  job done. Therefore, we need a tricky method to wait PerfService done.
 *
 *  If PerfService is ready for synchronized call, set this value to 0 and
 *  we won't wait PerfService anymore.
 */
const unsigned int gPerfServiceReadyDelay = 10000; // unit: usecond.
    ...
    for( MUINT32 i = 0 ; i < SCENARIO_SIZE; i++ )
    {
        if( gScenTable[i].scen == scen )
        {
            if( gScenHandle[i] != -1 )
            {
                ...
                gPerfEnabledIdx = i;
                /* Makes sure PerfService is ready */
                if (gPerfServiceReadyDelay)
                    usleep(gPerfServiceReadyDelay);
            }
            break;
        }
    }
...
{
    //CPU Affinity--------------------------------------------------------------------------/
    MY_LOGD("MFLL flow CPUAffinity Start Pid = %d , Tid = %d\n", getpid(), gettid());

    int cpu_msk = (int)gCpuEnableCore;  //Force run on big-core
    MY_LOGD("MFLL flow CPUAffinity Set CPU Affinity Mask= %d\n", cpu_msk);
    ...
    if (s <= 0)
    {
        MY_LOGD("MFLL flow CPUAffinity Get fail");
        return MFALSE;
    }
    else
    {
        MY_LOGD("MFLL flow CPUAffinity Thread %d Current CPU Affinity %08lx Backup Affinity %08lx", gettid(), cpuold.__bits[0], cpubackup.__bits[0]);
    }

    s = mt_sched_setaffinity(gettid(), sizeof(cpu_set_t), &cpuset);
    if (s != 0)
    {
        MY_LOGD("MFLL flow CPUAffinity Set fail");
        return MFALSE;
    }
    else
    {
        s = mt_sched_getaffinity(gettid(), sizeof(cpu_set_t), &cpuset, &cpubackup);
        MY_LOGD("MFLL flow CPUAffinity Thread %d New CPU Affinity %08lx Backup Affinity %08lx", gettid(), cpuset.__bits[0], cpubackup.__bits[0]);
    }
    /*--------------------------------------------------------------------------CPU Affinity*/
...
disableForceToBigCore()
{
	...
	if(!status)
    {
		MY_LOGD("MFLL flow CPUAffinity Exit affinity to CPU1 successfully, status=%d\n", status);

		return MTRUE;
	}
	else
	{
		MY_LOGD("MFLL flow CPUAffinity Exit affinity to CPU1 FAIL, status=%d\n", status);
...
/******************************************************************************
 * dual phase support related variables & functions
 *******************************************************************************/
/* operations of pass2 driver must be thread-safe, using mutex to implement it */
static pthread_mutex_t gPass2Mutex = PTHREAD_MUTEX_INITIALIZER_UNLOCK;
static pthread_mutex_t gDualPhaseMutex = PTHREAD_MUTEX_INITIALIZER_UNLOCK; // unlocked
static pthread_mutex_t gDualPhaseLastJpegLocker = PTHREAD_MUTEX_INITIALIZER_UNLOCK;
static int gDualPhaseLastJpegCount = 0;
/**
 *  To count used ZSD buffers.
 *
 *  Why we need to count used ZSD buffers, because during Dual Phase, sometimes ZSD buffers may
 *  be keeped until the next time Mfll asks CapBufMgr one more set of ZSD buffers. At this time,
 *  the previous set of ZSD buffers haven't been retruned, and you will have deadlock here because
 *  CapBufMgr won't response if there's any ZSD buffer not returned and asks new one.
 */
static pthread_mutex_t gZsdUsedBufferLocker = PTHREAD_MUTEX_INITIALIZER_UNLOCK;
static int gZsdUsedBuffer = 0;

static void dualPhaseUsedZsdBuffer(void)
{
    pthread_mutex_lock(&gZsdUsedBufferLocker);
    gZsdUsedBuffer++;
    pthread_mutex_unlock(&gZsdUsedBufferLocker);
}

static void dualPhaseReturnZsdBuffer(void)
{
    pthread_mutex_lock(&gZsdUsedBufferLocker);
    gZsdUsedBuffer--;
    pthread_mutex_unlock(&gZsdUsedBufferLocker);
...
static int dualPhaseGetZsdUsedBuffer(void)
{
    int r = 0;
    pthread_mutex_lock(&gZsdUsedBufferLocker);
    r = gZsdUsedBuffer;
...
static void dualPhaseResetZsdUsedBuffer()
{
    pthread_mutex_lock(&gZsdUsedBufferLocker);
    gZsdUsedBuffer = 0;
    pthread_mutex_unlock(&gZsdUsedBufferLocker);
...
static int dualPhaseLock(void)
{
    return pthread_mutex_lock(&gDualPhaseMutex);
}

static void dualPhaseUnlock(void)
{
    pthread_mutex_unlock(&gDualPhaseMutex);
}

static int pass2Lock(void)
{
    return pthread_mutex_lock(&gPass2Mutex);
}

static void pass2Unlock(void)
{
    pthread_mutex_unlock(&gPass2Mutex);
}

static int dualPhaseIncJpegCount(void)
{
    volatile int r;

    pthread_mutex_lock(&gDualPhaseLastJpegLocker);
    r = ++gDualPhaseLastJpegCount;
    pthread_mutex_unlock(&gDualPhaseLastJpegLocker);

    return r;
}

static int dualPhaseDecJpegCount(void)
{
    volatile int r;

    pthread_mutex_lock(&gDualPhaseLastJpegLocker);
    gDualPhaseLastJpegCount--;
    r = gDualPhaseLastJpegCount;
    pthread_mutex_unlock(&gDualPhaseLastJpegLocker);
...
/**
 *  This function returns true or false for determining if there is another P2
 *  being run.
 */
static bool dualPhaseIsStillProcessing(void)
{
    volatile bool b;

    pthread_mutex_lock(&gDualPhaseLastJpegLocker);
    b = (gDualPhaseLastJpegCount > 1 ? true : false);
    pthread_mutex_unlock(&gDualPhaseLastJpegLocker);
...
/**
 *  A wrapper class to retrieve IShotCallback instance from NormalShot or FBShot
 */
class WrapperImpShot : public NSShot::ImpShot {
public:
    sp<NSShot::IShotCallback> getShotCallback(void) {
        return mpShotCallback;
    }

public:
    bool makeCamExif(MUINT32 const u4CamMode, CamExif &rCamExif)
    {
        return makeCamExif(u4CamMode, rCamExif);
    };

    bool makeExifHeaderWrapper( MUINT32 const u4CamMode,
    ...
    {
        return makeExifHeaderEx(u4CamMode, puThumbBuf, u4ThumbSize, puExifBuf, u4FinalExifSize, rCamExif, pDbg);
    };
};
/******************************************************************************
...
static
MBOOL
allocImageBuffer(IImageBufferHeap **pHeap, sp<IImageBuffer>* pBuf, MUINT32 width, MUINT32 height, int const format, unsigned int usingGralloc = 0)
{
    ...
    /* Gralloc buffer extra param */
    IGrallocImageBufferHeap::AllocExtraParam gbExtraParam(eBUFFER_USAGE_HW_RENDER|eBUFFER_USAGE_HW_TEXTURE|eBUFFER_USAGE_SW_WRITE_RARELY);

    *pHeap = usingGralloc != 0
        ? (IImageBufferHeap*) IGrallocImageBufferHeap::create(LOG_TAG, imgParam, gbExtraParam)
        : (IImageBufferHeap*) IIonImageBufferHeap::create(LOG_TAG, imgParam);
    CHECK_OBJECT(*pHeap);
...
MfllShot(EShotMode const eShotMode, char const*const szCamShotName, MUINT32 mfb)
    ...
    , mRaw_Width(0)
    , mRaw_Height(0)
    , mMemoryRaw{NULL}             //array
    , mMemoryRawInCaptureOrder{NULL}
    , mMemoryRawB(NULL)
    , mMemoryRawR{NULL}             //array
    , mMemoryCapBufRaw{NULL}        //array
    ...
    , mYuvQ_Height(0)
    , mMemoryQYuvB(NULL)
    , mMemoryQYuvR{NULL}            //array

    , mMemoryLocalMotion{NULL}
    , mMemoryYuv(NULL)
    , mMemoryWeightingMap{NULL}
    , mMemoryFinalWeightingMapIndex(0)

    , mMemoryYuvMixing(NULL)
    ...
    , mMemoryJpeg_Thumbnail(NULL)
    , mThumbnailJpegRealSize(0)

    , mPostview_Width(0)
    , mPostview_Height(0)
    , mMemoryPostview(NULL)
    ...
    // RWB support
    , mTriggerRwbMemory(PTHREAD_MUTEX_INITIALIZER_LOCK)
    , mRwbMemoryDone(PTHREAD_MUTEX_INITIALIZER_LOCK)
    ...
    // PerformanceService
    , mPerfService(MFLL_FORCE_TO_ALL_BIG_CORES)

    // if using RWB sensor
    , mUsingRwbSensor((eRwbSupport)MFLL_RWB_SENSOR_CCM_METHOD)
    // MRP usage
    ...
    , mDualPhase(MFLL_DUAL_PHASE)
{
    ...
    for(MUINT32 ref=0; ref<MAX_REFERENCE_FRAMES; ref++) {
        ...
        /* parallel MEMC new parameters */
        mTriggerParallelMemc[ref] = PTHREAD_MUTEX_INITIALIZER_LOCK;
        mMemoryLocalMotion[ref] = NULL;
        mpMfllHal[ref] = NULL;
    }
    ...
    /* Parallel MEMC default enable */
    setUsingParallelMemc(MFLL_PARALLEL_MEMC != 0 ? MTRUE : MFALSE);
...
init()
{
    ...
    property_get("mediatek.mfll.dump.rwb", value, "0");
    MBOOL dumpRwb = atoi(value) || TEST_DUMP_RWB;
...
uninit()
{
    ...
    //
    for (MUINT32 i = 0; i < (mUsingParallelMemc ? mReferenceFrameLength : 1); i++) {
        if(mpMfllHal[i] != NULL) {
            mpMfllHal[i]->uninit();
            mpMfllHal[i]->destroyInstance(LOG_TAG);
            mpMfllHal[i] = NULL;
        }
    }
...
startOne(SensorParam const & rSensorParam)
{
    ...
    /* Increase dual phase live count ASAP */
    if (isUsingDualPhase())
        dualPhaseIncJpegCount();
    ...
    if (mPerfService) {
        if (!dualPhaseIsStillProcessing()) {
            ...
            MY_LOGD("ignore enable PerfService due to another P2 is still running.");
        }
    }
#endif
    ...
    ********************************************************************************/
    ...
    #endif
        /*
         * if RWB sensor support is using GPU mode, create the thread for
         * creating IImageBuffer from Gralloc
         */
        if (this->mUsingRwbSensor == eRwbSupport_GPU) {
            pthread_create(&mThreadRwbMemory, &attr, MfllShot::rwbMemoryProc, this);
        }

        // mfll
        pthread_create(&mThreadMfll, &attr, MfllShot::mfllProc, this);

        if (mUsingParallelMemc) {
            for (int i = 0; i < mReferenceFrameLength; i++) {
                /* create a memory chunk for saving two parameters, release it in parallelMemcProc */
                long *chunk = new long[2];
                chunk[0] = (long)(void*)this;
                chunk[1] = (long)i;
                pthread_create(&mThreadParallelMemc[i], &attr, MfllShot::parallelMemcProc, chunk);
            }
        }
        else {
            // memc
    ...
    ********************************************************************************/
    if (isUsingDualPhase()) {
        /**
         *  If using DualPhase, all threads (excludes CapturePostProc) are needed
         *  to be detach because we want to makes this function (startOne) to be
         *  as async call.
         */

        /* increase ImpShot life count */
        ((NSShot::ImpShot*)(mpCbUser))->incStrong(mpCbUser);

        /* Only join this thread */
        pthread_join(mThreadCapturePostProc, (void**)&threadRet);

        pthread_detach(mThreadMemory);
        pthread_detach(mThreadCapture);
        pthread_detach(mThreadRawToYuy2);
#if TEST_MEMC_YUY2
        pthread_detach(mThreadYuy2ToYv16);
#endif
        if (mThreadRwbMemory)
            pthread_detach(mThreadRwbMemory);

        pthread_detach(mThreadMfll);

        /* join threads depends on if using parallel MEMC */
        if (mUsingParallelMemc) {
            for (int i = 0; i < mReferenceFrameLength; i++)
                pthread_detach(mThreadParallelMemc[i]);
        }
        else {
            pthread_detach(mThreadMemc);
        }

        pthread_detach(mThreadBlending);
        pthread_detach(mThreadPostview);
        pthread_detach(mThreadFull);
        pthread_detach(mThreadThumbnail);
    }
    else {
        //  2.  join threads
        pthread_join(mThreadCapturePostProc, (void**)&threadRet);
...
#endif
        if (mThreadRwbMemory)
            pthread_join(mThreadRwbMemory, (void**)&threadRet);
        ...
        if (mUsingParallelMemc) {
            for (int i = 0; i < mReferenceFrameLength; i++)
                pthread_join(mThreadParallelMemc[i], (void**)&threadRet);
        }
        else {
...
lbExit:
#ifdef USING_MTK_PERFSERVICE_MFLL
    /* If using Dual Phase, we do not invoke PerfService here. */
    if (mPerfService && !isUsingDualPhase()) {
        disableForceToBigCore();
...
MVOID*
MfllShot::
parallelMemcProc(void *arg)
{
    ::prctl(PR_SET_NAME, "parallelMcProc", 0, 0, 0);
    FUNCTION_LOG_START;
    MUINTPTR 	ret = MTRUE;

    long *chunk = (long*)arg;
    MfllShot *self = (MfllShot*)(void*)chunk[0];
    int ref = (int)chunk[1];
    CHECK_OBJECT(self);
    self->flowParallelMemc(ref);

lbExit:
    delete [] chunk; // release
    FUNCTION_LOG_END_MUM;
    return (MVOID*)ret;
}
...
MVOID* MfllShot::rwbMemoryProc(void *arg)
{
    ::prctl(PR_SET_NAME,"rwbMemoryProc", 0, 0, 0);
    FUNCTION_LOG_START;
    MUINTPTR 	ret = MTRUE;

    MfllShot *self = (MfllShot*)arg;
    CHECK_OBJECT(self);
    ret = ret && self->flowRwbMemory();
...
flowCapture()
{
    ...
    if (eShotMode_ZsdMfllShot == meShotMode){
        ...
        /* check the current buffer length */
        while(dualPhaseGetZsdUsedBuffer() > 0) {
            MY_LOGD("There's %d buffers of capBufMgr used by Mfll, trying %d", dualPhaseGetZsdUsedBuffer(), tryCount);
            tryCount++;
            msleep(10);
            if (tryCount > 100) {
                MY_LOGD("Given up to wait used zsd buffer to be 0, reset it and keep going");
                dualPhaseResetZsdUsedBuffer();
            }
        }

        for(MUINT32 i=0; i < capBufLen; i++) {
            ret = ret && mpCapBufMgr->dequeBuf(mMemoryCapBufRaw[i]);
            dualPhaseUsedZsdBuffer();
            ...
            /**
             *  To make CamExif first.
             *
             *  In ZSD shot, we should ask 3A manager the "current" 3A info during
             *  the correct timing: ZSD manager has changed 3A
             *
             */
            if (i ==0)
                ((WrapperImpShot*)(mpCbUser))->makeCamExif(eAppMode_PhotoMode, mCamExif);
        }
    } else {
...
flowCapturePostProc()
{
...
#ifdef DUMP_UNPACKED_RAW
        dumpUnpackedRaw(0, mMemoryRawB.get());
        for (size_t i = 0; i < mReferenceFrameLength; i++)
            dumpUnpackedRaw(i + 1, mMemoryRawR[i].get());
#else
        //pass1
    ...
    /* callback for DNG info */
    this->handleDngInfoCb();
...
flowMfll()
{
    ...
    /**
     *  If using RWB sensor GPU mode, we need to do color correction by
     *  MTKRWB module.
     */
    if (this->mUsingRwbSensor == eRwbSupport_GPU) {
        this->doRwbColorCorrection();
    }
...
MBOOL
MfllShot::
flowParallelMemc(unsigned int ref)
{
    FUNCTION_LOG_START;
    MBOOL 	ret = MTRUE;
    MY_LOGD("rynn: parallelMemcProc %d", ref);

    WAIT(&mTriggerMemc[ref]);
    /* If this is the first ME, trigger the first MC directly */
    if (ref == 0)
        ANNOUNCE(&mTriggerParallelMemc[0]);

    /* invoke ME/MC with the certain MfbllHal instance */
    ret = ret && doMemc(ref, mpMfllHal[ref]);

    /* This MC done, trigger the next MC (ME has already run) */
    if (ref + 1 < mReferenceFrameLength) {
        ANNOUNCE(&mTriggerParallelMemc[ref + 1]);
    }

    /* release ME/MC working buffer in the last stage */
    if ((ref + 1) == mReferenceFrameLength) {
        /* but must wait other ME/MC done (MC runs parallelly) */
        for (int i = 0; i < (mReferenceFrameLength - 1); i++)
            WAIT(&mMemcDone[i]);
    ...
    /**
     *  we don't need to take care about the invoking order of mMemcDone,
     *  because Mfblending is always invoked in sequential.
     */
    ANNOUNCE(&mMemcDone[ref]);
...
MBOOL MfllShot::flowRwbMemory(void)
{
    FUNCTION_LOG_START;
    MBOOL ret = MTRUE;

    WAIT(&mTriggerRwbMemory);
    ret = ret && doRwbMemory();
    ANNOUNCE(&mRwbMemoryDone);
    FUNCTION_LOG_END;
    return ret;
}
...
void MfllShot::readProperty(void)
{
    ...
    /* RWB sensor support */
    property_get("mediatek.mfll.rwb", p, "-1");
    v = atoi(p);
    if (v != -1) {
        switch(v){
        case 0:
            mUsingRwbSensor = eRwbSupport_None;
            break;
        case 1:
            mUsingRwbSensor = eRwbSupport_GPU;
            break;
        case 2:
            mUsingRwbSensor = eRwbSupport_MDP;
            break;
        default:;
        }
        MY_LOGD("Manual set RWB support = %d", v);
    }

    /* Parallel MEMC */
    property_get("mediatek.mfll.pmm", p, "-1");
    v = atoi(p);
    if (v != -1) {
        setUsingParallelMemc( v ? MTRUE : MFALSE);
    }

    /* Force processes to big cores */
    property_get("mediatek.mfll.big", p, "-1");
    v = atoi(p);
    if (v != -1) {
        mPerfService = (v != 0 ? MTRUE : MFALSE);
    }

    /* Dual Phase */
    property_get("mediatek.mfll.dualphase", p, "-1");
    v = atoi(p);
    if (v != -1) {
        mDualPhase = (v != 0 ? 1 : 0);
    }
...
void MfllShot::phase1Done(void)
{
    /* phase 1 callback only supports ZSD mode */
    if (isUsingDualPhase() != MTRUE)
        return;

    MFLL_TRACE_CALL();

    /* lock to make sure only a phase 2 can be executed at one time */
    dualPhaseLock();

    /**
     *  CamShotImp::mpCbUser is a pointer for caller of CamShotImp instance (MfllShot instance).
     *  The caller of MfllShot should be NormalShot or FDShot, therefore we can get
     *  the ImpShot instance from CamShotImp::mpCbUser, where NormalShot/FDShot inherits
     *  ImpShot.
     */
    ((WrapperImpShot*)(mpCbUser))->getShotCallback()->onCB_P2done();
    MY_LOGD("phase1Done - unlocked");
}

void MfllShot::phase2Done(void)
{
    /* Make sure 100% ZSD + DualPhase */
    if (isUsingDualPhase() != MTRUE)
        return;
    ...
    dualPhaseUnlock();
    MY_LOGD("phase2Done - unlocked");
}

void MfllShot::selfRelease(void)
{
    /* Make sure 100% ZSD + DualPhase */
    if (isUsingDualPhase() != MTRUE)
        return;

    MY_LOGD("selfRelease - E");

#ifdef USING_MTK_PERFSERVICE_MFLL
    /* If Dual Phase is overlapped, do not disable PerfService */
    if (mPerfService && !dualPhaseIsStillProcessing()) {
        disableForceToBigCore();
        disablePerfService(gScenTable[0].scen);
        uninitPerf();
    }
    ...
    uninit();
    ((NSShot::ImpShot*)(mpCbUser))->decStrong(mpCbUser);
    delete this;

    MY_LOGD("selfRelease - X");
}

void MfllShot::dualphaseHandleJpeg(void)
{
    int jpegCount = dualPhaseDecJpegCount();
    bool lastFrame;

    /* check if the last frame */
    lastFrame = (jpegCount <= 0 ? true : false);

    IDbgInfoContainer* pDbg = mDebugInfo;
    MUINT8* puJpegBuf = (MUINT8*)mMemoryJpeg->getBufVA(0);
    MUINT32 u4JpegSize = mMemoryJpeg->getBitstreamSize();
    MUINT8* puThumbBuf = (MUINT8*)mMemoryJpeg_Thumbnail->getBufVA(0);
    MUINT32 u4ThumbSize = mMemoryJpeg_Thumbnail->getBitstreamSize();
    ...
    MY_LOGD("makeExifHeader = 0x%x", this);
    ((WrapperImpShot*)mpCbUser)->makeExifHeaderWrapper(
        eAppMode_PhotoMode,
        puThumbBuf,
        u4ThumbSize,
        puExifHeaderBuf,
     ...
     *  ImpShot.
     */
    ((WrapperImpShot*)(mpCbUser))->getShotCallback()->onCB_CompressedImage(
                                         0,
                                         u4JpegSize,
...
MBOOL MfllShot::isUsingDualPhase(void)
{
    if ((meShotMode != eShotMode_ZsdMfllShot) || (mDualPhase == 0))
        return MFALSE;
    return MTRUE;
}
...
allocateMemory()
{
    ...
    MUINT32 imgFmtRaw = eImgFmt_BAYER10;

    // Raw Buffer
    MY_LOGD("prepare Raw buffer");

    // always uses the first MfbllHal to query working buffer size
    CHECK_OBJECT(mpMfllHal[0]);
    mpMfllHal[0]->queryWorkingBufferSize(WorkingBufferSize);
    MY_LOGD("allocateMemory - algorithm WorkingBufferSize = %d", WorkingBufferSize);

    IImageBufferHeap *pHeap;

    /* query sesnor bits from shot param */
    {
        switch(mSensorParam.u4Bitdepth) {
        case 8:
            imgFmtRaw = eImgFmt_BAYER8;
            break;
        case 10:
            imgFmtRaw = eImgFmt_BAYER10;
            break;
        case 12:
            imgFmtRaw = eImgFmt_BAYER12;
            break;
        case 14:
            imgFmtRaw = eImgFmt_BAYER14;
            break;
        default:
            MY_LOGE("%s: un-support RAW bits %d",
                    __FUNCTION__, mSensorParam.u4Bitdepth);
            break;
        }
    }
    ...
    if(eShotMode_ZsdMfllShot != meShotMode){
        for(MUINT32 ref=0; ref<mReferenceFrameLength+1; ref++) {
            ret = ret && allocImageBuffer(&pHeap, &mMemoryRaw[ref], mRaw_Width, mRaw_Height, imgFmtRaw);
        }
    } else {
        if (!DONT_MEMCPY){
            for(MUINT32 ref=0; ref<mReferenceFrameLength+1; ref++) {
    ...
    // mMemoryLocalMotion;      30M
    {
        ...
        for (MUINT32 i = 0; i < (mUsingParallelMemc ? mReferenceFrameLength : 1); i++) {
            ret = ret && allocBlobBuffer(&pHeap, &mMemoryLocalMotion[i], WorkingBufferSize);
        }
    ...
    if(!(mRegistedBufferType&ECamShot_BUF_TYPE_YUV)) {
        ...
        ret = ret && allocImageBuffer(&pHeap,
            &mMemoryYuvMixing,
            mRotPicWidth, mRotPicHeight,
            mUsingRwbSensor == eRwbSupport_GPU ? eImgFmt_YV12 : eImgFmt_YUY2,
            mUsingRwbSensor == eRwbSupport_GPU // only GPU mode needs buffer from Gralloc
        );
...
releaseMemory()
{
    ...
    // mMemoryLocalMotion;                  30M
    for (MUINT32 i = 0; i < mReferenceFrameLength; i++) {
        freeImageBuffer(&mMemoryLocalMotion[i]);
    }
    ...
    /* free RWB if necessary */
    if (mMemoryRwbImgBuffer.get())
        freeImageBuffer(&this->mMemoryRwbImgBuffer);
#else
    msleep(50);
#endif
...
initInStartOne(SensorParam const & rSensorParam)
{
    ...
    ********************************************************************************/
...
#else
    {
        ...
        /* If sensor is RWB sensor, use MFLL_RWB_SENSOR_CCM_METHOD as default */
        if (sensorInfo.rawFmtType == SENSOR_RAW_RWB) {
            mUsingRwbSensor = (eRwbSupport)MFLL_RWB_SENSOR_CCM_METHOD;
        }
        else {
            mUsingRwbSensor = eRwbSupport_None;
    ...
    ********************************************************************************/
...
#if TEST_ALGORITHM
    {
    ...
    #ifdef MEMC_FLOW_V2
        /* If using parallel ME/MC, we need more instances of MfbllHal */
        for (unsigned int i = 0; i < (mUsingParallelMemc ? mReferenceFrameLength : 1); i++) {
            mpMfllHal[i] = new MfbllHal();
            CHECK_OBJECT(mpMfllHal[i]);
            mpMfllHal[i]->init(&MfbllPipeInitInfo);
        }
    #else
        mpMfllHal[0] = new MfbllHal();
        CHECK_OBJECT(mpMfllHal[0]);
        mpMfllHal[0]->init(&MfbllPipeInitInfo);
    #endif
        ...
        /* Use frame number from tuning data */
        mReferenceFrameLength = mTuningData.max_frame_number - 1;
        if (mReferenceFrameLength <= 0)
            MY_LOGE("mReferenceFrameLength is 0 !!!");
        MY_LOGD("mReferenceFrameLength(%d)", mReferenceFrameLength);
    }
#endif
...
fgCamShotDataCbImp(NSCamShot::CamShotDataInfo  const& msg)
{
    ...
    /* We need to feedback all callback info to EngShot here */
    if (meShotMode == eShotMode_EngShot) {
...
doBss()
{
    ...
    MF_BSS_PARAM_STRUCT param = {
        MF_BSS_ON                    : static_cast<MUINT8>(mEnableBss),
...
MBOOL
MfllShot::
doMemc(MUINT32 ref, MfbllHal *pMfb)
{
    MFLL_TRACE_CALL();
    ...
    /**
     *  doMemc supports parallel execution. If user wants to execute doMemc
     *  parallelly, user have to specify a MfbllHal instance for executing
     *  MFB algorithm.
     *
     *  If *pMfb is NULL, it means that user want to execute this sequentially
     *  and we will use mpMfllHal[0] as MFB algorithm handler.
     */
    if (pMfb == NULL) {
        pMfb = mpMfllHal[0];
        ret = ret && pMfb->setWorkingBuffer((MUINT8*)mMemoryLocalMotion[0]->getBufVA(0));
        /* do not block MC */
        ANNOUNCE(&mTriggerParallelMemc[ref]);
    }
    else {
        ret = ret && pMfb->setWorkingBuffer((MUINT8*)mMemoryLocalMotion[ref]->getBufVA(0));
    }
    ...
    if(ret) {
        /**
         *  mTriggerParallelMemc is a locker for parallel ME/MC control. In other
         *  words, ME/MC processing will be invoked parallelly, but the current
         *  MC processing depends on the result of the previous MC. Therefore,
         *  here we assign a synchronization object (locked mutex) to MfbllHal
         *  and MfbllHal will wait it to be unlocked and then to invoke MC
         *  processing.
         *
         *  This only effect if only if has new MEMC algorithm
         *  (MEMC_FLOW_V2 is defined.)
         */
        MBOOL isMemcSuccessed = pMfb->doMemc(gmvX, gmvY, baseBufAddr, refBufAddr, &badMvCount, &mTriggerParallelMemc[ref]);
        if(!isMemcSuccessed) {
            mSkipMfb = MTRUE;
...
encodeRawToYuv(sp<IImageBuffer> &rawBuffer, sp<IImageBuffer> &yuvBuffer, EIspProfile_T profile, void* privateData)
{
...

    {
        ...
        if (profile == EIspProfile_VSS_MFB_PostProc_ANR_EE_SWNR || profile == EIspProfile_VSS_MFB_PostProc_ANR_EE)
            p3A->setIspProfile(_3A_profile, mDebugInfo);
        else
            p3A->setIspProfile(_3A_profile);
    }
	p3A->destroyInstance(LOG_TAG);
...
doMixing()
{
    ...
    android::sp<IImageBuffer> spImgTarget; // indicates mixing output buffer.
    ...
    /**
     * Announce that we can prepare to allocate RWB support working buffer
     * if using RWB sesnor mode support.
     *
     * BTW, if Memory Reduce Plan is specified, we should late allocate RWB
     * buffers to avoid memory usage peak.
     */
    if (mUsingRwbSensor == eRwbSupport_GPU && mReduceMemoryUsage != eMrpMode_LowestMemory) {
        ANNOUNCE(&this->mTriggerRwbMemory);
    }
...
#if 0
    /* mt6755 not supports vip3i interface of mix anymore. */
    // - weighting map (mfb)
    ...
    moutput_wroto.mPortID.type = NSCam::NSIoPipe::EPortType_Memory;
#endif
    /* if using MDP conversion, set flag */
    if (this->mUsingRwbSensor == eRwbSupport_MDP) {
        moutput_wroto.bIsRwbEn = MTRUE;
        MY_LOGD("moutput_img3o enable RWB by MDP \n");
    }
    ...
    /**
     *  If using RWB sesnor and Memory Reduce Plan (Lowest Memory), later allocate RWB buffer
     *  after mixing related buffers have been released
     */
    if (mUsingRwbSensor == eRwbSupport_GPU && mReduceMemoryUsage == eMrpMode_LowestMemory) {
        ANNOUNCE(&this->mTriggerRwbMemory);
    }
    ...
    /**
     *  If support RWB sensor GPU mode, we will have one more stage after
     *  mixing done, GPU color correction.
     *
     *  Because GPU only accepts two buffers, one as input another as output,
     *  we want the final output of MFNR is mMemoryYuvMixing and if using
     *  GPU color correction the output of mixing stage should be a temporary
     *  buffer. Then GPU can takes this temporary buffer as input and output
     *  result to mMemoryYuvMixing.
     */
    if (this->mUsingRwbSensor == eRwbSupport_GPU) {
        WAIT(&this->mRwbMemoryDone); // wait memory is ready
        spImgTarget = this->mMemoryRwbImgBuffer;
    }
    else
        spImgTarget = this->mMemoryYuvMixing;
    ...
    switch(muNRType)
    {
        ...
        case ECamShot_NRTYPE_HWNR:
            {
                MFLL_TRACE_NAME("HWNR");
/**
 *  We don't need HWNR pass2 anymore due to hardware change. If your platform needs
 *  two pass HWNR, please define the macro "HWNR_PASS2".
 *
 *  #define HWNR_PASS2
 */
                ...
                if(mDump & MFLL_DUMP_MIXING)
                {
                    ...
                    char filename[MFB_MAX_DUMP_FILENAME_LENGTH] = {0};
                    //
                    snprintf(filename, sizeof(filename), "%s_%02d_doHwNR2_output__%04dx%04d.yuy2", mDumpFilename, 6, spImgTarget->getImgSize().w, spImgTarget->getImgSize().h);
                    MY_LOGD("flowCapture saveToFile %s", filename);
                    spImgTarget->saveToFile(filename);
                }

                /* release pInternalBuf if necessary */
                if( pInternalBuf.get() )
                {
                    IImageBufferAllocator* allocator = IImageBufferAllocator::getInstance();
        ...
        case ECamShot_NRTYPE_SWNR:
            {
                ...
                ret = ret && convertImage(pMemoryYuvMixing
                 ...
                 if(mDump & MFLL_DUMP_MIXING)
                 {
                     ...
                     char filename[MFB_MAX_DUMP_FILENAME_LENGTH] = {0};
                     //
                     snprintf(filename, sizeof(filename), "%s_swnr_output__%04dx%04d.yuy2", mDumpFilename, spImgTarget->getImgSize().w, spImgTarget->getImgSize().h);
                     MY_LOGD("flowCapture saveToFile %s", filename);
                     spImgTarget->saveToFile(filename);
                 }
            }
...
MBOOL MfllShot::doRwbMemory(void)
{
    MFLL_TRACE_CALL();
    FUNCTION_LOG_START;
    MBOOL ret = MTRUE;
    ...
    /*
     * Due to prototype of function "allocImageBuffer", here we have a tricky
     * method to make sure no memory leak for IImageBufferHeap
     */
    IImageBufferHeap     *pHeap = 0;

    /* allocate the same size, format buffer with mMemoryYuvMixing */
    ret = allocImageBuffer(
        &pHeap,
        &mMemoryRwbImgBuffer,
        mMemoryYuvMixing->getImgSize().w,
        mMemoryYuvMixing->getImgSize().h,
        eImgFmt_YV12,
        1 // use Gralloc to allocate buffer
    );

    if (ret != 0) {
        MY_LOGE("doRwbMemory - allocate gralloc buffer fail, ret=0x%x", ret);
    }
...
MBOOL MfllShot::doRwbColorCorrection(void)
{
    MFLL_TRACE_CALL();
    FUNCTION_LOG_START;
    MBOOL ret = MTRUE;

    /* Temporary buffer for RwbGpuProc usage if necessary */
    sp<IImageBuffer> spTempBuf = NULL;

    IImageBuffer *pImgSrc = this->mMemoryRwbImgBuffer.get();
    IImageBuffer *pImgDst = this->mMemoryYuvMixing.get();

    if (mDump & MFLL_DUMP_RWB) {
        char filename[MFB_MAX_DUMP_FILENAME_LENGTH] = {0};
        snprintf(filename, sizeof(filename), "%s_%02d_rwb_in_%02dx%04d.yv12", mDumpFilename, 8, pImgSrc->getBufStridesInBytes(0), pImgSrc->getImgSize().h);
        mMemoryRwbImgBuffer->saveToFile(filename);
    }

    /* Create RwbGpuProc instance */
    sp<RwbGpuProc> rwbProc = RwbGpuProc::createInstance(mSensorParam.u4OpenID);
    if (rwbProc.get() == NULL) {
        MY_LOGE("RwbGpuProc is NULL pointer");
        ret = MFALSE;
        goto lbExit;
    }

    /* If the mMemoryYuvMixing is not allocated by MfllShot, we must check if it's YV12 and allocated from Gralloc */
    if (mRegistedBufferType & ECamShot_BUF_TYPE_YUV) {
        /* Assume it's not, we have to allocate a temporary buffer for RwbGpuProc conversion */
        IImageBufferHeap *pHeap;
        ret = allocImageBuffer(&pHeap, &spTempBuf, pImgSrc->getImgSize().w, pImgSrc->getImgSize().h, eImgFmt_YV12, 1);
        if (ret != MTRUE) {
            MY_LOGE("doRwbColorCorrection - Allocate temp buffer failed");
            goto lbExit;
        }

        pImgDst = spTempBuf.get(); // update destination
    }

    /**
     *  Set source & destination buffer, notice, these two buffers MUST be created
     *  by Gralloc or GPU cannot access it.
     */
    rwbProc->setSrcDstBuf(pImgSrc, pImgDst);
    rwbProc->doGPUProc();
    rwbProc->destroyInstance();

    /* dump files */
    if(mDump & MFLL_DUMP_RWB) {
        char filename[MFB_MAX_DUMP_FILENAME_LENGTH] = {0};
        snprintf(filename, sizeof(filename), "%s_%02d_rwb_out_%02dx%04d.yv12", mDumpFilename, 8, pImgDst->getBufStridesInBytes(0), pImgDst->getImgSize().h);
        MY_LOGD("rwbGpuProc saveToFile %s", filename);
        mMemoryYuvMixing->saveToFile(filename);
    }

    /* If destination image is on spTempBuf, we need to convert it to mMemoryYuvMixing */
    if (spTempBuf.get()) {
        ret = ret && convertImage(spTempBuf, mMemoryYuvMixing, 0);
        if (ret != MTRUE) {
            MY_LOGE("%s - convert spTempBuf to mMemoryYuvMixing failed", __FUNCTION__);
        }
        freeImageBuffer(&spTempBuf);
    }
...
setDebugInfo(IDbgInfoContainer* pDbgInfoCtnr) const
{
    ...
    {   //save mfll, bss, memc, ais as tag-value format
        ...
        setDebugTag(rMfDebugInfo, MF_TAG_MEMC_BAD_MB_COUNT3,mDbgMfllInfo.memc.badMbCount[3]);
        setDebugTag(rMfDebugInfo, MF_TAG_MEMC_BAD_MB_COUNT4,mDbgMfllInfo.memc.badMbCount[4]);
        ...
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR_Y4LUT1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR_Y4LUT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR_Y4LUT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR_C4LUT1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR_C4LUT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR_C4LUT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR_A4LUT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR_A4LUT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR_L4LUT1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR_L4LUT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR_L4LUT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        ...
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR_LCE, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR_MED1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR_MED2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR_MED3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR_MED4, mDbgMfllInfo.reg[tabId][regId]); regId++;
        ...
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR_ACT1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR_ACT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR_ACT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        ...
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR2_CON1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR2_CON2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR2_YAD1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR2_Y4LUT1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR2_Y4LUT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR2_Y4LUT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR2_L4LUT1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR2_L4LUT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR2_L4LUT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR2_CAD, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR2_PTC, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR2_LCE, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR2_MED1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR2_MED2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR2_MED3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR2_MED4, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR2_ACTY, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BASE_CAM_ANR2_ACTC, mDbgMfllInfo.reg[tabId][regId]); regId++;
        ...
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR_Y4LUT1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR_Y4LUT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR_Y4LUT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR_C4LUT1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR_C4LUT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR_C4LUT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR_A4LUT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR_A4LUT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR_L4LUT1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR_L4LUT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR_L4LUT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        ...
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR_LCE, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR_MED1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR_MED2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR_MED3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR_MED4, mDbgMfllInfo.reg[tabId][regId]); regId++;
        ...
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR_ACT1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR_ACT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR_ACT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        ...
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR2_CON1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR2_CON2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR2_YAD1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR2_Y4LUT1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR2_Y4LUT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR2_Y4LUT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR2_L4LUT1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR2_L4LUT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR2_L4LUT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR2_CAD, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR2_PTC, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR2_LCE, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR2_MED1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR2_MED2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR2_MED3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR2_MED4, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR2_ACTY, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_BEFORE_MFB_CAM_ANR2_ACTC, mDbgMfllInfo.reg[tabId][regId]); regId++;
        ...
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR_Y4LUT1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR_Y4LUT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR_Y4LUT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR_C4LUT1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR_C4LUT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR_C4LUT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR_A4LUT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR_A4LUT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR_L4LUT1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR_L4LUT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR_L4LUT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        ...
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR_LCE, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR_MED1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR_MED2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR_MED3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR_MED4, mDbgMfllInfo.reg[tabId][regId]); regId++;
        ...
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR_ACT1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR_ACT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR_ACT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        ...
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR2_CON1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR2_CON2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR2_YAD1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR2_Y4LUT1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR2_Y4LUT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR2_Y4LUT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR2_L4LUT1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR2_L4LUT2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR2_L4LUT3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR2_CAD, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR2_PTC, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR2_LCE, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR2_MED1, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR2_MED2, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR2_MED3, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR2_MED4, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR2_ACTY, mDbgMfllInfo.reg[tabId][regId]); regId++;
        setDebugTag(rMfDebugInfo, MF_TAG_AFTER_MFB_CAM_ANR2_ACTC, mDbgMfllInfo.reg[tabId][regId]); regId++;
...
updateRegToDebugInfo(MFB_REGTABLE regtable)
{
...

    {
        MUINT32 reg[] = {
            ...
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_Y4LUT1),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_Y4LUT2),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_Y4LUT3),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_C4LUT1),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_C4LUT2),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_C4LUT3),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_A4LUT2),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_A4LUT3),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_L4LUT1),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_L4LUT2),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_L4LUT3),
            ...
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_LCE),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_MED1),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_MED2),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_MED3),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_MED4),
            ...
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_ACT1),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_ACT2),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_ACT3),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_ACTY),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR_ACTC),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR2_CON1),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR2_CON2),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR2_YAD1),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR2_Y4LUT1),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR2_Y4LUT2),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR2_Y4LUT3),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR2_L4LUT1),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR2_L4LUT2),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR2_L4LUT3),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR2_CAD),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR2_PTC),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR2_LCE),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR2_MED1),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR2_MED2),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR2_MED3),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR2_MED4),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR2_ACTY),
            ISP_READ_REG_NOPROTECT(pISPDrvObj, CAM_ANR2_ACTC),
...
dumpDebugInfoAsTextFile(const char * const filename) const
{
    ...
    {   //save gmv, eis, reg as hex list
        ...
        COMPILE_TIME_ASSERT_FUNCTION_SCOPE(MF_MAX_FRAME >= MAX_REFERENCE_FRAMES);
        for(MUINT32 frame=0; frame<mReferenceFrameLength; frame++) {
...
MBOOL MfllShot::setUsingParallelMemc(MBOOL bEnable)
{
#ifndef MEMC_FLOW_V2
    mUsingParallelMemc = false;
    MY_LOGE("Parallel MEMC is only supported when MEMC_FLOW_V2 is defined");
    return MFALSE;
#else
    mUsingParallelMemc = (bEnable != MFALSE ? true : false);
    return MTRUE;
#endif
}
...
MBOOL MfllShot::handleRawCb(void)
{
    /* if using EngShot, do not retrun RAW by this method, but using fgCamShotDataCbImp */
    if (meShotMode == eShotMode_EngShot)
        return TRUE;

    /* handle RAW */
    if (isDataMsgEnabled(NSCamShot::ECamShot_DATA_MSG_RAW)) {
        MY_LOGD("%s :handle RAW", __FUNCTION__);
        handleDataCallback(NSCamShot::ECamShot_DATA_MSG_RAW, NULL, NULL, mMemoryRawB.get());
        return MTRUE;
    }
    return MFALSE;
}

MBOOL MfllShot::handleDngInfoCb(void)
{
    /* callback for DNG info */
    if (isDataMsgEnabled(NSCamShot::ECamShot_DATA_MSG_META)) {
        MY_LOGD("%s :handle META (DNG)", __FUNCTION__);
        unsigned int err = 0;

        /**
         *  Tricky: using IHal3A to retrieve DNG info if using ZSD mode
         */
        DNGInfo_T dngInfo;
        EIspProfile_T profile;
        ...
        pass2Lock(); // avoid race condition
        p3A = IHal3A::createInstance(IHal3A::E_Camera_1, mSensorParam.u4OpenID, LOG_TAG);
        if (p3A != NULL) {
            if (eShotMode_ZsdMfllShot == meShotMode){
                profile = (muNRType==ECamShot_NRTYPE_SWNR ? EIspProfile_VSS_MFB_Blending_All_Off_SWNR : EIspProfile_VSS_MFB_Blending_All_Off);
            ...
            /* To retrieve DNG info */
            ParamIspProfile_T _3A_profile(profile, 0 /* magicNum */, MFALSE, ParamIspProfile_T::EParamValidate_P2Only, &dngInfo);
            p3A->setIspProfile(_3A_profile);
            p3A->destroyInstance("MFLL_DNGINFO");
        }
        else {
            err = 1;
        }
        ...
        if (err == 0) {
            handleDataCallback(NSCamShot::ECamShot_DATA_MSG_META, (MUINTPTR)&dngInfo, 0,0);
            return MTRUE;
        }
        else {
            MY_LOGE("Cannot create IHal3A instance for DNG metadata retrieving");
        }
...
void MfllShot::dumpUnpackedRaw(int index, IImageBuffer *pBuffer)
{
    if (pBuffer == NULL) {
        MY_LOGE("%s: RAW buffer is NULL", __FUNCTION__);
        return;
    }
    ...
    /**
     *  {{{ Using MTKDngOp to unpack RAW
     */
    MTKDngOp *MyDngop = MTKDngOp::createInstance(DRV_DNGOP_UNPACK_OBJ_SW);
    if (MyDngop == NULL) {
        MY_LOGE("Create MTKDngOp failed");
        return;
    }
    DngOpResultInfo MyDngopResultInfo;
    DngOpImageInfo MyDngopImgInfo;

    MUINT32 width = pBuffer->getImgSize().w;
    MUINT32 height = pBuffer->getImgSize().h;

    /** Initialize **/
    MY_LOGD("Start to simulate pack...\n");
    MyDngopImgInfo.Width = width;
    MyDngopImgInfo.Height = height;
    MyDngopImgInfo.Stride_src = pBuffer->getBufStridesInBytes(0);
    MyDngopImgInfo.Stride_dst = pBuffer->getImgSize().w * 2;
    MyDngopImgInfo.BIT_NUM = sensorbit; // buffer bit num
    MyDngopImgInfo.BIT_NUM_DST = 10;
    MyDngopImgInfo.Bit_Depth = 10; //for pack

    // query buffer size
    MUINT32 buf_size = (width * 2 * height); //(scrBuffer->getImgSize().w*2*scrBuffer->getImgSize().h);
    ...
    // assign raw data
    MyDngopImgInfo.srcAddr = (MUINT8*)pBuffer->getBufVA(0);
    ...
    MY_LOGD("after Unpack raw16: w (%d),h (%d),src-s (%d),dst-s (%d),buf-size(%d)",
            MyDngopImgInfo.Width,
            MyDngopImgInfo.Height,
            MyDngopImgInfo.Stride_src,
            MyDngopImgInfo.Stride_dst,
            buf_size);
    ...
    {
        char filename[MFB_MAX_DUMP_FILENAME_LENGTH] = {0};
        snprintf(filename, sizeof(filename), "%s_%02d_exp%04d_iso%04d_proc__%dx%d_%d_%d-0%d.raw"
            , mDumpFilename
            , 0
            , mDbgAisInfo.calculatedExposure
            , mDbgAisInfo.calculatedIso
        ...
        fwrite(p_buffer, 0x01, buf_size, fp);
        fclose(fp);
        delete [] p_buffer;
        MY_LOGD("Raw saved: %s", filename);
    }
}
