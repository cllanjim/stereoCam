...
MBOOL   IsSupportMfb(MUINT32 mfbMode, MUINT32 sensorId) {
    ...
    if(MfllShot::getCaptureInfo(mfbMode, sensorId, frameNumber, exp, iso, needUpdateAE)) {
        ...
        /* If using AIS and need to update AE, returns TRUE for ZsdShot reference */
        if (mfbMode == MFB_MODE_AIS && needUpdateAE == MTRUE) {
            runMfb = MTRUE;
        }
    }
    return runMfb;
...
MBOOL   getMFLLCapInfo(MUINT32 mfbMode, MUINT32 sensorId, NS3A::CaptureParam_T & Cap3AParam, MUINT32 &mfllFrameNum, MBOOL & needUpdateAE){
    ...
    // re-calculate gain by a simple linear calculation, notice that, the lightness
    // may be not the same
    Cap3AParam.u4AfeGain = Cap3AParam.u4AfeGain * iso / Cap3AParam.u4RealISO;
    return ret;
}
