...
createInstance(EShotMode const eShotMode, char const*const szCamShotName, MUINT32 sensorId, MUINT32 mfb, MBOOL *pIsMfb)
{
    ...
    /**
     *  Workaround: Now we have to force use MfllShot if using Zsd + Mfll. The reason is that while we're using
     *  ZSD shot, we CANNOT invoke SingleShot or device goes dead.
     */
    if (eShotMode == eShotMode_ZsdMfllShot && frameNumber <= 1) {
        MY_LOGD("Force use MfllShot due to ZSD");
        frameNumber = 4;
    }
