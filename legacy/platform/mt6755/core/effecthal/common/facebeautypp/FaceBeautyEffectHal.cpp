...
getCaptureRequirementImpl(EffectParameter *inputParam, Vector<EffectCaptureRequirement> &requirements) const
{
    ...
    char str[32];
    MY_LOGD("[%s] picture size %dx%d",__FUNCTION__,mParameter->getInt("picture-width"),mParameter->getInt("picture-height") );
...
getFaceBeautyBeautifiedPosition(Vector<SDK_FACE_BEAUTY_POS>& pos)
{
    ...
    if (p == NULL)
    {
        MY_LOGD("getFaceBeautyBeautifiedPosition fail");
        return MFALSE;
    }
...
abortImpl(EffectResult &result, EffectParameter const *parameter)
{
    ...
    {
      MY_LOGD("[%s] release mpInputFrame", __FUNCTION__);
    ...
    {
      MY_LOGD("[%s] release mpOutputFrame", __FUNCTION__);
    ...
    {
      MY_LOGD("[%s] release mOutputParameter", __FUNCTION__);
      mOutputParameter = NULL;
    }

    if(mInputParameter != NULL)
    {
      MY_LOGD("[%s] release mInputParameter", __FUNCTION__);
      mInputParameter = NULL;
    }

    if(mpRequest != NULL)
    {
      MY_LOGD("[%s] release mpRequest", __FUNCTION__);
      mpRequest = NULL;
    }
...
FBSDKCbFunc()
{
    ...
    if(mpisabort)
    {
        MY_LOGD("[%s] AP will call abort before doing CFB", __FUNCTION__);
        mpisabort = MFALSE;

    } else {
        MY_LOGD("[%s] call start done after doing CFB", __FUNCTION__);
...
updateEffectRequestImpl(const android::sp<EffectRequest> request)
{
    ...
    {
        MY_LOGD("[%s] oFrameInfo != NULL && iFrameInfo != NULL", __FUNCTION__);
        ...
        if (mpOutputFrame != NULL && mpInputFrame != NULL)
        {
            ...
            {
                MY_LOGD("[%s] Bad value", __FUNCTION__);
                return BAD_VALUE;
            }
            ...
            MY_LOGD("[%s] mpOutputFrame != NULL && mpInputFrame != NULL", __FUNCTION__);
            FUNCTION_LOG_END;
            return doEffect();
        }
    }
