...
#define EIS_LOG_IF(cond, ...)   do { if (UNLIKELY(cond)) { EIS_LOG(__VA_ARGS__); } }while(0)
#define EIS_LOG1( ...)   do { if (UNLIKELY(g_debugDump>=1)) { EIS_LOG(__VA_ARGS__); } }while(0)
...
#define EISO_CONDITION_WAIT_TIME 10
...
{
    return EisDrvImp_R::GetInstance(aSensorIdx);
}

/*******************************************************************************
*
********************************************************************************/
EisDrv_R *EisDrvImp_R::GetInstance(const MUINT32 &aSensorIdx)
{
    EIS_LOG("aSensorIdx(%u)",aSensorIdx);

    return new EisDrvImp_R(aSensorIdx);
}

/*******************************************************************************
*
********************************************************************************/
MVOID EisDrvImp_R::DestroyInstance()
{
    EIS_LOG("+");
    delete this;
}
...
*
********************************************************************************/
EisDrvImp_R::EisDrvImp_R(const MUINT32 &aSensorIdx) : EisDrv_R()
{
    // reference count
    mUsers = 0;
...
*
********************************************************************************/
MVOID *EisDrvImp_R::EisoThreadLoop(MVOID *arg)
{
    EisDrvImp_R *_this = reinterpret_cast<EisDrvImp_R *>(arg);
    ...
    EIS_LOG1("Sidx(%u),wait mEisoSem",_this->mSensorIdx);
    ::sem_wait(&_this->mEisoSem); // wait here until someone use sem_post() to wake this semaphore up
    EIS_LOG1("Sidx(%u),got mEisoSem",_this->mSensorIdx);
    ...
    EIS_LOG1("Sidx(%u),post mEisoSemEnd",_this->mSensorIdx);
...
MINT32 EisDrvImp_R::Init()
{
    ...
    if(MTRUE != m_pIMemDrv->init(LOG_TAG))
    {
    ...
    EisIrq_key = m_pISPDrvObj->registerIrq("EIS");


    //====== Create INormalPipe Object ======

    m_pNormalPipe = INormalPipe_FrmB::createInstance(mSensorIdx, EIS_DRV_NAME);
...
MINT32 EisDrvImp_R::Uninit()
{
    ...
    if(mUsers == 0)
    {
        ...
        if(m_pIMemDrv != NULL)
        {
            ...
            if(MTRUE != m_pIMemDrv->uninit(LOG_TAG))
            {
...
*
********************************************************************************/
MINT32 EisDrvImp_R::CreateMemBuf(MUINT32 &memSize,const MUINT32 &bufCnt, IMEM_BUF_INFO *bufInfo)
{
    MINT32 err = EIS_RETURN_NO_ERROR;
    ...
    if(bufCnt > 1)  // more than one
    {
        for(MUINT32 i = 0; i < bufCnt; ++i)
        {
            ...
            if(m_pIMemDrv->allocVirtBuf(LOG_TAG, &bufInfo[i]) < 0)
            {
    ...
    else
    {
        ...
        if(m_pIMemDrv->allocVirtBuf(LOG_TAG, bufInfo) < 0)
        {
...
*
*******************************************************************************/
MINT32 EisDrvImp_R::DestroyMemBuf(const MUINT32 &bufCnt, IMEM_BUF_INFO *bufInfo)
{
    ...
    if(bufCnt > 1)  // more than one
    {
        for(MUINT32 i = 0; i < bufCnt; ++i)
        {
            ...
            if (m_pIMemDrv->freeVirtBuf(LOG_TAG, &bufInfo[i]) < 0)
            {
    ...
    else
    {
        ...
        if (m_pIMemDrv->freeVirtBuf(LOG_TAG, bufInfo) < 0)
        {
...
*
********************************************************************************/
MINT32 EisDrvImp_R::ConfigEisReg(const EIS_SCENARIO_ENUM &aSce,const MUINT32 &aSensorTg)
{
    if(mIsConfig == 1)
    {
        if(g_debugDump >= 1)
    ...
    else
    {
        ...
        m_pEisP1Cb = new EisP1Cb_r(this);
        if( MTRUE != m_pNormalPipe->sendCommand(EPIPECmd_SET_EIS_CBFP,(MINTPTR)m_pEisP1Cb,-1,-1) )
        {
            EIS_ERR("Set pass1 EIS callback function failed via sendCommand.");
        }
...
*
*******************************************************************************/
MINT32 EisDrvImp_R::EnableEis(const MBOOL &aEn)
{
    EIS_LOG("aEn(%d)",aEn);
...
MINT32 EisDrvImp_R::UpdateEiso()
{
    ...
    EIS_LOG1("Eiso:idx(%u),PA(0x%08x)",mEiso_addrIdx,mEiso_phyAddr);
    ...
    if(MFALSE == m_pNormalPipe->sendCommand(EPIPECmd_SET_MODULE_CFG_DONE, handle,-1,-1))
    {
        ...
        {
            EIS_ERR("Release EISO handle failed via sendCommand.");
        }
        return EIS_RETURN_API_FAIL;
    }
...
*
********************************************************************************/
MINT32 EisDrvImp_R::UpdateEisoIdx()
{
    ...
    ISP_DRV_WAIT_IRQ_STRUCT waitIrq;
    memset(&waitIrq, 0, sizeof(waitIrq));
    waitIrq.Clear      = ISP_DRV_IRQ_CLEAR_NONE;
...

    {
        ...
        EIS_LOG1("eisoData.timeStamp %llu", eisoData.timeStamp);

        mEisoData.push(eisoData);

        EIS_LOG1("condition broadcast");
        mEisoCond.broadcast();
    }
    ...
    EIS_LOG1("Eiso:idx(%u->%u),NEXT(va,pa)=(0x%08x,0x%08x)",(mEiso_addrIdx == 0)?EISO_BUFFER_NUM-1:mEiso_addrIdx-1,
                                                             mEiso_addrIdx,
                                                             mEiso_virAddr,
                                                             mEiso_phyAddr);
...
*
********************************************************************************/
MVOID EisDrvImp_R::SetEisoThreadState(EIS_SW_STATE_ENUM aState)
{
...
*
*******************************************************************************/
MINT32 EisDrvImp_R::SetFirstFrame(const MUINT32 &aFirst)
{
...
*
*******************************************************************************/
MVOID EisDrvImp_R::SetFeBlockNum(const MUINT32 &aInputW, const MUINT32 &aInputH,const MUINT32 &aBlockNum)
{
...
*
********************************************************************************/
MUINT32 EisDrvImp_R::GetFirstFrameInfo()
{
    return mIsFirst ? 0 : 1;
}
...
*
********************************************************************************/
MUINT32 EisDrvImp_R::Get2PixelMode()
{
...
*
********************************************************************************/
MVOID EisDrvImp_R::GetEisInputSize(MUINT32 *aWidth, MUINT32 *aHeight)
{
    *aWidth  = mImgWidth;
    *aHeight = mImgHeight;
}
...
*
********************************************************************************/
MUINT32 EisDrvImp_R::GetEisDivH()
{
    if(g_debugDump >= 1)
    {
...
*
********************************************************************************/
MUINT32 EisDrvImp_R::GetEisDivV()
{
    if(g_debugDump >= 1)
    {
...
*
********************************************************************************/
MUINT32 EisDrvImp_R::GetEisMbNum()
{
    if(g_debugDump >= 1)
    {
...
*
********************************************************************************/
MVOID EisDrvImp_R::GetFeoRegInfo(FEO_COFIG_DATA *aFeoRegInfo)
{
...
*
********************************************************************************/
MBOOL EisDrvImp_R::GetEisSupportInfo(const MUINT32 &aSensorIdx)
{
    NormalPipe_EIS_Info eisInfo;

    if( MTRUE != m_pNormalPipe->sendCommand(EPIPECmd_GET_EIS_INFO,aSensorIdx,(MINTPTR)&eisInfo, -1) )
    {
        EIS_ERR("Get EIS info failed via sendCommand.");
    }
...
*
********************************************************************************/
MINT32 EisDrvImp_R::GetEisHwStatistic(EIS_STATISTIC_STRUCT *apEisStat,const MINT64 &aTimeStamp)
{
    EIS_LOG1("GetEisHwStatistic (%lld)",aTimeStamp);
    ...
    else //MW timestamp is valid
    {
...

        {
            ...
            while(!mEisoData.empty())
            {
                ...
                {
                    EIS_LOG("GT, (va,pa,ts)=(0x%08x,0x%08x,%lld)",mEisoData.front().va,mEisoData.front().pa,mEisoData.front().timeStamp);
                    hasMatch = 0;
                    break;
                }
            }
            ...
            {
                EIS_LOG1("Eiso condition wait");
                mEisoCond.waitRelative(mQueLock,(long long int)15*1000000LL); //conditaional wait 10ms
                EIS_LOG1("Eiso condition wait done");
        ...
        if(hasMatch == 1)
        {
            ...
            EIS_LOG1("pEisoAddr(%p)",pEisoAddr);
...
*
********************************************************************************/
MVOID EisDrvImp_R::GetEisHwStatistic(EIS_STATISTIC_T *apEisStat,const MINT64 &aTimeStamp)
{
    //====== Get EISO Data ======
...
*
********************************************************************************/
MVOID EisDrvImp_R::GetFeoStatistic(FEO_STAT_DATA *aFeoStatData)
{
...
*
*******************************************************************************/
MVOID EisDrvImp_R::FlushMemory(const EIS_DMA_ENUM &aDma,const EIS_FLUSH_ENUM &aFlush)
{
...
*
*******************************************************************************/
MVOID EisDrvImp_R::DumpReg(const EIS_PASS_ENUM &aEisPass)
{
    ...
    if(aEisPass == EIS_PASS_1)
    {
        ...
        EIS_LOG("dc_dl:(0x%08x)",(regVal & 0x3F00) >> 8);
        EIS_LOG("vert_shr:(0x%08x)",(regVal & 0xE0) >> 5);
        EIS_LOG("hori_shr:(0x%08x)",(regVal & 0x1C) >> 2);
        EIS_LOG("proc_gain:(0x%08x)",regVal & 0x3);
...
*
********************************************************************************/
MINT32 EisDrvImp_R::Complement2(MUINT32 value, MUINT32 digit)
{
    MINT32 Result;
...
*
*******************************************************************************/
MVOID EisDrvImp_R::BoundaryCheck(MUINT32 &aInput,const MUINT32 &upBound,const MUINT32 &lowBound)
{
    if(aInput > upBound)
    {
        aInput = upBound;
...
*
*******************************************************************************/
MINT64 EisDrvImp_R::GetTimeStamp(const MUINT32 &aSec,const MUINT32 &aUs)
{
    MINT64 tmpTime = ((MINT64)aSec*1000000000LL);
    tmpTime += ((MINT64)aUs*1000LL);
    return  tmpTime;
}
/*******************************************************************************
*
*******************************************************************************/
MINT32 EisDrvImp_R::WriteReg(const REG_NAME_ENUM &aRegName)
{
    MINTPTR handle = -1;
...
*
*******************************************************************************/
MINT32 EisDrvImp_R::ConfigMuxSel(const MUINT32 &aEisSel)
{
...
*
********************************************************************************/
MVOID EisDrvImp_R::SetEisState(const EIS_SW_STATE_ENUM &aState)
{
...
*
********************************************************************************/
EIS_SW_STATE_ENUM EisDrvImp_R::GetEisState()
{
    Mutex::Autolock lock(mSateLock);
    return mState;
}
...
*
********************************************************************************/
MVOID EisDrvImp_R::ChangeThreadSetting(char const *userName)
{
    //> set name
...
*
********************************************************************************/
void EisP1Cb_r::p1TuningNotify(MVOID *pInput,MVOID *pOutput)
{
    EisDrvImp_R *_this = reinterpret_cast<EisDrvImp_R *>(m_pClassObj);
    ...
    if(a_pEisCfgData->bEIS_Bypass == MTRUE)
    {
        ...
        EIS_LOG1("me_ctrl2(0x%08x)",_this->mEisRegSetting.reg_eis_prep_me_ctrl2);
...
MUINT32 EisDrvImp::GetTgRrzRatio()
{
    ...
    if( MTRUE != m_pNormalPipe->sendCommand(EPIPECmd_GET_RMX_OUT_SIZE, mSensorIdx, (MINT32)(&rmxW), (MINT32)(&rmxH)) )
    {
        EIS_ERR("Get RMX out size failed via sendCommand.");
    }
    ...
    EIS_LOG1("ratio(%u,%u)",ratioW,ratioH);
