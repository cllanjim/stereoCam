...
MINT32 EisDrvImp::Init()
{
    ...
    property_get("debug.eisDrv.dump", value, "0");
...
MINT32 EisDrvImp::UpdateEiso()
{
    ...
    if(MTRUE != m_pISPDrvObj->waitIrq(&waitIrq))
    {
        ...
        {
            EIS_LOG("final state(%d)",GetEisState());
            return EIS_RETURN_NO_ERROR;
        }
    }
...
MINT32 EisDrvImp::UpdateEisoIdx()
{
    ...
    if(MTRUE != m_pISPDrvObj->waitIrq(&waitIrq))
    {
        ...
        {
            EIS_LOG("[miss] final wait state(%d)",GetEisState());
        }
...

    {
        ...
        if(mEisoIsFirst == 1)
        {
            ...
            eisoData.pa = mEiso_phyAddr;
            mEisoIsFirst = 0;
            mTsForAlgoDebug = GetTimeStamp(waitIrq.EisMeta.tLastSOF2P1done_sec,waitIrq.EisMeta.tLastSOF2P1done_usec);
            EIS_LOG("mTsForAlgoDebug(%lld)",mTsForAlgoDebug);
        }
        else
        {
            MUINT32 tempEisoIdx = (mEiso_addrIdx == 0) ? EISO_BUFFER_NUM-1 : mEiso_addrIdx-1;
...
MVOID EisDrvImp::GetRegSetting(void *data)
{
    ...
    a_pGisCfgData->sensor_Width = mSensor_Width;
    a_pGisCfgData->sensor_Height = mSensor_Height;
    ...
    if ( (mRrz_scale_Width != mImgWidth) || (mRrz_scale_Height != mImgHeight))
    {
        EIS_LOG("mRrz_scale_Width(%d) is not sync to mImgWidth(%d)",mRrz_scale_Width,mImgWidth);
        EIS_LOG("mRrz_scale_Height(%d) is not sync to mImgHeight(%d)",mRrz_scale_Height,mImgHeight);
    }
...
void EisP1Cb::p1TuningNotify(MVOID *pInput,MVOID *pOutput)
{
    ...
    EIS_LOG("RRZ In(%u,%u), RRZ crop x/y(%u,%u), RRZ crop w/h(%u,%u), RMX(%u,%u)",
                                                   aEisInInfo->RRZcrop.full_w, aEisInInfo->RRZcrop.full_h,
                                                   aEisInInfo->RRZcrop.x, aEisInInfo->RRZcrop.y,
                                                   aEisInInfo->RRZcrop.w,aEisInInfo->RRZcrop.h,
                                                   aEisInInfo->sRMXOut.w, aEisInInfo->sRMXOut.h);
    ...
    //TBD: Holmes should review all the cases
    _this->mSensor_Width = aEisInInfo->RRZcrop.full_w;
    _this->mSensor_Height = aEisInInfo->RRZcrop.full_h;
    _this->mRrz_crop_Width = aEisInInfo->RRZcrop.w;
    _this->mRrz_crop_Height = aEisInInfo->RRZcrop.h;
    _this->mRrz_crop_X = aEisInInfo->RRZcrop.x;
    _this->mRrz_crop_Y= aEisInInfo->RRZcrop.y;
    _this->mRrz_scale_Width = ( _this->mIs2Pixel == 1)? (_this->mImgWidth << 1) : (_this->mImgWidth );
