...
int NvBufUtil::setCctMode(int isCctMode)
{
  DEF_AutoLock(m_cs);
  int err;
  void* buf;
	...
	if(err!=0)
	{
	    logE("readVerNvramNoLock error ln=%d", __LINE__);
		return err;
	}
	NVRAM_CAMERA_VERSION_STRUCT* verStruct = (NVRAM_CAMERA_VERSION_STRUCT*)buf;
	verStruct->isCct=isCctMode;
	err = writeVerNvramNoLock();
	if(err!=0)
	{
	   logE("writeVerNvramNoLock error ln=%d", __LINE__);
		 return err;
	}
	return 0;
}
int getCctModeNoLock(int* isCctMode)
{
  int err;
  void* buf;
	err = readVerNvramNoLock(buf);
...
{
logI("getBufAndReadNoDefault+ ln=%d ramId=%d sensorDev=%d",__LINE__,(int)nvRamId,sensorDev);
    DEF_AutoLock(m_cs);
	int err=0;
	int bRead;
	err = getMem(nvRamId, (CAMERA_DUAL_CAMERA_SENSOR_ENUM)sensorDev, p);
...
static int getBufAndReadNoLock(CAMERA_DATA_TYPE_ENUM nvRamId, int sensorDev, void*& p, int bForceRead)
{
logI("getBufAndReadNoLock+ ln=%d ramId=%d sensorDev=%d",__LINE__,(int)nvRamId,sensorDev);
...
//logI("getBufAndRead ln=%d p=%d",__LINE__,p);
    ...
    int isComb=0;
    isComb = cust_NvramNeedCombineDef(nvRamId);


    int cctModeInNvram;
    getCctModeNoLock(&cctModeInNvram);
    int cctModeProp = getPropInt("debug.nvram_is_cct", -1);
    ...
    {
      cctMode=cctModeInNvram;
    }


    if(isComb==1 && cctMode==0)
    {
        char* nv=0;
		int NvSz;
		err = getNvSize(nvRamId, NvSz);
		if(err!=0)
		{	
			logE("getNvSize err=%d",err);
			return err;
		}
		nv = new char[NvSz];
        err  = nvDrv->readDefaultData (
		...
		cust_NvramCombine(nvRamId, p, nv);
...
{
    logI("readDefault(with buf para ln=%d",__LINE__);
    DEF_AutoLock(m_cs);
    int err;
	int u4SensorID;
	void* buf;
