...
halFDVT::~halFDVT()
{
    ...
    MY_LOGD("[Destroy] m_pMTKFDVTObj->destroyInstance");
...
//**********************************************************************//
    ...
    if(g_CurrentStatus==2) {
        ...
        FDFTInitInfo.FDRefresh = 3;   //FB mode
    }
/*
...
void halFDVT::AcquireFTBuffer(void* arg)
{
    halFDVT *_this = reinterpret_cast<halFDVT*>(arg);
    while((_this->mFTBufReady) == 0) {
        MY_LOGW("FT Buffer is not ready");
    ...
    if(EnableSWResizerFlag)
    {
        ...
        else if(g_input_16_9 == 1)
        {
            if(SDEnable || mode == FDVT_GFD_MODE)
            {
                ...
                char filename[200];
                static int count = 0;
                sprintf(filename, "/storage/sdcard0/fd_data/Y_VGA_buffer_%d_%d_%d.raw", m_FDW, m_FDH, count1);
                fp = fopen(filename, "wb");
                fwrite((unsigned char*)y_vga, m_FDW*m_FDH, 1, fp);
                fclose(fp);
                count1++;
                */
...
)
{
    ...
    faceCnt=m_pMTKFDVTObj->FDVTGetResult((MUINT8 *) pbuf, FACEDETECT_TRACKING_DISPLAY);
...
halFDVT::halSDGetSmileResult()
{
    ...
    if(DetectResult[0].face_num==1)
    {
          int fd_size = DetectResult[0].x1 - DetectResult[0].x0;
        if(fd_size<=24)
            SmileCnt = 0;

            MY_LOGD("halFDVT fd_size is :%d,", fd_size);
    }
