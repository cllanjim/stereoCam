...
MINT32 VHdrDrvImp::Init()
{
    ...
    if(MFALSE == m_pIMemDrv->init(VHDR_DRV_NAME))
    {
...
MINT32 VHdrDrvImp::Uninit()
{
    ...
    if(mUsers == 0)
    {
        ...
        if(m_pIMemDrv != NULL)
        {
            ...
            if(MFALSE == m_pIMemDrv->uninit(VHDR_DRV_NAME))
            {
...
MINT32 VHdrDrvImp::ConfigLcs()
{
    ...
    if(mSensorTg == CAM_TG_1)
    {
        ...
        if(MFALSE == m_pNormalPipe->sendCommand(EPIPECmd_GET_TG_OUT_SIZE, mSensorIdx, (MINTPTR)(&lcsInWidth),(MINTPTR)(&lcsInHeight)))
        {
            VHDR_ERR("get TG out size fail");
            return VHDR_RETURN_API_FAIL;
        }
        if(MFALSE == m_pNormalPipe->sendCommand(EPIPECmd_GET_HBIN_INFO, mSensorIdx, (MINTPTR)(&hbinInfo), -1))
        {
            VHDR_ERR("get HBIN info fail");
            return VHDR_RETURN_API_FAIL;
        }
    ...
    else if(mSensorTg == CAM_TG_2)   //RAW_D
    {
        ...
        if(MFALSE == m_pNormalPipe->sendCommand(EPIPECmd_GET_TG_OUT_SIZE, mSensorIdx, (MINTPTR)(&lcsD_InWidth),(MINTPTR)(&lcsD_InHeight)))
        {
            VHDR_ERR("get TG out size fail");
            return VHDR_RETURN_API_FAIL;
        }
        if(MFALSE == m_pNormalPipe->sendCommand(EPIPECmd_GET_HBIN_INFO, mSensorIdx, (MINTPTR)(&hbinD_Info), -1))
        {
            VHDR_ERR("get HBIN info fail");
            return VHDR_RETURN_API_FAIL;
        }
...
MVOID VHdrDrvImp::ConfigRmg(const MUINT32 &aLeFirst)
{
    ...
    // so just put it here
    if(MFALSE == m_pNormalPipe->sendCommand(EPIPECmd_SET_LCS_CBFP,(MINTPTR)mpVHdrP1Cb,-1,-1))
    {
        VHDR_ERR("set LCS cbfp fail");
    }
}
...
MINT32 VHdrDrvImp::CreateMemBuf(MUINT32 &memSize,const MUINT32 &bufCnt, IMEM_BUF_INFO *bufInfo)
{
    ...
    if(m_pIMemDrv->allocVirtBuf(VHDR_DRV_NAME, bufInfo) < 0)
    {
...
MINT32 VHdrDrvImp::DestroyMemBuf(const MUINT32 &bufCnt, IMEM_BUF_INFO *bufInfo)
{
    ...
    if(bufCnt > 1)  // more than one
    {
        for(MUINT32 i = 0; i < bufCnt; ++i)
        {
            ...
            if (m_pIMemDrv->freeVirtBuf(VHDR_DRV_NAME, &bufInfo[i]) < 0)
            {
    ...
    else
    {
        ...
        if (m_pIMemDrv->freeVirtBuf(VHDR_DRV_NAME, bufInfo) < 0)
        {
...
MBOOL VHdrDrvImp::SaveToFile(char const *filepath,const MUINTPTR &addr)
{
    ...
    VHDR_LOG("write %d bytes to %s", size, filepath);
