...
#define CHECK_NR_RET( NR_ret )                          \
    ...
    void* gbuffer = mpSrcGpBuf->getImageBufferHeap()->getGraphicBuffer();
    sp<GraphicBuffer>* gpuSrcBuf = (sp<GraphicBuffer>*)gbuffer;
    gralloc_extra_ion_sf_info_t info;
    gralloc_extra_query((*gpuSrcBuf)->handle, GRALLOC_EXTRA_GET_IOCTL_ION_SF_INFO, &info);
    gralloc_extra_sf_set_status(&info, GRALLOC_EXTRA_MASK_YUV_COLORSPACE, GRALLOC_EXTRA_BIT_YUV_BT601_FULL);
    gralloc_extra_perform((*gpuSrcBuf)->handle, GRALLOC_EXTRA_SET_IOCTL_ION_SF_INFO, &info);
    ...
    //
    mRWB_info.SrcGraphicBuffer = mpSrcGpBuf->getImageBufferHeap()->getGraphicBuffer();//GB
    mRWB_info.DstGraphicBuffer = mpDstGpBuf->getImageBufferHeap()->getGraphicBuffer();//GB
    //
    if( mpRWB == NULL )
    {
        MY_LOGE("mpRWB is NULL");
