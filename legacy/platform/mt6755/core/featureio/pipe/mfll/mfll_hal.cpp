...
MfbllHal::MfbllHal()
    : Proc1_set_proc_info({0}) // POD structure
    , MfbllMeResult({0}) // POD structure
    , mTuning({0}) // POD structure
    , MfbllInitPara({0}) // POD structure
    , imgW(0)
    , imgH(0)
    , ImgSize(0)
    , WorkingBuffer(NULL)
...
MBOOL MfbllHal::init(void *pInitInData)
{
    ...
    //
	MfbllInitPara.Proc1_Init.Proc1_tuning.core_num = 8;
...
MBOOL MfbllHal::doMemc(MINT32 mvX, MINT32 mvY, MUINT8* baseBufAddr, MUINT8* refBufAddr, MUINT32 *badMvCount, pthread_mutex_t *triggerMc)
{
    FUNCTION_LOG_START;
...
#ifdef MEMC_FLOW_V2
    MfbllMcResult.Height = inMEHeight;
    MfbllMcResult.Width = inMEWidth;
    #if TEST_MEMC_YUY2
    MfbllMcResult.ImgFmt = PROC1_FMT_YUY2;            //@todo - PROC1_FMT_YV16 has better performance
    #else
    MfbllMcResult.ImgFmt = PROC1_FMT_YV16;            //@todo - PROC1_FMT_YV16 has better performance
    #endif
    MfbllMcResult.ResultImg = NULL;
#else
    ...
    MFLL_TRACE_BEGIN("ME");
    /* do ME directly */
    m_pMfbllDrv->MfbllMain(MFBLL_PROC_ME, &err, &MfbllMeResult);
    MFLL_TRACE_END();

    /* witing until triggerMc is unlocked */
    if (triggerMc) {
        pthread_mutex_lock(triggerMc);
    }

    MFLL_TRACE_BEGIN("MC");
    m_pMfbllDrv->MfbllMain(MFBLL_PROC_MC, &err, &MfbllMcResult);
    MFLL_TRACE_END();
#else
	//MEMC Main process
