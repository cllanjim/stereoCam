...
SDKcreateJpegImg()
{
    ...
    MINT32 bufDump = property_get_int32("effecthal.jpg.bufdump", 0);

    if(bufDump != 0)
    {
        char filename[256];
        sprintf(filename, "/sdcard/JpgEffecthalSrc_%dx%d_YV12.yuv", JPGSrcImgBuffer->getImgSize().w, JPGSrcImgBuffer->getImgSize().h);
        CAM_LOGD("Save jpg src success? (%d)", JPGSrcImgBuffer->saveToFile(filename));
        CAM_LOGD("Save src : %s", filename);
...
allocMem(MUINT32 fmt, MUINT32 w, MUINT32 h)
{
    ...
    if( fmt != eImgFmt_JPEG )
    {
        ...
        if (mpIMemDrv->allocVirtBuf(NAME, &bufMap.memBuf)) {  // IMemDrv now has no NAME parameter??
       // if (mpIMemDrv->allocVirtBuf(&bufMap.memBuf)) {
            CAM_LOGD("g_pIMemDrv->allocVirtBuf() error \n");
            return NULL;
        }
        //memset((void*)bufMap.memBuf.virtAddr, 0 , bufMap.memBuf.size);
        if (mpIMemDrv->mapPhyAddr(&bufMap.memBuf)) {
            CAM_LOGD("mpIMemDrv->mapPhyAddr() error \n");
            return NULL;
        }
    ...
    else
    {
                ...
                it++)
        {
            if (it->pImgBuf == pBuf)
            {
                ...
                {
                    CAM_LOGD("m_pIMemDrv->unmapPhyAddr() error");
                }
                if (mpIMemDrv->freeVirtBuf(NAME, &it->memBuf)) // Now IMemDrv has no NAME parameter?
                //if (mpIMemDrv->freeVirtBuf(&it->memBuf))
                {
                    CAM_LOGD("m_pIMemDrv->freeVirtBuf() error");
                }
                else
                {
