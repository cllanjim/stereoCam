 ...
 ************************************************************************************************/
#define LOG_TAG "MtkCam/MtkCam/featureio_CFBShot"
...
InitialAlgorithm(MUINT32 srcWidth, MUINT32 srcHeight, MINT32 gBlurLevel, MINT32 FBTargetColor)
{
    ...
    CAM_LOGD("[InitialAlgorithm] In srcWidth %d srcHeight %d",srcWidth,srcHeight);
    ...
    if(mSDKMode)
    {
        CAM_LOGD("[InitialAlgorithm] SDK use YV12 format");
    ...
    CAM_LOGD("[InitialAlgorithm] smooth-level=%d, skin-color=%d, enlarge-eye=%d, slim-face=%d, warp-face-num=%d", mSmoothLevel, mBrightLevel, mEnlargeEyeLevel, mSlimFaceLevel, FaceBeautyEnvInfo.pTuningPara->WarpFaceNum);
    
    err = mpFb->mHalFacebeautifyInit(&FaceBeautyEnvInfo);
    CAM_LOGD("[InitialAlgorithm] algorithm initail done");
    CPTLog(Event_FBShot_InitialAlgorithm, CPTFlagEnd);
    CAM_LOGD("[InitialAlgorithm] Out");
    if(err)
        return  MFALSE;
    else 
    return  MTRUE;
...
STEP2(IImageBuffer* Srcbufinfo, MUINT32 srcWidth, MUINT32 srcHeight, IImageBuffer* DSbufinfo, MtkCameraFaceMetadata* FaceInfo, void* FaceBeautyResultInfo)
{
    ...
    CAM_LOGD("[STEP2] srcAdr 0x%x DSbufinfo 0x%x size ds %d",(MUINT32)Srcbufinfo->getBufVA(0),(MUINT32)DSbufinfo->getBufVA(0),DSbufinfo->getBufSizeInBytes(0));
    CPTLog(Event_FBShot_STEP2, CPTFlagStart);
    MINT32 err = 0;
    if(mSDKMode)
    {
       CAM_LOGD("[STEP2] SDK VGA use YV12 format");
    ...
    CAM_LOGD("[STEP2] Save File done vga.adr 0x%x",mpDSImg->getBufVA(0));
    #endif
    ...
    if (!doFaceDetection(mpDSImg, 
                            ...
                            mSDKMode? mu4W_yuv: mu4W_yuv, 
                            mSDKMode? mu4H_yuv: mu4H_yuv,
                            0))
    {
        CAM_LOGD("[STEP2] FD fail");
        return  MFALSE;
    }
            ...
            it++)
    {
        ...
        CAM_LOGD("[STEP2] Beautified face @[%d,%d]",FacePos[i].x,FacePos[i].y);
        i++;
    }
    ...
    for(int i=0;i<sdkfaceNum;i++)
    {
	    ...
	    CAM_LOGD("face num %d x =%d y = %d",FBmetadata.number_of_faces,FacePos[i].x,FacePos[i].y);
    }
    ...
    if(err)
    {
    	 CAM_LOGD("[STEP2] Algo fail");
    	 goto lbExit;
    }
    CPTLog(Event_FBShot_STEP2, CPTFlagEnd);
    CAM_LOGD("[STEP2] Out err %d",err);
    //CAM_TRACE_END();
lbExit:
    if(err)
        return  MFALSE;
...
STEP3(IImageBuffer* Srcbufinfo, void* FaceBeautyResultInfo) const
{
    ...
    mpDSImg->saveToFile(szFileName); 
    CAM_LOGD("[STEP3] Save File done vga.adr 0x%x",mpDSImg->getBufVA(0));
    #endif
    CAM_LOGD("[STEP3] srcAdr 0x%x size  %d",(MUINT32)mpDSImg->getBufVA(0),mpDSImg->getBufSizeInBytes(0));
   ...
   // CAM_TRACE_END();
    ...
    CAM_LOGD("[STEP3] Save File done desAdr 0x%x",tmp->Step3ResultAddr_1);
    ...
    CAM_LOGD("[STEP3] Save File done desAdr 0x%x",tmp->Step3ResultAddr_2);
    #endif
    
    //CAM_TRACE_END();
    if(err)
    {
    	 CAM_LOGE("[STEP3] mHalSTEP3 fail");
    	 return  MFALSE;
    }
    else
    {
        CAM_LOGD("[STEP3] mHalSTEP3 out");
        return  MTRUE;
    }
}
/*******************************************************************************
...
STEP1(IImageBuffer* Srcbufinfo, MUINT32 srcWidth, MUINT32 srcHeight, IImageBuffer* Desbufinfo, IImageBuffer* Alphabufinfo, void* FaceBeautyResultInfo)
{
    ...
    MBOOL   ret = MFALSE;
    IImageBuffer* tmpSTEP1s, *tmpSTEP1d = NULL;
    CAM_LOGD("[STEP1] srcAdr 0x%x srcWidth %d srcHeight %d desAdr 0x%x ",(MUINT32)Srcbufinfo->getBufVA(0),srcWidth,srcHeight,(MUINT32)Desbufinfo->getBufVA(0));
    CPTLog(Event_FBShot_STEP1, CPTFlagStart);
    tmpSTEP1s = allocMem(eImgFmt_I420, mStep1Width, mStep1Height);
    if(!tmpSTEP1s)
    {
        CAM_LOGE("[STEP1] tmpSTEP1s alloc fail");
        ret = MFALSE;
        goto lbExit;
    }
    ...
    if(mSDKMode)
    {
        CAM_LOGD("[STEP1] SDK Resize1 use YV12 format");
    ...
    CAM_LOGD("[STEP1] Save File done tmpSTEP1s->getBufVA(0) 0x%x",tmpSTEP1s->getBufVA(0));
    #endif 
    
    CAM_LOGI("[STEP1] resize 1 done ");
    ...
    CAM_LOGI("[STEP1] algorithm done ret %d",ret);
    if(ret)
    {
    	 CAM_LOGD("[STEP1] STEP1 fail");
    	 goto lbExit;
    }
    ...
    CAM_LOGD("[STEP1] Save File done msFaceBeautyResultInfo.Step1ResultAddr 0x%x",msFaceBeautyResultInfo.Step1ResultAddr);
    #endif
    CPTLog(Event_FBShot_ResizeImg, CPTFlagStart);
    //Do resize Alphamap
    #ifdef Debug_Mode
    ...
    CAM_LOGD("[STEP1] Save File done tmpSTEP1->getBufVA(0) 0x%x",tmpSTEP1d->getBufVA(0));
    #endif
    //
    ...
    if(mSDKMode)
    {
        CAM_LOGD("[STEP1] SDK Resize2 use YV12 format");
    ...
    CAM_LOGI("[STEP1] resize 2 done ");

    //Sava Test
    #ifdef Debug_Mode
    ::sprintf(szFileName, "/sdcard/%s_%dx%d.yuv", "STEP1Img", srcWidth, srcHeight);
    Desbufinfo->saveToFile(szFileName);
    CAM_LOGD("[STEP1] Save File done Srcbufinfo 0x%x",Srcbufinfo->getBufVA(0));
    #endif
...
STEP4(IImageBuffer* Srcbufinfo, MUINT32 srcWidth, MUINT32 srcHeight, IImageBuffer* Blurbufinfo, IImageBuffer* Alphabufinfo, void* FaceBeautyResultInfo)
{
    ...
    if(!tmpStep4)
    {
        CAM_LOGE("[STEP4] tmpStep4 alloc fail");
        goto lbExit;
    }
    ...
    CAM_LOGD("[STEP4] algorithm done ret %d",ret);
    if(ret)
    {
    	 CAM_LOGD("[STEP4] STEP4 fail");
    	 goto lbExit;
    }
    ...
    CAM_LOGD("[STEP4] Save File done Step4ResultAddr 0x%x Result 0x%x",msFaceBeautyResultInfo.Step4ResultAddr,ret);
    #endif
    //------------------ Sava test ----------------//
lbExit:
    deallocMem(tmpStep4);
   	CAM_LOGD("[STEP4] out ret %d",ret);
    //CAM_TRACE_END();
    if(ret)
        return  MFALSE;
    else
...
STEP5(IImageBuffer* Srcbufinfo, MUINT32 srcWidth, MUINT32 srcHeight, IImageBuffer* Alphabufinfo, void* FaceBeautyResultInfo)
{
    //CAM_TRACE_BEGIN("STEP5");
	  MINT32 ret = 0;
	  CAM_LOGD("[STEP5] in");
	Srcbufinfo->saveToFile("/sdcard/STEP5.yuv");
	  MTKPipeFaceBeautyResultInfo* tmp=(MTKPipeFaceBeautyResultInfo*)FaceBeautyResultInfo;
    ...
    if(!tmpStep5)
    {
        CAM_LOGE("[STEP5] tmpStep5 alloc fail");
        goto lbExit;
    }
    ...
    CAM_LOGD("[STEP5] algorithm done ret %d",ret);
    if(ret)
    {
    	 CAM_LOGD("[STEP5] STEP5 fail");
    	 goto lbExit;
    }
    ...
    CAM_LOGD("[STEP5] Save File done Step5ResultAddr 0x%x Result 0x%x ",msFaceBeautyResultInfo.Step5ResultAddr,ret);
    #endif
    //------------------ Sava test ----------------//
lbExit:
    deallocMem(tmpStep5);
   	CAM_LOGD("[STEP5] out ret %d",ret);
    //CAM_TRACE_END();
   	if(ret)
        return  MFALSE;
    else
...
STEP6(IImageBuffer* Srcbufinfo, MUINT32 srcWidth, MUINT32 srcHeight, IImageBuffer* tmpbufinfo, void* FaceBeautyResultInfo) const
{
    ...
    //CAM_TRACE_END();
    CAM_LOGD("[STEP6] algorithm done ret %d",ret);
    ...
    CAM_LOGD("[STEP6] Save File done Step6ResultAddr 0x%x Result 0x%x ",msFaceBeautyResultInfo.Step6ResultAddr,ret);
    #endif
    //------------------ Sava test ----------------//
lbExit:
    //CAM_TRACE_END();
   	CAM_LOGD("[STEP6] out ret %d",ret);
   	if(ret)
        return  MFALSE;
    else
        return  MTRUE;
