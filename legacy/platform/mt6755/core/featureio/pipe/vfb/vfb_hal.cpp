...
static halVFBBase *pHalCFB = NULL;
...
halVFBBase*
halVFB::
getInstance(MBOOL IsVFB)
{
    MY_LOGD("[halVFB] getInstance +\n");
    if(IsVFB == true){
        if (pHalVFB == NULL) {
            MY_LOGD("[halVFB] new VFB \n");
            pHalVFB = new halVFB();
        }
        return pHalVFB;
    } else {
        if(pHalCFB == NULL){
            MY_LOGD("[halVFB] new CFB \n");
            pHalCFB = new halVFB();
        }
        return pHalCFB;
    }
...
void
halVFB::
destroyInstance(MBOOL IsVFB) 
{
    if(IsVFB == true){
        if (pHalVFB) {
            MY_LOGD("[halVFB] destroyInstance VFB\n");
            delete pHalVFB;
            pHalVFB = NULL;
        }
    } else {
        if (pHalCFB) {
            MY_LOGD("[halVFB] destroyInstance CFB\n");
            delete pHalCFB;
            pHalCFB = NULL;
        }
    }
...
#define FORMAT_CONVERSION(fmt) \
    ...
    vFaceBeautyEnvInfo.scenario = (VfbEnvInfo.RecordingHint)? MTK_VIDEO_FACE_BEAUTY_RECORD: MTK_VIDEO_FACE_BEAUTY_PREVIEW;
    if(VfbEnvInfo.scenario == MTK_VIDEO_FACE_BEAUTY_CAPTURE)
    {
        MY_LOGE("[mHalVFBInit] do CFB +\n");
        vFaceBeautyEnvInfo.scenario = MTK_VIDEO_FACE_BEAUTY_CAPTURE;
        vFaceBeautyEnvInfo.FDWidth = VfbEnvInfo.FDWidth;
        vFaceBeautyEnvInfo.FDHeight = VfbEnvInfo.FDHeight;
        MY_LOGE("[mHalVFBInit] do CFB - fd_w = %d fd_h = %d\n",vFaceBeautyEnvInfo.FDWidth,vFaceBeautyEnvInfo.FDHeight);
    }
...
halVFB::mHalVFBProc(MTKPipeVfbProcInfo VfbProcInfo, MTKPipeVfbResultInfo* VfbResultInfo)
{
    ...
    vFaceBeautyProcInfo.pTuningPara->cFB_EYETH = 3;
    vFaceBeautyProcInfo.pTuningPara->Sat_shift = 0;
    vFaceBeautyProcInfo.pTuningPara->GlobalColor = 0;
    ...
    for (int i=0; i<15; i++)
    {
        vFaceBeautyProcInfo.cfb_pos[i][0] = VfbProcInfo.cfb_pos[i][0];
        vFaceBeautyProcInfo.cfb_pos[i][1] = VfbProcInfo.cfb_pos[i][1];
    }
