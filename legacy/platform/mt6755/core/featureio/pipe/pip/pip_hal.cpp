...
#define PIP_SENSOR_SIZE_8M    (PIP_SENSOR_SIZE_16M>>1)
...
*
********************************************************************************/
MBOOL PipHalImp::GetHwMaxFrameRate(MINT32 i4SensorDev, MUINT32 &ZsdHwMaxFrameRate, MUINT32 &NonZsdHwMaxFrameRate, MBOOL isStereo)
{
    ...
    if (i4SensorType == NSCam::SENSOR_TYPE_RAW) // RAW sensor.
    {
        ...
        {
            if (i4SensorSize > PIP_SENSOR_SIZE_16M)   // SensorSize > 16M. PIP only support 16M+8M. So main > 16M will return 0 (indicate error).
            {
            ...
            {
                ZsdHwMaxFrameRate    = (isStereo) ? 24 : 15;
        ...
        {
            if (i4SensorSize > PIP_SENSOR_SIZE_8M)   // SensorSize > 8M. PIP only support 16M+8M. So sub > 8M will return 0 (indicate error).
            {
                ZsdHwMaxFrameRate    = 15;
                NonZsdHwMaxFrameRate = 30;
            }
            else    // 8M >= SensorSize
            {
        ...
        else if (i4SensorDev == SENSOR_DEV_MAIN_2)  // Main2 Cam.
        {
            if (i4SensorSize > PIP_SENSOR_SIZE_8M)
            {
                ZsdHwMaxFrameRate    = 15;
                NonZsdHwMaxFrameRate = 30;
            }
