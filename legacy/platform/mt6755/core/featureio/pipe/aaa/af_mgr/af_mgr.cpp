...
#define AF_TRACE_NAME_LENGTH                 64

#define AF_TRACE_CALL()                      ATRACE_CALL()
#define AF_TRACE_NAME(name)                  ATRACE_NAME(name)
#define AF_TRACE_BEGIN(name)                 ATRACE_BEGIN(name)
#define AF_TRACE_END()                       ATRACE_END()
#define AF_TRACE_FMT_BEGIN(fmt, arg...)                  \
do{                                                       \
    if( ATRACE_ENABLED() )                                \
    {                                                     \
...
#define AF_TRACE_FMT_END()                   AF_TRACE_END()
#else
#define AF_TRACE_CALL()
#define AF_TRACE_NAME(name)
#define AF_TRACE_BEGIN(name)
#define AF_TRACE_END()
#define AF_TRACE_FMT_BEGIN(fmt, arg)
#define AF_TRACE_FMT_END()
#endif
...
MRESULT AfMgr::Hal3AInitState(MINT32 i4SensorIdx, MINT32 isInitMCU)
{
    ...
    if( m_pLaserDrv == NULL && m_i4CurrSensorDev == ESensorDev_Main )
    {
        ...
        if( m_pLaserDrv )
        {
            ...
            {
                m_pLaserDrv = NULL;
                MY_LOG_IF(m_bDebugEnable, "[%s] m_pLaserDrv->init() fail \n", __FUNCTION__);
            }
            else
            {
                MY_LOG("[%s] m_pLaserDrv->init() done \n", __FUNCTION__);
            }
        }
    }
...
MRESULT AfMgr::init(MINT32 i4SensorIdx, MINT32 isInitMCU)
{
    ...
    m_PWrInit = isInitMCU==1 ? MTRUE : MFALSE; /*HAL1 : control in af_mgr*/

    if (m_Users > 0)
    {
        MY_LOG("[AF][%15s] no init, %d has created \n", __FUNCTION__, m_Users);
    ...
    else
    {
        m_i4EnableAF = -1;
        android_atomic_inc(&m_Users);
        MY_LOG("[AF][%15s] EnableAF %d", __FUNCTION__, m_i4EnableAF);
    }

    MY_LOG("[AF][%15s] start, m_Users: %d", __FUNCTION__, m_Users);
...
MRESULT AfMgr::uninit(MINT32 isInitMCU)
{
    ...
    MY_LOG("[AF][%15s] - m_Users: %d", __FUNCTION__, m_Users);
    return S_AF_OK;
}
...
MBOOL AfMgr::Start()
{
    MY_LOG("[AF][%15s] + Dev %d\n", __FUNCTION__, m_i4CurrSensorDev);

    //initial member.
    memset(&m_sAFInput,   0, sizeof(m_sAFInput));
    memset(&m_sAFOutput,   0, sizeof(m_sAFOutput));
    memset(&m_FocsingAFArea, 0, sizeof(m_FocsingAFArea));
    m_vFrmInfo.clear();
    ...
    if (m_i4EnableAF == -1)
    {
        ...
        MY_LOG("Sensor PDAF_Support mode %d", rSensorStaticInfo.PDAF_Support);
        ...
        MY_LOG("[AF][%15s] TG = %d, m_Pix_Id = %d (0:B,1:GB,2:GR,3:R)\n",
               __FUNCTION__,
               m_eSensorTG,
               m_Pix_Id);
    ...
    setAF_IN_HSIZE();
    MY_LOG("[AF][%15s] SensorDev %d [H]%d [V]%d\n", __FUNCTION__, m_i4CurrSensorDev, m_i4AF_in_Hsize,  m_i4AF_in_Vsize);
    ...
    //check ZSD or not
    MUINT32 isZSD = m_i4AF_in_Hsize==(MINT32)rSensorStaticInfo.captureWidth ? TRUE : FALSE;
    ...
    /*0: NO PDAF, 1: PDAF Raw Data mode, 2:PDAF VC mode(HDR), 3:PDAF VC mode(Binning)*/
    case 1:
        m_PDAF_Sensor_Support_Mode = 1;
        pdprofile.uSensorType = 1;
        break;
    case 2 :
        m_PDAF_Sensor_Support_Mode = 2;
    ...
    pdprofile.u4RawBit    = get_raw_bit_depth();
    ...
    // i4Coef[5] == 1:enable SensorListener, else: disable
    {
        ...
        if((m_NVRAM_LENS.rAFNVRAM.i4SensorEnable > 0) && (m_AndroidServiceState==TRUE) && (gpSensorListener==NULL))
        {
            ...
            gListenEnAcce = gpSensorListener->enableSensor(SensorListener::SensorType_Acc, SENSOR_ACCE_POLLING_MS);
            if(!gListenEnAcce)
            {
                gpSensorListener->disableSensor(SensorListener::SensorType_Acc);
            }
            ...
            gListenEnGyro = gpSensorListener->enableSensor(SensorListener::SensorType_Gyro, SENSOR_GYRO_POLLING_MS);
            if(!gListenEnGyro)
            {
                gpSensorListener->disableSensor(SensorListener::SensorType_Gyro);
            }

            gAcceValid = gListenEnAcce;
            gGyroValid = gListenEnGyro;
        }
    }
    ...
    //update non-volatilize information
    ...
    if( m_PWrInit==MFALSE && (m_eLIB3A_AFMode==LIB3A_AF_MODE_AFC || m_eLIB3A_AFMode==LIB3A_AF_MODE_AFC_VIDEO))
    ...
    MY_LOG("AFtbl_num : %d", AFtbl_num);
    if( (m_i4CurrSensorDev == ESensorDev_Main) && (m_i4CurrLensId != 0xffff) )
    {
    ...
    if(m_PDAF_Sensor_Support_Mode!=0)
    {
        ...
        else
        {
            ...
            MY_LOG("[AF][%15s] NVRAM Have PDAF calib data %d", __FUNCTION__, initPdData.rCaliData.i4Size);
        }
        ...
        if( ret==S_3A_OK)
        {
            ...
            MY_LOG("[AF] PD info (Sz=%d) to Hybrid AF after PD_mgr initailed %d %d\n", PDInfoSz, m_sAFInput.i4PDInfo[0], m_sAFInput.i4PDInfo[1]);
        }
        else
        {
            MY_LOG("[AF] Set PD calibration to PD mgr fail, pd buffer type (%d)\n", m_PDBuf_Type);
            m_PDAF_Sensor_Support_Mode=0;
    ...
    else if( mPDVCTest)
    {
        ...
        MY_LOG("[AF] Debug virtual channel only : for engineer testing only!!");
        //allocate virtual channel buffer
        m_AAOPDBuf = NULL;
        m_AAOPDBuf = new BufInfo_T;
    ...
    MY_LOG("[AF][%15s] PD Sensor support mode %d\n", __FUNCTION__, m_PDAF_Sensor_Support_Mode);
    m_FirstTimeAFON=0;
    ...
    if( m_pLaserDrv )
    {
        ...
        if( Disable == 0 )
        {
            m_pLaserDrv->setLaserCalibrationData((MUINT32)m_NVRAM_LENS.rAFNVRAM.i4Coefs[86], (MUINT32)m_NVRAM_LENS.rAFNVRAM.i4Coefs[87]);
        }
        m_pLaserDrv->setLaserGoldenTable((MUINT32 *)&m_NVRAM_LENS.rAFNVRAM.i4Coefs[71]);
        ...
        if(m_pLaserDrv->checkHwSetting() == 0 || Disable == 1)
        {
            m_pLaserDrv->uninit();
            m_pLaserDrv = NULL;
            MY_ERR("m_pLaserDrv->checkHwSetting() fail \n");
        }
    }
...
MBOOL AfMgr::Stop()
{
    ...
    MY_LOG("[AF][%15s] Dev %d, Non-volatilize : af mode (%d)%d, Lens pos %d\n",
           __FUNCTION__,
           m_i4CurrSensorDev,
...
{
    MY_LOG("[AF][%15s] + \n", __FUNCTION__);
    MY_LOG("[AF][%15s] Dev %d, InitMCU to %d", __FUNCTION__, m_i4CurrSensorDev, m_NonValInfo.sLensPos.i4CurrentPos);
    ...
    {
        m_pMcuDrv->moveMCU( m_NonValInfo.sLensPos.i4CurrentPos, m_i4CurrSensorDev);
...
{
    MY_LOG("cmd-%s TG(%d) : %d\n", __FUNCTION__, m_eSensorTG, m_eLIB3A_AFMode);
...
{
    MY_LOG_IF((m_bDebugEnable&1), "cmd-%s TG(%d) : ctl_afmode(%d)\n", __FUNCTION__, m_eSensorTG, eAFMode);
    ...
    MY_LOG_IF((m_bDebugEnable&1), "cmd-%s TG(%d) : (%d)->(%d)\n", __FUNCTION__, m_eSensorTG, preLib3A_AfMode, m_eLIB3A_AFMode);
...
{
    MY_LOG_IF((m_bDebugEnable&1), "[%s][Cnt]%d [L]%d [R]%d [U]%d [B]%d\n", __FUNCTION__, a_sAFArea.u4Count, a_sAFArea.rAreas[0].i4Left, a_sAFArea.rAreas[0].i4Right, a_sAFArea.rAreas[0].i4Top, a_sAFArea.rAreas[0].i4Bottom);
    ...
    if (m_sAFInput.sAFArea.i4Count != 0)
    {
        if( m_pLaserDrv )
        {
            m_sAFInput.sLaserInfo.i4AfWinPosX = ( a_sAFArea.rAreas[0].i4Left + a_sAFArea.rAreas[0].i4Right ) / 2;
            m_sAFInput.sLaserInfo.i4AfWinPosY = ( a_sAFArea.rAreas[0].i4Top + a_sAFArea.rAreas[0].i4Bottom ) / 2;
    ...
    MY_LOG_IF((m_bDebugEnable&1), "[%s][Cnt]%d [L]%d [R]%d [U]%d [B]%d to [Cnt]%d [W]%d [H]%d [X]%d [Y]%d\n", __FUNCTION__, a_sAFArea.u4Count, a_sAFArea.rAreas[0].i4Left, a_sAFArea.rAreas[0].i4Right, a_sAFArea.rAreas[0].i4Top, a_sAFArea.rAreas[0].i4Bottom, m_sAFInput.sAFArea.i4Count, m_sAFInput.sAFArea.sRect[0].i4W, m_sAFInput.sAFArea.sRect[0].i4H, m_sAFInput.sAFArea.sRect[0].i4X, m_sAFInput.sAFArea.sRect[0].i4Y);
...
MRESULT AfMgr::setFDInfo(MVOID* a_sFaces)
{
    ...
    m_FDArea.i4Score=0; /*for algorithm to check input is FD or object, 0:face, 100:object*/
...
MRESULT AfMgr::setOTInfo(MVOID* a_sOT)
{
    ...
    m_tcaf_mode=0;
    m_FDArea.i4Score=pObtinfo->faces->score; /*for algorithm to check input is FD or object, 0:face, 100:object*/
...
{
    MY_LOG_IF( m_bDebugEnable, "cmd-%s TG(%d) : %d\n", __FUNCTION__, m_eSensorTG, a_i4Pos);
    ...
    {
        MY_LOG("cmd-%s skip set MF pos (%d)->(%d), lib_afmode(%d)\n", __FUNCTION__, m_sAFOutput.i4AFPos, a_i4Pos, m_eLIB3A_AFMode);
    }
    return S_AF_OK;
}
...
MINT32 AfMgr::configHybridAFMode()
{
    ...
    MY_LOG_IF( m_bDebugEnable, "[%s] HybridAFMode : %x \n", __FUNCTION__, m_sAFInput.i4HybridAFMode);
...
MRESULT AfMgr::doAF(MVOID *pAFStatBuf)
{
    ...
    //For PDAF window
    memcpy(&m_LastAFArea,&m_sAFOutput.sAFArea.sRect[0],sizeof(AREA_T));
    ...
    if (m_eLIB3A_AFMode == LIB3A_AF_MODE_AFC || m_eLIB3A_AFMode == LIB3A_AF_MODE_AFC_VIDEO)
    {
        ...
        else
        {
            ...
            {
                memcpy(&m_FDArea.sRect[0],&m_sAFOutput.sAFArea.sRect[0], sizeof(AREA_T));
            }
            m_LastAFArea.i4Info = 0;    // monitor
    ...
    if( m_pLaserDrv )
    {
        ...
        //MY_LOG_IF( (m_bDebugEnable&1), "LaserData : %d - %d\n", m_sAFInput.sLaserInfo.i4CurPosDAC, m_sAFInput.sLaserInfo.i4CurPosDist);
        //MY_LOG("LaserData : %d - %d\n", m_sAFInput.sLaserInfo.i4CurPosDAC, m_sAFInput.sLaserInfo.i4CurPosDist);
        ...
        if( (m_eLIB3A_AFMode == LIB3A_AF_MODE_AFS) || (m_eLIB3A_AFMode == LIB3A_AF_MODE_MACRO) ) // 1 : TAF
        {
                    ...
                    m_sAFInput.sLaserInfo.i4AfWinPosCnt > 1 )
            {
                ...
                MY_LOG_IF((m_bDebugEnable&1), "Laser TAF, CONF(%d) DAC(%d)", m_sAFInput.sLaserInfo.i4Confidence, m_sAFInput.sLaserInfo.i4CurPosDAC);
            }
        }
        ...
        if (1 == m_sAFOutput.i4FDDetect)
        {
            m_sAFInput.sLaserInfo.i4Confidence = 20;
            m_sAFInput.sLaserInfo.i4CurPosDAC = 0;
            MY_LOG_IF((m_bDebugEnable&1), "Laser FaceAF, CONF(%d) DAC(%d)", m_sAFInput.sLaserInfo.i4Confidence, m_sAFInput.sLaserInfo.i4CurPosDAC);
        }
        MY_LOG_IF((m_bDebugEnable&1), "LaserInfo : DAC(%d), DIST(%d), CONF(%d)",
                  m_sAFInput.sLaserInfo.i4CurPosDAC, m_sAFInput.sLaserInfo.i4CurPosDist, m_sAFInput.sLaserInfo.i4Confidence);
    }
    ...
    else
    {
        ...
        if(m_i4CurrSensorDev == ESensorDev_Main)
        {
            m_DAF_TBL.daf_vec[i4curFrmNum % DAF_TBL_QLEN].af_dac_index = m_sAFOutput.i4AFPos;
        ...
        if (m_pMcuDrv)
        {
            MY_LOG_IF((m_bDebugEnable&1), "[AFStatH]%lld [AFStatV]%lld [moveMCUTG%d] %d", m_sAFInput.sAFStat.i8Stat24, m_sAFInput.sAFStat.i8StatV, m_eSensorTG,m_sAFOutput.i4AFPos);
        ...
        //call back for continuous af mode.
        if (m_eLIB3A_AFMode == LIB3A_AF_MODE_AFC || m_eLIB3A_AFMode == LIB3A_AF_MODE_AFC_VIDEO)
        {
...
MRESULT AfMgr::doPDTask()
{
...

    {
        ...
        {
            if( (pdWins[i].i4W <= 0)||(pdWins[i].i4H <= 0)||(pdWins[i].i4X < 0)||(pdWins[i].i4Y < 0) )
            {
                MY_LOG("[PDAF] Bypass PDblock %d %d %d %d %d\n",
                       i,
                       pdWins[i].i4X,
                       pdWins[i].i4Y,
                       pdWins[i].i4W,
        ...
        if( m_pdaf_rawbuf)
        {
            AF_TRACE_BEGIN("Prepare");
            //allocate pd input data
            SPDInputData_t PDInput( pdWinNum, m_i4AF_in_Vsize*m_i4AF_in_Hsize*get_raw_bit_depth()/8, reinterpret_cast<MUINT8 *>(m_pdaf_rawbuf));
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MVOID AfMgr::updateAFWinStatus(AREA_T &a_sAFArea)
{
    //compare
    if( a_sAFArea.i4X ==  m_HWAFArea.i4X &&
            a_sAFArea.i4Y ==  m_HWAFArea.i4Y &&
            a_sAFArea.i4W == m_HWAFArea.i4W &&
            a_sAFArea.i4H == m_HWAFArea.i4H )
    {
        if( m_flkwin_synccnt<2) //avoid overflow
            m_flkwin_synccnt++;
    }
    else
    {
    ...
    //record
    memcpy(&m_HWAFArea, &a_sAFArea, sizeof(AREA_T));
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MBOOL AfMgr::isAFWinReady()
{
    return (m_flkwin_synccnt>=2 ? MTRUE : MFALSE);
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MVOID AfMgr::setFlkWinConfig()
{
    ...
    if(m_bDebugAFwinW)
    {
        m_i4AF_in_Hsize=m_bDebugAFwinW;
    }
    if(m_bDebugAFwinH)
    {
        m_i4AF_in_Vsize=m_bDebugAFwinH;
    }
    ...
    // Update configuring HW stauts
    AREA_T a_sAFArea;
    a_sAFArea.i4X = WIN_POS_MIN;
    a_sAFArea.i4Y = WIN_POS_MIN;
    a_sAFArea.i4W = i4WOri-(WIN_POS_MIN*2);
    a_sAFArea.i4H = i4HOri-(WIN_POS_MIN*2);
    updateAFWinStatus( a_sAFArea);
...
MVOID AfMgr::setAFWinConfig(AF_AREA_T a_sAFArea)
{
    ...
    // Record searching ROI.
    //if(m_eLIB3A_AFMode != LIB3A_AF_MODE_AFC || m_eLIB3A_AFMode != LIB3A_AF_MODE_AFC_VIDEO)
        memcpy(&m_FocsingAFArea, &a_sAFArea.sRect[0], sizeof(AREA_T));

    // Update configuring HW stauts
    updateAFWinStatus( a_sAFArea.sRect[0]);
...
MVOID AfMgr::printAFConfigLog0()
{
    ...
    if(m_eSensorTG ==ESensorTG_1)
    {
        ...
        MY_LOG_IF((m_bDebugEnable&1), "DoAFTG1 [XY]%d %d [XY]%d %d [XY]%d %d [XY]%d %d [XY]%d %d [XY]%d %d [Sz]%d %d [G]%d %d %d %d %d %d %d [Hz]%d ",
    ...
    {
        MY_LOG_IF((m_bDebugEnable&1), "DoAFTG2 [XY]%d %d [Sz]%d %d  [G]%d %d %d %d %d %d %d [Hz]%d ",
...
AF_STAT_T AfMgr::Trans4WintoOneStat(MVOID *pAFStatBuf)
{
    ...
    if(m_tcaf_mode && !m_sAFOutput.i4FDDetect && m_FocsingAFArea.i4W!=0 && m_FocsingAFArea.i4H!=0)
    {
        i4posx = m_FocsingAFArea.i4X -m_sAFInput.sEZoom.i4X+ (m_FocsingAFArea.i4W>>1);
        i4posy = m_FocsingAFArea.i4Y -m_sAFInput.sEZoom.i4Y+ (m_FocsingAFArea.i4H>>1);
        if (i4posx > m_FocsingAFArea.i4W/12+FL_WIN_POS_MIN) i4posx-= (m_FocsingAFArea.i4W/12+FL_WIN_POS_MIN);
        else i4posx=0;
        if (i4posy > m_FocsingAFArea.i4H/12+FL_WIN_POS_MIN) i4posy-= (m_FocsingAFArea.i4H/12+FL_WIN_POS_MIN);
        else i4posy=0;
...
MRESULT AfMgr::readOTP()
{
    ...
    if ( 0 < i4MacroPos && i4MacroPos < 1024 && i4MacroPos > i4InfPos )
    {
        ...
        if( i4MacroPos > i450cmPos && i450cmPos > i4InfPos )
        {
            ...
            if(m_NVRAM_LENS.rAFNVRAM.i4Coefs[5] > 0)
            {
...
{
    MY_LOG("cmd-%s TG(%d) : lib_afmode(%d)\n", __FUNCTION__, m_eSensorTG, m_eLIB3A_AFMode);
    ...
    if( m_pLaserDrv ) //calibration flow testing
    {
        int Offset = 0;
        int XTalk = 0;
        ...
        if( Mode == 1 )
        {
            ...
            MY_LOG("LaserCali : getLaserOffsetCalib : %d\n", Offset);
        ...
        if( Mode == 2 )
        {
            ...
            MY_LOG("LaserCali : getLaserXTalkCalib : %d\n", XTalk);
...
MVOID AfMgr::setFocusDistance(MFLOAT lens_focusDistance)
{
    ...
    // 10 cm => dipoter 100
    // 1 cm => dipoter 1000
    MY_LOG_IF( m_bDebugEnable,"cmd-%s TG(%d) : %f\n", __FUNCTION__, m_eSensorTG, lens_focusDistance);
#if 0
    ...
    if( (m_eLIB3A_AFMode == LIB3A_AF_MODE_MF) &&
            ...
            (i4ResultDac<=1023))
    {
        MY_LOG("cmd-%s set lens pos (%d)->(%d) by dis %f\n", __FUNCTION__, m_sAFOutput.i4AFPos, i4ResultDac, lens_focusDistance);
    ...
    {
        MY_LOG("cmd-%s skip set lens pos (%d)->(%d), lib_afmode(%d)\n", __FUNCTION__, m_sAFOutput.i4AFPos, i4ResultDac, m_eLIB3A_AFMode);
    }
#endif
}
...
MFLOAT AfMgr::getFocusDistance   ()
{
    ...
    if (m_i4EnableAF == 0)
    {
        MY_LOG("cmd-%s no focus lens\n", __FUNCTION__, retDist);
        return 0.0;
    }
...
MVOID AfMgr::pauseAF(MBOOL a_bIsPaused)
{
    m_bPauseAF = a_bIsPaused;
}
...
MVOID AfMgr::VsyncSignal()
{
...

    {
        ...
        if( isDMAinited==MFALSE)
        {
            MY_LOG_IF(m_bDebugEnable, "[Dev %d] DMA is not inited", m_i4CurrSensorDev);
            return;
        }
        ...
        if( m_AAOPDBuf)
        {
            ...
            if( ret==MTRUE)
            {
                ...
                AF_TRACE_BEGIN("PostPD");
                ret=IPDMgr::getInstance().postToPDTask( m_i4CurrSensorDev, &PDInput);
                AF_TRACE_END();

                // Enqueue DMA buffer
                AF_TRACE_BEGIN("enqueue");
                ret = IAEBufMgr::getInstance().enqueueHwBuf( m_i4CurrSensorDev, rBufInfo, _PD_BUF_SEL_);
                AF_TRACE_END();

                // Update DMA base address for next frame
                AF_TRACE_BEGIN("update");
                ret = IAEBufMgr::getInstance().updateDMABaseAddr( m_i4CurrSensorDev, _PD_BUF_SEL_);
                AF_TRACE_END();
            }
            else
