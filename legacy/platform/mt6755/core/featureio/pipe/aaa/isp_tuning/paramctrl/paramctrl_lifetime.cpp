...
Paramctrl(
    ...
    , m_pCcmMgr(CcmMgr::createInstance(eSensorDev, m_rIspParam.ISPRegs, m_rIspParam.ISPMulitCCM, m_rIspParam.MDPMulitCCM, m_pIspTuningCustom))
    ...
    , m_pAaaTimer(MNULL)
    , m_i4FlashOnOff(0)
    , mbFlagDumpNvramMapping(MFALSE)
    , m_IspInterpCtrl(1)
{
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("debug.lsc_mgr.type", value, "2");
    int type = atoi(value);
    m_pLscMgr = LscMgr2::createInstance(eSensorDev, i4SensorIdx, eSensorTG, m_rIspParam, type);
    ...
    MY_LOG("[%s ctor] eSensorDev(%d), env(%p), m_pGmaMgr(%d)", __FUNCTION__, eSensorDev, m_pIspTuningCustom->get_custom_GMA_env_info(eSensorDev), m_pGmaMgr);
