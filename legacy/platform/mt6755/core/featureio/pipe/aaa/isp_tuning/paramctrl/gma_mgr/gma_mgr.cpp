    ...
    , mLogEn (0)
{
    ...
    for (int tb_num = 0; tb_num<GMA_TBL_NUM; tb_num++)
    {
        ...
        {
            mEnv.rNVRAMGmaParam.i4EncNVRAMGMA[tb_num][i] = m_rIspNvramReg.GGM[tb_num].lut_g.lut[i].GGM_G;
...
*/
    ...
    if (mAlgoEn)
    {
        ...
        if (mLogEn)
        {
            MY_LOG("%s: GMA EXIF:  %x/%x; %x/%x/%x/%x/%x; %x/%x/%x/%x/%x; %x/%x/%x/%x/%x; %x/%x/%x/%x/%x; %x/%x/%x/%x/%x; %x/%x/%x/%x/%x;"
                , __FUNCTION__
