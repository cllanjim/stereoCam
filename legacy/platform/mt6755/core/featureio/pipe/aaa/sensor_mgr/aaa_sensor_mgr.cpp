...
init(MINT32 const i4SensorIdx)
{
    ...
    m_pHwSyncDrvObj=HWSyncDrv::createInstance();
    if(m_pHwSyncDrvObj==NULL)
    {
        MY_ERR("[AAA Sensor Mgr] Can not create hwsync driver obj\n");
    }
    m_pHwSyncDrvObj->init(HW_SYNC_USER_AE, 0);
    android_atomic_inc(&m_Users);
...
setCaptureParams(MINT32 i4SensorDev, MUINT32 a_u4ExpTime, MUINT32 a_u4SensorGain)
{
    ...
    if(i4SensorDev == ESensorDev_Main) {
        ret = m_pHwSyncDrvObj->sendCommand(HW_SYNC_CMD_DISABLE_SYNC,NSCam::SENSOR_DEV_MAIN,0x0,0x0);
        ret_shutter = setSensorExpTime(NSCam::SENSOR_DEV_MAIN, a_u4ExpTime);
        ret_gain = setSensorGain(NSCam::SENSOR_DEV_MAIN, a_u4SensorGain);
        ret = m_pHwSyncDrvObj->sendCommand(HW_SYNC_CMD_ENABLE_SYNC,NSCam::SENSOR_DEV_MAIN,0x0,0x0);
    } else if(i4SensorDev == ESensorDev_Sub) {
        ret_shutter = setSensorExpTime(NSCam::SENSOR_DEV_SUB, a_u4ExpTime);
        ret_gain = setSensorGain(NSCam::SENSOR_DEV_SUB, a_u4SensorGain);
    } else if(i4SensorDev == ESensorDev_MainSecond) {
        ret = m_pHwSyncDrvObj->sendCommand(HW_SYNC_CMD_DISABLE_SYNC,NSCam::SENSOR_DEV_MAIN_2,0x0,0x0);
        ret_shutter = setSensorExpTime(NSCam::SENSOR_DEV_MAIN_2, a_u4ExpTime);
        ret_gain = setSensorGain(NSCam::SENSOR_DEV_MAIN_2, a_u4SensorGain);
        ret = m_pHwSyncDrvObj->sendCommand(HW_SYNC_CMD_ENABLE_SYNC,NSCam::SENSOR_DEV_MAIN_2,0x0,0x0);
    } else {
        MY_ERR("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;    
    }
...
MRESULT
AAASensorMgr::
set2ShutterControl(MINT32 i4SensorDev, MUINT32 a_u4LEExpTime, MUINT32 a_u4SEExpTime)
{
    MINT32 ret = S_AAA_SENSOR_MGR_OK;
    ...
    if ((a_u4LEExpTime == 0) || (a_u4SEExpTime == 0)){
        MY_ERR("setiVHDRParams() error Id:%d Shutter:%d %d \n", i4SensorDev, a_u4LEExpTime, a_u4SEExpTime);
        return MHAL_INVALID_PARA;
    }

    // Set exposure time
    if(i4SensorDev == ESensorDev_Main) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN, SENSOR_CMD_SET_SENSOR_HDR_SHUTTER, (MUINTPTR)&a_u4LEExpTime, (MUINTPTR)&a_u4SEExpTime, 0);            
    } else if(i4SensorDev == ESensorDev_Sub) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_SUB, SENSOR_CMD_SET_SENSOR_HDR_SHUTTER, (MUINTPTR)&a_u4LEExpTime, (MUINTPTR)&a_u4SEExpTime, 0);            
    } else if(i4SensorDev == ESensorDev_MainSecond) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_2, SENSOR_CMD_SET_SENSOR_HDR_SHUTTER, (MUINTPTR)&a_u4LEExpTime, (MUINTPTR)&a_u4SEExpTime, 0);            
    } else {
        MY_ERR("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;    
    }
    ...
    if(ret) {
        MY_ERR("Err CMD_SENSOR_SET_EXP_TIME, Sensor dev:%d Shutter:%d %d \n", i4SensorDev, a_u4LEExpTime, a_u4SEExpTime);
    }
