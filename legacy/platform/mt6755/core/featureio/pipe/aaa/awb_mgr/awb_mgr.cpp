...
MBOOL AwbMgr::setRgbGain(MINT32 const i4SensorDev, AWB_GAIN_T& rIspAWBGain)
{
    ...
    if((propR!=-1) && (propG!=-1) && (propB!=-1))
    {
      ...
      rIspAWBGain.i4B = propB;
    }
    MY_LOG("set RGB gain %d %d %d",rIspAWBGain.i4R, rIspAWBGain.i4G, rIspAWBGain.i4B);
    ...
    int awbTestSensorValue;
    awbTestSensorValue = getPropInt("z.awb_test_sensor",0);
    if(awbTestSensorValue==0)
    {
      MY_LOG("set sensor RGB gain %d",__LINE__);
      //AAASensorMgr::getInstance().setSensorAWBGain((ESensorDev_T)i4SensorDev, &rSensorAWBGain);
    }
    else
    {
      MY_LOG("no set sensor RGB gain",__LINE__);
...
MBOOL AwbMgr::cameraPreviewInit(MINT32 i4SensorIdx, Param_T &rParam)
{
    ...
    setRgbGain(m_eSensorDev, m_rAWBOutput.rPreviewAWBGain);
...
MRESULT AwbMgr::doPvAWB(MINT32 i4FrameCount, MBOOL bAEStable, MINT32 i4SceneLV, MVOID *pAWBStatBuf, MBOOL bApplyToHW)
{
        ...
        (m_eAWBMode != LIB3A_AWB_MODE_OFF))
    {
        ...
        else if ((m_bStrobeModeChanged) && (!m_bInitState))
        {
            ...
            /*
            *   0 ~ 255: flare application R/G/B 0 ~ 32
            *   256 ~ 511: RGB sum histogram
            *   512 ~ 767: Y histogram
            *   768 ~ 1023: Center Y histogram
            */
            rAWBInput.pAEHistBuf = &((UINT8*)pAWBStatBuf)[m_rAWBInitInput.rAWBStatParam.i4WindowNumX*m_rAWBInitInput.rAWBStatParam.i4WindowNumY*5];
            #if 0
                m_rAWBInitInput.rAWBStatParam.i4WindowNumX;
                m_rAWBInitInput.rAWBStatParam.i4WindowNumY;

            int i;
            UINT16* p;
             p = (UINT16*)rAWBInput.pAEHistBuf;
             int sum=0;
            for(i=0;i<128;i++)
              sum+=p[i];
            MY_LOG("qqsum = %d",sum);
            ...
            if (m_i4StrobeMode == AWB_STROBE_MODE_ON) {
                ...
                if (bApplyToHW)
                {
                    ...
                    setRgbGain(m_eSensorDev, m_rAWBOutput.rPreviewStrobeAWBGain);
            }
            }
            else {
                // update AE RAW pre-gain2
    ...
    if (m_bEnableAWB && (((!m_bAWBLock) && (!m_bAdbAWBLock)) || (m_i4StrobeMode == AWB_STROBE_MODE_ON)) && (m_eAWBMode != LIB3A_AWB_MODE_OFF))
    {
        ...
        */
        ...
        if (bApplyToHW)
        {
            ...
            setRgbGain(m_eSensorDev, m_rAWBOutput.rCaptureAWBGain);
        }
...
MBOOL AwbMgr::applyAWB(AWB_SYNC_OUTPUT_N3D_T &rAWBSyncOutput)
{
    ...
    setRgbGain(m_eSensorDev, m_rAWBOutput.rAWBInfo.rCurrentAWBGain);
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MBOOL AwbMgr::getAWBGain(AWB_GAIN_T& rAwbGain) const
{
    MY_LOG("%s()\n", __FUNCTION__);
...
MBOOL AwbMgr::AWBWindowConfig()
{
	...
	{
		    if(m_bHBIN2Enable)
		        m_rAWBWindowConfig.i4SizeX[sensorMode] = (m_rAWBWindowConfig.i4PitchX[sensorMode] / 4) * 4; // even number
	        else
...
static int getSensorID(CAMERA_DUAL_CAMERA_SENSOR_ENUM i4SensorDev, int& sensorId)
{
    ...
    default:    //  Shouldn't happen.
        MY_ERR("Merror: Invalid sensor device: %d", i4SensorDev);
        break;
        // return MFALSE;
    }
    MY_LOG("ln=%d %d", __LINE__, rSensorStaticInfo.sensorDevID);
    sensorId=rSensorStaticInfo.sensorDevID;

    if(sensorId==0)
    {
        MY_ERR("merror sensorId=0 (should not happen)");
        return -1;
    }
    return 0;
}
...
MBOOL AwbMgr::AWBInit(Param_T &rParam)
{
    ...
    int senId;
    getSensorID((CAMERA_DUAL_CAMERA_SENSOR_ENUM)m_eSensorDev, senId);
    AWB_PARAM2_T awbp2;
    awbp2 = getAWBParam2(senId);
    memcpy(&m_rAWBInitInput.rAWBNVRAM.rAlgoTuningParam.rLightPrefGain, &awbp2.rLightPrefGain, sizeof(AWB_PARAM2_T));
