...
Hal3A::Hal3A()
    ...
    , m_bKeepLastStatus(MFALSE)
    , m_3AMagicNumber(0)
    , m_3ATimeOutEnable(MFALSE)
    , mbAFUninit(MFALSE)
{
...
init(MINT32 i4SensorDevId, MINT32 i4SensorOpenIndex)
{
    ...
    mpAfStateMgr = new AfStateMgr(i4SensorDevId);
    ...
    MY_LOG("[%s] SensorDev(%d), PDAF_Support(%d)\n", __FUNCTION__, m_i4SensorDev, sensorStaticInfo.PDAF_Support);

    //mbEnPDThd = MTRUE;
    if(sensorStaticInfo.PDAF_Support==2 || sensorStaticInfo.PDAF_Support==3)
    {
    mbEnPDVCThd = MTRUE;
    }
    else
...
if (ENABLE_AF & m_3ACtrlEnable) {
    IAfMgr::getInstance().Hal3AInitState(m_i4SensorDev, m_i4SensorOpenIdx);
...
if (ENABLE_3A_GENERAL & m_3ACtrlEnable) {
if (ENABLE_AF & m_3ACtrlEnable) {
    IAfMgr::getInstance().Hal3AUninitState(m_i4SensorDev);
}
if (ENABLE_FLICKER & m_3ACtrlEnable) {
...
MBOOL Hal3A::postCommand(ECmd_T const eCmd, MINTPTR const i4Arg)
{
    ...
    if ((eCmd == ECmd_CameraPreviewEnd))
    {
        ...
        //Flush AF Irq
        {
            ...
            {
                AFFlushCnt++;
                MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "3A PreviewEnd, before flushAFirq");
                flushAFirq();
                MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "3A PreviewEnd, after flushAFirq");
                ::usleep(3000);//3ms
            }
            MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "3A PreviewEnd, after flushAFirq while");
        }
        ...
        mbEnAESenThd = MFALSE;
        {
            ...
            {
                MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "3A PreviewEnd, before flushVSirq");
                flushVSirq();
                MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "3A PreviewEnd, after flushVSirq");
                ::usleep(3000);//3ms
            }
            MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "3A PreviewEnd, after flushVSirq while");
        }
		...
		if(!m_3ATimeOutEnable)
            ::sem_wait(&semPDVCThdEnd); //break wait when it become 1, i.e., AF thread finish and post
        else
        {
            MBOOL bRet = sem_wait_relativeTime(&semPDVCThdEnd, SEM_TIME_LIMIT_NS, "postCommand::semPDVCThdEnd");
            if (!bRet) AEE_ASSERT_3A_HAL("sem_wait_relativeTime(semPDVCThdEnd).");
        }


        MY_LOG("3A PreviewEnd");
...
if (ENABLE_ISPTUNE & m_3ACtrlEnable) {
        ...
        if (!mbInCaptureProcess)
            if (mpScheduler->jobAssignAndTimerStart(E_Job_Af))
            {
                ...
                ::sem_getvalue(&mSemAFThreadstart, &Val);
                MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "[%s] post mSemAFThreadstart start Val(%d)", __FUNCTION__, Val);

                ::sem_post(&mSemAFThreadstart);

                ::sem_getvalue(&mSemAFThreadstart, &Val);
                MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "[%s] post mSemAFThreadstart done Val(%d)",__FUNCTION__, Val);
            }
        mAFMtxTimer.End();
    ...
    switch(eCmd)
    {
        case ECmd_Init:
        case ECmd_Uninit:
            mpAfStateMgr->sendCmd(eCmd);
            break;
        case ECmd_CameraPreviewStart:
            mpAfStateMgr->sendCmd(eCmd);
            EnableAFThread(MTRUE);
            break;
        case ECmd_CameraPreviewEnd:
            EnableAFThread(MFALSE);
            mpAfStateMgr->sendCmd(eCmd);
            break;
    }
...
if (ENABLE_AE & m_3ACtrlEnable) {
   ...
   // AE manual control
   ERROR_CHECK(IAeMgr::getInstance().setShutterValue(m_i4SensorDev,rNewParam.i4ShutterValue))
...
if (ENABLE_AF & m_3ACtrlEnable) {
   ...
   if (rNewParam.u4AfMode == AF_MODE_AFC ||
       rNewParam.u4AfMode == AF_MODE_AFC_VIDEO)
   {
       EState_T eCurrState = mpAfStateMgr->getStateStatus().eCurrState;
       if(eCurrState == eState_TAF)
           mpAfStateMgr->transitState(eCurrState,eState_CAF);
   } else
   {
       EState_T eCurrState = mpAfStateMgr->getStateStatus().eCurrState;
       if(eCurrState == eState_CAF)
           mpAfStateMgr->transitState(eCurrState,eState_TAF);
   }
...
if (ENABLE_AF & m_3ACtrlEnable) {
    mpAfStateMgr->sendCmd(ECmd_AFStart);
}
...
if (ENABLE_AF & m_3ACtrlEnable) {
    mpAfStateMgr->sendCmd(ECmd_AFEnd);
}
...
MBOOL Hal3A::setIspProfile(const ParamIspProfile_T& rParamIspProfile, IDbgInfoContainer* pDbgInfoCtnr)
{
    ...
    if (rParamIspProfile.pOutDngInfo)
    {
        MBOOL fgPureRaw =
            (rParamIspProfile.eIspProfile == EIspProfile_PureRAW_Capture)||
            (rParamIspProfile.eIspProfile == EIspProfile_PureRAW_Capture_SWNR)
            ? MTRUE : MFALSE;
        getDNGInfo(fgPureRaw, *rParamIspProfile.pOutDngInfo);
    }
...
if (ENABLE_3A_GENERAL & m_3ACtrlEnable) {
    ...
    LscMgr2::getInstance(static_cast<ESensorDev_T>(m_i4SensorDev))->setLock(bIsAELimiter);
}
    return S_3A_OK;
}
...
if (ENABLE_AF & m_3ACtrlEnable) {
    if (a_bEnable)  {
        ...
        if (mbAFThreadLoop== 0)
        {
            ...
            int Val;
            ::sem_getvalue(&mSemAFThreadstart, &Val);
            MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "[%s] sem_init mSemAFThreadstart start Val(%d)", __FUNCTION__, Val);

            sem_init(&mSemAFThreadstart, 0, 0);

            ::sem_getvalue(&mSemAFThreadstart, &Val);
            MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "[%s] sem_init mSemAFThreadstart done Val(%d)", __FUNCTION__, Val);
        ...
        if (mbAFThreadLoop == 1)
        {
            ...
            int Val;
            ::sem_getvalue(&mSemAFThreadstart, &Val);
            MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "[%s] sem_post mSemAFThreadstart start Val(%d)", __FUNCTION__, Val);

            ::sem_post(&mSemAFThreadstart);

            ::sem_getvalue(&mSemAFThreadstart, &Val);
            MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "[%s] sem_post mSemAFThreadstart done Val(%d)", __FUNCTION__, Val);
...
if (ENABLE_AF & _this->m_3ACtrlEnable) {
    ...
    while (_this->mbAFThreadLoop) {
        ...
        if (_this->mbAFThreadLoop != 0)
        {
            ...
            if ( _this->mpIspDrv_forAF && (_this->mpIspDrv_forAF->waitIrq(&waitIrq) > 0) ) // success
            {
...
MVOID*
Hal3A::PDVCThreadLoop(MVOID *arg)
{
    Hal3A *_this = reinterpret_cast<Hal3A*>(arg);
    // (1) change thread setting
    _this->changePDVCThreadSetting();

    // (2) thread-in-loop
    while(1)
	{
	    ::sem_wait(&_this->semPDVC);
		if ( ! _this->mbEnPDVCThd)
        {
            MY_LOG_IF(1, "[PDVCThreadLoop] Close, dev:%d\n", _this->m_i4SensorDev);
            break;
        }

		MY_LOG_IF(1, "[PDVCThreadLoop] PD VC task, dev:%d\n", _this->m_i4SensorDev);
		//

		IAfMgr::getInstance().VsyncSignal(_this->m_i4SensorDev);

        //
		//sync with 3A thread when PreviewEnd
		{
			int Val;
			MY_LOG_IF(_this->m_3ALogEnable & EN_3A_FLOW_LOG, "[Hal3A::PDVCThreadLoop] start waiting mPDMtx lock (3)\n");
			Mutex::Autolock autoLock(_this->mPDVCMtx);
			MY_LOG_IF(_this->m_3ALogEnable & EN_3A_FLOW_LOG, "[Hal3A::PDVCThreadLoop] get mPDMtx lock (3)\n");

			::sem_getvalue(&_this->semPDVCThdEnd, &Val);
			MY_LOG_IF(1, "[PDVCThreadLoop] semPDVCThdEnd before post = %d\n", Val);
			if (Val == 0) ::sem_post(&_this->semPDVCThdEnd); //to be 1, 1 means PD task done
		}
	}
	return NULL;
}
...
Hal3A::waitVSirq()
{
    ...
    ISP_DRV_WAIT_IRQ_STRUCT waitIrq;
    waitIrq.Clear=ISP_DRV_IRQ_CLEAR_WAIT;
    waitIrq.Timeout = 10000; // 10000 msec for supporting manual long exposure time control
    waitIrq.Type = (m_TgInfo == CAM_TG_1) ?
        ISP_DRV_IRQ_TYPE_INT_P1_ST : ISP_DRV_IRQ_TYPE_INT_P1_ST_D;
    waitIrq.Status = (m_TgInfo == CAM_TG_1) ?
        CAM_CTL_INT_P1_STATUS_VS1_INT_ST : CAM_CTL_INT_P1_STATUS_D_VS1_INT_ST;
    ...
    {
        MY_ERR("isp drv = NULL");
    }else if (mpIspDrv->waitIrq(&waitIrq) <= 0)
    {
...
MVOID Hal3A::querySensorStaticInfo()
{
    //Before phone boot up (before opening camera), we can query IHalsensor for the sensor static info (EX: MONO or Bayer)
    SensorStaticInfo sensorStaticInfo;
    IHalSensorList*const pHalSensorList = IHalSensorList::get();
    ...
    MUINT32 u4RawFmtType = sensorStaticInfo.rawFmtType; // SENSOR_RAW_MONO or SENSOR_RAW_Bayer or SENSOR_RAW_RWB

    MY_LOG("[%s] SensorDev(%d), SensorOpenIdx(%d), rawFmtType(%d)\n", __FUNCTION__, m_i4SensorDev, m_i4SensorOpenIdx, u4RawFmtType);
...
MRESULT Hal3A::queryTGInfoFromSensorHal()
{
    ...
    IAeMgr::getInstance().setTGInfo(m_i4SensorDev, m_TgInfo);
...
send3ACtrl(E3ACtrl_T e3ACtrl, MINTPTR iArg1, MINTPTR iArg2)
{
    ...
    case E3ACtrl_QueryAFStatus:
        ...
        break;
    case E3ACtrl_SetAFPause:
        IAfMgr::getInstance().pauseAF(m_i4SensorDev, (iArg1 ? MTRUE : MFALSE));
        break;
    // --------------------------------- PDAF ---------------------------------------
    ...
    case E3ACtrl_Enable3ASetParams:
        ...
        IspTuningMgr::getInstance().enableDynamicShading(m_i4SensorDev, iArg1);
        break;
    case E3ACtrl_KeepOnLastStatus:
        m_bKeepLastStatus = MTRUE;
        break;
    case E3ACtrl_GetCapPLineTable:
        IAeMgr::getInstance().getAECapPlineTable(m_i4SensorDev, (MINT32 *)iArg1,  (*reinterpret_cast<strAETable*>(iArg2)));
        break;
    ...
    if (s == -1)
    {
        if (errno == ETIMEDOUT)
        {
            if(!isWaitVsync)
                ...
                MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "[%s][%s]AE_sem_timedwait finish\n", __FUNCTION__, info);
            return MFALSE;
        }
        else
...
getStereo3DWarning()
{
    ...
    if (ISync3AMgr::getInstance()->isActive())
    {
        ...
        MY_LOG_IF(m_3ALogEnable & EN_3A_STEREO_LOG,
            "[%s] lv(%d,%d), fd(%d,%d,%d)", __FUNCTION__, i4LvMaster, i4LvSlave, i4FocusDistance, i4MaxFocusDistance, i4MinFocusDistance);
    }
    return i4Warninig;
}
...
MRESULT
Hal3A::
getDNGInfo(MBOOL fgPureRaw, DNGInfo_T& rDngInfo) const
{
...
#define OB_TO_INT(a) ((-(TO_INT((a), 13)))/4) /*10bit*/
    ...
    IHalSensor* pIHalSensor = pIHalSensorList->createSensor(LOG_TAG, m_i4SensorOpenIdx);
    pIHalSensorList->querySensorStaticInfo(m_i4SensorDev, &rSensorStaticInfo);

    // static
    // Active Array
    rDngInfo.i4SensorActiveArray[0] = 0;                                        // Start x
    rDngInfo.i4SensorActiveArray[1] = 0;                                        // Start Y
    rDngInfo.i4SensorActiveArray[2] = rSensorStaticInfo.captureActiveWidth;     // Width
    rDngInfo.i4SensorActiveArray[3] = rSensorStaticInfo.captureActiveHeight;    // Hight
    // CFA
    switch (rSensorStaticInfo.sensorFormatOrder)
    {
    case SENSOR_FORMAT_ORDER_RAW_B:
        rDngInfo.u1SensorInfoColorFilterArrangement = 3;
        break;
    case SENSOR_FORMAT_ORDER_RAW_Gb:
        rDngInfo.u1SensorInfoColorFilterArrangement = 2;
        break;
    case SENSOR_FORMAT_ORDER_RAW_Gr:
        rDngInfo.u1SensorInfoColorFilterArrangement = 1;
        break;
    default:
    case SENSOR_FORMAT_ORDER_RAW_R:
        rDngInfo.u1SensorInfoColorFilterArrangement = 0;
        break;
    }

    // white level (10bit)
    rDngInfo.i4SensorInfoWhiteLevel = 1023;

    // black level
    if (fgPureRaw)
    {
        ISP_NVRAM_OBC_T obc;
        IspTuningMgr::getInstance().getPureOBCInfo(m_i4SensorDev, &obc);
        switch (rSensorStaticInfo.sensorFormatOrder)
        {
        case SENSOR_FORMAT_ORDER_RAW_B:
            rDngInfo.i4SensorBlackLevelPattern[3] = OB_TO_INT(obc.offst3.bits.OBC_OFST_R);
            rDngInfo.i4SensorBlackLevelPattern[2] = OB_TO_INT(obc.offst1.bits.OBC_OFST_GR);
            rDngInfo.i4SensorBlackLevelPattern[1] = OB_TO_INT(obc.offst2.bits.OBC_OFST_GB);
            rDngInfo.i4SensorBlackLevelPattern[0] = OB_TO_INT(obc.offst0.bits.OBC_OFST_B);
            break;
        case SENSOR_FORMAT_ORDER_RAW_Gb:
            rDngInfo.i4SensorBlackLevelPattern[2] = OB_TO_INT(obc.offst3.bits.OBC_OFST_R);
            rDngInfo.i4SensorBlackLevelPattern[3] = OB_TO_INT(obc.offst1.bits.OBC_OFST_GR);
            rDngInfo.i4SensorBlackLevelPattern[0] = OB_TO_INT(obc.offst2.bits.OBC_OFST_GB);
            rDngInfo.i4SensorBlackLevelPattern[1] = OB_TO_INT(obc.offst0.bits.OBC_OFST_B);
            break;
        case SENSOR_FORMAT_ORDER_RAW_Gr:
            rDngInfo.i4SensorBlackLevelPattern[1] = OB_TO_INT(obc.offst3.bits.OBC_OFST_R);
            rDngInfo.i4SensorBlackLevelPattern[0] = OB_TO_INT(obc.offst1.bits.OBC_OFST_GR);
            rDngInfo.i4SensorBlackLevelPattern[3] = OB_TO_INT(obc.offst2.bits.OBC_OFST_GB);
            rDngInfo.i4SensorBlackLevelPattern[2] = OB_TO_INT(obc.offst0.bits.OBC_OFST_B);
            break;
        default:
        case SENSOR_FORMAT_ORDER_RAW_R:
            rDngInfo.i4SensorBlackLevelPattern[0] = OB_TO_INT(obc.offst3.bits.OBC_OFST_R);
            rDngInfo.i4SensorBlackLevelPattern[1] = OB_TO_INT(obc.offst1.bits.OBC_OFST_GR);
            rDngInfo.i4SensorBlackLevelPattern[2] = OB_TO_INT(obc.offst2.bits.OBC_OFST_GB);
            rDngInfo.i4SensorBlackLevelPattern[3] = OB_TO_INT(obc.offst0.bits.OBC_OFST_B);
            break;
        }
    ...
    {
        rDngInfo.i4SensorBlackLevelPattern[0] = 0;
        rDngInfo.i4SensorBlackLevelPattern[1] = 0;
        rDngInfo.i4SensorBlackLevelPattern[2] = 0;
        rDngInfo.i4SensorBlackLevelPattern[3] = 0;
    }

    MY_LOG("[%s] fgPureRaw(%d), CFA(%d), OB(%d)", __FUNCTION__,
        fgPureRaw, rDngInfo.u1SensorInfoColorFilterArrangement, rDngInfo.i4SensorBlackLevelPattern[0]);

    // color space
    MINT32 i, j;
    double cm1[3][3], fm1[3][3];
    IspTuningMgr::getInstance().getDNGColorMatrix1(m_i4SensorDev, cm1);
    IspTuningMgr::getInstance().getDNGForwardMatrix1(m_i4SensorDev, fm1);
    rDngInfo.u1SensorReferenceIlluminant1 = MTK_SENSOR_REFERENCE_ILLUMINANT1_D65;
    rDngInfo.i4TransformDenominator = 512;
    for (i = 0; i < 3; i++)
    {
        for (j = 0; j < 3; j++)
        {
            rDngInfo.i4SensorColorTransform1[i*3+j] = cm1[i][j]*512;
            rDngInfo.i4SensorForwardTransform1[i*3+j] = fm1[i][j]*512;
            rDngInfo.i4SensorCalibrationTransform1[i*3+j] = ((i == j) ? 512 : 0);
        }
    }
    ...
    AE_MODE_CFG_T rCaptureInfo;
    IAeMgr::getInstance().getCaptureParams(m_i4SensorDev, rCaptureInfo);
    rDngInfo.i8SensorExposureTime = (MINT64) rCaptureInfo.u4Eposuretime * 1000L;
    rDngInfo.i4SensorSensitivity = rCaptureInfo.u4RealISO;

    // neutral color point (take the inverse of awb gain)
    AWB_GAIN_T rAwbGain;
    IAwbMgr::getInstance().getAWBGain(m_i4SensorDev, rAwbGain);
    rDngInfo.i4SensorNeutralColorPointDenominator[0] = rAwbGain.i4R;
    rDngInfo.i4SensorNeutralColorPointDenominator[1] = rAwbGain.i4G;
    rDngInfo.i4SensorNeutralColorPointDenominator[2] = rAwbGain.i4B;
    rDngInfo.i4SensorNeutralColorPointNumerator[0] = 512;
    rDngInfo.i4SensorNeutralColorPointNumerator[1] = 512;
    rDngInfo.i4SensorNeutralColorPointNumerator[2] = 512;

    // lens
    AE_DEVICES_INFO_T rDeviceInfo;
    IAeMgr::getInstance().getSensorDeviceInfo(m_i4SensorDev, rDeviceInfo);
    rDngInfo.fLensAperture = (MFLOAT) rDeviceInfo.u4LensFno / 10.0f; // Format: F2.8 = 28
    rDngInfo.fLensFocalLength = (MFLOAT) rDeviceInfo.u4FocusLength_100x / 100.0f; // Format: FL 3.5 = 350

    // noise profile
    rDngInfo.dSensorNoiseProfile[0] = 0.00074349;
    rDngInfo.dSensorNoiseProfile[1] = 0.00005293;
    rDngInfo.dSensorNoiseProfile[2] = 0.00074349;
    rDngInfo.dSensorNoiseProfile[3] = 0.00005293;
    rDngInfo.dSensorNoiseProfile[4] = 0.00074349;
    rDngInfo.dSensorNoiseProfile[5] = 0.00005293;
    rDngInfo.dSensorNoiseProfile[6] = 0.00074349;
    rDngInfo.dSensorNoiseProfile[7] = 0.00005293;

    // lens shading map size
    rDngInfo.i4ShadingMapSize[0]=17;
    rDngInfo.i4ShadingMapSize[1]=17;

    // lens shading map
    rDngInfo.vecShadingMap.resize(17*17*4);
    MFLOAT* pfShadingMap = rDngInfo.vecShadingMap.editArray();
    if (fgPureRaw)
    {
        MINT32 i4Result = LscMgr2::getInstance(static_cast<ESensorDev_T>(m_i4SensorDev))->getGainTable(3, 17, 17, pfShadingMap);
        if (0 != i4Result)
        {
            for (MINT32 i = (17*17*4); i != 0; i--)
                *pfShadingMap++ = 1.0f;
        }
        MY_LOG("[%s] Gain Map Result (%d)", __FUNCTION__, i4Result);
    }
    else
    {
        MY_LOG("[%s] Processed RAW, force 1.0", __FUNCTION__);
        for (MINT32 i = (17*17*4); i != 0; i--)
            *pfShadingMap++ = 1.0f;
    }
...
MVOID Hal3A::
getMShutterCaptureParams(CaptureParam_T &a_rCaptureInfo)
{
   if (ENABLE_3A_GENERAL & m_3ACtrlEnable) {

        AE_MODE_CFG_T rCaptureInfo;

        IAeMgr::getInstance().getMShutterCaptureParams(m_i4SensorDev, rCaptureInfo);
...
getShutterValue()
{
    return IAeMgr::getInstance().getShutterValue(m_i4SensorDev);
}
