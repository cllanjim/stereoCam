...
#define EN_LSC_LOG_SHOW_TABLE   (0x1 << 10)
...
public:
    ...
    virtual MBOOL updateCfg(const ISP_NVRAM_LSC_T& rLscCfg) = 0;
    virtual MBOOL genHwTbl(MUINT32 ratio, const MUINT32* pSrc, MUINT32* pDest) = 0;
    virtual MBOOL genGainTbl(MUINT32 ratio, const MUINT32* pSrc, MUINT32* pDest) = 0;
};
...
public:
    ...
    static LscMgr2*         createInstance(ESensorDev_T const eSensorDev,
                                             MINT32 i4SensorIdx,
    ...
    virtual MBOOL           setSensorMode(ESensorMode_T eSensorMode, MBOOL fgForce=MFALSE);
    ...
    // check if double buffer effective
    MBOOL                   checkTblEffective() const;
    ...
    virtual MRESULT         getDebugInfo(SHADING_DEBUG_INFO_T &rShadingDbgInfo);
    ...
    virtual MINT32          setLscRatio(MINT32 i4Rto);
    ...
    virtual MBOOL           getTsfOnOff();
    virtual MBOOL           setLock(MBOOL fgOnOff);
    virtual MBOOL           backup();
    virtual MBOOL           restore();
protected:
    typedef struct
    {
    ...
    MBOOL                   calSl2Config(const SensorCropInfo_T& rCrop, ISP_NVRAM_NSL2_T& rSl2Cfg);
    ...
    MBOOL                   doShadingTrfm();
    MVOID                   logGainTbl(const MUINT8* ptrTbl, MUINT32 u4Size, const char* str);
    ...
    MINT32                  shadingTblCvtGainR(const ShadingTblCropCfg_T& rCropCfg, const ShadingTblInput_T& rInput, MUINT32 u4GridNumX, MUINT32 u4GridNumY, MUINT32* pu4OutputCoefTbl);
    ...
    MVOID                   doThreadFunc();
    virtual MVOID           initThreadFunc();
    virtual MVOID           uninitThreadFunc();
    MBOOL                   loadOtpDataForTsf();
    static MVOID *          threadLoop(void *arg);

    virtual MVOID           updateTsfCfg(UINT32 u4W, UINT32 u4H, const ISP_NVRAM_LSC_T& rLscCfg, ELscScenario_T eLscScn, MBOOL fgForce=MFALSE);

    virtual MBOOL           tsfInit();
    virtual MBOOL           tsfRun();
    virtual MBOOL           tsfBatch();
    virtual MBOOL           tsfBatchCap();
    virtual MBOOL           tsfCfgChg();
    virtual MBOOL           tsfReset(){return MTRUE;}
    virtual MBOOL           tsfResetTbl(ELscScenario_T eLscScn);
    virtual MBOOL           tsfSetProcInfo(MTK_TSF_SET_PROC_INFO_STRUCT& rProcInfo, MBOOL fgDump = MFALSE, const char* filename = NULL);
    virtual MBOOL           tsfPostCmd(E_TSF_CMD_T eCmd);
...
// SW ratio + per-frame
class LscMgr2Rto : public LscMgr2
{
public:
    static LscMgr2Rto*      createInstance(ESensorDev_T const eSensorDev,
                                             MINT32 i4SensorIdx,
                                             ESensorTG_T eSensorTG,
                                             NVRAM_CAMERA_ISP_PARAM_STRUCT& rIspNvram);
...
protected:
    ...
    virtual MBOOL           tsfBatchCap();
    virtual MBOOL           tsfCfgChg();
    virtual MBOOL           tsfReset();
    virtual MBOOL           tsfResetTbl(ELscScenario_T eLscScn);
    virtual MBOOL           tsfSetProcInfo(MTK_TSF_SET_PROC_INFO_STRUCT& rProcInfo, MBOOL fgDump = MFALSE, const char* filename = NULL);
    virtual MBOOL           tsfPostCmd(E_TSF_CMD_T eCmd);

    // ra
    ILscRatio*                      m_pLscRatio;
    ILscRatio*                      m_pTsfRatio;
    MUINT32*                        m_pu4TsfRaInputTbl;
    MUINT32*                        m_pu4RaResultTbl;

    // tsf
    MBOOL                           m_bTSF1stGroup;
    MUINT32*                        m_pu4ResultTblQ[4];
    std::vector<MUINT8>             m_vecBackup;
};

// SW ratio non-per-frame
class LscMgr2RtoCycle : public LscMgr2Rto
{
public:
    static LscMgr2RtoCycle*      createInstance(ESensorDev_T const eSensorDev,
                                             MINT32 i4SensorIdx,
                                             ESensorTG_T eSensorTG,
                                             NVRAM_CAMERA_ISP_PARAM_STRUCT& rIspNvram);
