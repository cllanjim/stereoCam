...
if (ENABLE_AAOBUF & m_pHal3A->m_3ACtrlEnable) {
    ...
    if ((m_pStateMgr->getPrecapState() == ePrecap_Num) && (m_pStateMgr->mAFStateCntSet.bNeedToDoPrecapAF == 0))
    {
        ...
        LscMgr2::getInstance(static_cast<ESensorDev_T>(m_SensorDevId))->backup();
...
sendPrecapIntent(intent2type<eIntent_VsyncUpdate>, state2type<ePrecap_AF_BeforeFlash>)
{
    ...
    {
        MY_LOG("[sendPrecapIntent] To execute Precapture AF before Flash, Timeout(%d)\n", m_pStateMgr->m_i4PrecapTimeOutFrmCnt);
        int isDoAF;
        int hasHw;
        MBOOL isSupportAF;
        MBOOL isTimeout = (m_pStateMgr->m_i4PrecapTimeOutFrmCnt > 30)? MTRUE : MFALSE;
        ...
        if((isDoAF==1 && hasHw==0) || !isSupportAF || isTimeout)
        {
            MY_LOG("no flash hw || isSupportAF(%d) || isTimeout(%d)", isSupportAF, isTimeout);
            m_pStateMgr->proceedPrecapState();
            return S_3A_OK;
        }

        if (IAfMgr::getInstance().IsFullScanMode(m_SensorDevId) == MFALSE)
        {
            m_pStateMgr->m_i4PrecapTimeOutFrmCnt++;
        }
        MBOOL isFocusFinish = IAfMgr::getInstance().isFocusFinish(m_SensorDevId);
        MBOOL isFocused = IAfMgr::getInstance().isFocused(m_SensorDevId);
        ...
        else
        {
            if(!isFocusFinish)
            {
                MY_LOG("[%s] wait for focus finish", __FUNCTION__);
                m_pStateMgr->m_bIsFocuseFinish = MFALSE;
                return S_3A_OK;
            }
            else
            {
                if( !m_pStateMgr->m_bIsFocuseFinish || isFocused )
                {
                    MY_LOG("[%s] m_bIsFocuseFinish = %d, isFocused = %d", __FUNCTION__, isFocusFinish, isFocused);
                    m_pStateMgr->m_bIsFocuseFinish = MTRUE;
                    m_pStateMgr->proceedPrecapState();
                    return S_3A_OK;
                }
                MY_ERR("[%s] incorrect precapture flow", __FUNCTION__);
            }
        }
    }
    else
    ...
    if(CUST_ENABLE_PRECAPTURE_AF() && CUST_PRECAPTURE_AF_AFTER_PREFLASH())
    {
        ...
        else
        {
            ...
            else
            {
                if( !m_pStateMgr->m_bIsFocuseFinish || isFocused )
                {
                    MY_LOG("[%s] m_bIsFocuseFinish = %d, isFocused = %d", __FUNCTION__, m_pStateMgr->m_bIsFocuseFinish, isFocused);
                    m_pStateMgr->m_bIsFocuseFinish = MTRUE;
                    m_pStateMgr->proceedPrecapState();
                    return S_3A_OK;
                }
...
if (ENABLE_AAOBUF & m_pHal3A->m_3ACtrlEnable) {
    ...
    IspTuningMgr::getInstance().sendIspTuningIOCtrl(m_SensorDevId, IspTuningMgr::E_ISPTUNING_SET_GMA_SCENARIO, IspTuningMgr::E_GMA_SCENARIO_CAPTURE, 0);
...
if (ENABLE_FLICKER & m_pHal3A->m_3ACtrlEnable) {
    ...
    MY_LOG("[%s] StatePrecapture::sendIntent(intent2type<eIntent_CameraPreviewEnd>) mbAFUninit(%d)->set to 1",__FUNCTION__,m_pHal3A->mbAFUninit);
    m_pHal3A->mbAFUninit = MTRUE;
