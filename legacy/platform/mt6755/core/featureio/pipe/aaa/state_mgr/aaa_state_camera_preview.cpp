...
if (ENABLE_AAOBUF & m_pHal3A->m_3ACtrlEnable) {
        ...
        IAEBufMgr::getInstance().dequeueHwBuf(m_SensorDevId, rBufInfo, _PD_BUF_SEL_);
        // Enqueue AE DMA buffer
        IAEBufMgr::getInstance().enqueueHwBuf(m_SensorDevId, rBufInfo, _PD_BUF_SEL_);
        // Update AE DMA base address for next frame
        IAEBufMgr::getInstance().updateDMABaseAddr(m_SensorDevId, _PD_BUF_SEL_);
...
if (ENABLE_AE & m_pHal3A->m_3ACtrlEnable) {
    ...
    MY_LOG("[%s] isSlowMotion = %d", __FUNCTION__, isSlowMotionUpdateI2C);
...
if (ENABLE_FLICKER & m_pHal3A->m_3ACtrlEnable) {
...
#ifdef MTK_TC1_FEATURE
    {
        ...
        kk++;
        if(kk%10==0)
            MY_LOG("video flicker en ln=%d",__LINE__);
    }
#else
...
sendIntent(intent2type<eIntent_RecordingStart>)
{
    ...
    bIsRecording = MTRUE;
    m_pStateMgr->m_bIsRecording = MTRUE;
    IspTuningMgr::getInstance().sendIspTuningIOCtrl(m_SensorDevId, IspTuningMgr::E_ISPTUNING_SET_GMA_SCENARIO, IspTuningMgr::E_GMA_SCENARIO_VIDEO, 0);
...
sendIntent(intent2type<eIntent_RecordingEnd>)
{
    ...
    bIsRecording = MFALSE;
    m_pStateMgr->m_bIsRecording = MFALSE;
