...
if (ENABLE_AE & m_pHal3A->m_3ACtrlEnable) {
    // AE : manual control
    if(IAeMgr::getInstance().getShutterValue(m_SensorDevId)){
        CaptureParam_T rCap3AParam;
        m_pHal3A->getMShutterCaptureParams(rCap3AParam);
        m_pHal3A->updateCaptureParams(rCap3AParam);
    }
...
if (ENABLE_AWB & m_pHal3A->m_3ACtrlEnable) {
    ...
    LscMgr2::getInstance(static_cast<ESensorDev_T>(m_SensorDevId))->restore();
...
if (ENABLE_FLASH & m_pHal3A->m_3ACtrlEnable) {
    ...
    if (!bRet) {
        MY_ERR("FlashMgr::getInstance().cameraPreviewStart or videoPreviewStart fail");
        return E_3A_ERR;
    }
}
if (ENABLE_FLICKER & m_pHal3A->m_3ACtrlEnable) {
