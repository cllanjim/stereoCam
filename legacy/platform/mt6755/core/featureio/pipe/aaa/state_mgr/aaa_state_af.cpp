...
if (ENABLE_AFOBUF & m_pHal3A->m_3ACtrlEnable) {
    ...
    IspTuningMgr::getInstance().sendIspTuningIOCtrl(m_SensorDevId, IspTuningMgr::E_ISPTUNING_NOTIFY_STOP, 0, 0);
...
if (ENABLE_AAOBUF & m_pHal3A->m_3ACtrlEnable) {
    ...
    if (!IAEBufMgr::getInstance().uninit(m_SensorDevId)) {
        MY_ERR("IAAEBufMgr::getInstance().uninit() fail");
        return E_3A_ERR;
    }
}
if (ENABLE_AFOBUF & m_pHal3A->m_3ACtrlEnable) {
...
//#endif
    ...
    if ((m_pStateMgr->mAFStateCntSet.PreAF_bNeedToTurnOnLamp == 0) ||
        ...
        IAeMgr::getInstance().doAFAEmonitor(m_SensorDevId, m_pStateMgr->getFrameCount(), reinterpret_cast<MVOID *>(pBuf->virtAddr),
        ...
        MY_LOG("[%s] isNeedUpdateI2C = %d", __FUNCTION__, isNeedUpdateI2C);
        MY_LOG("[%s] isSlowMotionUpdateI2C = %d", __FUNCTION__, isSlowMotionUpdateI2C);

        if(isNeedUpdateI2C || isSlowMotionUpdateI2C)
        {
            MY_LOG("[%s] postToAESenThread : wait to update I2C", __FUNCTION__);
        ...
        if (isNeedUpdateI2C || isSlowMotionUpdateI2C)
        {
            MY_LOG("[%s] postToAESenThread : ready to update I2C", __FUNCTION__);
...
#define AF_TIME_OUT_FRAME_COUNT 30
    ...
    MY_LOG("[StateAF::sendAFIntent](eIntent_VsyncUpdate,eAFState_AF), AFFrmCnt=%d, AFTimeOutFrmCnt=%d"
        , m_pStateMgr->mAFStateCntSet.AFFrmCnt, m_pStateMgr->mAFStateCntSet.AFTimeOutFrmCnt);
...
{m_pStateMgr->proceedAFState(); return S_3A_OK;}
    ...
    bIsSupportAF= IAfMgr::getInstance().getAFMaxAreaNum(m_SensorDevId);
    MY_LOG("[%s] bIsFocusFinish = %d, bIsSupportAF = %d", __FUNCTION__, bIsFocusFinish, bIsSupportAF);
    BufInfo_T* pBuf = reinterpret_cast<BufInfo_T*>(pBufInfo);
    m_pStateMgr->mAFStateCntSet.AFTimeOutFrmCnt++;

    // check time out on normal mode
    MINT32 i4OperMode = IspTuningMgr::getInstance().getOperMode(m_SensorDevId);
    if(i4OperMode == EOperMode_Normal && m_pStateMgr->mAFStateCntSet.AFTimeOutFrmCnt >= AF_TIME_OUT_FRAME_COUNT)
    {
        MY_LOG("[%s] AF Time out", __FUNCTION__);
        bIsTimeOut = MTRUE;
    }
    if (bIsSupportAF && !bIsTimeOut)
    {
