...
Hal3A::createThread()
{
    ...
    sem_init(&semPDVC, 0, 0);
    sem_init(&semPDVCThdEnd, 0, 1);
    pthread_create(&mThread, NULL, onThreadLoop, this);
    //pthread_create(&mPDThread, NULL, PDThreadLoop, this);
    if(mbEnPDVCThd) pthread_create(&mPDVCThread, NULL, PDVCThreadLoop, this);
}
...
MVOID
Hal3A::changePDVCThreadSetting()
{
    // (1) set name
    ::prctl(PR_SET_NAME,"PDVCThd", 0, 0, 0);
    ...
    // (2) set policy/priority
    {
        ...
        //
        MY_LOG(
            "[Hal3A::PDVCThreadLoop] policy:(expect, result)=(%d, %d), priority:(expect, result)=(%d, %d)"
            , expect_policy, policy, expect_priority, priority
        );
    }
...
Hal3A::sendCommand(ECmd_T const r3ACmd, MINTPTR const i4Arg)
{
    ...
    if (r3ACmd == ECmd_CameraPreviewStart)
    {
        ...
        ::sem_getvalue(&semPDVCThdEnd, &Val);
        if (Val == 0) ::sem_post(&semPDVCThdEnd); //to be 1
        MY_LOG_IF(1, "[Hal3A::sendCommand] PreviewStart semPDVCThdEnd(%d) before post", Val);
        //sem_init(&semPDVCThdEnd, 0, 1);
...
Hal3A::AESensorThreadLoop(MVOID *arg)
{
    ...
    while(1)
    {
        ...
        MY_LOG("waitVsync done.");
        ::sem_post(&_this->semPDVC);
        AAA_TRACE_END();
        MY_LOG_IF(_this->m_3ALogEnable & EN_3A_FLOW_LOG, "mbInCapturing = %d", _this->mbInCapturing);
        if((_this->mpStateMgr->getFrameCount() <= (-1)) || _this->mbInCapturing)
        {
            ...
            if (Val == 0)
            {
                MY_LOG_IF(_this->m_3ALogEnable & EN_3A_FLOW_LOG, "post semVsyncFromAE");
                ::sem_post(&_this->semVsyncFromAE); //to be 1
            }
        }
        ...
        if ( ! _this->mbEnAESenThd)
        {
            ...
            MY_LOG_IF(_this->m_3ALogEnable & EN_3A_FLOW_LOG, "[AESensorThreadLoop] mFlushVSIrqDone (1)");
            break;
        }
        ...
        {
            {
                ...
                MY_LOG_IF(_this->m_3ALogEnable & EN_3A_FLOW_LOG, "[%s][reset] semAEI2CSen = %d", __FUNCTION__, Val);

                ::sem_getvalue(&_this->semAESen, &Val);
                if (Val == 1) ::sem_wait(&_this->semAESen);
                MY_LOG_IF(_this->m_3ALogEnable & EN_3A_FLOW_LOG, "[%s][reset] semAESen = %d", __FUNCTION__, Val);
            }
            MY_LOG_IF(_this->m_3ALogEnable & EN_3A_FLOW_LOG, "Timeout : sem_wait_relativeTime(semAEI2CSen).");
            continue;
        }
        ...
        {
            _this->mFlushVSIrqDone = 1; // no need to flush
            MY_LOG_IF(_this->m_3ALogEnable & EN_3A_FLOW_LOG, "[AESensorThreadLoop] mFlushVSIrqDone (2)");
            break;
        }
        ...
        if (!bRet)
        {
            MY_LOG("Timeout : sem_wait_relativeTime(semAESen).");
            continue;
        }
        ...
        {
            _this->mFlushVSIrqDone = 1; // no need to flush
            MY_LOG_IF(_this->m_3ALogEnable & EN_3A_FLOW_LOG, "[AESensorThreadLoop] mFlushVSIrqDone (3)");
            break;
        }
        ...
        {
        MY_LOG_IF(1, "[AESensorThreadLoop] updateSensorbyI2C start\n");
        IAeMgr::getInstance().updateSensorbyI2C(_this->m_i4SensorDev);
        MY_LOG_IF(1, "[AESensorThreadLoop] updateSensorbyI2C end\n");
        }
