...
int FlashPline::aePline2FlashAlgPline(PLine* p, strAETable* pAE, int maxExp, int flickerMode)
{
	...
	*/
    if(flickerMode == 60)
        pAE->pCurrentTable = &pAE->sTable60Hz;
    else
        pAE->pCurrentTable = &pAE->sTable50Hz;
    ...
    for(i=0; i<(int)pAE->u4TotalIndex; i++)
    {
        ...
        else
        {
            ...
            if(exp>maxExp)
            {
                ...
                else if((gainLevel/1024 > maxAfe) && (maxAfe > 0))
                {
                    p->pCurrentTable[i].u4AfeGain = maxAfe;
