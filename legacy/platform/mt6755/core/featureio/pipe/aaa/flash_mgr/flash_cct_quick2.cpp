...
static int setexp(int dev, int exp, int afe, int isp)
{
    logI("setexp ln=%d dev=%d exp:%d afe:%d isp:%d",__LINE__,dev,exp,afe,isp);
    int err;
...
logI("cctCalibration st=%d ln=%d",state, __LINE__);
	...
	if(state==STATE_INIT)
	{
		...
		int calIsp;
		int calAfe;
		calExp = FlashUtil::getPropInt("z.flash_cal_exp",-1);
        calIsp = FlashUtil::getPropInt("z.flash_cal_isp",-1);
		calAfe = FlashUtil::getPropInt("z.flash_cal_afe",-1);
        logI("cctCalibrationQuick2 line=%d cal exp=%d, afe=%d, isp=%d",__LINE__, calExp, calAfe, calIsp);
		if(calExp!=-1 && calIsp!= -1 && calAfe!=-1)
		{
		    g_ExpBase=calExp;
		    g_AfeBase=calAfe;
		    g_IspBase=calIsp;
...
NextId:
		if(m_pfFrameCount==preStateEndCnt+1)
		{
			...
			if(g_dutyArr[curId]==-1 && g_dutyArr[curId]==-1)
    		{
    		    frameTar -= 25;
    		}
    		else if(curId==(int)g_dutyArr.size()-1)
    		{
    		    frameTar -= 25;
    		}
		}
	...
	else if(state==STATE_END)
	{
	    ...
	    //check overexposure and bright environment
	    double gtes=-1;
	    sz = g_CaliData1.size();
		for(j=0;j<sz;j++)
		{
			...
			for(i=0;i<e_CaFrm;i++)
			{
			    if(gtes<g_CaliData1[j].g[i])
			        gtes=g_CaliData1[j].g[i];
			}
			if(g_CaliData1[j].duty==-1 && g_CaliData1[j].dutyLT==-1)
			{
			    if(gtes>10)
			    {
			        g_flashQuickCalibrationResult = E_FLASH_CALI_ENVIRONMENT_NOT_DARK_ERR;
			        break;
                }
			}
			if(gtes>240)
			{
			    g_flashQuickCalibrationResult = E_FLASH_CALI_OVEREXPOSURE_ERR;
			    break;
            }

		}
		if(g_flashQuickCalibrationResult!=S_FLASH_OK)
		    goto END_WITH_ERROR;
		...
		int k;
		int calTestN;
		calTestN= g_flashQuickCalibrationNum;
		int expBase;
		int ispBase;
		int afeBase;
		...
		for (i = 0; i < calTestN; i++)
		{
			for (j = 0; j < g_CaliData1.size(); j++)
			{
					...
					g_algCalData[i].dutyLT == g_CaliData1[j].dutyLT)
				{
					...
					double fac;
					logI("ln%d fac from %d %d %d %d %d %d\n",__LINE__,expBase, afeBase, ispBase , g_CaliData1[j].exp , g_CaliData1[j].afe , g_CaliData1[j].isp);
	...
	logI("CalNum = %d\n", calTestN);
	   ...
	   nv3A = getNv3ABuf();
		...
		else
		{
		    ...
		    {
		        logI("duty=2 result");
		        logI("r\tg\tb value (duty=-1,0,1)");
		        logI("%lf\t%lf\t%lf",g_algCalData[0].R,g_algCalData[0].G,g_algCalData[0].B);
		        logI("%lf\t%lf\t%lf",g_algCalData[1].R,g_algCalData[1].G,g_algCalData[1].B);
		        logI("%lf\t%lf\t%lf",g_algCalData[2].R,g_algCalData[2].G,g_algCalData[2].B);
                /*
		        FILE* fp;
		        fp = fopen("/sdcard/dd.txt","wt");
		        fprintf(fp,"%lf\t%lf\t%lf\n",g_algCalData[0].R,g_algCalData[0].G,g_algCalData[0].B);
		        fprintf(fp,"%lf\t%lf\t%lf\n",g_algCalData[1].R,g_algCalData[1].G,g_algCalData[1].B);
		        fprintf(fp,"%lf\t%lf\t%lf\n",g_algCalData[2].R,g_algCalData[2].G,g_algCalData[2].B);
		        fclose(fp);
		        */
		        double mul;
		        mul = 9999.0/(g_algCalData[2].G-g_algCalData[0].G);

		        yTab[0] = mul*(g_algCalData[1].G-g_algCalData[0].G);
		        yTab[1] = mul*(g_algCalData[2].G-g_algCalData[0].G);
		        ...
		        double g2;
		        double b2;
		        r1 = g_algCalData[1].R-g_algCalData[0].R;
		        g1 = g_algCalData[1].G-g_algCalData[0].G;
		        b1 = g_algCalData[1].B-g_algCalData[0].B;
		        r2 = g_algCalData[2].R-g_algCalData[0].R;
		        g2 = g_algCalData[2].G-g_algCalData[0].G;
		        b2 = g_algCalData[2].B-g_algCalData[0].B;
...
END_WITH_ERROR:
	      ...
	      //  e_errBrightness_normal=0,
//	        e_errBrightness_environmentNotDark,
	//        e_errBrightness_overExposure,
        //};
	    //int errBrightness=0;
	    //double gtes=-1;
		...
		//write result:
		FILE* fpResult;
		fpResult = fopen("/sdcard/flash_cali_result","wt");
		if(fpResult==0)
		    logE("open file error: (/sdcard/flash_cali_result) ");
		else
		{
		    if(g_flashQuickCalibrationResult==S_FLASH_OK)
		        fprintf(fpResult, "@result pass\n");
		    else
		        fprintf(fpResult, "@result fail\n");

		    if(g_flashQuickCalibrationResult==E_FLASH_CALI_ENVIRONMENT_NOT_DARK_ERR)
		    {
				fprintf(fpResult, "@err_detail E_FLASH_CALI_ENVIRONMENT_NOT_DARK_ERR\n");
				fprintf(fpResult, "Environment is not dark. Calibration environment should be dark.\n");
			}
			else if(g_flashQuickCalibrationResult==E_FLASH_CALI_OVEREXPOSURE_ERR)
		    {
				fprintf(fpResult, "@err_detail E_FLASH_CALI_OVEREXPOSURE_ERR\n");
				fprintf(fpResult, "during calibration, y mean is overexposure. Please modify exposure setting or environment.\n");
			}
			else if(g_flashQuickCalibrationResult==E_FLASH_DUAL_CALI_PARAM_ERR)
			{
				fprintf(fpResult, "@err_detail E_FLASH_DUAL_CALI_PARAM_ERR\n");
				fprintf(fpResult, "There is error in calibration flow. Please provide log for further analyzing\n");
			}
			else if(g_flashQuickCalibrationResult==E_FLASH_DUAL_CALI_IN_Y_RANGE_ERR)
			{
				fprintf(fpResult, "@err_detail E_FLASH_DUAL_CALI_IN_Y_RANGE_ERR\n");
				fprintf(fpResult, "There is error in calibration flow. Please provide log for further analyzing\n");
			}
			else if(g_flashQuickCalibrationResult==E_FLASH_DUAL_CALI_IN_Y_VALUE_ERR)
			{
				fprintf(fpResult, "@err_detail E_FLASH_DUAL_CALI_IN_Y_VALUE_ERR\n");
				fprintf(fpResult, "Perhaps light leaks during calibration, please check the calibration envrironment.\n");
			}
			else if(g_flashQuickCalibrationResult==E_FLASH_DUAL_CALI_IN_WB_RANGE_ERR)
			{
				fprintf(fpResult, "@err_detail E_FLASH_DUAL_CALI_IN_WB_RANGE_ERR\n");
				fprintf(fpResult, "There is error in calibration flow. Please provide log for further analyzing\n");
			}
			else if(g_flashQuickCalibrationResult==E_FLASH_DUAL_CALI_IN_WB_VALUE_ERR)
			{
				fprintf(fpResult, "@err_detail E_FLASH_DUAL_CALI_IN_WB_VALUE_ERR\n");
				fprintf(fpResult, "The color or energy of flash leds is wrong. please check leds and driver\n");
			}
			else if(g_flashQuickCalibrationResult==E_FLASH_DUAL_CALI_GOLD_Y_RANGE_ERR)
			{
				fprintf(fpResult, "@err_detail E_FLASH_DUAL_CALI_GOLD_Y_RANGE_ERR\n");
				fprintf(fpResult, "There is error in calibration flow. Please provide log for further analyzing\n");
			}
			else if(g_flashQuickCalibrationResult==E_FLASH_DUAL_CALI_GOLD_Y_VALUE_ERR)
			{
				fprintf(fpResult, "@err_detail E_FLASH_DUAL_CALI_GOLD_Y_VALUE_ERR\n");
				fprintf(fpResult, "The color or energy of flash leds is wrong. please check leds and driver\n");
			}
			else if(g_flashQuickCalibrationResult==E_FLASH_DUAL_CALI_GOLD_WB_RANGE_ERR)
			{
				fprintf(fpResult, "@err_detail E_FLASH_DUAL_CALI_GOLD_WB_RANGE_ERR\n");
				fprintf(fpResult, "There is error in calibration flow. Please provide log for further analyzing\n");
			}
			else if(g_flashQuickCalibrationResult==E_FLASH_DUAL_CALI_GOLD_WB_VALUE_ERR)
			{
				fprintf(fpResult, "@err_detail E_FLASH_DUAL_CALI_GOLD_WB_VALUE_ERR\n");
				fprintf(fpResult, "The color or energy of flash leds is wrong. please check leds and driver\n");
			}
		    fclose(fpResult);
		}
