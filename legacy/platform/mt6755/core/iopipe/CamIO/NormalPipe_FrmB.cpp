...
#define RAW_D_SENSOR_RESOLUTION_LIMT  7990272//3264x2448
...
INormalPipe_FrmB::createInstance(MUINT32 *pSensorIndex, MUINT32 sensorCnt,char const* szCallerName, MINT32 burstQnum)
{
    ...
    if (!pAllSensorInfo){
        ...
        PIPE_DBG("[%d]createInstance, %s,Alloc pAllSensorInfo=0x%8x",pSensorIndex[0],szCallerName, pAllSensorInfo);
...
NormalPipe_FrmB::enque(QBufInfo const& rQBuf)
{
    ...
    if(this->m_bStarted == MFALSE){
        ...
        for (MUINT32 i = 0; i < rQBuf.mvOut.size(); i++)
        {
            ...
            }else if (CAM_SV_2 == _NORMALPIPE_GET_TG_IDX((mpSensorIdx[0]))){
                 if (PORT_CAMSV_IMGO == rQBuf.mvOut.at(i).mPortID){
...
NormalPipe_FrmB::DummyFrame(QBufInfo const& rQBuf){
    ...
    for(int i=0;i<rQBuf.mvOut.size();i++){
        ...
        PIPE_INF("MyEnq_dummy: dma(0x%x) pa(0x%x) io(0x%x_0x%x_0x%x_0x%x_0x%x_0x%x) mag(0x%x)\n",_buf.mPortID.index,\
                _buf.mBuffer->getBufPA(0),\
                _buf.FrameBased.mCropRect.p.x,_buf.FrameBased.mCropRect.p.y,_buf.FrameBased.mCropRect.s.w,_buf.FrameBased.mCropRect.s.h,_buf.FrameBased.mDstSize.w,_buf.FrameBased.mDstSize.h,\
                _buf.FrameBased.mMagicNum_tuning);
    }
        ...
        if ( rQTSBufInfo.vBufInfo.size() >= 1 )  {
            ...
            for (MUINT32 idx = 0; idx < rQTSBufInfo.vBufInfo.size(); idx++)  {
                ...
                if(buff.mMetaData.m_bDummyFrame == MTRUE){
                    ...
                    //
                    PIPE_INF("MyDeq: dummy frame: dma(0x%x) pa(0x%x) io(0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x) mag(0x%x_0x%x)\n",portID.index,\
                    buff.mPa,\
                    ...
                    PIPE_INF("MyDeq: dma(0x%x) pa(0x%x) io(0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x) mag(0x%x_0x%x)\n",portID.index,\
                    buff.mPa,\
                    result.mCrop_s.p.x,result.mCrop_s.p.y,result.mCrop_s.s.w,result.mCrop_s.s.h, \
                    result.mCrop_d.p.x,result.mCrop_d.p.y,result.mCrop_d.s.w,result.mCrop_d.s.h, \
                    result.mDstSize.w,result.mDstSize.h,\
...
//    pixelCnt2 = _NORMALPIPE_GET_PIX_MODE(S2Idx);
    ...
    /*if (mpEnabledSensorCnt == 1)*/
    {
        ...
        PIPE_DBG("1sensor tg[%d],PixelMode=[%d], ISPMode=[%d]", tg, pixelCnt, mTwinEnable[tg][1]);
    }
        ...
        for (int i=0; i<vInPorts.mPortInfo.size();i++) {
            ...
            if ( PORT_IMGO == vInPorts.mPortInfo.at(i).mPortID ) {
                ...
                if (CAM_TG_1 == _NORMALPIPE_GET_TG_IDX((this->mpSensorIdx[0]))){
                    ...
                    PIPE_DBG("configPipe PORT_RRZO, mRrzoOut_size:%d,%d, Crop:%d,%d,%d,%d", this->mRrzoOut_size.w, this->mRrzoOut_size.h);
                    pOut->index    = vInPorts.mPortInfo.at(i).mPortID.index;
                }
            ...
            if((pOut->index  == EPortIndex_IMGO)||(pOut->index  == EPortIndex_IMGO_D)){
                E_ISP_QUERY _op;
...
NormalPipe_FrmB::wait(MINT32 sensorIdx, EPipeSignal eType, uint64_t ns_timeout)
{
    ...
    if (this->mpCamIOPipe)
    {
        if ( MFALSE == this->mpCamIOPipe->irq((NSImageio::NSIspio::EPipePass)TG, _irq,(int)ISP_DRV_IRQ_USER_ISPDRV))
        {
...
sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    case EPIPECmd_GET_HBIN_INFO:
        ...
        PIPE_DBG("EPIPECmd_GET_HBIN_INFO SIdx(%d),twin=%d,hbin_en=%d w=0x%x, h=0x%x!",arg1,mbTwinEnable, hbin->mEnabled,hbin->size.w, hbin->size.h);
        break;
...
MBOOL NormalPipe_FrmB_Thread::DmaCfg(BufInfo* pParam)
{
    ...
    if( PORT_RRZO == port ){
        ...
        if(CAM_TG_2 == _NORMALPIPE_GET_TG_IDX((this->m_pNormalpipe_FrmB->mpSensorIdx[0])))
            port = PORT_IMGO_D;
        else if(CAM_SV_1 == _NORMALPIPE_GET_TG_IDX((this->m_pNormalpipe_FrmB->mpSensorIdx[0]))) {
            port = PORT_CAMSV_IMGO;
            PIPE_ERR("camsv enque flow r not supported yet\n");
            return MFALSE;
        }
        else if(CAM_SV_2 == _NORMALPIPE_GET_TG_IDX((this->m_pNormalpipe_FrmB->mpSensorIdx[0]))){
    ...
    if (this->m_pNormalpipe_FrmB->mpCamIOPipe) {
        ...
        if( PORT_RRZO == port || PORT_IMGO == port ){
            ...
            tgIdx = EPortIndex_CAMSV_TG1I;
            PIPE_ERR("camsv enque flow r not supported yet\n");
            NormalPipeGUnLock();
            return MFALSE;
        }
        else if( PORT_CAMSV2_IMGO == port ){
            tgIdx = EPortIndex_CAMSV_TG2I;
            PIPE_ERR("camsv enque flow r not supported yet\n");
            NormalPipeGUnLock();
            return MFALSE;
        }
...
MVOID NormalPipe_FrmB_Thread::EnqueRequest(MUINT32 sof_idx)
{
    ...
    if(_size == 0){
        ...
        PIPE_DBG("CUR FRAME STATUS:0x%lx\n",_drop_status);
