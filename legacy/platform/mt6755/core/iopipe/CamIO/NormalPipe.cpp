...
NormalPipe::init()
{
    ...
    CAM_LOGI("NormalPipe::init+ SenIdx=%d", mpSensorIdx[0]);
...
NormalPipe::uninit()
{
    ...
    CAM_LOGI("uninit+,  SenIdx=%d", mpSensorIdx[0]);
...
NormalPipe::enque(QBufInfo const& rQBuf)
{
    ...
    for (MUINT32 i = 0; i < rQBuf.mvOut.size(); i++)
    {
        ...
        //
        if ((portID.index == PORT_RRZO.index) ||(portID.index == PORT_RRZO_D.index)||(portID.index == PORT_IMGO.index)||(portID.index == PORT_IMGO_D.index)) {
...
NormalPipe::deque(QBufInfo& rQBuf, MUINT32 u4TimeoutMs)
{
        ...
        if ( rQTSBufInfo.vBufInfo.size() >= 1 )  {
            ...
            for (MUINT32 idx = 0; idx < rQTSBufInfo.vBufInfo.size(); idx++)  {
                ...
                if ((portID.index == PORT_RRZO.index) ||(portID.index == PORT_RRZO_D.index) || (portID.index == PORT_IMGO.index))
                    CAM_LOGD("[MyDeQ:%d]PortId(%d),va=0x%x,BufHeader=0x%x,PixelId=%x, MNum=0x%x, rrzDstSz=%d,%d,rrzSrcSz=%d,%d, time:%d.%d",mpSensorIdx[0], \
                             portID.index,\
                             ...
                             pBufHeader->timeStampS, pBufHeader->timeStampUs);
    ...
    mpHalSensor->sendCommand(_NORMALPIPE_GET_SENSOR_DEV_ID(mpSensorIdx[0]),SENSOR_CMD_DEBUG_P1_DQ_SENINF_STATUS,0,0,0);
    ...
    //update cq table num if mw member update burst queue number
        ...
        for (int i=0; i<vInPorts.mPortInfo.size();i++) {
            ...
            if (pOut != NULL) {
                ...
                CAM_LOGE("Dereferencing null pointer pOut");
                return MFALSE;
            }
            //
