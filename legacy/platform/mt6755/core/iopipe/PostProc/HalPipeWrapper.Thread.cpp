    ...
    ECmd &cmd)
{
    ...
    {   //no more cmd in cq1/cq2/cq3 dequeueCmd list, block wait for user add cmd
        ...
        mpPostProcPipe->endequeFrameCtrl(NSImageio::NSIspio::EPIPE_BUFQUECmd_WAKE_WAITFRAME,0,0,0,0,0);
    ...
    MUINT32 callerID)
{
    ...
    else
    {
        ...
        mpPostProcPipe->endequeFrameCtrl(cmd,callerID,0,p2dupCQidx,0,0);
...
dequeueJudgement()
{
    ...
    if (dequeP2Cq >= 3)
    {
    LOG_DBG("out of range : dequeCq(%d))",dequeP2Cq);
    return false;
    }
    switch(bufNode.bufTag)
    {
        ...
        default:
            //[2] wait for deque
            ret=mpPostProcPipe->endequeFrameCtrl(NSImageio::NSIspio::EPIPE_BUFQUECmd_WAIT_DEQUE,0,0,0,0,0);
...
)
{
    ...
    if(bufNode.portEn == ((1<<EPostProcPortIdx_IMGI) | 1<<EPostProcPortIdx_IMGO))
    {
        isIPRaw=MTRUE;
		//QQ
		LOG_ERR("QQ delay for work-around");
		usleep(100*1000);//QQ usleep(100*1000)
    }
...
//    LOG_INF("QQ id : (%x)  callback (%x)",bufNode.idxInBufBatchList,mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mpfnCallback);
...
//                LOG_INF("QQ idx(%d)",bufNode.idxInBufBatchList);
                BufParamPackage buf=mvEnqueuedBufBatch[bufNode.idxInBufBatchList];    //actually the first element
                releaseLock(ELockEnum_BufBatchEnQ);
                ...
                ioctrlret=mpPostProcPipe->endequeFrameCtrl(NSImageio::NSIspio::EPIPE_BUFQUECmd_WAIT_FRAME,bufNode.callerID,0,0,0,1000000);
                if(!ioctrlret)
                {
    ...
    switch(bufNode.drvSce)
    {
    	...
    	case NSImageio::NSIspio::eDrvScenario_VSS_MFB_Mixing:
    		...
    		mIsVssOccupied = false;
    		releaseLock(ELockEnum_VssOccupied);
    		//notify
    		::sem_post(&mSemVssOccupied);
    		break;
    	default:
    		break;
    }
