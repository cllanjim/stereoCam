...
SwHwScenarioPathMapping mSwHwPathMapping[eSoftwareScenario_total_num]=
{
    ...
    //
    {eSoftwareScenario_Main2_Normal_Stream,   "Main2_NormalStream",     NSImageio::NSIspio::eDrvScenario_CC,                 NSImageio::NSIspio::EPipePass_PASS2C,    EPathCQ_CQ3},
    {eSoftwareScenario_Main2_Normal_Capture,  "Main2_NormalCapture",    NSImageio::NSIspio::eDrvScenario_CC_SShot,           NSImageio::NSIspio::EPipePass_PASS2C,    EPathCQ_CQ3},
    ...
    {eSoftwareScenario_Main2_ZSD_Capture,     "Main2_ZSDCapture",       NSImageio::NSIspio::eDrvScenario_CC_SShot,           NSImageio::NSIspio::EPipePass_PASS2C,   EPathCQ_CQ3},
    {eSoftwareScenario_Main2_Mfb_Capture,     "Main2_MFBCapture",       NSImageio::NSIspio::eDrvScenario_CC_SShot,           NSImageio::NSIspio::EPipePass_PASS2C,   EPathCQ_CQ3},
    {eSoftwareScenario_Main2_Mfb_Blending,    "Main2_MFBBlending",      NSImageio::NSIspio::eDrvScenario_CC_MFB_Blending,    NSImageio::NSIspio::EPipePass_PASS2C,   EPathCQ_CQ3},
    {eSoftwareScenario_Main2_Mfb_Mixing,      "Main2_MFBMixing",        NSImageio::NSIspio::eDrvScenario_CC_MFB_Mixing,      NSImageio::NSIspio::EPipePass_PASS2C,   EPathCQ_CQ3},
    ...
    {eSoftwareScenario_Main2_Vfb_Stream_1,    "Main2_vFBStream1",       NSImageio::NSIspio::eDrvScenario_CC_vFB_Normal,      NSImageio::NSIspio::EPipePass_PASS2C,    EPathCQ_CQ3},
    ...
    {eSoftwareScenario_Main2_Pure_Raw_Stream, "Main2_PureRawStream",    NSImageio::NSIspio::eDrvScenario_IP,                 NSImageio::NSIspio::EPipePass_PASS2C,    EPathCQ_CQ3},
    {eSoftwareScenario_Main2_CC_Raw_Stream,   "Main2_CCRawStream",      NSImageio::NSIspio::eDrvScenario_CC_RAW,             NSImageio::NSIspio::EPipePass_PASS2C,    EPathCQ_CQ3}
    //
};
    ...
    QFeatureData& rFeatureData)
{
    ...
    {    //original version
        ...
        NSImageio::NSIspio::PortInfo ports[100]; // Fix N build error - use 100 to replace inPorts+outPorts
        ...
        for(int i=0;i<rParams.mvOut.size();i++)
        {
			...
			ports[cnt].bIsRwbEn	   = rParams.mvOut[i].bIsRwbEn;	// add for RWB sensor
			LOG_DBG("[1]ports[%d].bIsRwbEn = %d, rParams.mvOut[%d].bIsRwbEn = %d", cnt, ports[cnt].bIsRwbEn, i, rParams.mvOut[i].bIsRwbEn);	// add for RWB sensor
            for(int k=0;k<planeNum;k++)
            {
            ...
            ports[cnt].u4MagicNum  = rParams.mFrameNo;
			ports[cnt].bIsRwbEn	   = rParams.mvOut[i].bIsRwbEn;	// add for RWB sensor
			LOG_DBG("[2]ports[%d].bIsRwbEn = %d, rParams.mvOut[%d].bIsRwbEn = %d", cnt, ports[cnt].bIsRwbEn, i, rParams.mvOut[i].bIsRwbEn);	// add for RWB sensor
	    ...
	    LOG_INF("swScenario(0x%x)/EnPorts(0x%08x),featureEntag(0x%x),pixID(%d)",swScenario,bufPackage.portEn,featureEntag,pixID);
		...
		LOG_DBG("set iso: Qparam(%d), PipePackageInfo(%d)", rParams.isoValue, pipePackageInfo.isoValue);
        ...
        ret=callerPostProc->endequeFrameCtrl(NSImageio::NSIspio::EPIPE_BUFQUECmd_ENQUE_FRAME,bufNode.callerID,bufNode.p2BurstQIdx,bufPackage.p2cqDupIdx,bufPackage.frameNum);
    ...
    {   //enque by vector, v3 and v1 [v3 only support vector enque]
        ...
        for(int q=0;q<bufPackageNum;q++)
        {
            ...
            for(int i=outStartIdx;i<outEndIdx;i++)
            {
                ...
                ports[cnt].u4ImgHeight = rParams.mvOut[i].mBuffer->getImgSize().h;
				ports[cnt].bIsRwbEn	   = rParams.mvOut[i].bIsRwbEn;	// add for RWB sensor
				LOG_DBG("[3]ports[%d].bIsRwbEn = %d, rParams.mvOut[%d].bIsRwbEn = %d", cnt, ports[cnt].bIsRwbEn, i, rParams.mvOut[i].bIsRwbEn);	// add for RWB sensor
                for(int k=0;k<planeNum;k++)
                {
                    ports[cnt].u4Stride[k] = rParams.mvOut[i].mBuffer->getBufStridesInBytes(k);
                    ports[cnt].u4BufSize[k]= rParams.mvOut[i].mBuffer->getBufSizeInBytes(k);
			...
			pipePackageInfo.pixIdP2 = pixID;
			pipePackageInfo.isoValue = rParams.isoValue;	// set ISO
			LOG_DBG("set iso(HalPipeWrapper2): Qparam(%d), PipePackageInfo(%d)", rParams.isoValue, pipePackageInfo.isoValue);
    ...
    QFeatureData& rFeatureData)
{
    ...
    ret=callerPostProc->endequeFrameCtrl(NSImageio::NSIspio::EPIPE_BUFQUECmd_WAIT_FRAME,callerID,0,0,0,timeout);
    b=getUs();
