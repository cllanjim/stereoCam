...
static const NSCam::NSIoPipe::PortID RRZO( NSIoPipe::EPortType_Memory, NSImageio::NSIspio::EPortIndex_RRZO, 1);
...
void _test_camio_::startThread(void)
{
    ...
    pthread_attr_t const attr = {0, NULL, 1024 * 1024, 4096, SCHED_RR, 0};
...
MVOID* _test_camio_::_infiniteloop(void* arg)
{
    ...
    ::pthread_detach(::pthread_self());
        ...
        if( _this->m_enablePort & __RRZO_ENABLE) {
            //enque
            NSCam::NSIoPipe::NSCamIOPipe::BufInfo OutBuf(RRZO, 0);
            dequeBufInfo.mvOut.push_back(OutBuf);
            ...
            //in replace testing , use mag num:7
            NSCam::NSIoPipe::NSCamIOPipe::BufInfo InBuf(RRZO, _this->m_prepalce_Buf[__RRZO],_rrz, MRect(__CROP_Start(_this->m_TgSize,_cropsize,_this->m_prepalce_Buf[__RRZO]->getImgFormat()),_cropsize),(replace_buf + Enque_buf + Dummy_buf));
            _replace.mvOut.push_back(InBuf);
        }
        ...
        if(_this->m_pNormalP_FrmB->deque(dequeBufInfo) == MTRUE){
            ...
            else
            {
                ...
                for(int i=0;i<dequeBufInfo.mvOut.size();i++){
                    ...
                    else if(dequeBufInfo.mvOut.at(i).mPortID == RRZO)
            ...
            for(int i=0;i<p1dequeBufInfo.mvOut.size();i++)
            {
                if(i==0)
                {
                    ...
                    {    //life cycle for variable
                        ...
                        MUINT32 out_w=p1dequeBufInfo.mvOut.at(i).mMetaData.mCrop_d.s.w;
                        MUINT32 out_h=p1dequeBufInfo.mvOut.at(i).mMetaData.mCrop_d.s.h;
                        ...
                        wdmaoBuf.size=out_w*out_h*2;
                            pImemDrv->allocVirtBuf(NAME, &wdmaoBuf);
                        ...
                        wrotoBuf.size=out_w_2*out_h_2*2;
                            pImemDrv->allocVirtBuf(NAME, &wrotoBuf);
                    ...
                    for(int i=0;i<p1dequeBufInfo.mvOut.size();i++){
                        ...
                        else if(p1dequeBufInfo.mvOut.at(i).mPortID == RRZO)
...
int basic_flow(int num,int type)
{
 ...
 //           break;
        case 1: //prv_fps30_rrzo
            scenario = 0;
            enablePort = 0x1;
            printf("preview 30fps, DMA: rrzo, single sensor\n");
            break;
        case 2: //prv_fps30_imgo+rrzo
            scenario = 0;
            enablePort = 0x3;
            printf("preview 30fps, DMA: imgo+rrzo, single sensor\n");
            break;
  //      case 3: //prv_fps15_imgo_dualSensor
  //          scenario = 0;
  //          enablePort = 0x2;
  //          dualSensor = 2;
  //          sensorFps = 15;
  //          break;
        case 4: //prv_fps15_rrzo_dualSensor
            scenario = 0;
            enablePort = 0x1;
            dualSensor = 2;
            printf("preview 30fps, DMA: rrzo, dual sensor\n");
            break;
        case 5: //prv_fps30_rrzo+imgo_dualSensor
            scenario = 0;
            enablePort = 0x3;
            dualSensor = 2;
            printf("preview 30fps, DMA: imgo+rrzo, dual sensor\n");
            break;
   ...
   //         break;
        case 7: //cap_fps30_rrzo
            scenario = 1;
            enablePort = 0x1;
            printf("capture 30fps, DMA: rrzo\n");
            break;
        case 8: //cap_fps30_imgo+rrzo
            scenario = 1;
            enablePort = 0x3;
            sensorFps = 15;
            printf("capture 15fps, DMA: imgo+rrzo\n");
            break;
        default:
    ...
    for(int i=0;i<dualSensor;i++){
        ...
        if(enablePort & __IMGO_ENABLE){
            ...
            if( !getOutputFmt_2(IMGO,SEN_PIX_BITDEPTH,mSensorInfo[i], &fmt))
        ...
        if(enablePort & __RRZO_ENABLE){
            ...
            MSize _size;
            if( !getOutputFmt_2(RRZO,SEN_PIX_BITDEPTH,mSensorInfo[i], &fmt))
                LOG_INF("get pix fmt error\n");
    ...
    for(int k=0;k<dualSensor;k++){
        for(int i=0;i<(Enque_buf+Dummy_buf+replace_buf);i++){
            if(enablePort & __RRZO_ENABLE){
                ...
                if( !getOutputFmt_2(RRZO,SEN_PIX_BITDEPTH,mSensorInfo[k], &fmt))
                    LOG_INF("get pix fmt error\n");
                NSImageio::NSIspio::ISP_QuerySize(
                            NSImageio::NSIspio::EPortIndex_RRZO,
                            NSImageio::NSIspio::ISP_QUERY_X_PIX|
				...
				mpImemDrv->allocVirtBuf(NAME, &p1Buf);
            ...
            if(enablePort & __IMGO_ENABLE){
                if( !getOutputFmt_2(IMGO,SEN_PIX_BITDEPTH,mSensorInfo[k], &fmt))
                    LOG_INF("get pix fmt error\n");
                NSImageio::NSIspio::ISP_QuerySize(
                                    NSImageio::NSIspio::EPortIndex_IMGO,
                                    NSImageio::NSIspio::ISP_QUERY_X_PIX|
    ...
    for(int k=0;k<dualSensor;k++){
        for(int i=0;i<Enque_buf;i++){
            ...
            if(enablePort & __RRZO_ENABLE){
                ...
                if(bArbCrop && i==1){
                    _buf = NSCam::NSIoPipe::NSCamIOPipe::BufInfo(RRZO,pImgBuffer[k][__RRZO][i],_rrz,MRect(__CROP_Start_2(sensorSize[k],_cropsize,pImgBuffer[k][__RRZO][i]->getImgFormat(),1),_cropsize),i+1);
                }
                else{
                    _buf = NSCam::NSIoPipe::NSCamIOPipe::BufInfo(RRZO,pImgBuffer[k][__RRZO][i],_rrz,MRect(__CROP_Start(sensorSize[k],_cropsize,pImgBuffer[k][__RRZO][i]->getImgFormat()),_cropsize),i+1);
                }
            ...
            if(enablePort & __IMGO_ENABLE){
                ...
                NSCam::NSIoPipe::NSCamIOPipe::BufInfo _buf;
                if(bArbCrop && i==1){
                    _buf = NSCam::NSIoPipe::NSCamIOPipe::BufInfo(IMGO,pImgBuffer[k][__IMGO][i],_cropsize,MRect(__CROP_Start_2(sensorSize[k],_cropsize,pImgBuffer[k][__IMGO][i]->getImgFormat(),1),_cropsize),i+1);
                }
                else{
                    _buf = NSCam::NSIoPipe::NSCamIOPipe::BufInfo(IMGO,pImgBuffer[k][__IMGO][i],_cropsize,MRect(__CROP_Start(sensorSize[k],_cropsize,pImgBuffer[k][__IMGO][i]->getImgFormat()),_cropsize),i+1);
                }
                buf.mvOut.push_back(_buf);
            }
            mpCamIO[k]->enque(buf);
    ...
    for(int k=0;k<dualSensor;k++){
       for(int i=0;i<Enque_buf;i++){
           ...
           if(enablePort & __RRZO_ENABLE){
               ...
               printf("_rrz size : 0x%x 0x%x (0x%x 0x%x)(0x%x 0x%x) %d\n",_rrz.w,_rrz.h,sensorSize[k].w,sensorSize[k].h, _cropsize.w, _cropsize.h, pImgBuffer[k][__RRZO][i]->getImgFormat());
               NSCam::NSIoPipe::NSCamIOPipe::BufInfo _buf = NSCam::NSIoPipe::NSCamIOPipe::BufInfo(RRZO,pImgBuffer[k][__RRZO][i],_rrz,MRect(__CROP_Start(sensorSize[k],_cropsize,pImgBuffer[k][__RRZO][i]->getImgFormat()),_cropsize),i+1);
               buf.mvOut.push_back(_buf);
           }
           if(enablePort & __IMGO_ENABLE){
               _cropsize = __CROP_SIZE(__IMGO,pImgBuffer[k][__IMGO][i]->getImgFormat(),sensorSize[k],e_PixMode[k]);
               NSCam::NSIoPipe::NSCamIOPipe::BufInfo _buf = NSCam::NSIoPipe::NSCamIOPipe::BufInfo(IMGO,pImgBuffer[k][__IMGO][i],_cropsize,MRect(__CROP_Start(sensorSize[k],_cropsize,pImgBuffer[k][__IMGO][i]->getImgFormat()),_cropsize),i+1);
               buf.mvOut.push_back(_buf);
           }
           mpCamIO[k]->enque(buf);
       }
