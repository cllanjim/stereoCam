...
class _test_camio
{
    _test_camio()
    {
        ...
        m_CropTestSel = 0;
        m_sensoridx = 0;
        mPixMode = NSImageio::NSIspio::ISP_QUERY_UNKNOWN_PIX_MODE;
        m_Thread = 0;
        m_semThread.count = 0;
    };
    ~_test_camio(){};
...
MUINT32 _test_camio::startThread(void)
{
    ...
    if ((r = pthread_create(&this->m_Thread, &attr, this->_infiniteloop, this)) != 0) {
    ...
    while(1){
        ...
        if( bRet == MTRUE){
            ...
            else
            {
                ...
                //
                usleep(30000);//prevent send multiple requests ad one frame, sleep 30ms
                //
...
int test_camio(int argc, char** argv)
{
    ...
    if(CAM_V1 == ISPversion){
        for(int i=sensorIdx;i<dualSensor;i++){
            ...
            mpCamIO[i] = INormalPipe::createInstance((MINT32)sensorArray[i], LOG_TAG,1);
            mpCamIO[i]->init();
        }
    }
    else{
        for(int i=sensorIdx;i<dualSensor;i++){
            ...
            mpCamIO_FrmB[i] = INormalPipe_FrmB::createInstance((MINT32)sensorArray[i], LOG_TAG,1);
            mpCamIO_FrmB[i]->init();
        }
    }
    ...
    IMEM_BUF_INFO imemBuf[_MAXTG_][__MAXDMAO][(Enque_buf+Dummy_buf+replace_buf)];
    ...
    for(int k=sensorIdx;k<dualSensor;k++){
        for(int i=0;i<(Enque_buf+Dummy_buf+replace_buf);i++){
            if(enablePort & __RRZO_ENABLE){
                ...
                mpImemDrv->allocVirtBuf(NAME, &imgiBuf);
                imemBuf[k][__RRZO][i] = imgiBuf;
    ...
    for(int k=sensorIdx;k<dualSensor;k++){
        for(int i=0;i<Enque_buf;i++){
            ...
            if(CAM_V1 == ISPversion)
                ...
                for(MUINT32 nlp = 0; nlp < buf.mvOut.size(); nlp++){
                    buf.mvOut.at(nlp).FrameBased.mSOFidx = 0;//initial
                    printf("[%d]BufIdx(0x%x),Sofidx(0x%x)\n",nlp,buf.mvOut.at(nlp).mBufIdx,buf.mvOut.at(nlp).FrameBased.mSOFidx);
                }
   ...
   // printf("!!!!!!!! press enter to stop... !!!!!!!!\n");//*/
    ...
    for(int k=sensorIdx;k<dualSensor;k++){
        for(int i=0;i<(Enque_buf+Dummy_buf+replace_buf);i++){
            if(enablePort & __RRZO_ENABLE){
                printf("[%d]free RRZO(%d)\n", k, i);
                mpImemDrv->freeVirtBuf(NAME, &imemBuf[k][__RRZO][i]);
            }
            if(enablePort & __IMGO_ENABLE){
                printf("[%d]free IMGO(%d)\n", k, i);
                mpImemDrv->freeVirtBuf(NAME, &imemBuf[k][__IMGO][i]);

            }
            if(enablePort & __CAMSV_ENABLE){
                printf("[%d]free CAMSV(%d)\n", k, i);
                mpImemDrv->freeVirtBuf(NAME, &imemBuf[k][__CAMSV][i]);
            }
            if(enablePort & __CAMSV2_ENABLE){
                printf("[%d]free CAMSV2(%d)\n", k, i);
                mpImemDrv->freeVirtBuf(NAME, &imemBuf[k][__CAMSV2][i]);
            }
        }
    }
