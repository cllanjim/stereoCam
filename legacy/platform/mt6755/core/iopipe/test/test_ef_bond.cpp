...
int ef_test()
{
    ...
    ret = m_pICamIOPipe->init();
    if (ret != MTRUE)
    {
        LOG_ERR("CamIOPipe Init Fail!!!");
    }
    PortInfo m_tgi;
    PortInfo m_imgo;
           PortInfo m_rrzo;
    ...
    QTimeStampBufInfo rQTSBufInfo;
        ...
        for(int j=0;j<3;j++)
        {
			...
			mpImemDrv->allocVirtBuf(NAME, &(imgo_buf[j]));
            ret_mem = mpImemDrv->mapPhyAddr(&(imgo_buf[j]));
            if(ret_mem != 0)
            {
                LOG_ERR("map imgo physical memory fail!");
            }
			...
			mpImemDrv->allocVirtBuf(NAME, &(rrzo_buf[j]));
            ret_mem = mpImemDrv->mapPhyAddr(&(rrzo_buf[j]));
            if(ret_mem != 0)
            {
                LOG_ERR("map rrzo physical memory fail!");
            }
        ...
        for(int j=0;j<3;j++)
        {
            ret_mem = mpImemDrv->unmapPhyAddr(&(imgo_buf[j]));
            if(ret_mem != 0)
            {
                LOG_ERR("unmap imgo physical memory fail!");
            }
			mpImemDrv->freeVirtBuf(NAME, &(imgo_buf[j]));
        }
    }
