...
DECLARE_DBG_LOG_VARIABLE(IOpipeTest);
...
#undef LOG_ERR
#undef LOG_AST
#define LOG_VRB(fmt, arg...)        do { if (IOpipeTest_DbgLogEnable_VERBOSE) { BASE_LOG_VRB(fmt, ##arg); } } while(0)
#define LOG_DBG(fmt, arg...)        do { if (IOpipeTest_DbgLogEnable_DEBUG  ) { BASE_LOG_DBG(fmt, ##arg); } } while(0)
#if 0  //kk test
...
#define LOG_WRN(fmt, arg...)        do { if (IOpipeTest_DbgLogEnable_WARN   ) { BASE_LOG_WRN(fmt, ##arg); } } while(0)
#define LOG_ERR(fmt, arg...)        do { if (IOpipeTest_DbgLogEnable_ERROR  ) { BASE_LOG_ERR(fmt, ##arg); } } while(0)
#define LOG_AST(cond, fmt, arg...)  do { if (IOpipeTest_DbgLogEnable_ASSERT ) { BASE_LOG_AST(cond, fmt, ##arg); } } while(0)
...
static const NSCam::NSIoPipe::PortID WROTO( NSIoPipe::EPortType_Memory, NSImageio::NSIspio::EPortIndex_WROTO, 1);
static const NSCam::NSIoPipe::PortID IMG3O( NSIoPipe::EPortType_Memory, NSImageio::NSIspio::EPortIndex_IMG3O, 1);
static const NSCam::NSIoPipe::PortID IMG2O( NSIoPipe::EPortType_Memory, NSImageio::NSIspio::EPortIndex_IMG2O, 1);
static const NSCam::NSIoPipe::PortID IMGO( NSIoPipe::EPortType_Memory, NSImageio::NSIspio::EPortIndex_IMGO, 1);
///////////////////////////////////////////////
//test cases
int pass2_BasicTest(int casenum, int type);
int pass2_BasicTest_ip_raw(int casenum, int type);
int pass2_loopTest(int casenum, int type);
int pass2_multiEnqueTest(int casenum, int type);
int pass2_TuningTest(int casenum);
int pass2_BurstQueueTest(int testcaseNum, int testtype);
int applyTuningSetting(int mode, isp_reg_t **ppIspPhyReg);
int applyP2TuningSetting(isp_reg_t **ppIspPhyReg);
void applyV1Tunning(MINT32 magicNum);
//////////////////////////////////////////////
//global variables
static sem_t     mSem;
...
#include "pic/imgi_3264x2448_rgb888.h"
...
static MVOID* onThreadLoop(MVOID *arg)
{
    ...
    NSCam::NSIoPipe::NSPostProc::INormalStream* pIPStream;
    IMemDrv* mpImemDrv=NULL;
    QParams enqueParams;
    mpImemDrv=IMemDrv::createInstance();
    mpImemDrv->init(LOG_TAG);
    ...
    if(multiThread_case==6)
    {
        switch(multiThread_type)
        {
            case 0:
                {
                    //object init
                    pCCStream= NSCam::NSIoPipe::NSPostProc::INormalStream::createInstance("onThreadLoop", NSCam::NSIoPipe::NSPostProc::ENormalStreamTag_Prv,0xFFFF, isV3);
                    pCCStream->init();
                    //////////////////////////
                    //prepare dma data, imgi
                    MUINT32 bufStridesInBytes[3] = {1280, 0, 0};
                        imgiBuf.size=sizeof(g_imgi_array_640x480_yuy2);
                    mpImemDrv->allocVirtBuf(LOG_TAG, &imgiBuf);
                    ...
                    img2oBuf.size=640*480*2;
                        mpImemDrv->allocVirtBuf(LOG_TAG, &img2oBuf);
                    ...
                    if(isV3)
                    {
                        enqueParams.mvTuningData.clear();
                    }
                    ...
                    //save image
                    char filename[256];
                        sprintf(filename, "/data/multiThread_onThreadLoop_case0_img2o%dx%d.yuv", 640,480);
            ...
            case 1:
                {
                      NSCam::NSIoPipe::NSPostProc::IFeatureStream* pIPStream2;

                      pIPStream2 = NSCam::NSIoPipe::NSPostProc::IFeatureStream::createInstance(LOG_TAG, NSCam::NSIoPipe::NSPostProc::EFeatureStreamTag_PRaw_Stream,0xFFFF, isV3);
//                    pStream2= NSCam::NSIoPipe::NSPostProc::INormalStream::createInstance("onThreadLoop", NSCam::NSIoPipe::NSPostProc::ENormalStreamTag_Cap,0xFFFF, isV3);
                    pIPStream2->init();
                    //////////////////////////
                    //prepare dma data, imgi
                    MUINT32 bufStridesInBytes[3] = {1280, 0, 0};
                    ...
                    mpImemDrv->allocVirtBuf(LOG_TAG, &imgiBuf);
                        ...
                        //imem buffer 2 image heap
                        PortBufInfo_v1 portBufInfo = PortBufInfo_v1( imgiBuf.memID,imgiBuf.virtAddr,0,imgiBuf.bufSecu, imgiBuf.bufCohe);
                        IImageBufferAllocator::ImgParam imgParam = IImageBufferAllocator::ImgParam((eImgFmt_BAYER10),MSize(640, 480), bufStridesInBytes, bufBoundaryInBytes, 1);
                        sp<ImageBufferHeap> pHeap = ImageBufferHeap::create( LOG_TAG, imgParam,portBufInfo,true);
                    tempBuffer = pHeap->createImageBuffer();
                        tempBuffer->incStrong(tempBuffer);
                        tempBuffer->lockBuf(LOG_TAG,eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN);
                    ...
                    imgoBuf.size=640*480*2;
                        mpImemDrv->allocVirtBuf(LOG_TAG, &imgoBuf);
                    ...
                    IImageBufferAllocator::ImgParam imgParam_2 = IImageBufferAllocator::ImgParam((eImgFmt_BAYER10),MSize(640, 480),  bufStridesInBytes_2, bufBoundaryInBytes, 1);
                    sp<ImageBufferHeap> pHeap_2 = ImageBufferHeap::create( LOG_TAG, imgParam_2,portBufInfo_2,true);
                    IImageBuffer* outBuffer = pHeap_2->createImageBuffer();
                        outBuffer->incStrong(outBuffer);
                        outBuffer->lockBuf(LOG_TAG,eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN);
                    ...
                    for(int i=0;i<10;i++)
                    {
                        ...
                        ret = pIPStream2->enque(enqueParams);
						if(!ret)
						{
							printf(" : ERR enque fail\n");
						}
                        ...
                        QParams dequeParams;
                        ret = pIPStream2->deque(dequeParams);
						if(!ret)
						{
							printf(" : ERR deque fail\n");
						}
                        ...
                        //save image
                        char filename[256];
                            sprintf(filename, "/data/multiThread_onThreadLoop_case1_%d_imgo%dx%d.yuv",i, 640,480);
                            saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[0].mBuffer->getBufVA(0)), 640 *480 * 2);
                    ...
                    //destroy
                        pIPStream2->uninit();
                    pIPStream2->destroyInstance("onThreadLoop");
                }
                break;
            default:
                break;
    ...
    {    // temp use 960x640, TO DO 2560x1440 later
        ...
        switch (multiThread_type)
        {
            default:
            case 0:
                pStreamObj= NSCam::NSIoPipe::NSPostProc::INormalStream::createInstance("onThreadLoop", NSCam::NSIoPipe::NSPostProc::ENormalStreamTag_Prv,0xFFFF, bTuning);
                break;
            case 1:
                pStreamObj= NSCam::NSIoPipe::NSPostProc::INormalStream::createInstance("onThreadLoop", NSCam::NSIoPipe::NSPostProc::ENormalStreamTag_Vss,0xFFFF, bTuning);
                break;
        }
        pStreamObj->init();
        //////////////////////////
        ...
        enqueParams.mvIn.push_back(src);
        //default: w/o crop, w/o resize
        MUINT32 out_w=960;//960;
        MUINT32 out_h=640;//640;
        MCrpRsInfo crop;
        crop.mGroupID=1;
        crop.mCropRect.p_fractional.x=0;
        crop.mCropRect.p_fractional.y=0;
        ...
        wdmaoBuf.size=out_w*out_h*2;
            mpImemDrv->allocVirtBuf(LOG_TAG, &wdmaoBuf);
        ...
        for(int i=0;i<5;i++)
        {
            ...
            ret = pStreamObj->enque(enqueParams);
			if(!ret)
			{
				printf(" : ERR enque fail\n");
			}
            ...
            QParams dequeParams;
            ret = pStreamObj->deque(dequeParams);
			if(!ret)
			{
				printf(" : ERR deque fail\n");
			}
            ...
            //save image
            char filename[256];
                sprintf(filename, "/data/P2_case_%d_type_%d_onThreadLoop_%d_wdmao%dx%d.yuv",multiThread_case,multiThread_type,i, out_w,out_h);
...
{
    LOG_INF("+");
//    LOG_INF("QQ : testcaseType(%d),testcaseNum(%d),testtype(%d)"testcaseType,testcaseNum,testtype);
    int ret=0;
    switch(testcaseType)
    {
        case 1:
                if ((testcaseNum == 2) ||(testcaseNum == 3))
                ret=pass2_BasicTest_ip_raw(testcaseNum,testtype);
                else
        ...
        case 2:
            ...
            break;
        case 3:
            ret=pass2_BurstQueueTest(testcaseNum,testtype);
        default:
            break;
    }
...
int pass2_BasicTest(int casenum, int type)
{
...

    {
        ...
        NSCam::NSIoPipe::NSPostProc::INormalStream* pStream;
                pStream= NSCam::NSIoPipe::NSPostProc::INormalStream::createInstance(LOG_TAG, NSCam::NSIoPipe::NSPostProc::ENormalStreamTag_Prv,0xFFFF, isV3);
        pStream->init();
        IMemDrv* mpImemDrv=NULL;
        mpImemDrv=IMemDrv::createInstance();
        mpImemDrv->init(LOG_TAG);
        ...
        enqueParams.mvPrivaData.push_back(NULL);
			...
			enqueParams.isoValue = 250;// set ISO
        ...
        IImageBuffer* tempBuffer;
			...
			{
				printf("Case 10 DMAI setting: rgb888 input, size 3264x2448 \n");
				//imgi in/img3o out, test isp only mode (bypass mdp)
                MUINT32 bufStridesInBytes[3] = {9792, 0, 0};
                imgiBuf.size=sizeof(g_imgi_array_3264x2448_rgb888);
	            mpImemDrv->allocVirtBuf(LOG_TAG, &imgiBuf);
                memcpy( (MUINT8*)(imgiBuf.virtAddr), (MUINT8*)(g_imgi_array_3264x2448_rgb888), imgiBuf.size);
                //imem buffer 2 image heap
                PortBufInfo_v1 portBufInfo = PortBufInfo_v1( imgiBuf.memID,imgiBuf.virtAddr,0,imgiBuf.bufSecu, imgiBuf.bufCohe);
                IImageBufferAllocator::ImgParam imgParam = IImageBufferAllocator::ImgParam((eImgFmt_RGB888),
                                                                MSize(3264, 2448), bufStridesInBytes, bufBoundaryInBytes, 1);
                sp<ImageBufferHeap> pHeap = ImageBufferHeap::create( LOG_TAG, imgParam,portBufInfo,true);
	            tempBuffer = pHeap->createImageBuffer();
                tempBuffer->incStrong(tempBuffer);
                tempBuffer->lockBuf(LOG_TAG,eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN);
        ...
        case 2:
            ...
            case 9:
                ...
                break;
			case 10:
				printf("Case 10 crop setting \n");
				crop.mCropRect.p_fractional.x=0;
                crop.mCropRect.p_fractional.y=0;
                crop.mCropRect.p_integral.x=0;
                crop.mCropRect.p_integral.y=0;
                crop.mCropRect.s.w=3264;
                crop.mCropRect.s.h=2448;
                crop.mResizeDst.w=3264;
                crop.mResizeDst.h=2448;

                crop2.mCropRect.p_fractional.x=0;
                crop2.mCropRect.p_fractional.y=0;
                crop2.mCropRect.p_integral.x=0;
                crop2.mCropRect.p_integral.y=0;
                crop2.mCropRect.s.w=3264;
                crop2.mCropRect.s.h=2448;
                crop2.mResizeDst.w=3264;
                crop2.mResizeDst.h=2448;
				break;
            default:
                break;
        }
        ...
        else if(casenum==7)
        {
            if(type==0)
            {
                ...
                wrotoBuf.size=1280*720*2;
                    mpImemDrv->allocVirtBuf(LOG_TAG, &wrotoBuf);
		...
		{
			printf("Case 10 DMAO setting: img3o output, size 2560x1600 \n");
			if(type==0)
            {
                img2oBuf.size=2560*1600*2;
                mpImemDrv->allocVirtBuf(LOG_TAG, &img2oBuf);
                memset((MUINT8*)img2oBuf.virtAddr, 0xffffffff, img2oBuf.size);
                MUINT32 bufStridesInBytes_2[3] = {5120, 0, 0};
                PortBufInfo_v1 portBufInfo_2 = PortBufInfo_v1( img2oBuf.memID,img2oBuf.virtAddr,0,img2oBuf.bufSecu, img2oBuf.bufCohe);
                    IImageBufferAllocator::ImgParam imgParam_2 = IImageBufferAllocator::ImgParam((eImgFmt_YUY2),
                                                                    MSize(2560, 1600),  bufStridesInBytes_2, bufBoundaryInBytes, 1);
                sp<ImageBufferHeap> pHeap_2 = ImageBufferHeap::create( LOG_TAG, imgParam_2,portBufInfo_2,true);
                IImageBuffer* outBuffer = pHeap_2->createImageBuffer();
                outBuffer->incStrong(outBuffer);
                outBuffer->lockBuf(LOG_TAG,eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN);
        ...
        if(bV1Tunning)
        {
            applyV1Tunning(enqueParams.mFrameNo);
        }
...
//            ISP_WRITE_REG(mpIspDrv,CAM_CTL_SEL, 0, ISP_DRV_USER_ISPF);
 ...
 //           ISP_WRITE_BITS(mpIspDrv,CAM_CTL_SPARE3,INT_MRG, 1, ISP_DRV_USER_ISPF);
            ...
            if(isV3)
            {
                ...
                LOG_INF("CAM_CTL_EN_P1(0x%08x),CAM_CTL_EN_P2(0x%08x)",pIspPhyReg[0]->CAM_CTL_EN_P1.Raw,pIspPhyReg[0]->CAM_CTL_EN_P2.Raw);
                enqueParams.mvTuningData.push_back(pTuningQueBuf[0]);
            }
        ...
        {
            char filename[256];
                printf("QQ : saveBufToFile \n");
                sprintf(filename, "/data/P2UT_basicCC_rawin_img2o%dx%d.yuv", 640,480);
                saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[0].mBuffer->getBufVA(0)), 640 *480 * 2);
        }
        else if(casenum==1)
        {
            char filename[256];
                printf("QQ : saveBufToFile \n");
                sprintf(filename, "/data/P2UT_basicCC_yuvin_img2o%dx%d.yuv", 640,480);
                saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[0].mBuffer->getBufVA(0)), 640 *480 * 2);
        }
        else if(casenum==4)
        {
            char filename[256];
                printf("QQ : saveBufToFile \n");
                sprintf(filename, "/data/P2UT_basicCC_img2o_resized_%dx%d.yuv", 320,240);
        ...
        else if(casenum==5)
        {
            char filename[256];
                printf("QQ : saveBufToFile \n");
                sprintf(filename, "/data/P2UT_basicCC_img2o_crop_resized_%dx%d.yuv", 320,240);
                saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[0].mBuffer->getBufVA(0)), 320 *240 * 2);
        }
        else if(casenum==6)
        {
            if(type==0)
            {
                char filename[256];
                    printf("QQ : saveBufToFile \n");
                    sprintf(filename, "/data/P2UT_multiThread_CC_img2o%dx%d.yuv", 640,480);
                    saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[0].mBuffer->getBufVA(0)), 640 *480 * 2);
            }
        }
        else if(casenum==7)
        {
            char filename[256];
                printf("QQ : saveBufToFile \n");
                sprintf(filename, "/data/P2UT_directLink_7_0_wdmao%dx%d.yuv", 1280,720);
                saveBufToFile(filename, reinterpret_cast<MUINT8*>(wdmaoBuf.virtAddr), 1280 *720 * 2);
                char filename2[256];
                sprintf(filename2, "/data/P2UT_directLink_7_0_wroto%dx%d.yuv", 1280,720);
                saveBufToFile(filename2, reinterpret_cast<MUINT8*>(wrotoBuf.virtAddr), 1280 *720 * 2);
        }
		else if (casenum == 10)
		{
			char filename[256];
            printf("Case 10 saveBufToFile \n");
            sprintf(filename, "/data/P2UT_basicCC_rgbin_img3o%dx%d.yuv", 2560,1600);
            saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[0].mBuffer->getBufVA(0)), 2560 *1600 * 2);
		}
        #if 0
        else if(casenum==8)
        {
            char filename[256];
                sprintf(filename, "/data/basicP2_4_disp%dx%d.yuv", 640,480);
                saveBufToFile(filename, reinterpret_cast<MUINT8*>(wdmaoBuf.virtAddr), 640 *480 * 2);
                char filename2[256];
                sprintf(filename2, "/data/basicP2_4_vido%dx%d.yuv", 640,960);
...
int pass2_BasicTest_ip_raw(int casenum, int type)
{
        ...
        NSCam::NSIoPipe::NSPostProc::IFeatureStream* pStream;

        pStream = NSCam::NSIoPipe::NSPostProc::IFeatureStream::createInstance(LOG_TAG, NSCam::NSIoPipe::NSPostProc::EFeatureStreamTag_PRaw_Stream,0xFFFF, isV3);
        pStream->init();
        IMemDrv* mpImemDrv=NULL;
        mpImemDrv=IMemDrv::createInstance();
        mpImemDrv->init(LOG_TAG);
        ...
        IImageBuffer* tempBuffer;
            ...
            // ip raw should not use yuy2 as input
            else //if((casenum==3))
            {    //imgi in/img2o out, test isp only mode (bypass mdp)
                MUINT32 bufStridesInBytes[3] = {1280, 0, 0};
                imgiBuf.size=sizeof(g_imgi_array_640x480_yuy2);
        ...
        IMEM_BUF_INFO imgoBuf;
            ...
            mpImemDrv->allocVirtBuf(&imgoBuf);
                        printf("QQ : allocVirtBuf \n");
            memset((MUINT8*)imgoBuf.virtAddr, 0xffffffff, imgoBuf.size);
            MUINT32 bufStridesInBytes_2[3] = {1280, 0, 0};
            PortBufInfo_v1 portBufInfo_2 = PortBufInfo_v1( imgoBuf.memID,imgoBuf.virtAddr,0,imgoBuf.bufSecu, imgoBuf.bufCohe);
            IImageBufferAllocator::ImgParam imgParam_2 = IImageBufferAllocator::ImgParam((eImgFmt_YUY2),MSize(640, 480),  bufStridesInBytes_2, bufBoundaryInBytes, 1);
            #endif

                        printf("QQ : IImageBufferAllocator \n");
            sp<ImageBufferHeap> pHeap_2 = ImageBufferHeap::create( LOG_TAG, imgParam_2,portBufInfo_2,true);
            IImageBuffer* outBuffer = pHeap_2->createImageBuffer();
            outBuffer->incStrong(outBuffer);
            outBuffer->lockBuf(LOG_TAG,eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN);
       ...
       if(casenum==2)
        {
                ...
                char filename[256];
                    #if 0
                sprintf(filename, "/data/basicP2_IPRAW_imgo%dx%d.yuv", 640,480);
                saveBufToFile(filename, reinterpret_cast<MUINT8*>(wdmaoBuf.virtAddr), 640 *480 * 10/8);
                #else
                if(type == 0)
                {
                    sprintf(filename, "/data/P2_basicIPRAW_rawin_imgo_V1_%dx%d.yuv", 640,480);
                }
                else if(type == 1)
                {
                    sprintf(filename, "/data/P2_basicIPRAW_rawin_imgo_V3_%dx%d.yuv", 640,480);
                }
                saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[0].mBuffer->getBufVA(0)), 640 *480 * 2);
                #endif
        printf("QQ : saveBufToFile ");
        ...
        else //(casenum==3)
        {
                ...
                saveBufToFile(filename, reinterpret_cast<MUINT8*>(wdmaoBuf.virtAddr), 640 *480 * 10/8);
                #else
                sprintf(filename, "/data/P2_basicIPRAW_yuvin_imgo%dx%d.yuv", 640,480);
                saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[0].mBuffer->getBufVA(0)), 640 *480 * 2);
                #endif
        }
...
//                ISP_WRITE_BITS(mpIspDrv,CAM_CTL_SPARE3,INT_MRG, 1, ISP_DRV_USER_ISPF);
                ...
                NSCam::NSIoPipe::NSPostProc::INormalStream* pCCStream;
                //object init
                pCCStream= NSCam::NSIoPipe::NSPostProc::INormalStream::createInstance("pass2_loopTest", NSCam::NSIoPipe::NSPostProc::ENormalStreamTag_Prv,0xFFFF, isV3);
                pCCStream->init();
                //////////////////////////
                //prepare dma data, imgi
                IMEM_BUF_INFO imgiBuf;
                ...
                for(int i=0;i<10;i++)
                {
                    ...
                    //save image
                    char filename[256];
                        sprintf(filename, "/data/P2UT_multiThread_pass2_loopTest_case6_1_%d_img2o%dx%d.yuv",i, out_w,out_h);
                        saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[0].mBuffer->getBufVA(0)), out_w *out_h * 2);
                        //unmap and free buffer
                        outBuffer->unlockBuf("pass2_loopTest");
                        mpImemDrv->freeVirtBuf(LOG_TAG, &img2oBuf);
                }
 ...
 //               ISP_WRITE_BITS(mpIspDrv,CAM_CTL_SPARE3,INT_MRG, 1, ISP_DRV_USER_ISPF);
                ...
                for(int i=0;i<10;i++)
                {
                    ...
                    //save image
                    char filename[256];
                        sprintf(filename, "/data/P2UT_directLink_loopTest_case7_1_%d_wdmao%dx%d.yuv",i, out_w,out_h);
                        saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[0].mBuffer->getBufVA(0)), out_w *out_h * 2);
                        char filename_2[256];
                        sprintf(filename_2, "/data/P2UT_directLink_loopTest_case7_1_%d_wroto%dx%d.yuv",i, out_w_2,out_h_2);
                        ...
                        mpImemDrv->freeVirtBuf(LOG_TAG, &wdmaoBuf);
                        mpImemDrv->freeVirtBuf(LOG_TAG, &wrotoBuf);
                }
                    LOG_INF("pass2_loopTest loop end");
                //destroy
                    pCCStream->uninit();
 ...
 //               ISP_WRITE_BITS(mpIspDrv,CAM_CTL_SPARE3,INT_MRG, 1, ISP_DRV_USER_ISPF);
                ...
                NSCam::NSIoPipe::NSPostProc::INormalStream* pCCStream;
                //object init
                pCCStream= NSCam::NSIoPipe::NSPostProc::INormalStream::createInstance("pass2_loopTest", NSCam::NSIoPipe::NSPostProc::ENormalStreamTag_Prv,0xFFFF, bTuning);
                pCCStream->init();
                //////////////////////////
                //prepare dma data, imgi
                IMEM_BUF_INFO imgiBuf;
                ...
                for(int i=0;i<10;i++)
                {
                    ...
                    //save image
                    char filename[256];
                        sprintf(filename, "/data/P2_case_%d_type_%d_pass2loop_%d_wdmao%dx%d.yuv",casenum,type,i, out_w,out_h);
                        saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[0].mBuffer->getBufVA(0)), out_w *out_h * 2);
                        char filename_2[256];
                        sprintf(filename_2, "/data/P2_case_%d_type_%d_pass2loop_%d_wroto%dx%d.yuv",casenum,type,i, out_w,out_h);
...
int pass2_multiEnqueTest(int casenum, int type)
{
    ...
    NSCam::NSIoPipe::NSPostProc::INormalStream* pCCStream;
    //object init
    pCCStream= NSCam::NSIoPipe::NSPostProc::INormalStream::createInstance("pass2_multiEnqueTest", NSCam::NSIoPipe::NSPostProc::ENormalStreamTag_Prv,0xFFFF, isV3);
    pCCStream->init();
    ...
    LOG_INF("pass2_multiEnque start");
        ...
        //save image
        char filename[256];
            sprintf(filename, "/data/multiEnque_pass2_%d_0_img2o%dx%d.yuv",i, 640,480);
            saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[0].mBuffer->getBufVA(0)), 640 *480 * 2);
            char filename_2[256];
            sprintf(filename_2, "/data/multiEnque_pass2_%d_1_img2o%dx%d.yuv",i, 640,480);
...
void *Vss_Thread(void *data)
{
    ...
    MBOOL isV3 = MTRUE;
    threadData = *((int*)data);
    MUINT32 wrotoW, wrotoH;
    int cropW, cropH;
    MUINT32 imageW, imageH;
    ...
    //
    //objects
    NSCam::NSIoPipe::NSPostProc::INormalStream* pStream = NSCam::NSIoPipe::NSPostProc::INormalStream::createInstance(LOG_TAG,NSCam::NSIoPipe::NSPostProc::ENormalStreamTag_Vss,0xFFFF, isV3);
    ...
    pIspPhyReg = (isp_reg_t *)pTuningQueBuf;
    applyTuningSetting(0, &pIspPhyReg);
    LOG_INF("CAM_CTL_EN_P1(0x%08x),CAM_CTL_EN_P2(0x%08x)",pIspPhyReg->CAM_CTL_EN_P1.Raw,pIspPhyReg->CAM_CTL_EN_P2.Raw);
    enqueParams.mvTuningData.push_back(pTuningQueBuf);
    // in/out port
    //imem buffer 2 image heap
...
#define P2_TUNINGQUE_NUM    10
    ...
    int threadData;
    int threadStatus;
    MUINT32 wrotoW=0, wrotoH=0, wdmaoW, wdmaoH;
    int cropW=0, cropH=0;
    MUINT32 imageW=0, imageH=0;
    MRect p1SrcCrop;
    MSize p1Dst;
    MRect p1DstCrop;
    ...
    MCrpRsInfo crop2;
    crop2.mGroupID=2;
    MCrpRsInfo crop3;	// add for wroto v3
    crop3.mGroupID=3;	// add for wroto v3
    ...
    //objects
    NSCam::NSIoPipe::NSPostProc::INormalStream* pStream = NSCam::NSIoPipe::NSPostProc::INormalStream::createInstance(LOG_TAG,NSCam::NSIoPipe::NSPostProc::ENormalStreamTag_Prv,0xFFFF, isV3);
    ...
    crop3.mCropRect.p_integral.x=(imageW-wrotoW)>>1;
    crop3.mCropRect.p_integral.y=(imageH-wrotoH)>>1;
    crop3.mCropRect.s.w=cropW;
    crop3.mCropRect.s.h=cropH;
    crop3.mResizeDst.w=wrotoW;
    crop3.mResizeDst.w=wrotoH;
	// add for wroto v3 -----

    enqueParams.mvCropRsInfo.push_back(crop);
    enqueParams.mvCropRsInfo.push_back(crop2);
	enqueParams.mvCropRsInfo.push_back(crop3);	// add for wroto v3
    ...
    if(isV3)
    {
        ...
        pIspPhyReg[0] = (isp_reg_t *)pTuningQueBuf[0];
        applyP2TuningSetting(&pIspPhyReg[0]);

//        applyTuningSetting(1, &pIspPhyReg[0]);
        LOG_INF("CAM_CTL_EN_P1(0x%08x),CAM_CTL_EN_P2(0x%08x)",pIspPhyReg[0]->CAM_CTL_EN_P1.Raw,pIspPhyReg[0]->CAM_CTL_EN_P2.Raw);
    ...
    {
        char filename[256];
            sprintf(filename, "/data/basicP2_%d_wdmao%dx%d.yuv", casenum, wdmaoW,wdmaoH);
            saveBufToFile(filename, reinterpret_cast<MUINT8*>(wdmaoBuf.virtAddr), wdmaoW *wdmaoH * 2);
    } else if(casenum==2) {
        char filename[256];
            sprintf(filename, "/data/basicP2_%d_disp%dx%d.yuv", casenum, 640,480);
            saveBufToFile(filename, reinterpret_cast<MUINT8*>(wdmaoBuf.virtAddr), 640 *480 * 2);
            char filename2[256];
            sprintf(filename2, "/data/basicP2_%d_vido%dx%d.yuv", casenum, 1280,720);
            saveBufToFile(filename2, reinterpret_cast<MUINT8*>(wrotoBuf.virtAddr), 1280 *720 * 2);
    } else if(casenum==3) {
        char filename[256];
        sprintf(filename, "/data/basicP2_%d_wdmao%dx%d.yuv", casenum, wdmaoW,wdmaoH);
        ...
        //
        char filename2[256];
        sprintf(filename2, "/data/basicP2_%d_wroto%dx%d.yuv", casenum, wrotoW,wrotoH);
        saveBufToFile(filename2, reinterpret_cast<MUINT8*>(wrotoBuf.virtAddr), wrotoW *wrotoH * 2);
    } else if(casenum==4) {
        char filename[256];
            sprintf(filename, "/data/basicP2_%d_wroto%dx%d.yuv", casenum, wrotoW,wrotoH);
            saveBufToFile(filename, reinterpret_cast<MUINT8*>(wrotoBuf.virtAddr), wrotoW *wrotoH * 2);
        //
    ...
    //G2G
    #if 0   //original tuning setting, yuv to rgb
    TUNING_MGR_REG_IO_STRUCT g_g2gData[] =
        {{0x00004920, 0x00000200},
        {0x00004924, 0x000002CE},
        {0x00004928, 0x1F500200},
        {0x0000492C, 0x00001E92},
        {0x00004930, 0x038B0200},
        {0x00004934, 0x00000200},
        {0x00004938, 0x00000009}};
    #else   //special tuning setting
    TUNING_MGR_REG_IO_STRUCT g_g2gData[] =
        {{0x00004920, 0x00000200},
        {0x00004924, 0x00000000},
        {0x00004928, 0x02000000},
        {0x0000492C, 0x00000000},
        {0x00004930, 0x00000000},
        {0x00004934, 0x00000200},
        {0x00004938, 0x00000009}};
    #endif

    #if 1   //original tuning setting, yuv to rgb
    TUNING_MGR_REG_IO_STRUCT g_g2cConvData[] =
        {{0x00004BA0, 0x012D0099},
        {0x00004BA4, 0x0000003A},
        {0x00004BA8, 0x075607AA},
        {0x00004BAC, 0x00000100},
        {0x00004BB0, 0x072A0100},
        {0x00004BB4, 0x000007D6}};
    TUNING_MGR_REG_IO_STRUCT g_g2cShadeData[] =
        {{0x00004BB8, 0x0118000E},   // CAM_G2C_SHADE_CON_1.Bits.G2C_SHADE_EN (bit28)=0
        {0x00004BBC, 0x0074B740},
        {0x00004BC0, 0x00000133},
        {0x00004BC4, 0x079F0A5A},
        {0x00004BC8, 0x00000000}};
    #else   //special tuning setting
    #endif

void applyV1Tunning(MINT32 magicNum)
{
    //other setting preparation
    ...
    for(i=0;i<sizeof(g_g2gData)/sizeof(TUNING_MGR_REG_IO_STRUCT);i++){
        regIo[i].Addr = g_g2gData[i].Addr;
        regIo[i].Data = g_g2gData[i].Data;
    }
    TuningMgr::getInstance().tuningMgrWriteRegs(eSoftwareScenario_Main_Normal_Stream, regIo, sizeof(g_g2gData)/sizeof(TUNING_MGR_REG_IO_STRUCT));
    //
    //G2C conv
    TuningMgr::getInstance().updateEngine(eSoftwareScenario_Main_Normal_Stream, eTuningMgrFunc_G2c_Conv);
    for(i=0;i<sizeof(g_g2cConvData)/sizeof(TUNING_MGR_REG_IO_STRUCT);i++){
        regIo[i].Addr = g_g2cConvData[i].Addr;
        regIo[i].Data = g_g2cConvData[i].Data;
    }
    TuningMgr::getInstance().tuningMgrWriteRegs(eSoftwareScenario_Main_Normal_Stream, regIo, sizeof(g_g2cConvData)/sizeof(TUNING_MGR_REG_IO_STRUCT));

    //
    //G2C shade
    TuningMgr::getInstance().updateEngine(eSoftwareScenario_Main_Normal_Stream, eTuningMgrFunc_G2c_Shade);
    for(i=0;i<sizeof(g_g2cShadeData)/sizeof(TUNING_MGR_REG_IO_STRUCT);i++){
        regIo[i].Addr = g_g2cShadeData[i].Addr;
        regIo[i].Data = g_g2cShadeData[i].Data;
    }
    TuningMgr::getInstance().tuningMgrWriteRegs(eSoftwareScenario_Main_Normal_Stream, regIo, sizeof(g_g2cShadeData)/sizeof(TUNING_MGR_REG_IO_STRUCT));
#endif
...
int applyP2TuningSetting(isp_reg_t **ppIspPhyReg)
{
        ...
        ppIspPhyReg[0]->CAM_CTL_EN_P2.Bits.NSL2_EN=0;
        ...
        pIspSetting[0x4BB8>>2] = 0x0118000E;
        pIspSetting[0x4BBC>>2] = 0x0074B740;
        pIspSetting[0x4BC0>>2] = 0x00000133;
        pIspSetting[0x4BC4>>2] = 0x079F0A5A;
        pIspSetting[0x4BC8>>2] = 0x00000000;

	pIspSetting[0x000048A0>>2] = 0x38303060;
    	pIspSetting[0x000048A4>>2] = 0x1430063F;
    	pIspSetting[0x000048A8>>2] = 0x00600600;
    	pIspSetting[0x000048AC>>2] = 0x07FF0100;
    	pIspSetting[0x000048B0>>2] = 0x02008020;
    	pIspSetting[0x000048B4>>2] = 0x003FFFE0;
    	pIspSetting[0x000048B8>>2] = 0x08421000;
    	pIspSetting[0x000048BC>>2] = 0x81028000;
    	pIspSetting[0x000048C0>>2] = 0x00000050;
    	pIspSetting[0x000048C4>>2] = 0x00000000;
    	pIspSetting[0x000048C8>>2] = 0x145034DC;
    	pIspSetting[0x000048CC>>2] = 0x0034FF55;
    	pIspSetting[0x000048D0>>2] = 0x00DF2064;
    	pIspSetting[0x000048D4>>2] = 0x007FAC00;
    	pIspSetting[0x000048D8>>2] = 0x0000000F;
    	pIspSetting[0x000048DC>>2] = 0x00000000;
    	pIspSetting[0x000048E0>>2] = 0x00000000;
    	pIspSetting[0x000048E4>>2] = 0x00000000;
    	pIspSetting[0x000048E8>>2] = 0x00000000;
...
int applyTuningSetting(int mode, isp_reg_t **ppIspPhyReg)
{
    ...
    if(mode==0){
        ...
        #else
        ppIspPhyReg[0]->CAM_CTL_EN_P1.Bits.LSC_EN=1;
...
* case5 : [preview path][zoom test] imgi in/img2o out, isp only&frame mode, enable cdrz, 640x480 in,crop 400x200, /320x240 out (input format: bayer 10)
********************************************************************************/
int pass2_BurstQueueTest(int casenum, int type)
{
        int ret=0;
            MBOOL isV3 = MFALSE;    //temp disable tuning path
        sem_init(&mSem, 0, 0);
        ...
        printf("QQ : casenum(%d),type(%d) \n",casenum,type);
            ...
            IspDrvShell* m_pIspDrvShell = IspDrvShell::createInstance();
            m_pIspDrvShell->init();
            m_pIspDrvShell->updateBurstQueueNum(2);
            ...
            IImageBuffer* imageBuffer_640x480_b10;
            IImageBuffer* imageBuffer_640x480_yuy2;
            IImageBuffer* imageBuffer_1280x720_b10;


                if(casenum==0)
                {    //imgi in/img2o out, test isp only mode (bypass mdp)
                    MINT32 bufStridesInBytes[3] = {800, 0, 0};
                    imgiBuf.size=sizeof(g_imgi_array_640x480_b10);
                    mpImemDrv->allocVirtBuf(LOG_TAG, &imgiBuf);
                    memcpy( (MUINT8*)(imgiBuf.virtAddr), (MUINT8*)(g_imgi_array_640x480_b10), imgiBuf.size);
                    //imem buffer 2 image heap
                    ...
                    IImageBufferAllocator::ImgParam imgParam = IImageBufferAllocator::ImgParam((eImgFmt_BAYER10), MSize(640, 480), bufStridesInBytes, bufBoundaryInBytes, 1);
                    sp<ImageBufferHeap> pHeap = ImageBufferHeap::create( LOG_TAG, imgParam,portBufInfo,true);
                    imageBuffer_640x480_b10 = pHeap->createImageBuffer();
                    imageBuffer_640x480_b10->incStrong(imageBuffer_640x480_b10);
                    imageBuffer_640x480_b10->lockBuf(LOG_TAG,eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN);

                    MINT32 bufStridesInBytes_2[3] = {1280, 0, 0};
                    imgiBuf_2.size=sizeof(g_imgi_array_640x480_yuy2);
                    mpImemDrv->allocVirtBuf(LOG_TAG, &imgiBuf_2);
                    memcpy( (MUINT8*)(imgiBuf_2.virtAddr), (MUINT8*)(g_imgi_array_640x480_yuy2), imgiBuf_2.size);
                    //imem buffer 2 image heap
                    PortBufInfo_v1 portBufInfo_2 = PortBufInfo_v1( imgiBuf_2.memID,imgiBuf_2.virtAddr,0,imgiBuf_2.bufSecu, imgiBuf_2.bufCohe);
                    IImageBufferAllocator::ImgParam imgParam_2 = IImageBufferAllocator::ImgParam((eImgFmt_YUY2), MSize(640, 480), bufStridesInBytes_2, bufBoundaryInBytes_2, 1);
                    sp<ImageBufferHeap> pHeap_2 = ImageBufferHeap::create( LOG_TAG, imgParam_2,portBufInfo_2,true);
                    imageBuffer_640x480_yuy2 = pHeap_2->createImageBuffer();
                    imageBuffer_640x480_yuy2->incStrong(imageBuffer_640x480_yuy2);
                    imageBuffer_640x480_yuy2->lockBuf(LOG_TAG,eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN);
                }
                else if((casenum==1))
                {
                    MINT32 bufStridesInBytes[3] = {800, 0, 0};
                    ...
                    MINT32 bufStridesInBytes_2[3] = {1600, 0, 0};
                    imgiBuf_2.size=sizeof(g_imgi_array_1280x720_b10);
                    mpImemDrv->allocVirtBuf(LOG_TAG, &imgiBuf_2);
                    memcpy( (MUINT8*)(imgiBuf_2.virtAddr), (MUINT8*)(g_imgi_array_1280x720_b10), imgiBuf_2.size);
                    //imem buffer 2 image heap
                    PortBufInfo_v1 portBufInfo_2 = PortBufInfo_v1( imgiBuf_2.memID,imgiBuf_2.virtAddr,0,imgiBuf_2.bufSecu, imgiBuf_2.bufCohe);
                    IImageBufferAllocator::ImgParam imgParam_2 = IImageBufferAllocator::ImgParam((eImgFmt_YUY2), MSize(640, 480), bufStridesInBytes_2, bufBoundaryInBytes_2, 1);
                    sp<ImageBufferHeap> pHeap_2 = ImageBufferHeap::create( LOG_TAG, imgParam_2,portBufInfo_2,true);
                    imageBuffer_1280x720_b10 = pHeap_2->createImageBuffer();
                    imageBuffer_1280x720_b10->incStrong(imageBuffer_1280x720_b10);
                    imageBuffer_1280x720_b10->lockBuf(LOG_TAG,eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN);
            ...
            if(casenum==0)
            {
                src.mPortID=IMGI;
                src.mPortID.group=0;
                src.mBuffer=imageBuffer_640x480_b10;
                enqueParams.mvIn.push_back(src);

                src_2.mPortID=IMGI;
                src_2.mPortID.group=1;
                src_2.mBuffer=imageBuffer_640x480_yuy2;
                enqueParams.mvIn.push_back(src_2);
            }
            else if(casenum==1)
            {
                ...
                src_2.mPortID=IMGI;
                src_2.mPortID.group=1;
                src_2.mBuffer=imageBuffer_1280x720_b10;
                enqueParams.mvIn.push_back(src_2);
            }
            ...
            IImageBuffer* outBuffer;
            IImageBuffer* outBuffer_2;
            ...
            {    //imgi in/img2o out, test isp only mode (bypass mdp)
                ...
                memset((MUINT8*)img2oBuf.virtAddr, 0xffffffff, img2oBuf.size);
                MUINT32 bufStridesInBytes[3] = {1280, 0, 0};
                PortBufInfo_v1 portBufInfo = PortBufInfo_v1( img2oBuf.memID,img2oBuf.virtAddr,0,img2oBuf.bufSecu, img2oBuf.bufCohe);
                IImageBufferAllocator::ImgParam imgParam = IImageBufferAllocator::ImgParam((eImgFmt_YUY2), MSize(640, 480),  bufStridesInBytes, bufBoundaryInBytes, 1);
                sp<ImageBufferHeap> pHeap = ImageBufferHeap::create( LOG_TAG, imgParam,portBufInfo,true);
                outBuffer = pHeap->createImageBuffer();
                outBuffer->incStrong(outBuffer);
                outBuffer->lockBuf(LOG_TAG,eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN);

                img2oBuf_2.size=640*480*2;
                mpImemDrv->allocVirtBuf(LOG_TAG, &img2oBuf_2);
                memset((MUINT8*)img2oBuf_2.virtAddr, 0xffffffff, img2oBuf_2.size);
                MUINT32 bufStridesInBytes_2[3] = {1280, 0, 0};
                PortBufInfo_v1 portBufInfo_2 = PortBufInfo_v1( img2oBuf_2.memID,img2oBuf_2.virtAddr,0,img2oBuf_2.bufSecu, img2oBuf_2.bufCohe);
                IImageBufferAllocator::ImgParam imgParam_2 = IImageBufferAllocator::ImgParam((eImgFmt_YUY2), MSize(640, 480),  bufStridesInBytes_2, bufBoundaryInBytes_2, 1);
                sp<ImageBufferHeap> pHeap_2 = ImageBufferHeap::create( LOG_TAG, imgParam_2,portBufInfo_2,true);
                outBuffer_2 = pHeap_2->createImageBuffer();
                outBuffer_2->incStrong(outBuffer_2);
                outBuffer_2->lockBuf(LOG_TAG,eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN);
            }
            else if(casenum==1)
            {
                img2oBuf.size=640*480*2;
                ...
                img2oBuf_2.size=1280*720*2;
                mpImemDrv->allocVirtBuf(LOG_TAG, &img2oBuf_2);
                memset((MUINT8*)img2oBuf_2.virtAddr, 0xffffffff, img2oBuf_2.size);
                MUINT32 bufStridesInBytes_2[3] = {1280, 0, 0};
                PortBufInfo_v1 portBufInfo_2 = PortBufInfo_v1( img2oBuf_2.memID,img2oBuf_2.virtAddr,0,img2oBuf_2.bufSecu, img2oBuf_2.bufCohe);
                IImageBufferAllocator::ImgParam imgParam_2 = IImageBufferAllocator::ImgParam((eImgFmt_YUY2), MSize(1280, 720),  bufStridesInBytes_2, bufBoundaryInBytes_2, 1);
                sp<ImageBufferHeap> pHeap_2 = ImageBufferHeap::create( LOG_TAG, imgParam_2,portBufInfo_2,true);
                outBuffer_2 = pHeap_2->createImageBuffer();
                outBuffer_2->incStrong(outBuffer_2);
                outBuffer_2->lockBuf(LOG_TAG,eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN);
    ...
    //            ISP_WRITE_REG(mpIspDrv,CAM_CTL_SEL, 0, ISP_DRV_USER_ISPF);
     ...
     //           ISP_WRITE_BITS(mpIspDrv,CAM_CTL_SPARE3,INT_MRG, 1, ISP_DRV_USER_ISPF);
            ...
            if(casenum==0)
            {
                char filename[256];
                    printf("QQ : saveBufToFile \n");
                    sprintf(filename, "/data/P2UT_burst_case0_img2o%dx%d.yuv", 640,480);
                    saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[0].mBuffer->getBufVA(0)), 640 *480 * 2);

                char filename_2[256];
                    printf("QQ : saveBufToFile_2 \n");
                    sprintf(filename_2, "/data/P2UT_burst_case0_img2o%dx%d_2.yuv", 640,480);
                    saveBufToFile(filename_2, reinterpret_cast<MUINT8*>(dequeParams.mvOut[1].mBuffer->getBufVA(0)), 640 *480 * 2);
            }
            else if(casenum==1)
            {
                char filename[256];
                    printf("QQ : saveBufToFile \n");
                    sprintf(filename, "/data/P2UT_burst_case1_wdmao%dx%d.yuv", 640,480);
                    saveBufToFile(filename, reinterpret_cast<MUINT8*>(dequeParams.mvOut[0].mBuffer->getBufVA(0)), 640 *480 * 2);

                char filename_2[256];
                    printf("QQ : saveBufToFile \n");
                    sprintf(filename_2, "/data/P2UT_burst_case1_wroto%dx%d_2.yuv", 1280,720);
                    saveBufToFile(filename_2, reinterpret_cast<MUINT8*>(dequeParams.mvOut[1].mBuffer->getBufVA(0)), 1280 *720 * 2);
            }
            ////
            //temp enter while to attach code viser
            //LOG_INF("ENTER WHILE");
