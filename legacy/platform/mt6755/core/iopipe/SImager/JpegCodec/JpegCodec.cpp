...
)
{
    ...
    if (checkIfNeedImgTransform(eCodecType, pSrcBufInfo, pDstBufInfo, rROI, u4Transform))
    {
...

        {
            ...
            getAlignment(eCodecType, format, &w_align, &h_align);
...
MBOOL
JpegCodec::
getAlignment(MUINT32 const eCodecType, MINT32 const eFmt, MUINT32* width, MUINT32* height)
{
...
)
{
    ...
    {
        MUINT32 w_align, h_align;
        if( getAlignment(eCodecType, pSrcBufInfo->getImgFormat(), &w_align, &h_align) )
        {
            if (((pSrcBufInfo->getImgSize().w & (w_align-1) ) != 0) ||
                    ((pSrcBufInfo->getImgSize().h & (h_align-1) ) != 0))
            {
                MY_LOGD("encType 0x%x, src fmt 0x%x, width/height not aligh to %dx/%dx, src =(%d, %d)",
                        eCodecType,
                        pSrcBufInfo->getImgFormat(),
                        w_align,
                        h_align,
