...
//
    ...
    =   {
            ...
            {CAM_ISP_NBC_TBL,        0x7600,  256  },	// for ANR table
            {CAM_ISP_NBC,            0x7A00,  32   },	// for ANR table
            ...
            {CAM_ISP_NSL2,           0x4A78,  10   },
            //{CAM_ISP_SL2B,           0x4F60,  7    },
            {CAM_ISP_NSL2B,          0x4A50,  10   },
            ...
            {CAM_ISP_NBC2,           0x7B00,  48   },
            {CAM_ISP_NSL2A,          0x4A20,  10   },
            ...
            {CAM_ISP_RWB,     		 0x7BC0,  9    },	// add for RWB sensor
            {CAM_ISP_ISO,     		 0x7BE4,  1    },	// add for ISO adaptive
 ...
 ISP_TURNING_FUNC_BIT_MAPPING gIspTuningFuncBitMapp[eIspTuningMgrFuncBit_Num]
    ={{eIspTuningMgrFuncBit_Dbs             ,  eTuningCtlByte_P1, 24, -1, -1, -1, -1, -1, CAM_ISP_DBS,         CAM_DUMMY_},
      {eIspTuningMgrFuncBit_Dbs_d           , eTuningCtlByte_P1D, -1, 24, -1, -1, -1, -1, CAM_ISP_DBS_D,       CAM_DUMMY_},
      ...
      {eIspTuningMgrFuncBit_Rmg             ,  eTuningCtlByte_P1, 22, -1, -1, -1, -1, -1, CAM_CTL_VHDR,         CAM_DUMMY_},
      {eIspTuningMgrFuncBit_Rmg_d           , eTuningCtlByte_P1D, -1, 22, -1, -1, -1, -1, CAM_CTL_VHDR_D,       CAM_DUMMY_},
      ...
      {eIspTuningMgrFuncBit_Nsl2            ,  eTuningCtlByte_P2, -1, -1,  2, -1, -1, -1, CAM_ISP_NSL2,         CAM_DUMMY_},
      //{eIspTuningMgrFuncBit_Cfa             ,  eTuningCtlByte_P2, -1, -1,  3, -1, -1, -1, CAM_ISP_CFA,          CAM_DUMMY_},
      {eIspTuningMgrFuncBit_Udm             ,  eTuningCtlByte_P2, -1, -1,  3, -1, -1, -1, CAM_ISP_UDM,          CAM_DUMMY_},
      ...
      {eIspTuningMgrFuncBit_Nsl2b           ,  eTuningCtlByte_P2, -1, -1, 25, -1, -1, -1, CAM_ISP_NSL2B,        CAM_DUMMY_},
      ...
      {eIspTuningMgrFuncBit_Nbc2            ,  eTuningCtlByte_P2, -1, -1, 16, -1, -1, -1, CAM_ISP_NBC2,         CAM_DUMMY_},
      {eIspTuningMgrFuncBit_Nsl2a           ,  eTuningCtlByte_P2, -1, -1, 24, -1, -1, -1, CAM_ISP_NSL2A,        CAM_DUMMY_},
      {eIspTuningMgrFuncBit_Nbc_Tbl         ,  eTuningCtlByte_P2, -1, -1, 11, -1, -1, -1, CAM_ISP_NBC_TBL,      CAM_DUMMY_},	// for ANR table
      {eIspTuningMgrFuncBit_Rwb          	,  eTuningCtlByte_P2, -1, -1,  0, -1, -1, -1, CAM_ISP_RWB,     		CAM_DUMMY_},	// add for RWB sensor
      {eIspTuningMgrFuncBit_Iso          	,  eTuningCtlByte_P2, -1, -1,  0, -1, -1, -1, CAM_ISP_ISO,     		CAM_DUMMY_}};	// add for ISO adaptive
...
MBOOL IspDrvImp::init(const char* userName)
{
    ...
    for(int i=0;i<ISP_DRV_P2_CQ_NUM;i++){
        for(int j=0;j<ISP_TUNING_QUEUE_NUM;j++){
            ...
            if ( m_pIMemDrv->allocVirtBuf(NAME, &m_ispTmpBufInfo) ) {
            ...
            memset((MUINT8*)mTuningQueInf[i][j].pTuningQue,0,mTuningQueInf[i][j].queSize);
            LOG_DBG("p2cq(%d),que(%d):pTuningQue(0x%x)",i,j,mTuningQueInf[i][j].pTuningQue);
        }
        ...
        for(int i=0;i<ISP_DRV_P2_CQ_NUM;i++){
            for(int j=0;j<ISP_TUNING_QUEUE_NUM;j++){
                if((mTuningQueInf[i][j].queFd>=0) && (mTuningQueInf[i][j].pTuningQue!=0)){
                    ...
                    if ( m_pIMemDrv->freeVirtBuf(NAME, &m_ispTmpBufInfo) ) {
...
MBOOL IspDrvImp::cqTableControl(ISP_DRV_CQTABLE_CTRL_ENUM cmd,int burstQnum)
{
    ...
    switch(cmd)
    {
        case ISP_DRV_CQTABLE_CTRL_ALLOC:
            ...
            LOG_DBG("m_ispVirRegBufInfo.size: %d.", m_ispVirRegBufInfo.size);
            //
            if ( m_pIMemDrv->allocVirtBuf(NAME, &m_ispVirRegBufInfo) ){
                LOG_ERR("ERROR:m_pIMemDrv->allocVirtBuf");
                ret=false;
                break;
            }
            ...
            for (int i = 0; i < mTotalCQNum; i++) {
                ...
                LOG_DBG("cq:[%d],virtIspAddr:virt[0x%08x]/phy[0x%08x]",i,mpIspVirRegAddrVA[i], mpIspVirRegAddrPA[i] );
            }
            ...
            if ( m_pIMemDrv->allocVirtBuf(NAME, &m_ispCQDescBufInfo) ) {
                LOG_ERR("ERROR:m_pIMemDrv->allocVirtBuf");
                ret=false;
                break;
            }
            ...
            for (int i = 0; i < mTotalCQNum; i++)
            {
                ...
                LOG_DBG("cq:[%d],mpIspCQDescriptor:Virt[0x%08x]/Phy[0x%08x],size/num(%d/%d/%d)",i,mpIspCQDescriptorVirt[i],mpIspCQDescriptorPhy[i],sizeof(ISP_DRV_CQ_CMD_DESC_STRUCT),CAM_MODULE_MAX,sizeof(mIspCQDescInit));
            }
        ...
        case ISP_DRV_CQTABLE_CTRL_DEALLOC:
            ...
            if ( 0 != mpIspVirRegBufferBaseAddr )
            {
                ...
                //
                if ( m_pIMemDrv->freeVirtBuf(NAME, &m_ispVirRegBufInfo) ) {
                    LOG_ERR("ERROR:m_pIMemDrv->freeVirtBuf");
                }
            ...
            if( 0 != mpIspCQDescBufferVirt )
            {
                ...
                //
                if ( m_pIMemDrv->freeVirtBuf(NAME, &m_ispCQDescBufInfo) ) {
                    LOG_ERR("ERROR:m_pIMemDrv->freeVirtBuf");
                }
...
//QQ    LOG_INF("CAM_FEO_ERR_STAT=0x%08x",readReg((((UINT32)&ispReg.CAM_FEO_ERR_STAT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    ...
    for(i=0x7000;i<=0x7C00;i+=20){
        LOG_INF("(0x%04x,0x%08x)(0x%04x,0x%08x)(0x%04x,0x%08x)(0x%04x,0x%08x)(0x%04x,0x%08x)",
            i,readReg(i, ISP_DRV_USER_ISPF),i+4,readReg(i+4, ISP_DRV_USER_ISPF),i+8,readReg(i+8, ISP_DRV_USER_ISPF),i+12,readReg(i+12, ISP_DRV_USER_ISPF),i+16,readReg(i+16, ISP_DRV_USER_ISPF));
    }

	// Check HW rule
	MUINT32 regCtlEnP2;	// 0x15004018
	MUINT32 regCtlTcmEn; // 0x15004084
	MUINT32 regAnr1Con1; // 0x15007A00
	MUINT32 regAnr2Con1; // 0x15007B00
	MUINT32 nsl2aEn, nsl2bEn, nbcEn, nbc2En, tdrEn;
	MUINT32 anr1_LceLink, anr2_LceLink, anr2_Mode;

	regCtlEnP2 = readReg(0x4018, ISP_DRV_USER_ISPF);
	regCtlTcmEn = readReg(0x4084, ISP_DRV_USER_ISPF);
	regAnr1Con1 = readReg(0x7A00, ISP_DRV_USER_ISPF);
	regAnr2Con1 = readReg(0x7B00, ISP_DRV_USER_ISPF);
	LOG_INF("regCtlEnP2(0x%08x) regCtlTcmEn(0x%08x) regAnr1Con1(0x%08x) regAnr2Con1(0x%08x)",regCtlEnP2, regCtlTcmEn, regAnr1Con1, regAnr2Con1);

	nsl2aEn = (regCtlEnP2&0x01000000)>>24;
	nsl2bEn = (regCtlEnP2&0x02000000)>>25;
	nbcEn = (regCtlEnP2&0x00000800)>>11;
	nbc2En = (regCtlEnP2&0x00010000)>>16;
	tdrEn = (regCtlTcmEn&0x80000000)>>31;
	anr1_LceLink = (regAnr1Con1&0x01000000)>>24;
	anr2_LceLink = (regAnr2Con1&0x01000000)>>24;
	anr2_Mode = (regAnr2Con1&0x00001000)>>12;
	LOG_INF("NSL2A(%x) NSL2B(%x) NBC(%x) NBC2(%x) TDR(%x)",nsl2aEn,nsl2bEn,nbcEn,nbc2En,tdrEn);
	LOG_INF("anr1_LceLink(%x) anr2_LceLink(%x) anr2_Mode(%x)",anr1_LceLink,anr2_LceLink,anr2_Mode);

	if (tdrEn && nsl2aEn)
	{
		if (nbcEn == 0)
			LOG_ERR("For NSL2A enable, NBC must enable, too!");
	}
	if (tdrEn && nsl2bEn)
	{
		if (nbc2En == 0)
			LOG_ERR("For NSL2B enable, NBC2 must enable, too!");
	}
	if ((nbcEn) && (nsl2aEn == 0))
	{
		if (anr1_LceLink == 1)
			LOG_ERR("For NBC enable and NSL2A disable, ANR_CON1.ANR1_LCE_LINK must be 0!");
	}
	if ((nbc2En) && (nsl2bEn == 0))
	{
		if (anr2_LceLink == 1)
			LOG_ERR("For NBC2 ebable and NSL2B disable, ANR2_CON1.ANR2_LCE_LINK must be 0!");
	}
	if (nbc2En)
	{
		if (anr2_Mode == 1)
			LOG_ERR("For NBC2 enable, ANR2_MODE should be 0!");
	}		
	//
...
MBOOL IspDrvImp::waitIrq(ISP_DRV_WAIT_IRQ_STRUCT* pwaitIrq)
{
    ...
    MUINT32 Ta=0,Tb=0;
    //
    if(pwaitIrq->UserInfo.UserKey<0)/*cam1*/
    ...
    if(Ret < 0) {
        ...
        LOG_ERR("ISP_WAIT_IRQ fail(%d). Type(%d), Status(0x%08x), Timeout(%d).", Ret, pwaitIrq->Type, pwaitIrq->Status, pwaitIrq->Timeout);
            ...
            LOG_ERR("ISP_WAIT_IRQ fail(%d). Type(%d), Status(0x%08x), Timeout(%d).", Ret, pwaitIrq->UserInfo.Type, pwaitIrq->UserInfo.Status, pwaitIrq->Timeout);
        return MFALSE;
    }
...
MBOOL   IspDrvImp::queryirqtimeinfo(ISP_DRV_WAIT_IRQ_STRUCT* Irqinfo)
{
    ...
    else
    {
        ...
        LOG_INF("christopher-2 sigNum(%d/%d)",qirq.TimeInfo.passedbySigcnt,Irqinfo->TimeInfo.passedbySigcnt);
    ...
    if(path == eTuningQueAccessPath_featureio){
        ...
        do {
            ...
            mTuningQueIdx[p2Cq].keepP2PreviewUpdateFuncBit = (EIspTuningMgrFunc)
                    ((MINT64)mTuningQueIdx[p2Cq].keepP2PreviewUpdateFuncBit | (MINT64)mTuningQueInf[p2Cq][currQueueIdx].eUpdateFuncBit);
            //
            ...
            for(i=0;i<eIspTuningMgrFuncBit_Num;i++){
                if((1LL<<gIspTuningFuncBitMapp[i].eTuningFuncBit) & mTuningQueIdx[p2Cq].eCurReadP2UpdateFuncBit){
...
//set by imageio path                            mTuningQueIdx[p2Cq].keepTpipeField.nr3d.nr3d_on_height = pReadP2TuningQue->CAM_NR3D_ON_SIZ.Bits.NR3D_ON_HT;
        ...
        for (k=0;k<checkNum;k++){
            ...
            mTuningQueIdx[p2Cq].keepP1UpdateFuncBit = (EIspTuningMgrFunc)
                    ((MINT64)mTuningQueIdx[p2Cq].keepP1UpdateFuncBit | (MINT64)mTuningQueInf[p2Cq][currQueueIdx].eUpdateFuncBit);
            //
            // [4]update the current queue info.
            pIspRegMap = (isp_reg_t*)mTuningQueInf[p2Cq][currQueueIdx].pTuningQue;
            if (NULL == pIspRegMap){
...
//-----------------------------------------------------------------------------
MUINT64 IspDrvImp::getTuningUpdateFuncBit(
...
)
{
    ...
    pTuningField.nsl2.nsl2_hrz_comp = pTuningRegMappingQue->CAM_NSL2_RZ.Bits.SL2_HRZ_COMP;
    pTuningField.nsl2.nsl2_vrz_comp = pTuningRegMappingQue->CAM_NSL2_RZ.Bits.SL2_VRZ_COMP;
    //
    ...
    pTuningField.udm.bayer_bypass = pTuningRegMappingQue->CAM_UDM_INTP_CRS.Bits.UDM_BYP;
    //
    ...
    pTuningField.nsl2b.nsl2b_hrz_comp = pTuningRegMappingQue->CAM_NSL2B_RZ.Bits.SL2_HRZ_COMP;
    pTuningField.nsl2b.nsl2b_vrz_comp = pTuningRegMappingQue->CAM_NSL2B_RZ.Bits.SL2_VRZ_COMP;
    pTuningField.nsl2a.nsl2a_hrz_comp = pTuningRegMappingQue->CAM_NSL2A_RZ.Bits.SL2_HRZ_COMP;
    pTuningField.nsl2a.nsl2a_vrz_comp = pTuningRegMappingQue->CAM_NSL2A_RZ.Bits.SL2_VRZ_COMP;
    //
    ...
    pTuningField.nbc.anr1_eny = pTuningRegMappingQue->CAM_ANR_CON1.Bits.ANR1_ENY;
    pTuningField.nbc.anr1_enc = pTuningRegMappingQue->CAM_ANR_CON1.Bits.ANR1_ENC;
    pTuningField.nbc.anr1_iir_mode = 0;
    pTuningField.nbc.anr1_scale_mode = pTuningRegMappingQue->CAM_ANR_CON1.Bits.ANR1_SCALE_MODE;
    //
    pTuningField.nbc2.anr2_eny = pTuningRegMappingQue->CAM_ANR2_CON1.Bits.ANR2_ENY;
    pTuningField.nbc2.anr2_enc = pTuningRegMappingQue->CAM_ANR2_CON1.Bits.ANR2_ENC;
    pTuningField.nbc2.anr2_scale_mode = pTuningRegMappingQue->CAM_ANR2_CON1.Bits.ANR2_SCALE_MODE;
    //
	...
	memcpy((char*)&pTuningField.nsl2, (char*)&mTuningQueIdx[p2Cq].keepTpipeField.nsl2, sizeof(ISP_TPIPE_CONFIG_NSL2_STRUCT));
    //memcpy((char*)&pTuningField.cfa, (char*)&mTuningQueIdx[p2Cq].keepTpipeField.cfa, sizeof(ISP_TPIPE_CONFIG_CFA_STRUCT));
	memcpy((char*)&pTuningField.udm, (char*)&mTuningQueIdx[p2Cq].keepTpipeField.udm, sizeof(ISP_TPIPE_CONFIG_UDM_STRUCT));
	...
	memcpy((char*)&pTuningField.nsl2a, (char*)&mTuningQueIdx[p2Cq].keepTpipeField.nsl2a, sizeof(ISP_TPIPE_CONFIG_NSL2A_STRUCT));
	memcpy((char*)&pTuningField.nsl2b, (char*)&mTuningQueIdx[p2Cq].keepTpipeField.nsl2b, sizeof(ISP_TPIPE_CONFIG_NSL2B_STRUCT));
    memcpy((char*)&pTuningField.nbc, (char*)&mTuningQueIdx[p2Cq].keepTpipeField.nbc, sizeof(ISP_TPIPE_CONFIG_NBC_STRUCT));
	memcpy((char*)&pTuningField.nbc2, (char*)&mTuningQueIdx[p2Cq].keepTpipeField.nbc2, sizeof(ISP_TPIPE_CONFIG_NBC2_STRUCT));
    ...
    LOG_DBG("tarUdm.bypass(%d),desUdm.bypass(%d)",mTuningQueIdx[p2Cq].keepTpipeField.udm.bayer_bypass,pTuningField.udm.bayer_bypass);
...
)
{
    ...
    switch(softScenario){
        ...
        case eSoftwareScenario_Main2_Vfb_Stream_2:
...
//-----------------------------------------------------------------------------
MBOOL IspDrvImp::enqueP2Frame(MUINT32 callerID,MINT32 p2burstQIdx,MINT32 p2dupCQIdx ,MINT32 frameNum)
{
    MBOOL ret=MTRUE;
