...
enum eIONCacheFlushType
{
 ...
 struct stIMEM_USER_INFO
{
    char name[32];
    MINT32 count;   //count for calling init from the same user
    MUINT32 totalbufSize; //total size of buffer usage
};

struct stIMEM_BUF_INFO
{
    MUINTPTR va;
    MUINT32 size;
    stIMEM_BUF_INFO()
...
class IMemDrvImp : public IMemDrv
{
    ...
    private:
    /**
     * @brief Create a thread dedicated for information dump.
     *
     * @details
     *
     * @note
    ...
    MVOID createThread();
    /**
     * @brief Destroy the thread dedicated for information dump.
     *
     * @details
     *
     * @note
...
/**
     * @brief Create a file to save dbg information.
     *
     * @details
     *
     * @note
...
//            stIspIonHandleData handle_data);
    ...
    private:
        ...
        list<stIMEM_USER_INFO> mUserInfoList;
        vector<vector<stIMEM_BUF_INFO> > mBufInfoList;
