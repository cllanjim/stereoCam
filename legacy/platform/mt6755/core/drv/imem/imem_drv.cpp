...
MBOOL IMemDrvImp::init(char const* szCallerName)
{
    ...
    property_get("camera.memdbgcheck.enable", value, "0");
    ...
    if(!get)
    {
        ...
        strcpy((char*)(user.name),szCallerName);
        user.count=1;
        ...
        if(memdbgcheckEnable)
        {
            LOG_INF("list size(%s/%d/%d)",szCallerName,(int)mUserInfoList.size(), (int)mBufInfoList.size());
        }
    }
    pthread_mutex_unlock(&dbgMutex);
    //
    ...
    IMEM_BUF_INFO* pInfo)
{
    ...
    IMEM_DBG("%s mID[0x%x]/size[0x%x]/VA[0x%x]/Imem usage[0x%x]",szCallerName, pInfo->memID,pInfo->size,pInfo->virtAddr,total_size);
...

    {
        ...
        if(getmatch && (index>=0))
        {
            ...
            if(memdbgcheckEnable)
            {
                IMEM_INF("alloc check(%s_%d, size(%d))",szCallerName,index,mBufInfoList[index].size());
            }
        }
        else
        {
            LOG_WRN("user(%s) do not do init function before, va/sz(0x%x/0x%x)", szCallerName, pInfo->virtAddr, pInfo->size);
        }
        pthread_mutex_unlock(&dbgMutex);
    }
    return 0;
    ...
    IMEM_BUF_INFO* pInfo)
{
	...
	IMEM_DBG("szCallerName(%s), mID[0x%x]/size[0x%x]/VA[0x%x]/PA[0x%x]/Imem usage[0x%x]",szCallerName, pInfo->memID,pInfo->size,pInfo->virtAddr,pInfo->phyAddr,total_size);
    //get property for memdbgdump
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("camera.memdbgcheck.enable", value, "0");
    int memdbgcheckEnable=atoi(value);
        ...
        if(getmatch && (index >= 0))
        {
            ...
            for (vector<stIMEM_BUF_INFO>::iterator it = mBufInfoList[index].begin(); it != mBufInfoList[index].end(); it++)
            {
                if(pInfo->virtAddr == (*it).va)
                {
                    ...
                    if(memdbgcheckEnable)
                    {
                        IMEM_INF("free_check bf(%s/0x%x/0x%x/%d)",szCallerName,(*it).va,(*it).size,mBufInfoList[index].size());
                        mBufInfoList[index].erase(it);
                        IMEM_INF("free_check af(%s_/%d)",szCallerName,mBufInfoList[index].size());
                    }
                    else
                    {
                        mBufInfoList[index].erase(it);
            ...
            if(!getmatch)
            {
                IMEM_WRN("do not alloc before(%s/0x%x/0x%x/%d)",szCallerName,pInfo->virtAddr,pInfo->size,mBufInfoList[index].size());
            }
        }
        else
        {
            IMEM_WRN("do not allocate this buffer before user(%s), va/sz(0x%x/0x%x)", szCallerName, pInfo->virtAddr, pInfo->size);
        }
        pthread_mutex_unlock(&dbgMutex);
    }
    return 0;
...
MBOOL IMemDrvImp::saveDbgInfo()
{
    ...
    else
    {
        ...
        int index=-1;
        IMEM_INF("dgbList(%d), mCount(%d_%d)",mUserInfoList.size(),mInitCount,mLocal_InitCount);
        fprintf(f, "dgbList(%d), mCount(%d_%d)\n",mUserInfoList.size(),mInitCount,mLocal_InitCount);
        for (list<stIMEM_USER_INFO>::iterator it = mUserInfoList.begin(); it != mUserInfoList.end(); it++)
        {
            index++;
            IMEM_INF("User: %s, used memory buffer Num: %d, totalsize: 0x%x,  detail is as below\n",  (*it).name, mBufInfoList[index].size()-1, (*it).totalbufSize);
            fprintf(f, "User: %s, used memory buffer Num: %d, totalsize: 0x%x, detail is as below\n",  (*it).name, mBufInfoList[index].size()-1, (*it).totalbufSize);

            vector<stIMEM_BUF_INFO>::iterator itt = mBufInfoList[index].begin();
            IMEM_DBG("begin VA(0x%x), size(0x%x)\n",  (*itt).va, (*itt).size);
            itt++; //bypass the first element(dummy head)
            for(int i=1 ; i < mBufInfoList[index].size() ;i++)
            {//corresponding buffer in user-buffer list
                IMEM_INF("VA(0x%x), size(0x%x)\n",  (*itt).va, (*itt).size);
                fprintf(f, "VA(0x%x), size(0x%x)\n",  (*itt).va, (*itt).size);
                itt++;
            }
        }
        pthread_mutex_unlock(&dbgMutex);
