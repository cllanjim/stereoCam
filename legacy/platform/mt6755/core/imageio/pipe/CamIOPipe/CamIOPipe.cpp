...
MUINT32 p1ConvModuleID(MUINT32 id) {
    ...
    switch (id) {
        ...
        case EModule_vHDR: newId = CAM_CTL_VHDR; break;
        ...
        case EModule_vHDR_D: newId = CAM_CTL_VHDR_D; break;
        case EModule_CAMSV_IMGO: newId = CAM_DMA_IMGO_SV; break;
        case EModule_CAMSV2_IMGO: newId = CAM_DMA_IMGO_SV2; break;
        default:
            PIPE_ERR("Not support module(%d)",id);
            newId= -1;
            break;
...
PIPE_DBG("+, CQ:%d/%d",m_pass1_Raw_CQ, m_pass1_RawD_CQ);
    ...
    for (MUINT32 i = 0 ; i < vOutPorts.size() ; i++ ) {
        ...
        if ( EPortIndex_IMGO == vOutPorts[i]->index ) {
            ...
            en_camsv2 |= CAMSV_CAMSV2_MODULE_TG_EN;
            en_camsv2 |= CAMSV_CAMSV2_MODULE_PAK_EN;
            en_camsv2 |= CAMSV_CAMSV2_MODULE_DB_EN;
            bCamsv2_2pix = MFALSE;
    ...
    switch ( cmd ) {
        ...
        case EPIPECmd_SET_MODULE_EN:
            ...
            if ( EModule_CAMSV_IMGO == arg1)  {
                ...
                if (true == this->m_pass1TwinMode)
                {
                    ISP_WRITE_REG(pPhyIspDrv,CAMSV_CAMSV_PAK,0x07,ISP_DRV_USER_ISPF);
                }
                else
                {
            ...
            else if ( EModule_CAMSV2_IMGO == arg1)  {
                IspDrv* pPhyIspDrv = this->m_pIspDrvShell->getPhyIspDrv();
                ISP_WRITE_REG(pPhyIspDrv,CAMSV_CAMSV2_CLK_EN,0x00008005,ISP_DRV_USER_ISPF);
                ISP_WRITE_REG(pPhyIspDrv,CAMSV_CAMSV2_INT_EN ,0x481,ISP_DRV_USER_ISPF);
                ISP_WRITE_REG(pPhyIspDrv,CAMSV_TG2_VF_CON,0x00001001,ISP_DRV_USER_ISPF);
                ISP_WRITE_REG(pPhyIspDrv,CAMSV_CAMSV2_MODULE_EN,0x40000015,ISP_DRV_USER_ISPF);
                if (true == this->m_pass1TwinMode)
                {
                    ISP_WRITE_REG(pPhyIspDrv,CAMSV_CAMSV2_PAK,0x07,ISP_DRV_USER_ISPF);
                }
                else
                {
                    ISP_WRITE_REG(pPhyIspDrv,CAMSV_CAMSV2_PAK,0x03,ISP_DRV_USER_ISPF);
                }
                ISP_WRITE_REG(pPhyIspDrv,CAMSV_CAMSV2_FMT_SEL,0x01000000, ISP_DRV_USER_ISPF);
                ret = 0;
            }
            else {
                tg = _p1GetModuleTgIdx(arg1,this->m_pass1TwinMode);
