...
uninit()
{
    ...
    if(buf_info.virtAddr != 0x0 )
    {
        if ( m_pIspDrvShell->m_pIMemDrv->freeVirtBuf("IspDrvShell", &buf_info) ) {
...
enqueInBuf(PortID const portID, QBufInfo const& rQBufInfo,MINT32 const enqueCq,MINT32 const dupCqIdx)
{
    ...
    memset(&bufInfo, 0x0, sizeof(stISP_BUF_INFO));
...
//    }
    ...
    else
    {
        ...
        else
        {
            ...
            if ( m_pIspDrvShell->m_pIMemDrv->allocVirtBuf("IspDrvShell", &jpg_buf_info) ) {
                PIPE_ERR("ERROR:m_pIMemDrv->allocVirtBuf");
                return MFALSE;
            }
            jpg_WorkingMem_memId = jpg_buf_info.memID;
...
//                en_p2_dma |=  CAM_CTL_DMA_EN_VIPI_EN;
    ...
    {  //eRawPxlID
        case ISP_SUB_MODE_RAW:
            ...
            en_p2 |= CAM_CTL_EN_P2_UDM_EN|CAM_CTL_EN_P2_UNP_EN;
            g2g_sel_en=1;
            g2g_sel=1;
            break;
    ...
    if(pPipePackageInfo->isV3==true){
            ...
            PIPE_DBG("[V3]hwScenario(%d),p2SubMode(%d),tuningEn1Tag(0x%x),tuningEn2Tag(0x%x),tuningDmaTag(0x%x)",hwScenario,p2SubMode,tuningEn1Tag,tuningEn2Tag,tuningDmaTag);
            ...
            PIPE_DBG("[V3]en1(0x%08x),en2(0x%08x),dma(0x%08x)",en_p1,en_p2,en_p1_dma);
        } else {
    ...
    switch (meScenarioID) {
        ...
        case eDrvScenario_VSS_MFB_Mixing:
            ...
            en_p2 |= CAM_CTL_EN_P2_MIX3_EN;	// MFB_MIXING does not need mfb module
            ...
            */
			//ctl_sel=0x00001060;
			ctl_sel=0x00001040;	// MFB_MIXING does not need mfb module, so c24_sel must be 0s
            break;
        case eDrvScenario_VSS:
    ...
    PIPE_DBG("drvScenario(%d),mdpCheck1(%d),mdpCheck2(%d),crop1TableIdx(%d)",drvScenario,mdpCheck1,mdpCheck2,crop1TableIdx);
    ...
    PIPE_DBG("Pass(%d),imgi.fmt(0x%x),meScenarioID(%d),isCropG1En(%d),m_CrzEn(%d),mdpSrcFmt(0x%x),mdp_src(%d,%d,%d,%d),en_p2_dma(0x%08x),crop1TableIdx(%d),mdpCheck1(%d),mdpCheck2(%d),drvScenario(%d),tcm_en(0x%x)",\
  ...
  m_camPass2Param.isp_top_ctl.CAM_CTL_SEL_P2.Bits.MFB_MODE=mfb_mode;
    ...
    m_camPass2Param.isoValue = pPipePackageInfo->isoValue;	// set ISO
	PIPE_DBG("set iso: PipePackageInfo(%d), CamPathPass2Parameter(%d)", pPipePackageInfo->isoValue, m_camPass2Param.isoValue);
    ...
    if ( -1 != idx_imgi ) {
		...
		else if (portInfo_imgi.crop1.w != 0)
        	{
        m_camPass2Param.croppara.mdp_crop.x = portInfo_imgi.crop1.x;
        m_camPass2Param.croppara.mdp_crop.floatX = portInfo_imgi.crop1.floatX;
        m_camPass2Param.croppara.mdp_crop.y = portInfo_imgi.crop1.y;
        m_camPass2Param.croppara.mdp_crop.floatY = portInfo_imgi.crop1.floatY;
        m_camPass2Param.croppara.mdp_crop.w = portInfo_imgi.crop1.w;
        m_camPass2Param.croppara.mdp_crop.h = portInfo_imgi.crop1.h;
        	}
		else
			{
        m_camPass2Param.croppara.mdp_crop.x = portInfo_imgi.crop2.x;
    ...
    if ( -1 != idx_wroto) {
        ...
        else
        {
			...
			else if (portInfo_imgi.crop3.w != 0)
			{
			    m_camPass2Param.wroto.srcCropX = portInfo_imgi.crop3.x;
            	m_camPass2Param.wroto.srcCropFloatX = portInfo_imgi.crop3.floatX;
        	    m_camPass2Param.wroto.srcCropY= portInfo_imgi.crop3.y;
			...
			{
				PIPE_ERR("crop2 & crop3 are all 0!");
				return MFALSE;
			}
        }
...
*
********************************************************************************/
MBOOL PostProcPipe::endequeFrameCtrl(EPIPE_BUFQUECmd cmd,MUINT32 callerID,MINT32 p2burstQIdx,MINT32 p2dupCQIdx,MINT32 frameNum,MINT32 timeout)
{
    MBOOL ret=MFALSE;
    ...
    switch(cmd)
    {
        case EPIPE_BUFQUECmd_ENQUE_FRAME:
            ret=m_pIspDrvShell->getPhyIspDrv()->enqueP2Frame(callerID,p2burstQIdx,p2dupCQIdx,frameNum);
            break;
