...
public:
    ...
    MUINT32  tgFps;          //tgFps
    IspDMACfg():
        pixel_byte(1),swap(0), format_en(0),format(0),bus_size_en(0),bus_size(0),ring_en(0),ring_size(0),capbility(0x00),memBuf_c_ofst(0),memBuf_v_ofst(0),v_flip_en(0),lIspColorfmt(eImgFmt_IMPLEMENTATION_DEFINED),tgFps(30)
        {};
};
...
public:
    ...
    MBOOL Flip;
    MUINT32         capbility;      //port capbility
    MBOOL	bIsRwbEn;	// add for RWB sensor
    public:
        MdpRotDMACfg():
            pixel_byte(1),capbility(0x00),crop_en(0),uv_plane_swap(0),Format(CRZ_DRV_FORMAT_YUV422),Plane(CRZ_DRV_PLANE_1),Sequence(CRZ_DRV_SEQUENCE_YVYU),Rotation(CRZ_DRV_ROTATION_0),withCropM(0),enSrcCrop(0),srcCropX(0),srcCropFloatX(0),srcCropY(0),srcCropFloatY(0),srcCropW(0),srcCropH(0),Flip(0),bIsRwbEn(0)
            {};
};
