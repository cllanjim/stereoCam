              ...
              m_pVirIspDrv(NULL)
{
    ...
    m_isTwinMode = MFALSE;
    m_TGIdx = 0;
    m_chkP1SettingBeforeLatch =MFALSE;
    ::memset((void*)&m_pVirtIspReg,0,sizeof(m_pVirtIspReg));
    UnLock();
...
HwRWCtrl::setModuleEn(CAM_MODULE_ENUM module, MBOOL en)
{
    ...
    _VirDrv = this->m_pPhyIspDrv->getCQInstance(this->m_pPhyIspDrv->getRealCQIndex(this->m_CQ,this->m_BurstQNum,((this->m_DupCqIdx+i)%ISP_DRV_P1_CQ_DUPLICATION_NUM)));
        ...
        case CAM_DMA_IMGO_SV:
            ...
            break;
        case CAM_DMA_IMGO_SV2:
            ISP_WRITE_REG(_VirDrv,CAMSV_CAMSV2_CLK_EN,0x00008005,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_VirDrv,CAMSV_CAMSV2_INT_EN ,0x481,ISP_DRV_USER_ISPF);
            ISP_WRITE_BITS(_VirDrv,CAMSV_TG2_VF_CON,SINGLE_MODE,0,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_VirDrv,CAMSV_CAMSV2_MODULE_EN,40000015,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_VirDrv,CAMSV_CAMSV2_PAK,0x03,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_VirDrv,CAMSV_CAMSV2_FMT_SEL,0x01000000, ISP_DRV_USER_ISPF);
            break;
