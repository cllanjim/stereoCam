...
MINT32
IspDrv_B::
cam_isp_nsl2_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ISP_FUNC_DBG("+,cq(%d),subMode(%d),isApplyTuning(%d)",imageioPackage.cq,imageioPackage.subMode,imageioPackage.isApplyTuning);
	isp_reg_t ispReg;
    /*
    	NSL2_CENTR_X = (NSL2_CENTR_X_LSC - crop_initial_x)*RRZ_Ratio
	NSL2_CENTR_Y = (NSL2_CENTR_Y_LSC- crop_initial_y) *RRZ_Ratio
	NSL2_R_0 = NSL2_R_0_LSC *RRZ_Ratio
	NSL2_R_1 = NSL2_R_1_LSC *RRZ_Ratio
	NSL2_R_2 = NSL2_R_2_LSC *RRZ_Ratio
	NSL2_HRZ_COMP = NSL2_HRZ_COMP = 2048
	NSL2_VRZ_COMP = NSL2_VRZ_COMP = 2048
	NSL2_GAIN_0 = NSL2_GAIN_0_LSC
	NSL2_GAIN_1 = NSL2_GAIN_1_LSC
	NSL2_GAIN_2 = NSL2_GAIN_2_LSC
	NSL2_GAIN_3 = NSL2_GAIN_3_LSC
	NSL2_GAIN_4 = NSL2_GAIN_4_LSC
	NSL2_SLP_1 = NSL2_SLP_1_LSC / RRZ_Ratio
	NSL2_SLP_2 = NSL2_SLP_2_LSC / RRZ_Ratio
	NSL2_SLP_3 = NSL2_SLP_3_LSC / RRZ_Ratio
	NSL2_SLP_4 = NSL2_SLP_4_LSC / RRZ_Ratio
    */
    if(imageioPackage.isApplyTuning){ //0x15004A78 ~ 0x15004A9C
        ISP_DRV_REG_IO_STRUCT regsStruct[ISP_DRV_MAX_CQ_GROUP_SIZE];
        MUINT32 addrOffset, moduleSize;
        MUINT32 nsl2CentrXLsc, nsl2CentrYLsc, nsl2Rr0Lsc, nsl2Rr1Lsc, nsl2Rr2Lsc, nsl2Slp1Lsc, nsl2Slp2Lsc, nsl2Slp3Lsc, nsl2Slp4Lsc;
        //
        ISP_FUNC_DBG("eTuningCqFunc1(%d),eTuningCqFunc2(%d)",tuningPackage.eTuningCqFunc1,tuningPackage.eTuningCqFunc2);
        //
        if(tuningPackage.eTuningCqFunc1==CAM_ISP_NSL2){
           m_pP2IspDrv->getCqModuleInf(tuningPackage.eTuningCqFunc1,addrOffset,moduleSize);
            ...
            for(MUINT32 i=0;i<moduleSize;i++){
                ...
                regsStruct[i].Data = tuningPackage.pTuningBuf[addrOffset>>2];
                addrOffset += 4;
                ISP_FUNC_VRB("[NSL2](0x%08x,0x%08x)",regsStruct[i].Addr,regsStruct[i].Data);
            }
            ISP_BURST_WRITE_REGS(m_pP2IspDrv, regsStruct, moduleSize);
            // update nsl2
            // get raw nsl2 data from CQ set by tuningMgr just now
            nsl2CentrXLsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_CEN, SL2_CENTR_X);
            nsl2CentrYLsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_CEN, SL2_CENTR_Y);
            nsl2Rr0Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_RR_CON0, SL2_R_0);
            nsl2Rr1Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_RR_CON0, SL2_R_1);
            nsl2Rr2Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_RR_CON1, SL2_R_2);
			nsl2Slp1Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_SLP_CON0, SL2_SLP_1);
			nsl2Slp2Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_SLP_CON0, SL2_SLP_2);
			nsl2Slp3Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_SLP_CON1, SL2_SLP_3);
			nsl2Slp4Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_SLP_CON1, SL2_SLP_4);
            // calculate the right nsl2 data
            nsl2CentrXLsc = (nsl2CentrXLsc-tuningPackage.rrzCropX)*tuningPackage.rrzRatio;
            nsl2CentrYLsc = (nsl2CentrYLsc-tuningPackage.rrzCropY)*tuningPackage.rrzRatio;
            nsl2Rr0Lsc = nsl2Rr0Lsc*tuningPackage.rrzRatio;
            nsl2Rr1Lsc = nsl2Rr1Lsc*tuningPackage.rrzRatio;
            nsl2Rr2Lsc = nsl2Rr2Lsc*tuningPackage.rrzRatio;
			nsl2Slp1Lsc = nsl2Slp1Lsc/tuningPackage.rrzRatio;
			nsl2Slp2Lsc = nsl2Slp2Lsc/tuningPackage.rrzRatio;
			nsl2Slp3Lsc = nsl2Slp3Lsc/tuningPackage.rrzRatio;
			nsl2Slp4Lsc = nsl2Slp4Lsc/tuningPackage.rrzRatio;
            ISP_FUNC_DBG("rrzCropX(%d),rrzCropY(%d),rrzRatio(%f)",tuningPackage.rrzCropX,tuningPackage.rrzCropY,tuningPackage.rrzRatio);
            // set the nsl2 data caculated to CQ
            ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2_CEN, SL2_CENTR_X, nsl2CentrXLsc);
            ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2_CEN, SL2_CENTR_Y, nsl2CentrYLsc);
            ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2_RR_CON0, SL2_R_0, nsl2Rr0Lsc);
            ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2_RR_CON0, SL2_R_1, nsl2Rr1Lsc);
            ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2_RR_CON1, SL2_R_2, nsl2Rr2Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2_SLP_CON0, SL2_SLP_1, nsl2Slp1Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2_SLP_CON0, SL2_SLP_2, nsl2Slp2Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2_SLP_CON1, SL2_SLP_3, nsl2Slp3Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2_SLP_CON1, SL2_SLP_4, nsl2Slp4Lsc);
            ISP_FUNC_DBG("[NSL2]CentrXLsc(%d),CentrYLsc(%d),Rr0Lsc(%d),Rr1Lsc(%d),Rr2Lsc(%d),Slp1Lsc(%d),Slp2Lsc(%d),Slp3Lsc(%d),Slp4Lsc(%d)",nsl2CentrXLsc,nsl2CentrYLsc,nsl2Rr0Lsc,nsl2Rr1Lsc,nsl2Rr2Lsc,nsl2Slp1Lsc,nsl2Slp2Lsc,nsl2Slp3Lsc,nsl2Slp4Lsc);
        }
    }
...
cam_isp_nbc_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ISP_FUNC_DBG("+,cq(%d),subMode(%d),isApplyTuning(%d)",imageioPackage.cq,imageioPackage.subMode,imageioPackage.isApplyTuning);
    if(imageioPackage.isApplyTuning){  //0x15007600 ~ 0x15007A7C
        ISP_DRV_REG_IO_STRUCT regsStruct[ISP_DRV_MAX_CQ_GROUP_SIZE];
        MUINT32 addrOffset, moduleSize;
        //
        ISP_FUNC_DBG("eTuningCqFunc1(%d),eTuningCqFunc2(%d)",tuningPackage.eTuningCqFunc1,tuningPackage.eTuningCqFunc2);
        //
        if(tuningPackage.eTuningCqFunc1==CAM_ISP_NBC){	//0x15007A00 ~ 0x15007A7C
           m_pP2IspDrv->getCqModuleInf(tuningPackage.eTuningCqFunc1,addrOffset,moduleSize);
		...
		if(tuningPackage.eTuningCqFunc2==CAM_ISP_NBC_TBL){ //(ANR_TBL)0x15007600 ~ 0x150079FF	// for ANR table
           m_pP2IspDrv->getCqModuleInf(tuningPackage.eTuningCqFunc2,addrOffset,moduleSize);
...
MINT32
IspDrv_B::
cam_isp_nbc2_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ISP_FUNC_DBG("+,cq(%d),subMode(%d),isApplyTuning(%d)",imageioPackage.cq,imageioPackage.subMode,imageioPackage.isApplyTuning);
    if(imageioPackage.isApplyTuning){  //0x15007B00 ~ 0x15007BBC
        ISP_DRV_REG_IO_STRUCT regsStruct[ISP_DRV_MAX_CQ_GROUP_SIZE];
        MUINT32 addrOffset, moduleSize;
        //
        ISP_FUNC_DBG("eTuningCqFunc1(%d),eTuningCqFunc2(%d)",tuningPackage.eTuningCqFunc1,tuningPackage.eTuningCqFunc2);
        //
        if(tuningPackage.eTuningCqFunc1==CAM_ISP_NBC2){
           m_pP2IspDrv->getCqModuleInf(tuningPackage.eTuningCqFunc1,addrOffset,moduleSize);
...
MINT32
IspDrv_B::
cam_isp_nsl2a_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ISP_FUNC_DBG("+,cq(%d),subMode(%d),isApplyTuning(%d)",imageioPackage.cq,imageioPackage.subMode,imageioPackage.isApplyTuning);
	isp_reg_t ispReg;
    /*
    	NSL2A_CENTR_X = NSL2_CENTR_X
	NSL2A_CENTR_Y = NSL2_CENTR_Y
	NSL2A_R_0 = NSL2_R_0
	NSL2A_R_1 = NSL2_R_1
	NSL2A_R_2 = NSL2_R_2
	NSL2A_HRZ_COMP = NSL2_HRZ_COMP = 2048
	NSL2A_VRZ_COMP = NSL2_VRZ_COMP = 2048
	NSL2A_GAIN_0 = NSL2_GAIN_0
	NSL2A_GAIN_1 = NSL2_GAIN_1
	NSL2A_GAIN_2 = NSL2_GAIN_2
	NSL2A_GAIN_3 = NSL2_GAIN_3
	NSL2A_GAIN_4 = NSL2_GAIN_4
	NSL2A_SLP_1 = NSL2_SLP_1
	NSL2A_SLP_2 = NSL2_SLP_2
	NSL2A_SLP_3 = NSL2_SLP_3
	NSL2A_SLP_4 = NSL2_SLP_4
    */
    if(imageioPackage.isApplyTuning){ //0x15004A20 ~ 0x15004A44
        ISP_DRV_REG_IO_STRUCT regsStruct[ISP_DRV_MAX_CQ_GROUP_SIZE];
        MUINT32 addrOffset, moduleSize;
        MUINT32 nsl2CentrXLsc, nsl2CentrYLsc, nsl2Rr0Lsc, nsl2Rr1Lsc, nsl2Rr2Lsc, nsl2HrzComp, nsl2VrzComp, nsl2Gain0Lsc, nsl2Gain1Lsc, nsl2Gain2Lsc, nsl2Gain3Lsc, nsl2Gain4Lsc, nsl2Slp1Lsc, nsl2Slp2Lsc, nsl2Slp3Lsc, nsl2Slp4Lsc;
        //
        ISP_FUNC_DBG("eTuningCqFunc1(%d),eTuningCqFunc2(%d)",tuningPackage.eTuningCqFunc1,tuningPackage.eTuningCqFunc2);
        //
        if(tuningPackage.eTuningCqFunc1==CAM_ISP_NSL2A){
           m_pP2IspDrv->getCqModuleInf(tuningPackage.eTuningCqFunc1,addrOffset,moduleSize);
            ...
            for(MUINT32 i=0;i<moduleSize;i++){
                ...
                regsStruct[i].Data = tuningPackage.pTuningBuf[addrOffset>>2];
                addrOffset += 4;
                ISP_FUNC_VRB("[NSL2A](0x%08x,0x%08x)",regsStruct[i].Addr,regsStruct[i].Data);
            }
            ISP_BURST_WRITE_REGS(m_pP2IspDrv, regsStruct, moduleSize);
            // update nsl2a
            // get raw nsl2 data from CQ set by tuningMgr just now
            ...
            nsl2Rr1Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_RR_CON0, SL2_R_1);
            nsl2Rr2Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_RR_CON1, SL2_R_2);
			nsl2HrzComp = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_RZ, SL2_HRZ_COMP);
			nsl2VrzComp = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_RZ, SL2_VRZ_COMP);
			nsl2Gain0Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_RR_CON1, SL2_GAIN_0);
			nsl2Gain1Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_RR_CON1, SL2_GAIN_1);
			nsl2Gain2Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_GAIN, SL2_GAIN_2);
			nsl2Gain3Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_GAIN, SL2_GAIN_3);
			nsl2Gain4Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_GAIN, SL2_GAIN_4);
			nsl2Slp1Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_SLP_CON0, SL2_SLP_1);
			nsl2Slp2Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_SLP_CON0, SL2_SLP_2);
			nsl2Slp3Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_SLP_CON1, SL2_SLP_3);
			nsl2Slp4Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_SLP_CON1, SL2_SLP_4);
            // set the nsl2a data caculated to CQ
            ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2A_CEN, SL2_CENTR_X, nsl2CentrXLsc);
            ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2A_CEN, SL2_CENTR_Y, nsl2CentrYLsc);
            ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2A_RR_CON0, SL2_R_0, nsl2Rr0Lsc);
            ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2A_RR_CON0, SL2_R_1, nsl2Rr1Lsc);
            ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2A_RR_CON1, SL2_R_2, nsl2Rr2Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2A_RZ, SL2_HRZ_COMP, nsl2HrzComp);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2A_RZ, SL2_VRZ_COMP, nsl2VrzComp);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2A_RR_CON1, SL2_GAIN_0, nsl2Gain0Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2A_RR_CON1, SL2_GAIN_1, nsl2Gain1Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2A_GAIN, SL2_GAIN_2, nsl2Gain2Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2A_GAIN, SL2_GAIN_3, nsl2Gain3Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2A_GAIN, SL2_GAIN_4, nsl2Gain4Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2A_SLP_CON0, SL2_SLP_1, nsl2Slp1Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2A_SLP_CON0, SL2_SLP_2, nsl2Slp2Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2A_SLP_CON1, SL2_SLP_3, nsl2Slp3Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2A_SLP_CON1, SL2_SLP_4, nsl2Slp4Lsc);
            ISP_FUNC_DBG("[NSL2A]CentrXLsc(%d),CentrYLsc(%d),Rr0Lsc(%d),Rr1Lsc(%d),Rr2Lsc(%d),HrzComp(%d),VrzComp(%d),Gain0Lsc(%d),Gain1Lsc(%d),Gain2Lsc(%d),Gain3Lsc(%d),Gain4Lsc(%d),Slp1Lsc(%d),Slp2Lsc(%d),Slp3Lsc(%d),Slp4Lsc(%d)",nsl2CentrXLsc,nsl2CentrYLsc,nsl2Rr0Lsc,nsl2Rr1Lsc,nsl2Rr2Lsc,nsl2HrzComp,nsl2VrzComp,nsl2Gain0Lsc,nsl2Gain1Lsc,nsl2Gain2Lsc,nsl2Gain3Lsc,nsl2Gain4Lsc,nsl2Slp1Lsc,nsl2Slp2Lsc,nsl2Slp3Lsc,nsl2Slp4Lsc);
        }
    }
...
MINT32
IspDrv_B::
cam_isp_nsl2b_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ISP_FUNC_DBG("+,cq(%d),subMode(%d),isApplyTuning(%d)",imageioPackage.cq,imageioPackage.subMode,imageioPackage.isApplyTuning);
	isp_reg_t ispReg;
    /*
    	NSL2B_CENTR_X = NSL2_CENTR_X
	NSL2B_CENTR_Y = NSL2_CENTR_Y
	NSL2B_R_0 = NSL2_R_0
	NSL2B_R_1 = NSL2_R_1
	NSL2B_R_2 = NSL2_R_2
	NSL2B_HRZ_COMP = NSL2_HRZ_COMP = 2048
	NSL2B_VRZ_COMP = NSL2_VRZ_COMP = 2048
	NSL2B_GAIN_0 = NSL2_GAIN_0
	NSL2B_GAIN_1 = NSL2_GAIN_1
	NSL2B_GAIN_2 = NSL2_GAIN_2
	NSL2B_GAIN_3 = NSL2_GAIN_3
	NSL2B_GAIN_4 = NSL2_GAIN_4
	NSL2B_SLP_1 = NSL2_SLP_1
	NSL2B_SLP_2 = NSL2_SLP_2
	NSL2B_SLP_3 = NSL2_SLP_3
	NSL2B_SLP_4 = NSL2_SLP_4
    */
    if(imageioPackage.isApplyTuning){ //0x15004A50 ~ 0x15004A74
        ISP_DRV_REG_IO_STRUCT regsStruct[ISP_DRV_MAX_CQ_GROUP_SIZE];
        MUINT32 addrOffset, moduleSize;
        MUINT32 nsl2CentrXLsc, nsl2CentrYLsc, nsl2Rr0Lsc, nsl2Rr1Lsc, nsl2Rr2Lsc, nsl2HrzComp, nsl2VrzComp, nsl2Gain0Lsc, nsl2Gain1Lsc, nsl2Gain2Lsc, nsl2Gain3Lsc, nsl2Gain4Lsc, nsl2Slp1Lsc, nsl2Slp2Lsc, nsl2Slp3Lsc, nsl2Slp4Lsc;
        //
        ISP_FUNC_DBG("eTuningCqFunc1(%d),eTuningCqFunc2(%d)",tuningPackage.eTuningCqFunc1,tuningPackage.eTuningCqFunc2);
        //
        if(tuningPackage.eTuningCqFunc1==CAM_ISP_NSL2B){
           m_pP2IspDrv->getCqModuleInf(tuningPackage.eTuningCqFunc1,addrOffset,moduleSize);
            ...
            for(MUINT32 i=0;i<moduleSize;i++){
                ...
                regsStruct[i].Data = tuningPackage.pTuningBuf[addrOffset>>2];
                addrOffset += 4;
                ISP_FUNC_VRB("[NSL2B](0x%08x,0x%08x)",regsStruct[i].Addr,regsStruct[i].Data);
            }
            ISP_BURST_WRITE_REGS(m_pP2IspDrv, regsStruct, moduleSize);
            // update nsl2b
            // get raw nsl2 data from CQ set by tuningMgr just now
			...
			nsl2Slp3Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_SLP_CON1, SL2_SLP_3);
			nsl2Slp4Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_SLP_CON1, SL2_SLP_4);
            // set the nsl2b data caculated to CQ
            ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2B_CEN, SL2_CENTR_X, nsl2CentrXLsc);
            ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2B_CEN, SL2_CENTR_Y, nsl2CentrYLsc);
            ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2B_RR_CON0, SL2_R_0, nsl2Rr0Lsc);
            ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2B_RR_CON0, SL2_R_1, nsl2Rr1Lsc);
            ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2B_RR_CON1, SL2_R_2, nsl2Rr2Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2B_RZ, SL2_HRZ_COMP, nsl2HrzComp);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2B_RZ, SL2_VRZ_COMP, nsl2VrzComp);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2B_RR_CON1, SL2_GAIN_0, nsl2Gain0Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2B_RR_CON1, SL2_GAIN_1, nsl2Gain1Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2B_GAIN, SL2_GAIN_2, nsl2Gain2Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2B_GAIN, SL2_GAIN_3, nsl2Gain3Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2B_GAIN, SL2_GAIN_4, nsl2Gain4Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2B_SLP_CON0, SL2_SLP_1, nsl2Slp1Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2B_SLP_CON0, SL2_SLP_2, nsl2Slp2Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2B_SLP_CON1, SL2_SLP_3, nsl2Slp3Lsc);
			ISP_WRITE_BITS(m_pP2IspDrv, CAM_NSL2B_SLP_CON1, SL2_SLP_4, nsl2Slp4Lsc);
			ISP_FUNC_DBG("[NSL2B]CentrXLsc(%d),CentrYLsc(%d),Rr0Lsc(%d),Rr1Lsc(%d),Rr2Lsc(%d),HrzComp(%d),VrzComp(%d),Gain0Lsc(%d),Gain1Lsc(%d),Gain2Lsc(%d),Gain3Lsc(%d),Gain4Lsc(%d),Slp1Lsc(%d),Slp2Lsc(%d),Slp3Lsc(%d),Slp4Lsc(%d)",nsl2CentrXLsc,nsl2CentrYLsc,nsl2Rr0Lsc,nsl2Rr1Lsc,nsl2Rr2Lsc,nsl2HrzComp,nsl2VrzComp,nsl2Gain0Lsc,nsl2Gain1Lsc,nsl2Gain2Lsc,nsl2Gain3Lsc,nsl2Gain4Lsc,nsl2Slp1Lsc,nsl2Slp2Lsc,nsl2Slp3Lsc,nsl2Slp4Lsc);
        }
    }
...
cam_isp_sl2c_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ISP_FUNC_DBG("+,cq(%d),subMode(%d),isApplyTuning(%d)",imageioPackage.cq,imageioPackage.subMode,imageioPackage.isApplyTuning);
    /*
    	SL2C_CENTR_X = NSL2_CENTR_X
	SL2C_CENTR_Y = NSL2_CENTR_Y
	SL2C_RR_0 = NSL2_R_0* NSL2_R_0
	SL2C_RR_1 = NSL2_R_1* NSL2_R_1
	SL2C_RR_2 = NSL2_R_2* NSL2_R_2
	SL2C_HRZ_COMP = NSL2_HRZ_COMP = 2048
    */
    if(imageioPackage.isApplyTuning){  //0x15004FC0 ~ 0x15004FD8
        MUINT32 nsl2CentrXLsc, nsl2CentrYLsc, nsl2Rr0Lsc, nsl2Rr1Lsc, nsl2Rr2Lsc, nsl2HrzComp;
        ...
        if(tuningPackage.eTuningCqFunc1==CAM_ISP_SL2C){
           m_pP2IspDrv->getCqModuleInf(tuningPackage.eTuningCqFunc1,addrOffset,moduleSize);
            ...
            sl2cCentrXLsc = nsl2CentrXLsc;
            sl2cCentrYLsc = nsl2CentrYLsc;
            sl2cRr0Lsc = nsl2Rr0Lsc*nsl2Rr0Lsc;
            sl2cRr1Lsc = nsl2Rr1Lsc*nsl2Rr1Lsc;
            sl2cRr2Lsc = nsl2Rr2Lsc*nsl2Rr2Lsc;
            sl2cHrzComp = nsl2HrzComp;
...
MINT32
IspDrv_B::
cam_isp_udm_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ISP_FUNC_DBG("+");
    isp_reg_t ispReg;
    MUINT32 fgModeRegBit, reg_cam_udm_dsb;

    fgModeRegBit = (imageioPackage.enFgMode&0x01)<<10;
    reg_cam_udm_dsb = ((MUINT32)((MUINT8*)&ispReg.CAM_UDM_DSB-(MUINT8*)&ispReg));

    if(imageioPackage.isApplyTuning){  //0x150048A0 ~ 0x150048E8
        ISP_DRV_REG_IO_STRUCT regsStruct[ISP_DRV_MAX_CQ_GROUP_SIZE];
        MUINT32 addrOffset, moduleSize;
        //
        ISP_FUNC_DBG("eTuningCqFunc1(%d),eTuningCqFunc2(%d),fgModeRegBit(0x%x),reg_cam_udm_dsb(0x%x)",tuningPackage.eTuningCqFunc1,tuningPackage.eTuningCqFunc2,fgModeRegBit,reg_cam_udm_dsb);
        //
        if(tuningPackage.eTuningCqFunc1==CAM_ISP_UDM){
           m_pP2IspDrv->getCqModuleInf(tuningPackage.eTuningCqFunc1,addrOffset,moduleSize);

            ISP_FUNC_DBG("addrOffset(0x%x),moduleSize(%d)",addrOffset,moduleSize);
            for(MUINT32 i=0;i<moduleSize;i++){
                if(reg_cam_udm_dsb != addrOffset){
                    regsStruct[i].Addr = addrOffset;
                    regsStruct[i].Data = tuningPackage.pTuningBuf[addrOffset>>2];
                } else {  // set fg mode
                    regsStruct[i].Addr = addrOffset;
                    regsStruct[i].Data = (tuningPackage.pTuningBuf[addrOffset>>2] & ~fgModeRegBit) | fgModeRegBit;
                }
                addrOffset += 4;
                ISP_FUNC_VRB("[UDM](0x%08x,0x%08x)",regsStruct[i].Addr,regsStruct[i].Data);
            }
            ISP_BURST_WRITE_REGS(m_pP2IspDrv, regsStruct, moduleSize);
        }
    } else {

        m_pP2IspDrv->writeReg(((MUINT32)((MUINT8*)&ispReg.CAM_UDM_INTP_CRS-(MUINT8*)&ispReg)), 0x38303060, ISP_DRV_USER_ISPF, 0);
        m_pP2IspDrv->writeReg(((MUINT32)((MUINT8*)&ispReg.CAM_UDM_INTP_NAT-(MUINT8*)&ispReg)), 0x1430063F, ISP_DRV_USER_ISPF, 0);
        m_pP2IspDrv->writeReg(((MUINT32)((MUINT8*)&ispReg.CAM_UDM_INTP_AUG-(MUINT8*)&ispReg)), 0x00600600, ISP_DRV_USER_ISPF, 0);
        m_pP2IspDrv->writeReg(((MUINT32)((MUINT8*)&ispReg.CAM_UDM_LUMA_LUT1-(MUINT8*)&ispReg)), 0x07FF0100, ISP_DRV_USER_ISPF, 0);
        m_pP2IspDrv->writeReg(((MUINT32)((MUINT8*)&ispReg.CAM_UDM_LUMA_LUT2-(MUINT8*)&ispReg)), 0x02008020, ISP_DRV_USER_ISPF, 0);
        m_pP2IspDrv->writeReg(((MUINT32)((MUINT8*)&ispReg.CAM_UDM_SL_CTL-(MUINT8*)&ispReg)), 0x003FFFE0, ISP_DRV_USER_ISPF, 0);
        m_pP2IspDrv->writeReg(((MUINT32)((MUINT8*)&ispReg.CAM_UDM_HFTD_CTL-(MUINT8*)&ispReg)), 0x08421000, ISP_DRV_USER_ISPF, 0);
        m_pP2IspDrv->writeReg(((MUINT32)((MUINT8*)&ispReg.CAM_UDM_NR_STR-(MUINT8*)&ispReg)), 0x81028000, ISP_DRV_USER_ISPF, 0);
        m_pP2IspDrv->writeReg(((MUINT32)((MUINT8*)&ispReg.CAM_UDM_NR_ACT-(MUINT8*)&ispReg)), 0x00000050, ISP_DRV_USER_ISPF, 0);
        m_pP2IspDrv->writeReg(((MUINT32)((MUINT8*)&ispReg.CAM_UDM_HF_STR-(MUINT8*)&ispReg)), 0x00000000, ISP_DRV_USER_ISPF, 0);
        m_pP2IspDrv->writeReg(((MUINT32)((MUINT8*)&ispReg.CAM_UDM_HF_ACT1-(MUINT8*)&ispReg)), 0x145034DC, ISP_DRV_USER_ISPF, 0);
        m_pP2IspDrv->writeReg(((MUINT32)((MUINT8*)&ispReg.CAM_UDM_HF_ACT2-(MUINT8*)&ispReg)), 0x0034FF55, ISP_DRV_USER_ISPF, 0);
        m_pP2IspDrv->writeReg(((MUINT32)((MUINT8*)&ispReg.CAM_UDM_CLIP-(MUINT8*)&ispReg)), 0x00DF2064, ISP_DRV_USER_ISPF, 0);
        m_pP2IspDrv->writeReg(((MUINT32)((MUINT8*)&ispReg.CAM_UDM_DSB-(MUINT8*)&ispReg)), (0x007FA800|fgModeRegBit), ISP_DRV_USER_ISPF, 0);
        m_pP2IspDrv->writeReg(((MUINT32)((MUINT8*)&ispReg.CAM_UDM_TILE_EDGE-(MUINT8*)&ispReg)), 0x0000000F, ISP_DRV_USER_ISPF, 0);
        m_pP2IspDrv->writeReg(((MUINT32)((MUINT8*)&ispReg.CAM_UDM_DSL-(MUINT8*)&ispReg)), 0x00000000, ISP_DRV_USER_ISPF, 0);
        m_pP2IspDrv->writeReg(((MUINT32)((MUINT8*)&ispReg.CAM_UDM_SPARE_1-(MUINT8*)&ispReg)), 0x00000000, ISP_DRV_USER_ISPF, 0);
        m_pP2IspDrv->writeReg(((MUINT32)((MUINT8*)&ispReg.CAM_UDM_SPARE_2-(MUINT8*)&ispReg)), 0x00000000, ISP_DRV_USER_ISPF, 0);
        m_pP2IspDrv->writeReg(((MUINT32)((MUINT8*)&ispReg.CAM_UDM_SPARE_3-(MUINT8*)&ispReg)), 0x00000000, ISP_DRV_USER_ISPF, 0);
...
MINT32
IspDrv_B::
cam_isp_dbs_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ISP_FUNC_DBG("+,cq(%d),subMode(%d),isApplyTuning(%d)",imageioPackage.cq,imageioPackage.subMode,imageioPackage.isApplyTuning);
    if(imageioPackage.isApplyTuning){  //0x15004740 ~ 0x15004754
        ISP_DRV_REG_IO_STRUCT regsStruct[ISP_DRV_MAX_CQ_GROUP_SIZE];
        MUINT32 addrOffset, moduleSize;
        //
        ISP_FUNC_DBG("eTuningCqFunc1(%d),eTuningCqFunc2(%d)",tuningPackage.eTuningCqFunc1,tuningPackage.eTuningCqFunc2);
        //
        if(tuningPackage.eTuningCqFunc1==CAM_ISP_DBS){
           m_pP2IspDrv->getCqModuleInf(tuningPackage.eTuningCqFunc1,addrOffset,moduleSize);
...
cam_isp_rmg_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ...
    if(imageioPackage.isApplyTuning){  //0x15004104 ~ 0x15004104
        ...
        ISP_FUNC_DBG("eTuningCqFunc1(%d),eTuningCqFunc2(%d)",tuningPackage.eTuningCqFunc1,tuningPackage.eTuningCqFunc2);
        //
        if(tuningPackage.eTuningCqFunc1==CAM_CTL_VHDR){
           m_pP2IspDrv->getCqModuleInf(tuningPackage.eTuningCqFunc1,addrOffset,moduleSize);
...
MINT32
IspDrv_B::
cam_isp_rwb_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ISP_FUNC_DBG("+,cq(%d),subMode(%d),isApplyTuning(%d)",imageioPackage.cq,imageioPackage.subMode,imageioPackage.isApplyTuning);

    if(imageioPackage.isApplyTuning){  //0x15007BC0 ~ 0x15007BE0
        ISP_DRV_REG_IO_STRUCT regsStruct[ISP_DRV_MAX_CQ_GROUP_SIZE];
        MUINT32 addrOffset, moduleSize;
        //
        ISP_FUNC_DBG("eTuningCqFunc1(%d),eTuningCqFunc2(%d)",tuningPackage.eTuningCqFunc1,tuningPackage.eTuningCqFunc2);
        //
        if(tuningPackage.eTuningCqFunc1==CAM_ISP_RWB){
           m_pP2IspDrv->getCqModuleInf(tuningPackage.eTuningCqFunc1,addrOffset,moduleSize);
            ...
            for(MUINT32 i=0;i<moduleSize;i++){
                ...
                regsStruct[i].Data = tuningPackage.pTuningBuf[addrOffset>>2];
                addrOffset += 4;
				ISP_FUNC_DBG("Addr(%x), Data(%x)",regsStruct[i].Addr,regsStruct[i].Data);
            }
        }
    }
...
// add for ISO adaptive +++++
MINT32
IspDrv_B::
cam_isp_iso_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ISP_FUNC_DBG("+,cq(%d),subMode(%d),isApplyTuning(%d)",imageioPackage.cq,imageioPackage.subMode,imageioPackage.isApplyTuning);

    if(imageioPackage.isApplyTuning){  //0x15007BE4
        ISP_DRV_REG_IO_STRUCT regsStruct[ISP_DRV_MAX_CQ_GROUP_SIZE];
        MUINT32 addrOffset, moduleSize;
        //
        ISP_FUNC_DBG("eTuningCqFunc1(%d),eTuningCqFunc2(%d)",tuningPackage.eTuningCqFunc1,tuningPackage.eTuningCqFunc2);
        //
        if(tuningPackage.eTuningCqFunc1==CAM_ISP_ISO){
           m_pP2IspDrv->getCqModuleInf(tuningPackage.eTuningCqFunc1,addrOffset,moduleSize);
...
{
    public:
        IspDrvShellImp():mInitCount(0),mpM4UDrv(NULL),lsciSwWorkaroundBufFd(0),lsciSwWorkaroundBufVa(0x0){}
...
int ret = 0;
    ...
    =============================================*/
    {
       ...
       //
       if ( m_pIMemDrv->allocVirtBuf(NAME, &lsciSwWorkaroundBufInfo) ) {
...
//js_test move to _disable.
...

    {
        ...
        //
        if ( m_pIMemDrv->freeVirtBuf(NAME, &lsciSwWorkaroundBufInfo) ) {
...
controlBurstQueBuffer(EIspControlBufferCmd cmd, MUINT32 burstQueueNum)
{
    ...
    if(eIspControlBufferCmd_Alloc == cmd) {
        ...
        if ( m_pIMemDrv->allocVirtBuf(NAME, &gMdpBufInfo) ) {
        ...
        for(k=0;k<burstQueueNum;k++){
            for(i=0;i<ISP_DRV_P2_CQ_NUM;i++){
                for(j=0;j<ISP_DRV_P2_CQ_DUPLICATION_NUM;j++){
                    ...
                    //
                    if ( m_pIMemDrv->allocVirtBuf(NAME, &tpipeBufInfo[k][i][j]) ) {
        ...
        {
            m_pVirtIspDrv[i] = (IspDrv *)(((uintptr_t)m_pVirtIspDrv + totalCQNum) + i * 1 *sizeof(IspDrv));
        }
        if(m_pVirtIspDrv==NULL)
        {
        ...
        for( int i=0; i<totalCQNum; i++ )
        {
            m_pVirtIspReg[i] = (isp_reg_t *)(((uintptr_t)m_pVirtIspReg + totalCQNum) + i * 1 * sizeof(isp_reg_t));
        }
        if(m_pVirtIspReg==NULL)
        {
        ...
        for(k=0;k<burstQueueNum;k++){
            for(i=0;i<ISP_DRV_P2_CQ_NUM;i++){
                for(j=0;j<ISP_DRV_P2_CQ_DUPLICATION_NUM;j++){
                    ...
                    //
                    if ( m_pIMemDrv->freeVirtBuf(NAME, &tpipeBufInfo[k][i][j]) ) {
        ...
        gMdpBufInfo.size = 0;
        #else   // Not PMEM.
        if ( m_pIMemDrv->freeVirtBuf(NAME, &gMdpBufInfo) ) {
...
//            ISP_WRITE_REG(_targetVirDrv,CAM_BPCI_CON,0x80000020,ISP_DRV_USER_ISPF); // ultra-
            ISP_WRITE_REG(_targetVirDrv,CAM_BPCI_CON2,0x00080800,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_targetVirDrv,CAM_BPCI_CON,0x80101020,ISP_DRV_USER_ISPF); // ultra--
...
//            ISP_WRITE_REG(_targetVirDrv,CAM_BPCI_D_CON,0x80000020,ISP_DRV_USER_ISPF); // ultra-
            ISP_WRITE_REG(_targetVirDrv,CAM_BPCI_D_CON2,0x00080800,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_targetVirDrv,CAM_BPCI_D_CON,0x80101020,ISP_DRV_USER_ISPF); // ultra--
...
//            ISP_WRITE_REG(_targetVirDrv,CAM_AAO_CON2,0x00202000,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_targetVirDrv,CAM_AAO_CON2,0x00080800,ISP_DRV_USER_ISPF);
            if(this->m_fps > FPS_BOUNDARY)
                ISP_WRITE_REG(_targetVirDrv,CAM_AAO_CON,0x80101020,ISP_DRV_USER_ISPF); // ultra-
            else
...
//            ISP_WRITE_REG(_targetVirDrv,CAM_AAO_D_CON,0x80000020,ISP_DRV_USER_ISPF); // ultra-
            ISP_WRITE_REG(_targetVirDrv,CAM_AAO_D_CON2,0x00080800,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_targetVirDrv,CAM_AAO_D_CON,0x80101020,ISP_DRV_USER_ISPF); // ultra--
...
//            ISP_WRITE_REG(_targetVirDrv,CAM_ESFKO_CON,0x80000020,ISP_DRV_USER_ISPF); // ultra-
            ISP_WRITE_REG(_targetVirDrv,CAM_ESFKO_CON2,0x00080800,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_targetVirDrv,CAM_ESFKO_CON,0x80101020,ISP_DRV_USER_ISPF); // ultra--
...
//            ISP_WRITE_REG(_targetVirDrv,CAM_LCSO_CON,0x80000020,ISP_DRV_USER_ISPF); // ultra-
            ISP_WRITE_REG(_targetVirDrv,CAM_LCSO_CON2,0x00080800,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_targetVirDrv,CAM_LCSO_CON,0x80101020,ISP_DRV_USER_ISPF); // ultra--
...
//            ISP_WRITE_REG(_targetVirDrv,CAM_LCSO_D_CON,0x80000020,ISP_DRV_USER_ISPF); // ultra-
            ISP_WRITE_REG(_targetVirDrv,CAM_LCSO_D_CON2,0x00080800,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_targetVirDrv,CAM_LCSO_D_CON,0x80101020,ISP_DRV_USER_ISPF); // ultra--
...
//			ISP_WRITE_REG(_targetVirDrv,CAM_IMGO_CON,0x801010B0,ISP_DRV_USER_ISPF); // ultra-
            ISP_WRITE_REG(_targetVirDrv,CAM_IMGO_CON2,0x00202000,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_targetVirDrv,CAM_IMGO_CON,0x804040B0,ISP_DRV_USER_ISPF); // ultra--
...
//			ISP_WRITE_REG(_targetVirDrv,CAM_RRZO_CON,0x80202080,ISP_DRV_USER_ISPF); // ultra-
            ISP_WRITE_REG(_targetVirDrv,CAM_RRZO_CON,0x80404080,ISP_DRV_USER_ISPF); // ultra--
...
//            ISP_WRITE_REG(_targetVirDrv,CAM_RRZO_CON2,0x00808000,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_targetVirDrv,CAM_RRZO_CON2,0x00202000,ISP_DRV_USER_ISPF);
            ...
            ISP_FUNC_DBG("ISP_DMA_RRZO, pxl_byte=%d, w=0x%x,x=0x%x,stride=0x%x, cropXY=%d,%d",this->dma_cfg.pixel_byte, this->dma_cfg.size.w, this->dma_cfg.size.xsize, this->dma_cfg.size.stride,this->dma_cfg.crop.y, this->dma_cfg.crop.x);
            //currently , slow motion will map to this case
            if(this->m_fps > FPS_BOUNDARY){
#if SRAM_SWITCH
...
//            ISP_WRITE_REG(_targetVirDrv,CAM_IMGO_D_CON2,0x00B0B000,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_targetVirDrv,CAM_IMGO_D_CON,0x804040B0,ISP_DRV_USER_ISPF); //ultra--
            ISP_WRITE_REG(_targetVirDrv,CAM_IMGO_D_CON2,0x00202000,ISP_DRV_USER_ISPF);
...
//            ISP_WRITE_REG(_targetVirDrv,CAM_RRZO_D_CON2,0x00808000,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_targetVirDrv,CAM_RRZO_D_CON,0x80404080,ISP_DRV_USER_ISPF); // ultra--
            ISP_WRITE_REG(_targetVirDrv,CAM_RRZO_D_CON2,0x00202000,ISP_DRV_USER_ISPF);
            ...
            ISP_FUNC_DBG("ISP_DMA_RRZOD, pxl_byte=%d, w=0x%x,x=0x%x,stride=0x%x, cropXY=%d,%d",\
               this->dma_cfg.pixel_byte, this->dma_cfg.size.w, this->dma_cfg.size.xsize, this->dma_cfg.size.stride,\
...
ISP_RAW_CENTRAL_CTRL_INFO::config2( ISP_TOP_CTRL * pSrcTopCtrl )
{
    ...
    if (( ISP_PASS1 == pSrcTopCtrl->path         || ISP_PASS1_D == pSrcTopCtrl->pathD) ||
        ...
        if (NULL == mpPhyIspDrv) {
           ...
           if (NULL == mpPhyIspDrv) {
              ISP_FUNC_ERR("RAW_CENTRAL NULL == mpPhyIspDrv");
              return MFALSE;
           }
        }
...
int ISP_TOP_CTRL::_config( void )
{
    ...
    //add mutex for global var:rawCentralCtrl protection
    Mutex::Autolock lock(rawCentralCtrl.mLock);
    ...
    if ((ISP_PASS1 == this->path)||(ISP_PASS1_D == this->pathD)){
           ...
           IspDrv* mRawDIspDrv = this->m_pIspDrvShell->ispDrvGetCQIspDrv(ISP_DRV_CQ0_D,0, 0/*mTopCtrl.burstQueIdx*/);
            ...
            if (rawCentralCtrl.mEnablePath == 0) {
                ...
                MINT32 nQueryRst = 10;
                ISP_FUNC_INF("[PIP][TopCtrl::config] Sw Reset");
                ISP_WRITE_BITS(m_pPhyIspDrv,CAM_CTL_SW_CTL, SW_RST_Trig, 0,ISP_DRV_USER_ISPF);
                ISP_WRITE_BITS(m_pPhyIspDrv,CAM_CTL_SW_CTL, SW_RST_Trig, 1,ISP_DRV_USER_ISPF);
                while(ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAM_CTL_SW_CTL) != 0x2 && nQueryRst){
                    ISP_FUNC_DBG("TOP reseting...\n");
                    nQueryRst--;
            	}
    ...
    if ( this->isp_top_ctl.CAMSV_CAMSV2_MODULE_EN.Bits.TG_EN ) {
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_CAMSV2_CLK_EN,0x00008005,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_CAMSV2_INT_EN ,this->isp_top_ctl.CAMSV_CAMSV2_INT_EN.Raw,ISP_DRV_USER_ISPF);
        ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_TG2_VF_CON,SINGLE_MODE,0,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_CAMSV2_MODULE_EN,this->isp_top_ctl.CAMSV_CAMSV2_MODULE_EN.Raw,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_CAMSV2_PAK,0x03,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_CAMSV2_FMT_SEL,0x01000000, ISP_DRV_USER_ISPF);
    }
    ...
    if ( ISP_PASS1   == this->path || ISP_PASS1_D == this->pathD ) {
        ...
        /* disable DCM */
        ISP_WRITE_REG(m_pPhyIspDrv,CAM_CTL_RAW_DCM_DIS,0xFFFFFFFF,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAM_CTL_RAW_D_DCM_DIS,0xFFFFFFFF,ISP_DRV_USER_ISPF);
        ...
        p1_log[4] = function_DbgLogEnable_DEBUG;//function_DbgLogEnable_INFO;//kernel log reduction
        p1_log[5] = p1_log[4];
        for (int ii=0; ii < 2; ii++)  {
           ...
           switch ( path[ii] ) {
               case ISP_PASS1:
                   ...
                   ISP_FUNC_INF("[PIP][TopCtrl]ISP_PASS1 P1:EN(0x%x/0x%x),DMA(0x%x/0x%x),fmt(0x%x/0x%x),sel(0x%x/0x%x),BYP(0x%x/0x%x),GLO(0x%x/0x%x/0x%x),rawDCM(0x%x)\n", \
                       ...
                       ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAM_CTL_RAW_DCM_DIS));
        ...
        for (int ii=0; ii < 2; ii++)  {
           ...
           case ISP_PASS1_CAMSV:
               ...
               m_pPhyIspDrv->dumpDBGLog(p1_log,NULL);
               //--TG setting
               ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_TG_VF_CON,SINGLE_MODE,this->b_continuous[ii]?0:1,ISP_DRV_USER_ISPF);
               ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_CAMSV_MODULE_EN,this->isp_top_ctl.CAMSV_CAMSV_MODULE_EN.Raw,ISP_DRV_USER_ISPF);
               #ifdef _rtbc_buf_que_2_0_
           ...
           case ISP_PASS1_CAMSV_D:
               ...
               m_pPhyIspDrv->dumpDBGLog(p1_log,NULL);
               //--TG setting
               ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_TG2_VF_CON,SINGLE_MODE,this->b_continuous[ii]?0:1,ISP_DRV_USER_ISPF);
               ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_CAMSV2_MODULE_EN,this->isp_top_ctl.CAMSV_CAMSV2_MODULE_EN.Raw,ISP_DRV_USER_ISPF);
               #ifdef _rtbc_buf_que_2_0_
    ...
    if ((this->path==ISP_PASS2)||(this->path==ISP_PASS2B)||(this->path==ISP_PASS2C))
    {
        ...
        if (this->isV3)
        {
            ...
            if(this->isp_top_ctl.CAM_CTL_SCENARIO.Bits.SCENARIO==ISP_HW_SCENARIO_IP)
            {
                ...
                p2EnConfig = 0x40000000; // For IP Raw path, p2 module should not be turned on except for UNP and IMGI port.
            }
...
//        }
        ...
        //
        // The GGM module transfer 12-bits to 10-bits, so GGM top must always enable while data passing through.
        // However, the inner GGM enable control of CAM_GGM_CTRL will still depend on tuning.
        p2EnConfig = (p2EnConfig | CAM_CTL_EN_P2_GGM_EN);
        ISP_WRITE_REG(m_pP2IspDrv, CAM_CTL_EN_P2, p2EnConfig, ISP_DRV_USER_ISPF);
		//
        // for scenario IP
        if(this->isp_top_ctl.CAM_CTL_SCENARIO.Bits.SCENARIO==ISP_HW_SCENARIO_IP){
...
int ISP_TOP_CTRL::_enable( void* pParam  )
{
    ...
    if ( CAM_ISP_PASS2_START == *start || CAM_ISP_PASS2B_START == *start || CAM_ISP_PASS2C_START == *start ){
        ISP_FUNC_DBG("path(%d),start(%d),isP2IspOnly(%d),dupCqIdx(%d),burstQueIdx(%d),SCEN(%d)",this->path,*start,this->isP2IspOnly,this->dupCqIdx,this->burstQueIdx,this->isp_top_ctl.CAM_CTL_SCENARIO.Bits.SCENARIO);
...
int ISP_TOP_CTRL::_disable( void )
{
    ...
    if ( bP1Path ) {
        /* FBC reset flow must protect by mutex to avoid PIP stop->start might not reset FBC,
           we move "wait VD operation" to here because it is not suitable in the mutex protection metioned above.
        */
        if (ISP_PASS1 == this->path) {
            ...
            //if( CAMSV_CAMSV2_MODULE_TG_EN & this->isp_top_ctl.CAMSV_CAMSV2_MODULE_EN.Raw)
			{
                ...
                ISP_FUNC_INF("cam_sv2 vf:(0x%x)",ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_TG2_VF_CON));
                ISP_FUNC_INF("cam_sv2 module_en:(0x%x)",ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_CAMSV2_MODULE_EN));
            }
                ...
                while ((pactive_irq_vs_int[0]) && ( wait_vs_num-- )) {
                    ...
                    if ( MFALSE == ret ) {
                        ...
                        if(ISP_READ_BITS_NOPROTECT(this->m_pPhyIspDrv,CAM_TG_INTER_ST,TG_CAM_CS) != 1){
                            ISP_FUNC_INF("[TG1]TG is not idle, wait 1 VD again.\n");
                            ret = this->m_pPhyIspDrv->waitIrq( pactive_irq_vs_int[0] );
                            if ( MFALSE == ret ) {
                                ISP_FUNC_ERR("waitIrq( irq_TG1_DONE ) fail");
                            }
                ...
                while ((pactive_irq_vs_int[0]) && ( wait_vs_num-- )) {
                    ...
                    if ( MFALSE == ret ) {
                        ...
                        if(ISP_READ_BITS_NOPROTECT(this->m_pPhyIspDrv,CAM_TG2_INTER_ST,TG_CAM_CS) != 1){
                            ISP_FUNC_INF("[TG2]TG is not idle, wait 1 VD again.\n");
                            ret = this->m_pPhyIspDrv->waitIrq( pactive_irq_vs_int[0] );
                            if ( MFALSE == ret ) {
                                ISP_FUNC_ERR("waitIrq( irq_TG2_DONE ) fail");
                            }
                                ...
                                if(ISP_READ_BITS_NOPROTECT(this->m_pPhyIspDrv,CAM_TG2_INTER_ST,TG_CAM_CS) != 1)
                                    ISP_FUNC_INF("[TG2]TG is not idle.(0x%x)\n", ISP_READ_REG_NOPROTECT(this->m_pPhyIspDrv,CAM_TG2_INTER_ST));
                            }
                         }
                         }
                    }
        ...
        /* [end] wait VD operation for DMA stop */
        /* add mutex for global var:rawCentralCtrl protection */
        if ( bP1Path ) {
            Mutex::Autolock lock(rawCentralCtrl.mLock);
            /* */
...
///////////////////S remove later after fbc ECO
                ...
                if (rawCentralCtrl.mEnablePath != 0) {  //workaround
...

                    {
                        ...
                        //Polling
                        wait_vs_num = 10;
                        if(this->isp_top_ctl.CAM_CTL_EN_P1_DMA.Bits.RRZO_EN){
                        ...
                        while ((temp != 0x1100000) && (wait_vs_num-- > 0)) {
                            ...
                            MUINT32 temp = (ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAM_CTL_IMGO_FBC))&0xFF0000F;
                            while ((temp != 0x1100000) && (wait_vs_num-- > 0)) {
                                    ISP_FUNC_INF("[PIP]IMGO FBC=0x%x", temp);
                                    temp = (ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAM_CTL_IMGO_FBC))&0xFF0000F;
                            }
                        }
                    }
...
///////////////////E
        ...
        if (ISP_PASS1_D == this->pathD) {
                ...
                {  //workaround
                    ...
                    if (rawCentralCtrl.mEnablePath != 0){
                        ...
                        //Polling to check
                        wait_vs_num = 10;
                        if(this->isp_top_ctl.CAM_CTL_EN_P1_DMA_D.Bits.RRZO_D_EN){
                        MUINT32 temp = (ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAM_CTL_RRZO_D_FBC))&0xFF0000F;
                        while ((temp != 0x1100000) && (wait_vs_num-- > 0)) {
                                    ISP_FUNC_INF("[PIP]RRZO FBC_D=0x%x", temp);
                                temp = (ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAM_CTL_RRZO_D_FBC))&0xFF0000F;
                        }
                    }
                        else{//IMGO_D FBC
                            MUINT32 temp = (ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAM_CTL_IMGO_D_FBC))&0xFF0000F;
                            while ((temp != 0x1100000) && (wait_vs_num-- > 0)) {
                                    ISP_FUNC_INF("[PIP]IMGO FBC_D=0x%x", temp);
                                    temp = (ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAM_CTL_IMGO_D_FBC))&0xFF0000F;
                            }
                        }
                    }
                    ISP_WRITE_BITS(m_pPhyIspDrv,CAM_TG2_VF_CON,VFDATA_EN,0,ISP_DRV_USER_ISPF);
        ...
        }/* [end] mutex for global var:rawCentralCtrl protection*/
...
{
    /*//mark, these codes always return 1
...
int ISP_RAW_PIPE::_config( void )
{
    ...
    if ( ISP_PASS2 == this->path || \
        ...
        MUINT32 i;
        MUINT64 tuningUpdateBit=eIspTuningMgrFunc_Null;
        ...
        if (this->isV3)  //QQ??
        {
            ...
            if(imageioPackage.isApplyTuning==MTRUE){
	            tuningUpdateBit = 0xffffffffffffffff;
                ...
                ISP_FUNC_DBG("tuningUpdateBit(0x%016llx),pTuningBuf(0x%08x)",tuningUpdateBit,tuningPackage.pTuningBuf);
            }

            tuningEnP1 = this->pTuningIspReg->CAM_CTL_EN_P1.Raw;
            tuningEnP1Dma = this->pTuningIspReg->CAM_CTL_EN_P1_DMA.Raw;
            tuningEnP2 = this->pTuningIspReg->CAM_CTL_EN_P2.Raw;
            tuningEnP2Dma = this->pTuningIspReg->CAM_CTL_EN_P2_DMA.Raw;
        ...
        if(imageioPackage.isApplyTuning==MTRUE){
            ...
            ISP_FUNC_DBG("tuningUpdateBit(0x%016llx),tuningEnP2(0x%08x),tuningEnP2Dma(0x%08x),pTuningBuf(0x%08x)",tuningUpdateBit,tuningEnP2,tuningEnP2Dma,tuningPackage.pTuningBuf);
        }
        if(NSImageio::NSIspio::eDrvScenario_IP == this->drvScenario){
            finalEnP1 = ((~tuningEnP1Tag)&this->en_p1)|(tuningEnP1Tag&tuningEnP1);
    ...
    if (!this->isV3)
    {
        ...
        if(CAM_CTL_EN_P1_RRZ_EN & this->en_p1 && 1 == this->isTwinMode) {
            ...
            ISP_FUNC_DBG("ISP_RAW_PIPE::_config(setP1RrzCfg:0x%x_0x%x):Twin lsc_en=%d E:path(%d),rrz_in_w/h(%d/%d),rrz_crop_x/y/fx/fy/w/h(%d/%d/%d/%d/%d/%d),rrz_out_w/h(%d/%d), sd_lw/xn(%d/%d),m_num(0x%x)", \
                this->path,this->pathD,\
                rrz_cfg.lsc_en,\
                rrz_cfg.pass1_path, \
...
int ISP_RAW_PIPE::_write2CQ(void)
{
    ...
    if ( ISP_PASS2 == this->path || \
        ...
        if(this->isV3)
        {
            ISP_FUNC_DBG("ISP_RGB_PIPE::_write2CQ");
            ...
            if(isApplyTuning==MTRUE){
				tuningUpdateBit = 0xffffffffffffffff;
				//ISP_FUNC_DBG("tuningUpdateBit(0x%016llx)",tuningUpdateBit);
            }
            finalEnP2 = ((~tuningEnP2Tag)&this->en_p2)|(tuningEnP2Tag&tuningEnP2);
        ...
        if(isApplyTuning==MTRUE){
            ...
            ISP_FUNC_DBG("tuningUpdateBit(0x%016llx),tuningEnP2(0x%08x),tuningEnP2Dma(0x%08x)",tuningUpdateBit,tuningEnP2,tuningEnP2Dma);
        }
        if(NSImageio::NSIspio::eDrvScenario_IP == this->drvScenario){
            finalEnP1 = ((~tuningEnP1Tag)&this->en_p2)|(tuningEnP1Tag&tuningEnP1);
...
MUINT32 ISP_RAW_PIPE::setEIS(void){
    ...
    if(this->pP1Tuning){
		...
		/* RRZ crop info for GIS */
		input.RRZcrop.x = this->rrz_in_roi.x;
		input.RRZcrop.y = this->rrz_in_roi.y;
		input.RRZcrop.w = this->rrz_in_roi.w;
		input.RRZcrop.h = this->rrz_in_roi.h;
		input.RRZcrop.floatX = this->rrz_in_roi.floatX;
		input.RRZcrop.floatY = this->rrz_in_roi.floatY;
        input.RRZcrop.full_w = this->rrz_in_size.w;
        input.RRZcrop.full_h = this->rrz_in_size.h;
        ...
        if(_eis_cfg.bEIS_Bypass == 0){
            ...
            if(_eis_cfg.bEIS_EN){
                ...
                if(!_targetVirDrv->getIspCQModuleInfo(CAM_ISP_EIS, outInfo)){
                    ISP_FUNC_ERR("getIspCQModuleInfo fail\n");
                    return 0;
                }
...
int ISP_RAW_PIPE::_suspendTuneCfg(MUINT32 mNum)
{
    ...
    if(mNum & _DUMMY_MAGIC){
        ...
        if(this->m_Dmao_Reg[_targetDupQueIdx] == 0xffffffff){
            ...
            ISP_FUNC_DBG("path:0x%x B:(0x%x):0x%x_0x%x\n",path,_targetDupQueIdx,_reg);
        return 1;
    }
    return 0;
}
...
int ISP_RAW_PIPE::setP1TuneCfg(MUINT32 mNum, MINT32 senDev)
{
    ...
    if (ISP_PASS1 == this->path){
        ...
        if (cfg.tuningUpdateBit & eIspTuningMgrFunc_Dbs) {
            ISP_FUNC_DBG("Dbs");

            memset(&outInfo, 0, sizeof(ISP_DRV_CQ_MODULE_INFO_STRUCT));
            if (!_targetVirDrv->getIspCQModuleInfo(CAM_ISP_DBS, outInfo)) {
                ISP_FUNC_ERR("[ERROR] CAM_ISP_DBS");
            ...
            m_pPhyIspDrv->cqAddModule((ISP_DRV_CQ_ENUM)this->CQ,this->burstQueIdx,_targetDupQueIdx,CAM_ISP_DBS);
            if(this->isTwinMode){
            	m_pPhyIspDrv->cqAddModule((ISP_DRV_CQ_ENUM)this->CQ,this->burstQueIdx,_targetDupQueIdx, CAM_ISP_DBS_D);
            }
            if (cfg.tuningEnP1 & CAM_CTL_EN_P1_DBS_EN) {
               //Enable
               memcpy((((MUINT8*)cqVirtReg)        + (outInfo.addr_ofst)), \
                      (((MUINT8*)(cfg.pTuningBuf))  + (outInfo.addr_ofst)), \
                      (outInfo.reg_num)*sizeof(UINT32));
               ISP_WRITE_BITS(_targetVirDrv, CAM_CTL_EN_P1, DBS_EN, 1,ISP_DRV_USER_ISPF);
               ISP_FUNC_DBG("Enable Dbs");

            }else {
               //Disable
               ISP_WRITE_BITS(_targetVirDrv, CAM_CTL_EN_P1, DBS_EN, 0,ISP_DRV_USER_ISPF);
               memcpy((((MUINT8*)cqVirtReg)        + (outInfo.addr_ofst)), \
                      (((MUINT8*)(cfg.pTuningBuf)) + (outInfo.addr_ofst)), \
                      (outInfo.reg_num)*sizeof(UINT32));
               ISP_FUNC_DBG("Disable Dbs");
            }
        }
        ...
        if (cfg.tuningUpdateBit & eIspTuningMgrFunc_Rmg) {
            ...
            memset(&outInfo, 0, sizeof(ISP_DRV_CQ_MODULE_INFO_STRUCT));
            if (!_targetVirDrv->getIspCQModuleInfo(CAM_CTL_VHDR, outInfo)) {
                ISP_FUNC_ERR("[ERROR] CAM_CTL_VHDR");
                if(m_pPhyIspDrv->enTuningQueByCq(eTuningQueAccessPath_imageio_P1, cq, mNum , eIspTuningMgrFunc_Null)==MFALSE){
                    ISP_FUNC_ERR("[Error]enTuningQue fail");
                }
                return -1;
            ...
            m_pPhyIspDrv->cqAddModule((ISP_DRV_CQ_ENUM)this->CQ,this->burstQueIdx,_targetDupQueIdx,CAM_CTL_VHDR);
            if(this->isTwinMode){
            	m_pPhyIspDrv->cqAddModule((ISP_DRV_CQ_ENUM)this->CQ,this->burstQueIdx,_targetDupQueIdx, CAM_CTL_VHDR_D);
            }
        ...
        if (cfg.tuningUpdateBit & eIspTuningMgrFunc_Bnr) {
            ...
            if (!_targetVirDrv->getIspCQModuleInfo(CAM_DMA_BPCI, outInfo)) {
                ISP_FUNC_ERR("[ERROR] CAM_ISP_BPCI");
                if(m_pPhyIspDrv->enTuningQueByCq(eTuningQueAccessPath_imageio_P1, cq, mNum , eIspTuningMgrFunc_Null)==MFALSE){
                    ISP_FUNC_ERR("[Error]enTuningQue fail");
                }
                return -1;
        ...
        if (cfg.tuningUpdateBit & eIspTuningMgrFunc_Rpg) {
            ...
            if (!_targetVirDrv->getIspCQModuleInfo(CAM_ISP_RPG, outInfo)) {
                ISP_FUNC_ERR("[ERROR] CAM_ISP_RPG");
                if(m_pPhyIspDrv->enTuningQueByCq(eTuningQueAccessPath_imageio_P1, cq, mNum , eIspTuningMgrFunc_Null)==MFALSE){
                    ISP_FUNC_ERR("[Error]enTuningQue fail");
                }
                return -1;
    ...
    if (ISP_PASS1_D == this->pathD){
        ...
        if (cfg.tuningUpdateBit & eIspTuningMgrFunc_Dbs_d) {
			ISP_FUNC_DBG("DBS_D");
            memset(&outInfo, 0, sizeof(ISP_DRV_CQ_MODULE_INFO_STRUCT));
            if (!_targetVirDrv->getIspCQModuleInfo(CAM_ISP_DBS_D, outInfo)) {
                ISP_FUNC_ERR("[ERROR] CAM_ISP_DBS_D");
                if(m_pPhyIspDrv->enTuningQueByCq(eTuningQueAccessPath_imageio_P1, cq, mNum , eIspTuningMgrFunc_Null)==MFALSE){
                    ISP_FUNC_ERR("[Error]enTuningQue fail");
                }
                return -1;
            ...
            m_pPhyIspDrv->cqAddModule((ISP_DRV_CQ_ENUM)this->CQ_D,this->burstQueIdx,_targetDupQueIdx,CAM_ISP_DBS_D);
            if (cfg.tuningEnP1D & CAM_CTL_EN_P1_D_DBS_D_EN) {
               //Enable
               memcpy((((MUINT8*)cqDVirtReg)        + (outInfo.addr_ofst)), \
                      (((MUINT8*)(cfg.pTuningBuf)) + (outInfo.addr_ofst)), \
                      (outInfo.reg_num)*sizeof(UINT32));
               ISP_WRITE_BITS(_targetVirDrv, CAM_CTL_EN_P1_D, DBS_D_EN, 1,ISP_DRV_USER_ISPF);
            }else {
               //Disable
               ISP_WRITE_BITS(_targetVirDrv, CAM_CTL_EN_P1_D, DBS_D_EN, 0,ISP_DRV_USER_ISPF);
               memcpy((((MUINT8*)cqDVirtReg)       + (outInfo.addr_ofst)), \
                      (((MUINT8*)(cfg.pTuningBuf)) + (outInfo.addr_ofst)), \
                      (outInfo.reg_num)*sizeof(UINT32));
            }
        ...
        if (cfg.tuningUpdateBit & eIspTuningMgrFunc_Rmg_d) {
            ...
            memset(&outInfo, 0, sizeof(ISP_DRV_CQ_MODULE_INFO_STRUCT));
            if (!_targetVirDrv->getIspCQModuleInfo(CAM_CTL_VHDR_D, outInfo)) {
                ISP_FUNC_ERR("[ERROR] CAM_CTL_VHDR_D");
                if(m_pPhyIspDrv->enTuningQueByCq(eTuningQueAccessPath_imageio_P1, cq, mNum , eIspTuningMgrFunc_Null)==MFALSE){
                    ISP_FUNC_ERR("[Error]enTuningQue fail");
                }
                return -1;
            ...
            m_pPhyIspDrv->cqAddModule((ISP_DRV_CQ_ENUM)this->CQ_D,this->burstQueIdx,_targetDupQueIdx,CAM_CTL_VHDR_D);
...
int ISP_RGB_PIPE::_config( void )
{
    ...
    if(this->isV3)
    {
        ...
        if(imageioPackage.isApplyTuning==MTRUE){
            ...
            ISP_FUNC_DBG("tuningUpdateBit(0x%016llx),pTuningBuf(0x%08x),hrz_crop(%d,%d)", \
                tuningUpdateBit,tuningPackage.pTuningBuf,tuningPackage.rrzCropX,tuningPackage.rrzCropY);
        }
        finalEnP2 = ((~tuningEnP2Tag)&this->en_p2)|(tuningEnP2Tag&tuningEnP2);
        imageioPackage.finalEnP2 = finalEnP2;

        ISP_FUNC_DBG("(ISP_RGB_PIPE),isApplyTuning(0x%x),finalEnP2(0x%x),tuningUpdateBit(0x%x)",this->isApplyTuning,finalEnP2,tuningUpdateBit);
    }
    else
    {
    getP2EnTuningTag(MTRUE,(ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario, this->sub_mode, tuningEnP2Tag, tuningEnP2DmaTag); // get P2 tuning En & Dma Tag
...
int ISP_YUV_PIPE::_config( void )
{
    ...
    if (this->isV3)
    {
        ...
        if(imageioPackage.isApplyTuning==MTRUE){
            ...
            ISP_FUNC_DBG("tuningUpdateBit(0x%016llx),tuningEnP2(0x%08x),tuningEnP2Dma(0x%08x),pTuningBuf(0x%08x),nr3dGainWeighting(0x%x)",tuningUpdateBit,tuningEnP2,tuningEnP2Dma,tuningPackage.pTuningBuf,tuningPackage.nr3dGainWeighting);
        }
        finalEnP2 = ((~tuningEnP2Tag)&this->en_p2)|(tuningEnP2Tag&tuningEnP2);
        imageioPackage.finalEnP2 = finalEnP2;

        ISP_FUNC_DBG("(ISP_YUV_PIPE),isApplyTuning(0x%x),finalEnP2(0x%x),tuningUpdateBit(0x%x)",this->isApplyTuning,finalEnP2,tuningUpdateBit);
    }
    else
    {
    getP2EnTuningTag(MTRUE,(ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario, this->sub_mode, tuningEnP2Tag, tuningEnP2DmaTag); // get P2 tuning En & Dma Tag
    ...
    if(imageioPackage.isApplyTuning==MTRUE){
		...
		ISP_DRV_P2_CQ_ENUM p2Cq;
		this->m_pIspDrvShell->m_pPhyIspDrv_bak->mapCqToP2Cq((ISP_DRV_CQ_ENUM)this->CQ, p2Cq);
		//MUINT32 rwbMatrix[ISP_RWB_MATRIX_CELL_NUM] = {0};
		for (i=0;i<ISP_RWB_MATRIX_CELL_NUM;i++)
		{
			gpMdpMgrCfgData[this->burstQueIdx][p2Cq][this->dupCqIdx].rwbMatrix[i] = tuningPackage.pTuningBuf[(0x7BC0>>2)+i];
			//rwbMatrix[i] = tuningPackage.pTuningBuf[(0x7BC0>>2)+i];
			ISP_FUNC_DBG("gpMdpMgrCfgData[%d][%d][%d].rwbMatrix[%d] = %x", this->burstQueIdx, p2Cq, this->dupCqIdx, i, gpMdpMgrCfgData[this->burstQueIdx][p2Cq][this->dupCqIdx].rwbMatrix[i]);
			//ISP_FUNC_INF("(ISP_YUV_PIPE), rwb[%d] = %x", i, rwbMatrix[i]);
		}
		// add for RWB sensor -----

		// add for ISO adaptive +++++
		gpMdpMgrCfgData[this->burstQueIdx][p2Cq][this->dupCqIdx].isoValue = tuningPackage.pTuningBuf[(0x7BE4>>2)];
		ISP_FUNC_DBG("gpMdpMgrCfgData[%d][%d][%d].isoValue = %x", this->burstQueIdx, p2Cq, this->dupCqIdx, gpMdpMgrCfgData[this->burstQueIdx][p2Cq][this->dupCqIdx].isoValue);
		// add for ISO adaptive -----
    }
...
int ISP_YUV_PIPE::_write2CQ(void)
{
    ...
    if (this->isV3)
    {
         ISP_FUNC_DBG("ISP_YUV_PIPE::_write2CQ");
        getP2EnTuningTag(MTRUE,(ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario, this->sub_mode, tuningEnP2Tag, tuningEnP2DmaTag); // get P2 tuning En & Dma Tag
           isApplyTuning = this->isApplyTuning;
        if(isApplyTuning==MTRUE){
	        tuningUpdateBit = 0xffffffffffffffff;
...
//        this->src_crop.h);
    ...
    if(this->CQ!=ISP_DRV_CQ01 && this->CQ!=ISP_DRV_CQ02 && this->CQ!=ISP_DRV_CQ03){
		...
		//gpMdpMgrCfgData[this->burstQueIdx][p2Cq][this->dupCqIdx].isoValue = this->isoValue;	// set ISO	// remove for ISO adaptive
    }

	//ISP_FUNC_DBG("set iso: ISP_MDP_PIPE(%d), MDPMGR_CFG_STRUCT_S(%d)", this->isoValue, gpMdpMgrCfgData[this->burstQueIdx][p2Cq][this->dupCqIdx].isoValue);
    ...
    TdriDrvCfg* pSrcImageio)
{
    ...
    pDst->top.udm_en = pSrcImageio->top.udm_en;
    ...
    pDst->top.nsl2a_en = pSrcImageio->top.nsl2a_en;
    pDst->top.nsl2b_en = pSrcImageio->top.nsl2b_en;
    pDst->top.nbc_en = pSrcImageio->top.nbc_en;
	pDst->top.nbc2_en = pSrcImageio->top.nbc2_en;
    //pDst->top.sl2_en = pSrcImageio->top.sl2_en;
    pDst->top.nsl2_en = pSrcImageio->top.nsl2_en;
	...
	pDst->top.dbs_en = pSrcImageio->top.dbs_en;
...
//    pDst->feo.feo_stride = pSrcImageio->feo.feo_stride;
    ...
    pDst->nsl2.nsl2_hrz_comp = pSrcImageio->tuningFunc.nsl2.nsl2_hrz_comp;
    pDst->nsl2.nsl2_vrz_comp = pSrcImageio->tuningFunc.nsl2.nsl2_vrz_comp;
    //
    ...
    //
    pDst->nbc.anr1_eny = pSrcImageio->tuningFunc.nbc.anr1_eny;
    pDst->nbc.anr1_enc = pSrcImageio->tuningFunc.nbc.anr1_enc;
    pDst->nbc.anr1_iir_mode = pSrcImageio->tuningFunc.nbc.anr1_iir_mode;
    pDst->nbc.anr1_scale_mode = pSrcImageio->tuningFunc.nbc.anr1_scale_mode;
    //
    pDst->nbc2.anr2_eny = pSrcImageio->tuningFunc.nbc2.anr2_eny;
    pDst->nbc2.anr2_enc = pSrcImageio->tuningFunc.nbc2.anr2_enc;
    pDst->nbc2.anr2_scale_mode = pSrcImageio->tuningFunc.nbc2.anr2_scale_mode;
    //
    ...
    pDst->udm.bayer_bypass = pSrcImageio->tuningFunc.udm.bayer_bypass;
    //
    ...
    pDst->nsl2a.nsl2a_hrz_comp = pSrcImageio->tuningFunc.nsl2a.nsl2a_hrz_comp;
    pDst->nsl2a.nsl2a_vrz_comp = pSrcImageio->tuningFunc.nsl2a.nsl2a_vrz_comp;
    pDst->nsl2b.nsl2b_hrz_comp = pSrcImageio->tuningFunc.nsl2b.nsl2b_hrz_comp;
    pDst->nsl2b.nsl2b_vrz_comp = pSrcImageio->tuningFunc.nsl2b.nsl2b_vrz_comp;
    //
	...
	::memcpy( (char*)&pDst->nsl2, (char*)&pSrcImageio->tuningFunc.nsl2, sizeof(ISP_TPIPE_CONFIG_NSL2_STRUCT));
	...
	::memcpy( (char*)&pDst->nbc2, (char*)&pSrcImageio->tuningFunc.nbc2, sizeof(ISP_TPIPE_CONFIG_NBC2_STRUCT));
	...
	::memcpy( (char*)&pDst->udm, (char*)&pSrcImageio->tuningFunc.udm, sizeof(ISP_TPIPE_CONFIG_UDM_STRUCT));
	...
	::memcpy( (char*)&pDst->nsl2a, (char*)&pSrcImageio->tuningFunc.nsl2a, sizeof(ISP_TPIPE_CONFIG_NSL2A_STRUCT));
	::memcpy( (char*)&pDst->nsl2b, (char*)&pSrcImageio->tuningFunc.nsl2b, sizeof(ISP_TPIPE_CONFIG_NSL2B_STRUCT));
    ...
    if(isApplyTuning==MTRUE){
        ...
        p2TuningEn = m_pPhyIspDrv->getTuningTop(eTuningQueAccessPath_imageio_P2, eTuningTopEn_p2, this->CQ, this->magicNum);
        p2TuningDma = m_pPhyIspDrv->getTuningTop(eTuningQueAccessPath_imageio_P2, eTuningTopEn_p2_dma, this->CQ, this->magicNum);
        p1TuningEn = m_pPhyIspDrv->getTuningTop(eTuningQueAccessPath_imageio_P1, eTuningTopEn_p1, this->CQ, this->magicNum);
        p1TuningDma = m_pPhyIspDrv->getTuningTop(eTuningQueAccessPath_imageio_P1, eTuningTopEn_p1_dma, this->CQ, this->magicNum);
        ...
        */
        ...
        if(p2TuningTopEnTag & CAM_CTL_EN_P2_NSL2_EN ){
            pDst->top.nsl2_en = ((p2TuningEn&CAM_CTL_EN_P2_NSL2_EN)?(1):(0));
        }
        /*
        ...
        */
        //
        if(p2TuningTopEnTag & CAM_CTL_EN_P2_UDM_EN ){
            pDst->top.udm_en = ((p2TuningEn&CAM_CTL_EN_P2_UDM_EN)?(1):(0));
        }
		...
		if(p2TuningTopEnTag & CAM_CTL_EN_P2_NBC2_EN ){
            pDst->top.nbc2_en = ((p2TuningEn&CAM_CTL_EN_P2_NBC2_EN)?(1):(0));
        }
...
//            }
        ...
        */
    	...
    	if(p2TuningTopEnTag & CAM_CTL_EN_P2_NSL2B_EN){
            pDst->top.nsl2b_en = ((p2TuningEn&CAM_CTL_EN_P2_NSL2B_EN)?(1):(0));
        }

    	if(p2TuningTopEnTag & CAM_CTL_EN_P2_NSL2A_EN){
            pDst->top.nsl2a_en = ((p2TuningEn&CAM_CTL_EN_P2_NSL2A_EN)?(1):(0));
        }
        ...
        */
        //set for p2_en field
        if(p2TuningFieldEnTag & CAM_CTL_EN_P2_NSL2_EN ){
            #if 1
            pDst->nsl2.nsl2_hrz_comp = tuningField.nsl2.nsl2_hrz_comp;
			pDst->nsl2.nsl2_vrz_comp = tuningField.nsl2.nsl2_vrz_comp;
            #else
            ::memcpy( (char*)&pDst->nsl2, (char*)&tuningField.nsl2, sizeof(ISP_TPIPE_CONFIG_NSL2_STRUCT));
            #endif
        }
		/*
        ...
        */
        ...
        if(p2TuningFieldEnTag & CAM_CTL_EN_P2_UDM_EN ){
            // dm_fg_mode be set from path imageio
            pDst->udm.bayer_bypass = tuningField.udm.bayer_bypass;
        }
        ...
        if(p2TuningFieldEnTag & CAM_CTL_EN_P2_NBC_EN ){
            #if 1
            pDst->nbc.anr1_eny = tuningField.nbc.anr1_eny;
            pDst->nbc.anr1_enc = tuningField.nbc.anr1_enc;
            pDst->nbc.anr1_iir_mode = tuningField.nbc.anr1_iir_mode;
            pDst->nbc.anr1_scale_mode = tuningField.nbc.anr1_scale_mode;
            #else
            ::memcpy( (char*)&pDst->nbc, (char*)&tuningField.nbc, sizeof(ISP_TPIPE_CONFIG_NBC_STRUCT));
            #endif
        }

		if(p2TuningFieldEnTag & CAM_CTL_EN_P2_NBC2_EN ){
            #if 1
            pDst->nbc2.anr2_eny = tuningField.nbc2.anr2_eny;
            pDst->nbc2.anr2_enc = tuningField.nbc2.anr2_enc;
            pDst->nbc2.anr2_scale_mode = tuningField.nbc2.anr2_scale_mode;
            #else
            ::memcpy( (char*)&pDst->nbc2, (char*)&tuningField.nbc2, sizeof(ISP_TPIPE_CONFIG_NBC2_STRUCT));
            #endif
        }
        ...
        */
        ...
        if(p2TuningFieldEnTag & CAM_CTL_EN_P2_NSL2A_EN){
            pDst->nsl2a.nsl2a_hrz_comp = tuningField.nsl2.nsl2_hrz_comp;
			pDst->nsl2a.nsl2a_vrz_comp = tuningField.nsl2.nsl2_vrz_comp;
            //::memcpy( (char*)&pDst->nsl2a, (char*)&tuningField.nsl2a, sizeof(ISP_TPIPE_CONFIG_NSL2A_STRUCT));
        }

        if(p2TuningFieldEnTag & CAM_CTL_EN_P2_NSL2B_EN){
            pDst->nsl2b.nsl2b_hrz_comp = tuningField.nsl2.nsl2_hrz_comp;
			pDst->nsl2b.nsl2b_vrz_comp = tuningField.nsl2.nsl2_vrz_comp;
            //::memcpy( (char*)&pDst->nsl2a, (char*)&tuningField.nsl2a, sizeof(ISP_TPIPE_CONFIG_NSL2A_STRUCT));
        }
        if(p2TuningFieldEnTag & CAM_CTL_EN_P2_SL2C_EN){
            pDst->sl2c.sl2c_hrz_comp = tuningField.nsl2.nsl2_hrz_comp;
        ...
        if(this->drvScenario == NSImageio::NSIspio::eDrvScenario_IP)
        {
            ...
            //
            //set for p1_en top
            if(p1TuningTopEnTag & CAM_CTL_EN_P1_DBS_EN ){
                pDst->top.dbs_en = ((p1TuningEn&CAM_CTL_EN_P1_DBS_EN)?(1):(0));
            }
            if(p1TuningTopEnTag & CAM_CTL_EN_P1_BNR_EN ){
                pDst->top.bnr_en = ((p1TuningEn&CAM_CTL_EN_P1_BNR_EN)?(1):(0));
            }
            if(p1TuningTopEnTag & CAM_CTL_EN_P1_LSC_EN ){
                pDst->top.lsc_en = ((p1TuningEn&CAM_CTL_EN_P1_LSC_EN)?(1):(0));
            }
            //set for p1_dma top
...
int ISP_PASS1_CTRL::setP1RrzCfg( IspRrzCfg const rrz_cfg )
{
    ...
    if ( rrz_cfg.isTwinMode) {
        ...
        TWIN_GET_IN_CFG()->SW.TWIN_RAWI_XSIZE = 0;//
        ...
        //ISP_WRITE_REG(m_pP1IspDrv,CAM_RMG_D_HDR_CFG,ISP_READ_REG_NOPROTECT(m_pP1IspDrv, CAM_RMG_HDR_CFG),ISP_DRV_USER_ISPF);
        //ISP_WRITE_REG(m_pP1IspDrv,CAM_RMG_D_HDR_GAIN,ISP_READ_REG_NOPROTECT(m_pP1IspDrv, CAM_RMG_HDR_GAIN),ISP_DRV_USER_ISPF);
        //debug out param
...
waitBufReady( MUINT32 dmaChannel )
{
    ...
    MUINT32 bWaitBufRdy;
    MUINT32 loopCnt = 0;
    //for supporting manual long exposure time control
    ...
    if ( _imgo_ == rt_dma || _imgo_d_ == rt_dma || _rrzo_ == rt_dma || _rrzo_d_ == rt_dma || _camsv_imgo_ == rt_dma || _camsv2_imgo_ == rt_dma) {
        ...
        ISP_FUNC_DBG("waitBufReady[%d]:[%x,%x] rtBufCtrl.ctrl(%d)/id(%d)/ptr(0x%x)",rt_dma,this->path, this->path_D,buf_ctrl.ctrl,buf_ctrl.buf_id,buf_ctrl.data_ptr);
#if 0//mark for kernel log reduction, only check buf rdy once
...
dequeueHwBuf( MUINT32 dmaChannel, stISP_FILLED_BUF_LIST& bufList )
{
	...
	char value[PROPERTY_VALUE_MAX] = {'\0'};
	property_get("camera.dumpHwBuf.enable", value, "0");
	int dumpHwBufEnable=atoi(value);
...
class IspEventThreadImp : public IspEventThread
{
    IspEventThreadImp()
        ...
        ,m_bBreak(MFALSE)
    {
        memset(&m_semIspEventthread, 0, sizeof(m_semIspEventthread));
        memset(&m_ispEventthread, 0, sizeof(m_ispEventthread));
    }
    //
...
MVOID* IspEventThreadImp::ispEventThread(void *arg)
{
    ...
    while (1)
    {
        ...
        //1:SOF
        if(_this->m_pPhyIspDrv->waitIrq( &irq_SOF ) == MFALSE){
            ISP_FUNC_ERR("wait SOF fail");
            continue;
        }
        ISP_FUNC_DBG("SOF");
    }
