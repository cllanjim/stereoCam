...
{
    MINT32 err = 0 ; // MDPMGR_NO_ERROR; Fix Inferred misuse of enum (MIXED_ENUMS)
    DP_STATUS_ENUM err2;
	...
	float rwbMatrix[ISP_RWB_MATRIX_CELL_NUM];	// add for RWB sensor

    ISP_TRACE_CALL();
	LOG_DBG("set iso: %d", cfgData.isoValue);	// set ISO
	LOG_DBG("cfgData.dstDma[ISP_MDP_DL_WDMAO].bIsRwbEn = %d", cfgData.dstDma[ISP_MDP_DL_WDMAO].bIsRwbEn);	// add for RWB sensor
	LOG_DBG("cfgData.dstDma[ISP_MDP_DL_WROTO].bIsRwbEn = %d", cfgData.dstDma[ISP_MDP_DL_WROTO].bIsRwbEn);	// add for RWB sensor
	for(MUINT32 i=0; i<ISP_RWB_MATRIX_CELL_NUM; i++)
	{
		float* tmp = (float*)&cfgData.rwbMatrix[i];
		rwbMatrix[i] = *tmp;
		LOG_DBG("cfgData.rwbMatrix[%d] = %x; frwbMatrix[%d] = %f", i,cfgData.rwbMatrix[i], i,rwbMatrix[i]);
	}
...
//            cfgData.srcCropX,cfgData.srcCropFloatX,cfgData.srcCropY,cfgData.srcCropFloatY,cfgData.srcCropW,cfgData.srcCropH,cfgData.mdpSrcPlaneNum);
	...
	// for CTS test case, fix different view angle +++++
	if ((cfgData.dstPortCfg[0] == 1) && (cfgData.dstPortCfg[1] == 1))
	{
		if (cfgData.dstDma[0].srcCropX > cfgData.dstDma[1].srcCropX)
		{
			if ((cfgData.dstDma[0].srcCropX - cfgData.dstDma[1].srcCropX) >= 256)
			{
				LOG_INF("For CTS:(Before)cfgData.dstDma[0].srcCrop(%u, %u), cfgData.dstDma[1].srcCrop(%u, %u)", cfgData.dstDma[0].srcCropX, cfgData.dstDma[0].srcCropW, cfgData.dstDma[1].srcCropX, cfgData.dstDma[1].srcCropW);
				cfgData.dstDma[0].srcCropX = cfgData.dstDma[1].srcCropX;
				cfgData.dstDma[0].srcCropW = cfgData.dstDma[1].srcCropW;
				LOG_INF("For CTS:(After)cfgData.dstDma[0].srcCrop(%u, %u), cfgData.dstDma[1].srcCrop(%u, %u)", cfgData.dstDma[0].srcCropX, cfgData.dstDma[0].srcCropW, cfgData.dstDma[1].srcCropX, cfgData.dstDma[1].srcCropW);				
			}
		}
		else
		{
			if ((cfgData.dstDma[1].srcCropX - cfgData.dstDma[0].srcCropX) >= 256)
			{
				LOG_INF("For CTS:(Before)cfgData.dstDma[0].srcCrop(%u, %u), cfgData.dstDma[1].srcCrop(%u, %u)", cfgData.dstDma[0].srcCropX, cfgData.dstDma[0].srcCropW, cfgData.dstDma[1].srcCropX, cfgData.dstDma[1].srcCropW);
				cfgData.dstDma[1].srcCropX = cfgData.dstDma[0].srcCropX;
				cfgData.dstDma[1].srcCropW = cfgData.dstDma[0].srcCropW;
				LOG_INF("For CTS:(After)cfgData.dstDma[0].srcCrop(%u, %u), cfgData.dstDma[1].srcCrop(%u, %u)", cfgData.dstDma[0].srcCropX, cfgData.dstDma[0].srcCropW, cfgData.dstDma[1].srcCropX, cfgData.dstDma[1].srcCropW);
			}
		}		
	}
    ...
    for(MINT32 index = 0; index < ISP_MDP_DL_NUM; index++)
    {
        ...
        //set src crop if need
           LOG_DBG("QQ idx[%d]_src_crop(%d/%d) : X(%u),FloatX(%u),Y(%u),FloatY(%u),W(%u),H(%u)", index,cfgData.dstDma[index].withCropM,cfgData.dstDma[index].enSrcCrop,\
...
//         if(cfgData.dstDma[index].withCropM && cfgData.dstDma[index].enSrcCrop)
        {
            ...
            switch(cfgData.dstDma[index].capbility)
               {
                case NSImageio::NSIspio::EPortCapbility_Rcrd:
					...
					err2 = pDpStream->setPQParameter(index, ISPParam);	// Set sharpness disable because change capbility issue
                    break;
                case NSImageio::NSIspio::EPortCapbility_Disp:
                    ...
                    // add for ISO adaptive +++++
                    //DpPqParam	 ISPParam;	
		    ISPParam.enable = true;  
		    //ISPParam.scenario = MEDIA_ISP_PREVIEW; 
		    //ISPParam.u.isp.iso = cfgData.isoValue; 
		    err2 = pDpStream->setPQParameter(index, ISPParam);
		    // add for ISO adaptive -----
                    if(err2 != DP_STATUS_RETURN_SUCCESS)
                    {
			...
			// for RWB sensor +++++
			DpColorMatrix DpColorMatrixCfg;
			DpColorMatrixCfg.enable = cfgData.dstDma[index].bIsRwbEn;
			DpColorMatrixCfg.c00 = rwbMatrix[0];
			DpColorMatrixCfg.c01 = rwbMatrix[1];
			DpColorMatrixCfg.c02 = rwbMatrix[2];
			DpColorMatrixCfg.c10 = rwbMatrix[3];
			DpColorMatrixCfg.c11 = rwbMatrix[4];
			DpColorMatrixCfg.c12 = rwbMatrix[5];
			DpColorMatrixCfg.c20 = rwbMatrix[6];
			DpColorMatrixCfg.c21 = rwbMatrix[7];
			DpColorMatrixCfg.c22 = rwbMatrix[8];

			pDpStream->setPortMatrix(index, &DpColorMatrixCfg);
            // for RWB sensor -----
...
MINT32 MdpMgrImp::startMdp(MDPMGR_CFG_STRUCT cfgData)
{
    ...
    LOG_DBG("+,cqIndx(%u),sceID(%u),cqPA(0x%x),tpipVa(0x%x),tpipPa(0x%x),isWaitBuf(%d),venc(%d)", \
...
MVOID dumpIspTPipeInfo(ISP_TPIPE_CONFIG_STRUCT a_info, MBOOL dump_en)
{
    if(dump_en)
    {
		...
		LOG_INF("top.nsl2_en(%d)",                                 a_info.top.nsl2_en);
		...
		LOG_INF("top.nsl2a_en(%d)",                                a_info.top.nsl2a_en);
		LOG_INF("top.nsl2b_en(%d)",                                a_info.top.nsl2b_en);
		LOG_INF("top.dbs_en(%d)",                                  a_info.top.dbs_en);
		LOG_INF("top.rmg_en(%d)",                                  a_info.top.rmg_en);
		...
		LOG_INF("nsl2.nsl2_hrz_comp(%d)",                          a_info.nsl2.nsl2_hrz_comp);
		LOG_INF("nsl2.nsl2_vrz_comp(%d)",                          a_info.nsl2.nsl2_vrz_comp);
		...
		LOG_INF("nsl2a.nsl2a_hrz_comp(%d)",                        a_info.nsl2a.nsl2a_hrz_comp);
		LOG_INF("nsl2a.nsl2a_vrz_comp(%d)",                        a_info.nsl2a.nsl2a_vrz_comp);
		LOG_INF("nsl2b.nsl2b_hrz_comp(%d)",                        a_info.nsl2b.nsl2b_hrz_comp);
		LOG_INF("nsl2b.nsl2b_vrz_comp(%d)",                        a_info.nsl2b.nsl2b_vrz_comp);
	    LOG_INF("nbc.anr1_eny(%d)",                                 a_info.nbc.anr1_eny);
	    LOG_INF("nbc.anr1_enc(%d)",                                 a_info.nbc.anr1_enc);
	    LOG_INF("nbc.anr1_iir_mode(%d)",                            a_info.nbc.anr1_iir_mode);
	    LOG_INF("nbc.anr1_scale_mode(%d)",                          a_info.nbc.anr1_scale_mode);
