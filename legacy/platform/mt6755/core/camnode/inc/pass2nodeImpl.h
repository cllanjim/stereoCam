...
class Pass2NodeImpl : public Pass2Node
{
    public: // ctor & dtor
        ...
        // for capture only: < 0: none 1:mdp  2:gpu  >
        virtual MBOOL  setGPUProcType(MUINT32 const type) {mbIsGPUProcType= type; return MTRUE;};

        //
        virtual void    setCropRatio(float ratio) { mCropRatio = ratio; }
...
class FeaturePass2 : public PrvPass2
{
    public:
        ...
        virtual MBOOL enableSDPreview(MBOOL en);
...
class CapPass2 : public Pass2NodeImpl
{
    ...
    protected:
        ...
        MBOOL allocMemory(IImageBuffer* srcbuf, MINT32 const fmt, sp<IImageBuffer>& targetBuf, MBOOL isGralloc = MFALSE);
        MBOOL freeMemory(sp<IImageBuffer>& targetBuf);

        MVOID dumpBuffer(IImageBuffer* pBuf, const char* filename, const char* fileext);
        MBOOL doRwbGpu(IImageBuffer* pBuf, IImageBuffer* dst0, IImageBuffer* dst1, MINT32 trans);
    ...
    private:
        ...
        SwNR*                   mpSwNR;
        //
        sp<RwbGpuProc>          mspRwbGpuProc;
        sp<IImageBuffer>        mspGPUDstBuf;
        //
        MUINT32                 muDumpFlag;
};
