...
Pass2NodeImpl(Pass2NodeType const type)
    ...
    , mCropRatio(1.0f)
{
    addNotifySupport( CONTROL_P2DONE );
}
...
#define DUMP_IImageBufferStride( type, pbuf, fileExt, cnt)     \
    ...
    switch( data )
    {
        ...
        case PASS2_CAP_DST_3:
            DUMP_IImageBuffer( PASS2_CAP_DST_3, buf, "yuv", muDeqFrameCnt  );
            break;
        default:
...
enquePass2(MBOOL const doCallback)
{
    ...
    while( pData != vP2data.end() )
    {
        ...
        MCropRect p2InCrop, p2InCropWithoutZoom;
        MSize srcSize;
        MINT64 timestamp;
        ...
        if( !isPreviewPath() ) {
            ...
            //Precrop main1 IMGO if target FOV is different to sensor FOV
            //RRZO cropping is written in stereo_hal by chaning rrz_usage size
            //Ratio is set in StereoShot
            if(!isRrzo &&
               mCropRatio < 1.0f &&
               mCropRatio > 0.0f)
            {
                MSize origSize = p2InCrop.s;
                p2InCrop.s.w = (int)(p2InCrop.s.w * mCropRatio) & ~1;
                p2InCrop.s.h = (int)(p2InCrop.s.h * mCropRatio) & ~1;
                p2InCrop.p_integral.x += (origSize.w - p2InCrop.s.w)/2;
                p2InCrop.p_integral.y += (origSize.h - p2InCrop.s.h)/2;
                MY_LOGD("Crop ratio %.2f (%d, %d) %dx%d", mCropRatio,
                        p2InCrop.p_integral.x, p2InCrop.p_integral.y,
        ...
        {
            MY_LOGD("do pass2 start callback");
            handleNotify(CONTROL_P2DONE);
        }
        ...
        crop2.mCropRectExceptDZ = p2InCropWithoutZoom;
                ...
                pOutReq != pData->vDstReq.end(); pOutReq++, i++ )
        {
            ...
            else
            {
                if( (mbIsGPUProcType == 2)
                    && !isPreviewPath()
                    && (mapToPortID(dstDataType) == WROTO) )
                {   // in RWB case, only jpeg enable GPU proc.
                    dst.bIsRwbEn = MFALSE;
                }
                else
                {   // enable MDP proc.
...
mapToNodeDataType(PortID const portId)
{
    ...
    if( _portId == IMG2O )
        return isPreviewPath() ? PASS2_PRV_DST_2 : PASS2_CAP_DST_3;
    else
    if( _portId == VENC)
...
MBOOL
Pass2NodeImpl::
waitCurEnqueFrameDone()
{
    FUNC_START;
    MUINT32 curEnqFrameCnt = 0;
    {
        Mutex::Autolock lock(mRecordingLock);
        curEnqFrameCnt = muEnqFrameCnt;
    }
    //
    while(curEnqFrameCnt > muDeqFrameCnt)
    {
        Mutex::Autolock lock(mLock);
        MY_LOGD("wait lock enq %d > deq %d", curEnqFrameCnt, muDeqFrameCnt);
        mCondDeque.wait(mLock);
        MY_LOGD("wait done");
    }
    FUNC_END;
