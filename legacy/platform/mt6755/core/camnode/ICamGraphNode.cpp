...
handlePostBuffer(MUINT32 const data, MUINTPTR const buf, MUINT32 const ext) const
{
    ...
    IImageBuffer* pImgBuf = (IImageBuffer*)buf;
    snprintf( key, PROPERTY_KEY_MAX, "debug.camnode.scanline.%d", data);//see nodeDataTypes.h
    property_get( key, propertyValue, "0");
    if( atoi(propertyValue) == 1 && debugScanLine != NULL )
    {
        debugScanLine->drawScanLine(pImgBuf->getImgSize().w,
