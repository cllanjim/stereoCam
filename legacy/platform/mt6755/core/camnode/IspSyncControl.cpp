...
class IspSyncControlImp : public IspSyncControlHw
{
    ...
    public: //operations
        ...
        virtual MBOOL   getPass2Info(
                            ...
                            MUINT32&        outPrivateDataSize,
                            MCropRect&      outInCrop,
                            MCropRect&      outInCropWithoutZoom);
...
createInstance(MUINT32 const sensorIdx)
{
    ...
    pSyncCtrl->muUserCnt++;
    MY_LOGI("sensorIdx(%d), this(0x%x), userCnt(%d)", sensorIdx, pSyncCtrl, pSyncCtrl->muUserCnt );
...
destroyInstance()
{
    Mutex::Autolock _l(g_lock);
    muUserCnt--;
    MY_LOGI("this(0x%x), userCnt(%d)", this, muUserCnt);
    if( muUserCnt == 0 )
    {
...
IspSyncControlImp(MUINT32 const sensorIdx)
    ...
    , mSensorType(0)
{
    MY_LOGI("new instance(0x%x), sensorIdx(%d)", this, getSensorIdx());
    mlZoomRatio.clear();
...
calRrzoMaxZoomRatio()
{
    ...
    //
    EXIT:
    MY_LOGI("mRrzoMaxZoomRatio(%d)",mRrzoMaxZoomRatio);
    return MTRUE;
}
    ...
    MSize   prvSize)
{
    ...
    MY_LOGV("%d: F(0x%08X),prvSize(%dx%d),C(%d,%d,%dx%d),S(%dx%d)",
            ...
            size.w,
            size.h);
    MY_LOGE_IF(size.w == 0 || size.h == 0,"%d: F(0x%08X),prvSize(%dx%d),C(%d,%d,%dx%d),S(%dx%d)",
            getSensorIdx(),
            mRrzoFormat,
            prvSize.w,
            prvSize.h,
    ...
    MSize&          size)
{
    ...
    MY_LOGD("%d: F(0x%08X),pixelMode(%d,%d),fixed(%d),C(%d,%d,%dx%d),S(%dx%d)",
            ...
            size.w,
            size.h);
    MY_LOGE_IF(size.w == 0 || size.h == 0,"%d: F(0x%08X),pixelMode(%d,%d),fixed(%d),C(%d,%d,%dx%d),S(%dx%d)",
            getSensorIdx(),
            mRrzoFormat,
            pixelMode,
            mIspPixelMode,
    ...
    MSize&          outOutSize)
{
    ...
    if(inIsRrzo)
    {
        if( mbInitRrzOn )
        {
            outInCrop   = mInitRrzCrop;
            outOutSize  = mInitRrzSize;
        }
        else
        if(mHDRState == SENSOR_VHDR_MODE_IVHDR)
        {
    ...
    #endif
    //
    MY_LOGD("%d: TS(%d),# 0x%X,IsR(%d),ZR(%d,%d/%d),fixed(%d),C(%d,%d,%dx%d),S(%dx%d)",
            getSensorIdx(),
            mlPass1OutSize.size(),
            inMagicNum,
            inIsRrzo,
            ...
            outOutSize.h);
    //
    MY_LOGE_IF(outOutSize.w == 0 || outOutSize.h == 0,"%d: TS(%d),# 0x%X,IsR(%d),ZR(%d,%d/%d),fixed(%d),C(%d,%d,%dx%d),S(%dx%d)",
            getSensorIdx(),
            mlPass1OutSize.size(),
            inMagicNum,
            inIsRrzo,
    ...
    MCropRect&      outInCropWithoutZoom)
{
            ...
            for(iterOutSize = mlPass1OutSize.begin(); iterOutSize != mlPass1OutSize.end(); iterOutSize++)
            {
                if((*iterOutSize).magicNum == (*iterMetadata).magicNum)
                {
                    if((*iterMetadata).isRrzo)
                    {
                        ...
                        //
                        outInCropWithoutZoom.p_integral.x = (((*iterOutSize).outSize.w - CropRect.w)+1) / 2;
                        outInCropWithoutZoom.p_integral.y = (((*iterOutSize).outSize.h - CropRect.h)+1) / 2;
                        outInCropWithoutZoom.p_fractional.x = 0;
                        outInCropWithoutZoom.p_fractional.y = 0;
                        outInCropWithoutZoom.s.w = ALIGN_UP_SIZE(CropRect.w, 2);
                        outInCropWithoutZoom.s.h = ALIGN_UP_SIZE(CropRect.h, 2);
                    }
                    else
                    {
                        NSCamHW::Rect SrcRect(0, 0, mSensorSize.w, mSensorSize.h);
    ...
    if(eisEnable)
    {
        MY_LOGD("%d: ImgBuf(0x%X),# 0x%X,PD(0x%X/%d),ZR(%d,%d/%d),S(%dx%d),C(%d/%10d,%d/%10d,%dx%d),CWOZ(%d,%d,%dx%d),EIS:X(%d/%10d),Y(%d/%10d)",
                getSensorIdx(),
                (MUINTPTR)inpImgBuf,
                outMagicNum,
                ...
                outInCrop.s.w,
                outInCrop.s.h,
                outInCropWithoutZoom.p_integral.x,
                outInCropWithoutZoom.p_integral.y,
                outInCropWithoutZoom.s.w,
                outInCropWithoutZoom.s.h,
                (*iterMetadata).eisCrop.xInt,
                (*iterMetadata).eisCrop.xFlt,
                (*iterMetadata).eisCrop.yInt,
                (*iterMetadata).eisCrop.yFlt);
    ...
    {
        MY_LOGD("%d: ImgBuf(0x%X),# 0x%X,PD(0x%X/%d),ZR(%d,%d/%d),S(%dx%d),C(%d,%d,%dx%d),CWOZ(%d,%d,%dx%d)",
                getSensorIdx(),
                (MUINTPTR)inpImgBuf,
                outMagicNum,
                (MUINTPTR)outpPrivateData,
...
{
    MY_LOGI("set replace buf count %d, %d", fullcnt, resizedcnt);
    //
