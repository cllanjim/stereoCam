...
class DualITNodeImpl : public DualImgTransformNode
{
    ...
    public: // ctor & dtor
        DualITNodeImpl(MUINT32 const pos, MUINT32 const transfromFlag);
        ~DualITNodeImpl();
    ...
    private:
        ...
        sp<IImageBuffer>        mspDstBuf_L;
        sp<IImageBuffer>        mspDstBuf_R;
        ...
        // JPS transform flag: indicate image needs to rotate or flip.
        MUINT32                 muTransformFlag;
};
...
DualImgTransformNode::
createInstance(MUINT32 const pos, MUINT32 const transformFlag)
{
    return new DualITNodeImpl(pos, transformFlag);
}
 ...
 ********************************************************************************/
DualITNodeImpl::
    DualITNodeImpl(MUINT32 const pos, MUINT32 const transformFlag)
    : DualImgTransformNode()
...
threadLoopUpdate()
{
    ...
    //    0 is main-main2 (main in L)
    //    1 is main2-main (main in R)
    MY_LOGD_IF(StereoSettingProvider::bEnableLog, "Stereo_Profile: doImageTransform Buf1: 0x%08X buf2: 0x%08X +", postBuf_0.buf, postBuf_1.buf);
    ...
    MY_LOGD_IF(StereoSettingProvider::bEnableLog, "Stereo_Profile: doImageTransform -");
    //
    if ( pDstBuf_SBS != NULL )
    {
        pDstBuf_SBS->lockBuf( LOG_TAG, eBUFFER_USAGE_SW_MASK | eBUFFER_USAGE_HW_MASK );
        // uncomment if you want to dump images
        // char szFileName[512];
        // sprintf(szFileName, "/sdcard/mtklog/DUALIT_%dx%d_dstdata_%d.yuv",pDstBuf_SBS->getImgSize().w, pDstBuf_SBS->getImgSize().h,DUALIT_DST_2);
        // pDstBuf_SBS->saveToFile(szFileName);
...
)
{
    ...
    sp<IImageBuffer> pDstBuf_SBS = (isRightBuf) ? mspDstBuf_R : mspDstBuf_L;
    ...
    // Swap crop size since we do clockwise rotation here
    MRect crop_original = crop;
    //crop.s.w = crop_original.s.h;
    //crop.s.h = crop_original.s.w;
    ...
    MY_LOGD_IF( pDstBuf_SBS != 0, "SBS:(%p),isR(%d),S(%dx%d),stride(%d),F(%d),TS(%lld),crop_original(%d,%d,%d,%d)", pDstBuf_SBS.get(), isRightBuf,
            ...
            crop_original.p.x, crop_original.p.y, crop_original.s.w, crop_original.s.h);
    //
    CAM_TRACE_FMT_BEGIN("execP2:%d", muDeqFrameCnt);

    // Originally we dont rotate image here
    MY_LOGD_IF(StereoSettingProvider::bEnableLog, "Stereo_Profile: mpImgTransform->execute +");
     ret = ret && mpImgTransform->execute(buf, pDstBuf, pDstBuf_SBS.get(), crop, trans, 0xFFFFFFFF);
    MY_LOGD_IF(StereoSettingProvider::bEnableLog, "Stereo_Profile: mpImgTransform->execute -");
    // Rotate image because of the sensor layout
    /*if( data == DUALIT_SRC_0 ){
        // main image rotation fix at 270 degree
        ret = ret && mpImgTransform->execute(buf, pDstBuf_SBS.get(), NULL,      crop_original, muTransformFlag, 0xFFFFFFFF);
        // thumnail scale down and rotation(scaling is done automatically by the size of pDstBuf)
        ret = ret && mpImgTransform->execute(buf, pDstBuf,           NULL,      crop_original, trans,              0xFFFFFFFF);
    }else{
        // pDstBuf_SBS rotate
        // pDstBuf scale down
        ret = ret && mpImgTransform->execute(buf, pDstBuf_SBS.get(), pDstBuf, crop_original, muTransformFlag, 0xFFFFFFFF);
    }*/
    ...
    if ( pDstBuf != NULL )
    {
        // uncomment if you want to dump images
        // char szFileName[512];
        // sprintf(szFileName, "/sdcard/mtklog/DUALIT_%dx%d_dstdata_%d.yuv",pDstBuf->getImgSize().w, pDstBuf->getImgSize().h,dstDataType);
        // pDstBuf->saveToFile(szFileName);
...
pushBuf(MUINT32 const data, IImageBuffer* const buf, MUINT32 const ext)
{
    ...
    MBOOL ret = MTRUE;
    #if 1 //AARON FIXME
    MRect crop = ( ext != 0 ) ? *reinterpret_cast<MRect*>(ext) : MRect(MPoint(), buf->getImgSize());
    #else
