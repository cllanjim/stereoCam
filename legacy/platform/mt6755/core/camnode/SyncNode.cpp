...
#define SYNC_THRESHOLD_10 33
#define SYNC_THRESHOLD_33 66
    ...
    , mbEnable_CapBuf(MFALSE)
{
    ...
    property_get( "debug.stereocam.slowfps", value, "-1");
    ...
    property_get( "debug.stereocam.syncdump", value, "0");
...
pushBuf(MUINT32 const data, IImageBuffer* const buf, MUINT32 const ext)
{
    ...
    if ( !mbEnable )
    {
        MY_LOGD("syncNode disabled, return buffer immediately");
    ...
    if(syncResult == SYNC_PASS){
        ...
        if(isCapturePath){
            ...
            if(miSyncDump){
                ...
                IImageBuffer* buffer = postBuf_0.buf;
                char filename[256];
                sprintf(filename, "/sdcard/mtklog/syncdump/main1_rrzo_%dx%d_%d.raw", buffer->getImgSize().w,buffer->getImgSize().h,buffer->getBufStridesInBytes(0));
                buffer->saveToFile(filename);

                buffer = postBuf_1.buf;
                sprintf(filename, "/sdcard/mtklog/syncdump/main2_rrzo_%dx%d_%d.raw", buffer->getImgSize().w,buffer->getImgSize().h,buffer->getBufStridesInBytes(0));
                buffer->saveToFile(filename);
