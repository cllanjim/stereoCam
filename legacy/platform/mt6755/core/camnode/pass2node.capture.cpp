    ...
    , muDumpFlag(0)
{
    ...
    muDequeOrder[3] = PASS2_CAP_DST_3;
...
//should remove after sync with RWB owner
#if 1
...
threadLoopUpdate()
{
    MBOOL ret = MFALSE;
    //
    //RWB sensor dont go this way
    //(for normal without pureRaw & NR,it will direct callback to jpegnode)
    if(mbIsGPUProcType == 0)
    {
    ...
    MY_LOGD("muNRType = %d, mbIsGPUProcEn = %d",muNRType, mbIsGPUProcType);
    //
...
//should remove after sync with RWB owner
            ...
            if( mpInternalNR2Buf.get() == NULL &&
                    !allocMemory(
                        ...
                        ( (mbIsGPUProcType == 2) ? eImgFmt_YV12 : eImgFmt_YUY2 ), //RWB GPU proc: YV12
                        mpInternalNR2Buf,
                        ( (mbIsGPUProcType == 2) ? MTRUE : MFALSE ) ) )
            {
...
//should remove after sync with RWB owner
    ...
    if( muNRType != 0 )
    {
        ...
        for( uint32_t i = 0 ; i < mvOutRequest.size() ; i++ )
        {
        ...
        if( muNRType == 1 ) //HW NR
        {
            ...
            if(mbIsGPUProcType == 2)
            {
                ...
                if( muDumpFlag & DUMP_HWNR )
                {
                    dumpBuffer(mpInternalNR1Buf.get(), "nr_original", "raw");
                    dumpBuffer(mpInternalNR2Buf.get(), "hwnr_nr1", "yuv");
                }
                if( ! doRwbGpu( mpInternalNR2Buf.get(), out[0], out[1], trans) )
                {
                    MY_LOGE("doRwbGpu failed");
                    goto lbExit;
                }
            }
            //
        ...
        else if ( muNRType == 2 ) //SW NR
        {
            ...
            if(mbIsGPUProcType == 2)
            {
                if( ! doRwbGpu( mpInternalNR1Buf.get(), out[0], out[1], trans ) )
                {
                    MY_LOGE("doRwbGpu failed");
                    goto lbExit;
                }
...
getPass2Buffer(vector<p2data>& vP2data)
{
    ...
    if( muNRType == 0 )
    {
        ...
        if(mbIsGPUProcType == 2)
        {
            //GPU process - allocate internal buffers
    ...
    else
    {
        ...
        else if ( muNRType == 2 )
        {
            if( mpInternalNR1Buf.get() == NULL &&
                    !allocMemory(
                        pSrcBuf,
                        ( (mbIsGPUProcType == 2) ? eImgFmt_YV12 : eImgFmt_I420 ),
                        mpInternalNR1Buf,
                        ( (mbIsGPUProcType == 2) ? MTRUE : MFALSE) ) )
            {
                MY_LOGE("failed to allocate memory, nr1");
...
handleP2Done(QParams& rParams)
{
    ...
    if( muNRType == 0 )
    {
        if(mbIsGPUProcType != 2)//pure/process raw postbuffer to next node
        {
        ...
        {
            MY_LOGD("pure/process raw without NR, but GPU porcess");
        }
    }
    //
    {
...
MBOOL
CapPass2::
allocMemory(IImageBuffer* srcbuf, MINT32 const fmt, sp<IImageBuffer>& targetBuf, MBOOL isGralloc)
{
    MBOOL ret = MTRUE;
    ...
    MY_LOGD("alloc %d x %d, stride %d, fmt 0x%x", size.w, size.h, bufStridesInBytes, fmt);
    ...
    if(isGralloc)
    {
        IImageBufferAllocator::ExtraParam extParam(eBUFFER_USAGE_HW_TEXTURE|eBUFFER_USAGE_SW_READ_OFTEN|eBUFFER_USAGE_SW_WRITE_OFTEN);
        targetBuf = allocator->alloc_gb(LOG_TAG, imgParam, extParam);
    }
    else
    {
...
MBOOL
CapPass2::
doRwbGpu(IImageBuffer* pBuf, IImageBuffer* dst0, IImageBuffer* dst1, MINT32 trans)
{
    //
    MBOOL ret = MFALSE;
    //
    mspRwbGpuProc = RwbGpuProc::createInstance(getSensorIdx());
    //
    if(mspRwbGpuProc != NULL)
    {
        MY_LOGD("muNRType,fmt=%d, %d", muNRType, dst0->getImgFormat());
        if((muNRType==0)
            &&(dst0->getImgFormat() == eImgFmt_YV12)
            &&(dst0->getImageBufferHeap()->getGraphicBuffer() != NULL ) // gralloc buffer
        )
        {
            mspRwbGpuProc->setSrcDstBuf(pBuf, dst0);
            mspRwbGpuProc->doGPUProc();
        }
        else
        {
            //alloc graphic buffer for gpuPoc output
            {
                if(!allocMemory(pBuf, eImgFmt_YV12, mspGPUDstBuf, MTRUE))
                {
                    MY_LOGE("failed to allocate memory, gpuDstBuf");
                    goto lbExit;
                }
            }
            //
            mspRwbGpuProc->setSrcDstBuf(pBuf, mspGPUDstBuf.get());
            mspRwbGpuProc->doGPUProc();
            //
            // format transform
            if( !transform(mspGPUDstBuf.get(), dst0, dst1, mCapCrop, trans) )
            {
                MY_LOGE("format transform failed");
                goto lbExit;
            }
        ...
        //
        if( muDumpFlag & DUMP_GPU )
        {
            dumpBuffer(pBuf,"doRwbGpu_in", "yuv");
            dumpBuffer(mspGPUDstBuf.get(), "doRwbGpu_out", "yuv");
            dumpBuffer(dst0, "gpu_main", "yuv");
            dumpBuffer(dst1, "gpu_post", "yuv");
         }
         //
         mspRwbGpuProc->destroyInstance();
         mspRwbGpuProc = NULL;
    }
    else
    {
        MY_LOGE("mspRwbGpuProc is NULL");
        goto lbExit;
    }
    //
    ret = MTRUE;
