...
threadLoopUpdate()
{
	...
	MUINT32 i4MagicNum = postBufData.ext;
    // currently force main sensor (1)
    pIAfMgr->doPDBuffer(1, (MVOID *)pImgBuf->getBufVA(0), pImgBuf->getImgSize().w, pImgBuf->getImgSize().h, pImgBuf->getBufStridesInBytes(0), i4MagicNum);
    pIAfMgr->doPDTask(1);
...
pushBuf(MUINT32 const data, IImageBuffer* const buf, MUINT32 const ext)
{
    ...
    MY_LOGD("size(%d), data(%d), buf(0x%x), frmNum(%d)",
            mlPostBufData.size(),
            postBufData.data,
            postBufData.buf,
            ext);
