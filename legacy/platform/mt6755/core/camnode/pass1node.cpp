...
class Pass1NodeImpl : public Pass1Node
{
    ...
    private:
        ...
        volatile MINT32             mRepBufAllocCnt;
        //     infos
        ...
        list<HwPortConfig_t>        mlHwPortCfg;
        threadData                  mThData;
};
...
onInit()
{
    ...
    //
    if( sem_init(&mSemBufAlloc, 0, 0) != 0 )
    {
        MY_LOGE("sem init fail");
        goto lbExit;
    }
...
onStart()
{
    ...
    if( !getHwPortConfig(&mlHwPortCfg) )
    {
    ...
    {
        pResMgrDrv->setItem(
                        ResMgrDrv::ITEM_SENSOR_FPS,
                        mInitCfg.muFrameRate);
        pResMgrDrv->triggerCtrl();
        pResMgrDrv->destroyInstance();
        pResMgrDrv = NULL;
    }
    else
...
allocBuffers(list<HwPortConfig_t> & lPortCfg)
{
    ...
    MBOOL ret = MFALSE;
    MUINT32 maxReplaceBufCnt =  mpIspSyncCtrlHw->getPass1ReplaceCnt(MTRUE) > mpIspSyncCtrlHw->getPass1ReplaceCnt(MFALSE) ?
                                mpIspSyncCtrlHw->getPass1ReplaceCnt(MTRUE) : mpIspSyncCtrlHw->getPass1ReplaceCnt(MFALSE);

    list< HwPortConfig_t >::const_iterator pConfig = lPortCfg.begin();
    while( pConfig != lPortCfg.end() )
    {
        MY_LOGD("ring PortID(%x)", pConfig->mPortID);
    ...
    ::android_atomic_write(0, &mRepBufAllocCnt);
    sem_post(&mSemBufAlloc);

    MY_LOGD("maxReplaceBufCnt(%d)",maxReplaceBufCnt);
    for(MUINT32 cnt = 0; cnt < maxReplaceBufCnt ; cnt++ )
    {
        MY_LOGD("cnt(%d)",cnt);
        pConfig = lPortCfg.begin();
        while( pConfig != lPortCfg.end() )
        {
            MY_LOGD("replace PortID(%x)", pConfig->mPortID);
            MUINT32 nodedatatype = mapToNodeDataType(pConfig->mPortID);
            ICamBufHandler* pBufHdl = getBufferHandler(nodedatatype);
            if( !pBufHdl )
            {
            ...
            if(cnt < rep_count)
            {
                MY_LOGD("cnt(%d/%d),nodedatatype(%d)",
                        cnt,
                        rep_count,
                        nodedatatype);
                if( !pBufHdl->requestBuffer(nodedatatype, allocinfo) )
        ...
        sem_post(&mSemBufAlloc);
        ::android_atomic_write(cnt, &mRepBufAllocCnt);
    }
    ret = MTRUE;
lbExit:
    if( !ret ) {
...
startHw(list<HwPortConfig_t> & plPortCfg)
{
    ...
    mThData.pSelf = this;
    mThData.plPortCfg = &plPortCfg;
    if( pthread_create(&mThreadHandle, NULL, doThreadAllocBuf, &mThData) != 0 )
    {
    ...
    //
    MY_LOGD("wait ring alloc done +");
    sem_wait(&mSemBufAlloc);
    MY_LOGD("wait ring alloc done -");
    //
    {
        //enque
...
dequeLoop()
{
    ...
    //
    if(muFrameCnt < maxReplaceBufCnt)
    {
        MY_LOGD("wait replace alloc done +");
        sem_wait(&mSemBufAlloc);
        MY_LOGD("wait replace alloc done -");
