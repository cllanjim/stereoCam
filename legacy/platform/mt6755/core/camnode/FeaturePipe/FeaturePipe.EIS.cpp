...
MBOOL FeaturePipeEisNode::onInit()
{
    ...
    if (mRecordingHint && (mFeatureOnMask&FEATURE_ON_MASK_BIT_EIS) == FEATURE_ON_MASK_BIT_EIS )
    {
        ...
        //
        cpuRangeInfo.bigFreqMin = 286000;
        cpuRangeInfo.littleFreqMin = 1144000;
        //
        cpuRangeInfo.bigFreqMax = 286000;
        cpuRangeInfo.littleFreqMax = 1144000;
        //
...
MBOOL FeaturePipeEisNode::onPostBuffer(MUINT32 const data, MUINTPTR const buf, MUINT32 const ext)
{
    ...
    mqJobInput.push(job);
#endif
    MUINT64 ts = reinterpret_cast<IImageBuffer*>(buf)->getTimestamp();
...
MBOOL FeaturePipeEisNode::threadLoopUpdate()
{
    ...
    if (mRecordingHint && (mFeatureOnMask&FEATURE_ON_MASK_BIT_EIS) == FEATURE_ON_MASK_BIT_EIS )
    {
        ...
        mpEisHal->DoEis(EIS_PASS_2, &eisHalCfgData, ts);
        mFrameCount++;
    }else
    {
