...
#endif
#if 1
    { ResMgrDrv::SCEN_SW_CAM_PRV, 4, 1950000, 4, 1950000, 1 },
    { ResMgrDrv::SCEN_SW_CAM_CAP, 4, 1950000, 4, 1950000, 1 },
#endif
#else
...
FeaturePipe::initFeatureGraph()
{
    ...
    if (FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask) ||FEATURE_MASK_IS_EIS_CALIBRATION_ENABLED(mFeatureOnMask))
    {
    ...
    }else if (FEATURE_MASK_IS_EIS_CALIBRATION_ENABLED(mFeatureOnMask))
    {
        mpCamGraph->setBufferHandler(P2A_TO_EISSW,  mpAllocBufHdl);
        mpCamGraph->connectData(P2A_TO_EISSW, P2A_TO_EISSW, mpP2aNode, mpEisSwNode);
    }
    ...
    if (FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask) || FEATURE_MASK_IS_EIS_CALIBRATION_ENABLED(mFeatureOnMask))
    {
        reinterpret_cast<FeaturePipeEisNode*>(mpEisSwNode)->set(RECORDING_HINT, (MINTPTR) mRecordingHint);
    }
...
FeaturePipe::uninitFeatureGraph()
{
    ...
    if (FEATURE_MASK_IS_VFB_ENABLED(mFeatureOnMask) && (misPerfService == MTRUE))
    {
        ...
        LOG_DBG("uninitFeatureGraph: Uninit PerfService");
    }
#endif
...
#ifdef USING_MTK_PERFSERVICE
    if (FEATURE_MASK_IS_VFB_ENABLED(mFeatureOnMask) && (misPerfService == MTRUE) && misPerfService_count == 3)
    {
        disablePerfService(gScenTable[0].scen);
        uninitPerf();
        misPerfService = MFALSE;
        LOG_DBG("performCallback: Uninit PerfService");
    } else {
        if(misPerfService == MTRUE)
            misPerfService_count++;
    }
#endif
...
FeaturePipe::setFeature(MUINT32 featureMask)
{
    ...
    if (featureMask == 1)
    {
        ...
        misPerfService_count = 0;
        LOG_DBG("setFeature: enable PerfService");
    }
#endif
    ...
    if (FEATURE_MASK_IS_3DNR_ENABLED(featureDiff) ||
        ...
        FEATURE_MASK_IS_EIS_CALIBRATION_ENABLED(featureDiff) ||
...
FeaturePipeP2aNode::onInit()
{
    ...
    // CKH, stereo does not support EIS
    if (!FEATURE_MASK_IS_STEREO_CAMERA_ENABLED(mFeatureOnMask))
    {
        if (NSCam::Utils::CamManager::getInstance()->getEISScenarioType() !=
            NSCam::Utils::CamManager::EIS_SCE_TYPE_NULL)
        {
            mpEisHalFor3DNR = EisHal::CreateInstance("FeaturePipe_3DNR", mOpenedSensorIndex);
            if (UNLIKELY(mpEisHalFor3DNR == NULL))
            {
    ...
    if (!FEATURE_MASK_IS_STEREO_CAMERA_ENABLED(mFeatureOnMask)) // CKH, stereo does not support EIS
    {
...
onPostBuffer(MUINT32 const data, MUINT32 const buf, MUINT32 const ext)
{
    ...
    MBOOL ret = MTRUE;
		...
		sgmkDebugFlag[0] = '1';
#endif
	if (sgmkDebugFlag[0] != '0') { LOG_DBG("mkdbg_fpp2a-onpb_001: frm: %d", ext); }

    if(!FEATURE_MASK_IS_VFB_ENABLED(mFeatureOnMask) &&
       !FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask) &&
       !FEATURE_MASK_IS_STEREO_CAMERA_ENABLED(mFeatureOnMask) &&
       !FEATURE_MASK_IS_EIS_CALIBRATION_ENABLED(mFeatureOnMask) &&
	...
	if (sgmkDebugFlag[0] != '0') { LOG_DBG("mkdbg_fpp2a-onpb_002: frm: %d", ext); }
...
p2aCbFunc(QParams& rParams)
{
    ...
    if(!FEATURE_MASK_IS_VFB_ENABLED(pP2ANodeObj->mFeatureOnMask) &&
       ...
       !FEATURE_MASK_IS_EIS_CALIBRATION_ENABLED(pP2ANodeObj->mFeatureOnMask) &&
    ...
    )
    {
        ...
        pP2ANodeObj->mpFeaturePipeObj->performCallback(rParams);
    }else if (FEATURE_MASK_IS_EIS_CALIBRATION_ENABLED(pP2ANodeObj->mFeatureOnMask))
    {
        Mutex::Autolock lock(pP2ANodeObj->mLock);
...
handleP2Done(QParams& rParams)
{
    ...
    if (FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask) ||FEATURE_MASK_IS_EIS_CALIBRATION_ENABLED(mFeatureOnMask))
    {
        ...
        handlePostBuffer(P2A_TO_EISSW, (MUINTPTR) (rParams.mvIn[0].mBuffer), getMagicNo(rParams));
...
executeP2AByPassMode()
{
    ...
    CAM_TRACE_FMT_BEGIN("P2ANStrm ByPassMode: %d", getMagicNo((InQParams)));
    ...
    if (InQParams.mpfnCallback != NULL)
    {
        ...
        /* Change to P2A callback func */
        mqCookie.push(InQParams.mpCookie);
        mqfnCallback.push(InQParams.mpfnCallback);
        InQParams.mpCookie = (void*) this;
        InQParams.mpfnCallback = p2aCbFunc;
    }
    ...
    for (int i=0; i < InQParams.mvOut.size(); i++)
    {
        ...
        u4Transform =  InQParams.mvOut[i].mTransform;
...
#define TransCase( trans, rot, flip ) \
        ...
        default:
            LOG_ERR("not supported transform(0x%x)", u4Transform);
            u4Rotation = 0;
            u4Flip = 0;
            return MFALSE;
            break;
#undef TransCase
        ...
        if (u4Rotation == 0 || u4Rotation == 180)
        {
        ...
        {
            // Use largest size of pass 2 output.
            if ( (PostProcOutWidth < InQParams.mvOut[i].mBuffer->getImgSize().h) || (PostProcOutHeight < InQParams.mvOut[i].mBuffer->getImgSize().w))
            {
                PostProcOutWidth = InQParams.mvOut[i].mBuffer->getImgSize().h;
                PostProcOutHeight = InQParams.mvOut[i].mBuffer->getImgSize().w;
            }
        }
    }
    ...
    {
        memset(&eisConfigData,0,sizeof(eisConfigData));
        ...
        //FOR GIS calibration
        eisConfigData.crzOutW = PostProcOutWidth;
        eisConfigData.crzOutH = PostProcOutHeight;
        eisConfigData.srzOutW = PostProcOutWidth;
        eisConfigData.srzOutH = PostProcOutHeight;
    ...
    ret = mpINormalStream->enque(InQParams);
    if (ret == MFALSE)
    {
        LOG_DBG("NormalStream enqueue failure of frame %d", getMagicNo(InQParams));
    }
    CAM_TRACE_FMT_END();
...
#undef TransCase
            ...
            if (u4Rotation == 0 || u4Rotation == 180)
            {
                ...
                if ( (PostProcOutWidth < InQParams.mvOut[i].mBuffer->getImgSize().w) || (PostProcOutHeight < InQParams.mvOut[i].mBuffer->getImgSize().h))
                {
                    if(InQParams.mvOut[i].mBuffer->getImgSize().w == 176 || InQParams.mvOut[i].mBuffer->getImgSize().h == 144)
                    {
                        PostProcOutWidth = InQParams.mvOut[i].mBuffer->getImgSize().w*2;
                        PostProcOutHeight = InQParams.mvOut[i].mBuffer->getImgSize().h*2;
                        LOG_DBG("[p2a] PostProcOutWidth =%d PostProcOutHeight =%d", PostProcOutWidth, PostProcOutHeight);
                    }else{
                        PostProcOutWidth = InQParams.mvOut[i].mBuffer->getImgSize().w;
                        PostProcOutHeight = InQParams.mvOut[i].mBuffer->getImgSize().h;
                    }
                ...
                if ( (InQParams.mvOut[i].mPortID.capbility == NSCam::NSIoPipe::EPortCapbility_Disp) && FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
                {
                    mEISDispW = InQParams.mvOut[i].mBuffer->getImgSize().w;
                    mEISDispH = InQParams.mvOut[i].mBuffer->getImgSize().h;
                }
            ...
            }else
            {
                ...
                if ( (PostProcOutWidth < InQParams.mvOut[i].mBuffer->getImgSize().h) || (PostProcOutHeight < InQParams.mvOut[i].mBuffer->getImgSize().w))
                {
                    if(InQParams.mvOut[i].mBuffer->getImgSize().w == 176 || InQParams.mvOut[i].mBuffer->getImgSize().h == 144)
                    {
                        PostProcOutWidth = InQParams.mvOut[i].mBuffer->getImgSize().h*2;
                        PostProcOutHeight = InQParams.mvOut[i].mBuffer->getImgSize().w*2;
                        LOG_DBG("[p2a] PostProcOutWidth =%d PostProcOutHeight =%d", PostProcOutWidth, PostProcOutHeight);
                    }else{
                        PostProcOutWidth = InQParams.mvOut[i].mBuffer->getImgSize().h;
                        PostProcOutHeight = InQParams.mvOut[i].mBuffer->getImgSize().w;
                ...
                if ( (InQParams.mvOut[i].mPortID.capbility == NSCam::NSIoPipe::EPortCapbility_Disp) && FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
                {
                    mEISDispW = InQParams.mvOut[i].mBuffer->getImgSize().h;
                    mEISDispH = InQParams.mvOut[i].mBuffer->getImgSize().w;
                }
            }
        }
        ...
        if(FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
        {
            ...
            fullWidth = InQParams.mvIn[0].mBuffer->getImgSize().w;
            fullHeight = InQParams.mvIn[0].mBuffer->getImgSize().h;
            LOG_DBG("[EIS] Full W/H(%u,%u)",fullWidth,fullHeight);
...
#if 1
            //NSCamHW::Rect srcRect(InQParams.mvCropRsInfo[eisCrop2Idx].mCropRectExceptDZ.p_integral.x,
            //                                  InQParams.mvCropRsInfo[eisCrop2Idx].mCropRectExceptDZ.p_integral.y,
            //                                  fullWidth, fullHeight);
            NSCamHW::Rect srcRect(0, 0, fullWidth, fullHeight);
            ...
            u4Transform =  InQParams.mvOut[0].mTransform;
...
#undef TransCase
            ...
            if (u4Rotation == 0 || u4Rotation == 180)
            {
                if (!mEISDispW)
                {
                    LOG_DBG("[EIS] no disp width before and new disp width is %d\n", InQParams.mvOut[0].mBuffer->getImgSize().w);
                    mEISDispW = InQParams.mvOut[0].mBuffer->getImgSize().w;
                }

                if (!mEISDispH)
                {
                    LOG_DBG("[EIS] no disp height before and new disp height is %d\n", InQParams.mvOut[0].mBuffer->getImgSize().h);
                    mEISDispH = InQParams.mvOut[0].mBuffer->getImgSize().h;
                }
            }else
            {
                if (!mEISDispW)
                {
                    LOG_DBG("[EIS] no disp width before and new disp width is %d\n", InQParams.mvOut[0].mBuffer->getImgSize().h);
                    mEISDispW = InQParams.mvOut[0].mBuffer->getImgSize().h;
                }

                if (!mEISDispH)
                {
                    LOG_DBG("[EIS] no disp height before and new disp height is %d\n", InQParams.mvOut[0].mBuffer->getImgSize().w);
                    mEISDispH = InQParams.mvOut[0].mBuffer->getImgSize().w;
                }
            }

            NSCamHW::Rect dstRect(0, 0,mEISDispW,mEISDispH);  // only use display ratio

            //eisCropRect = android::MtkCamUtils::calCrop(srcRect,dstRect,100);
            //eisCropRect.x += srcRect.x;
            //eisCropRect.y += srcRect.y;
            eisCropRect =  NSCamHW::Rect( InQParams.mvCropRsInfo[eisCrop2Idx].mCropRectExceptDZ.p_integral.x,
                                                        InQParams.mvCropRsInfo[eisCrop2Idx].mCropRectExceptDZ.p_integral.y,
                                                        InQParams.mvCropRsInfo[eisCrop2Idx].mCropRectExceptDZ.s.w,
                                                        InQParams.mvCropRsInfo[eisCrop2Idx].mCropRectExceptDZ.s.h);

            LOG_DBG("[EIS]Idx(%u),ROI(%u,%u,%u,%u),ROI_noDZ(%u,%u,%u,%u),Dst(0,0,%u,%u),Crop(%u,%u,%u,%u)",eisCrop2Idx,
                                                                                     InQParams.mvCropRsInfo[eisCrop2Idx].mCropRect.p_integral.x,
                                                                                     InQParams.mvCropRsInfo[eisCrop2Idx].mCropRect.p_integral.y,
                                                                                     InQParams.mvCropRsInfo[eisCrop2Idx].mCropRect.s.w,
                                                                                     InQParams.mvCropRsInfo[eisCrop2Idx].mCropRect.s.h,
                                                                                     InQParams.mvCropRsInfo[eisCrop2Idx].mCropRectExceptDZ.p_integral.x,
                                                                                     InQParams.mvCropRsInfo[eisCrop2Idx].mCropRectExceptDZ.p_integral.y,
                                                                                     InQParams.mvCropRsInfo[eisCrop2Idx].mCropRectExceptDZ.s.w,
                                                                                     InQParams.mvCropRsInfo[eisCrop2Idx].mCropRectExceptDZ.s.h,
                                                                                     mEISDispW,
...
static const NSCam::NSIoPipe::PortID WDMAO( NSIoPipe::EPortType_Memory, NSImageio::NSIspio::EPortIndex_WDMAO, 1);
        ...
        MSize inputImgSize = InQParams.mvIn[0].mBuffer->getImgSize();
        MSize dsImgSize;
#ifdef MTK_CAM_VIDEO_FACEBEAUTY_SUPPORT
        reinterpret_cast<FeaturePipeVfbNode*>(mpFeaturePipeObj->mpVfbSwNode)->getDSImgSize(inputImgSize, dsImgSize);
#endif
...
#if 0//TBD Holmes discard this because FE is not exist
...
setVfbExtreme(MBOOL mode)
{
    ...
    mVfbExtreme = false;
    LOG_DBG("vFB doesn't support Extreme mode(%d)",mVfbExtreme);
}
MBOOL
FeaturePipeSwitcherNode::
onPostBuffer(MUINT32 const data, MUINTPTR const buf, MUINT32 const ext)
 ...
 *
 ******************************************************************************/
#define GPU_YV12_SUPPORTED (1) //TBD query GPU driver for supported format
#undef LOG_VRB
#undef LOG_DBG
#undef LOG_DBG
#undef LOG_WRN
...
{
    return new FeaturePipeGpuNode("FeaturePipeGpuNode", SingleTrigger, SCHED_POLICY, (SCHED_PRIORITY-4), featureOnMask, pFeaturePipeObj);
}
MVOID
...
threadInit()
{
...

    {
        ...
        {
            AllocInfo allocinfo( FULL_GPU_MAX_OUTPUT_WIDTH,
                                 FULL_GPU_MAX_OUTPUT_HEIGHT,
#if GPU_YV12_SUPPORTED
                                 eImgFmt_YV12,
#else
                                 eImgFmt_RGBA8888,
#endif
...
#if GPU_YV12_SUPPORTED
            mGfxRgbaBuffer[i] = new GraphicBuffer(FULL_GPU_MAX_OUTPUT_WIDTH, FULL_GPU_MAX_OUTPUT_HEIGHT, HAL_PIXEL_FORMAT_YV12, GraphicBuffer::USAGE_HW_RENDER | GraphicBuffer::USAGE_SW_READ_OFTEN | GraphicBuffer::USAGE_SW_WRITE_OFTEN);
#else                                            ,
            mGfxRgbaBuffer[i] = new GraphicBuffer(FULL_GPU_MAX_OUTPUT_WIDTH, FULL_GPU_MAX_OUTPUT_HEIGHT, PIXEL_FORMAT_RGBA_8888, GraphicBuffer::USAGE_HW_RENDER | GraphicBuffer::USAGE_SW_READ_OFTEN | GraphicBuffer::USAGE_SW_WRITE_OFTEN);
#endif
    ...
    gpu_tuning.GLESVersion = 3; //Holmes 2 ==> 3
    gpu_tuning.Demo = 0;
...
#endif //_WORKAROUND_GPU_FLASH
#if GPU_YV12_SUPPORTED
    warp_info.OutImgFmt = WARP_IMAGE_YV12;
#else
...
{
    return new FeaturePipeMdpNode("FeaturePipeMdpNode", SingleTrigger, SCHED_POLICY, (SCHED_PRIORITY-4), featureOnMask, pFeaturePipeObj);
}
MVOID
...
threadLoopUpdate()
{
    ...
    else if(FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
    {
        ...
        {
            if (OutQParams.mvCropRsInfo[i].mGroupID == 2)
            {
                eisCrop2Idx = i;
            }
        }
        ...
        MCropRect mdpCrop;
        MCropRect imgiCrop = OutQParams.mvCropRsInfo[eisCrop2Idx].mCropRect;
        MCropRect tmpCrop = OutQParams.mvCropRsInfo[eisCrop2Idx].mCropRectExceptDZ;


        MUINT32 fullWidth,fullHeight,eisFeBlock,PostProcOutWidth,PostProcOutHeight,gpuTargetW,gpuTargetH;
        fullWidth = OutQParams.mvIn[0].mBuffer->getImgSize().w;
        fullHeight = OutQParams.mvIn[0].mBuffer->getImgSize().h;
        //LOG_DBG("MDPNode IMGI W/H(%u,%u)",fullWidth,fullHeight);
        ...
        PostProcOutWidth = ALIGN_SIZE(imgiCrop.s.w, eisFeBlock);
        PostProcOutHeight = ALIGN_SIZE(imgiCrop.s.h, eisFeBlock);
        //LOG_DBG("MDPNode PostProcOut W/H(%u,%u)",PostProcOutWidth,PostProcOutHeight);

        gpuTargetW=PostProcOutWidth/(reinterpret_cast<FeaturePipeEisNode*>(mpFeaturePipeObj->mpEisSwNode)->GetEisPlusCropRatio()/100.0);
        gpuTargetH=PostProcOutHeight/(reinterpret_cast<FeaturePipeEisNode*>(mpFeaturePipeObj->mpEisSwNode)->GetEisPlusCropRatio()/100.0);

        gpuTargetW = ROUND_TO_2X(gpuTargetW);
        gpuTargetH = ROUND_TO_2X(gpuTargetH);
        //LOG_DBG("MDPNode gpuTarget W/H(%u,%u)", gpuTargetW, gpuTargetH);

        mdpCrop.p_integral.x = ( (imgiCrop.p_integral.x-tmpCrop.p_integral.x) * gpuTargetW) / PostProcOutWidth;
        mdpCrop.p_integral.y = ( (imgiCrop.p_integral.y-tmpCrop.p_integral.y) * gpuTargetH) / PostProcOutHeight;

        mdpCrop.p_fractional.x = imgiCrop.p_fractional.x >> 11;
        mdpCrop.p_fractional.y = imgiCrop.p_fractional.y >> 11;

        mdpCrop.s.w = gpuTargetW;
        mdpCrop.s.h = gpuTargetH;

        LOG_DBG("MDPNode mdpCrop W/H(%u,%u)",mdpCrop.s.w,mdpCrop.s.h);
        ...
        if(EnableOption[0] == '1' || EnableOption[0] == '3')
        {
            ...
            pImageBuffer->syncCache(eCACHECTRL_INVALID);
            pImageBuffer->saveToFile(szFileName);
            //pBufHandler->enqueBuffer(8001,fullImgReq.mBuffer);
        }

        DpColorFormat MDPInputFormat;

#if GPU_YV12_SUPPORTED
        MDPInputFormat = DP_COLOR_YV12;
#else
        MDPInputFormat = DP_COLOR_RGBA8888;
#endif


#if GPU_YV12_SUPPORTED
        /* Copy result to PostProc output buffer */
        if (mpStream->setSrcConfig(pImageBuffer->getImgSize().w,
                                   pImageBuffer->getImgSize().h,
                                   FULL_GPU_MAX_OUTPUT_WIDTH,
                                   (FULL_GPU_MAX_OUTPUT_WIDTH >> DP_COLOR_GET_H_SUBSAMPLE(MDPInputFormat)),
                                   MDPInputFormat,
        ...
        if (mpStream->setSrcConfig(pImageBuffer->getImgSize().w,
                                   pImageBuffer->getImgSize().h,
                                   (FULL_GPU_MAX_OUTPUT_WIDTH*(DP_COLOR_BITS_PER_PIXEL(MDPInputFormat)>>3)),
                                   0,
                                   MDPInputFormat,
                                   DP_PROFILE_FULL_BT601,
                                   eInterlace_None,
        ...
        MUINT32 va[3] = {(MUINT32) pImageBuffer->getBufVA(0),(MUINT32) pImageBuffer->getBufVA(1),(MUINT32) pImageBuffer->getBufVA(2)};
        MUINT32 mva[3] = {(MUINT32) pImageBuffer->getBufPA(0),(MUINT32) pImageBuffer->getBufPA(1),(MUINT32) pImageBuffer->getBufPA(2)};
        MUINT32 size[3] = {(MUINT32)(pImageBuffer->getImgSize().w * pImageBuffer->getImgSize().h),
                           (MUINT32)((pImageBuffer->getImgSize().w * pImageBuffer->getImgSize().h) >> 2),
                           (MUINT32)((pImageBuffer->getImgSize().w * pImageBuffer->getImgSize().h) >> 2)};
#else

        MUINT32 va[3] = {(MUINT32) pImageBuffer->getBufVA(0),0,0};
        MUINT32 mva[3] = {(MUINT32) pImageBuffer->getBufPA(0),0,0};
        MUINT32 size[3] = {(MUINT32)(pImageBuffer->getImgSize().w * pImageBuffer->getImgSize().h* (DP_COLOR_BITS_PER_PIXEL(MDPInputFormat)>>3)) ,
                           0,
                           0};

#endif
        //LOG_DBG("MDP Input,mva(%d,%d,%d),size(%d,%d,%d)",mva[0], mva[1], mva[2], size[0], size[1], size[2]);
        if (mpStream->queueSrcBuffer((void**)va,
                                     mva,
                                     size,
                                     DP_COLOR_GET_PLANE_COUNT(MDPInputFormat)) < 0)
        {
        ...
        if(mpStream->setSrcCrop(mdpCrop.p_integral.x, mdpCrop.p_fractional.x, mdpCrop.p_integral.y, mdpCrop.p_fractional.y, mdpCrop.s.w,mdpCrop.s.h) < 0)
        {
        ...
        #define FMT_SUPPORT( fmt, dpfmt ) \
          ...
          #undef FMT_SUPPORT
                ...
                u4Transform = OutQParams.mvOut[i].mTransform;
...
#undef TransCase
            ...
            LOG_DBG("u4Transform %d, Flip %d Rotation %d", u4Transform, u4Rotation, u4Flip);

            if(mpStream->setRotation(i, u4Rotation))
            {
                LOG_ERR("DpIspStream->setDstConfig failed");
                return MFALSE;
            }

            if(mpStream->setFlipStatus(i, u4Flip))
            {
                LOG_ERR("DpIspStream->setDstConfig failed");
                return MFALSE;
            }
            ...
            MUINT32 tmpstride0 = 0, tmpstride1 = 0;
            tmpstride0 = OutQParams.mvOut[i].mBuffer->getBufStridesInBytes(0);
            if (OutQParams.mvOut[i].mBuffer->getPlaneCount()> 1)
            {
                tmpstride1 = OutQParams.mvOut[i].mBuffer->getBufStridesInBytes(1);
            }else
            {
                tmpstride1 = 0;
            }
            ...
            if (OutQParams.mvOut[i].mPortID.capbility == NSCam::NSIoPipe::EPortCapbility_Rcrd)
            {
                bProfile = DP_PROFILE_BT601;
                LOG_DBG("mvOut record: %d", i);
            }
