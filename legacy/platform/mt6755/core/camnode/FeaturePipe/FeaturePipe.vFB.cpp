          ...
          mp3AHal(NULL)
{
    ...
    for(int i = 0 ; i < 15 ; i++ )
    {
        mGSFaceInfo[i].score = 0;
        mGSFaceInfo[i].id = 0;
        mGSPoseInfo[i].rop_dir = 0;
        mGSPoseInfo[i].rip_dir = 0;
    }
...
onNotify(MUINT32 const datamsg, MUINT32 const ext1, MUINT32 const ext2)
{
    ...
    switch( datamsg )
    {
        case FEATURE_MASK_CHANGE:
            ...
            if (FEATURE_MASK_IS_SMILE_SHOT_ENABLED(mFeatureOnMask) != FEATURE_MASK_IS_SMILE_SHOT_ENABLED(ext1))
            {
                LOG_DBG("Smile Shot is %s", (FEATURE_MASK_IS_GESTURE_SHOT_ENABLED(ext1))? "enabled": "disabled");
            }
...
threadLoopUpdate()
{
    ...
    MINT32 FaceCount = doFaceDetection(const_cast<IImageBuffer*>(jobDsImg.imgReq.mBuffer), &pFDResult->mFaceMetadata, IsDetected_SD);
    ...
    /* Smile Shot */
    if (FEATURE_MASK_IS_SMILE_SHOT_ENABLED(mFeatureOnMask) && (FaceCount))
    {
        if (!IsDetected_SD)
        {
            char EnableOption[PROPERTY_VALUE_MAX] = {'\0'};
            property_get("camera.featurepipe.fakesmile", EnableOption, "0");
            if (EnableOption[0] == '1')
            {
                LOG_DBG("[fake] enable IsDetected_SD");
                IsDetected_SD = true;
                property_set("camera.featurepipe.fakesmile", "0");
            }
        }

        if(IsDetected_SD) {
            LOG_DBG("Send Smile callback");
            mpFeaturePipeObj->performCallback(CAMERA_MSG_PREVIEW_METADATA, MTK_CAMERA_MSG_EXT_NOTIFY_SMILE_DETECT);
        }
    }
...
MINT32
FeaturePipeFdNode::doFaceDetection(IImageBuffer* pImageBuffer, MtkCameraFaceMetadata *faceMetadata, bool &IsDetected_SD)
{
//#define _WORKAROUND_FD_NOT_SUPPORT_YUY2
    ...
    do {
        ...
        mpFDHalObj->halFDDo(0,
                             ...
                             mFDYBuffer,
#endif
                            FEATURE_MASK_IS_SMILE_SHOT_ENABLED(mFeatureOnMask),
                            mRotation,
#ifdef _WORKAROUND_FD_NOT_SUPPORT_YUY2
                            3);
#else
    ...
    //for fake face debug mode setting
    char EnableOption[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("debug.camera.fd.fakefaceen", EnableOption, "0");
    MINT32 mFakeFaceEn = atoi(EnableOption);
    if ( mFakeFaceEn )
    {
        numFace = 1;
        faceMetadata->number_of_faces = 1;
        // set a width and height = 80 pixel fake face
        faceMetadata->faces[0].rect[0] = 0;
        faceMetadata->faces[0].rect[1] = 0;
        faceMetadata->faces[0].rect[2] = ((80*2000) / mFDWidth);
        faceMetadata->faces[0].rect[3] = ((80*2000) / mFDHeight);
        faceMetadata->faces[0].score = 100;
    }

    LOG_DBG("Face detection tried %d times and found %d faces", count, numFace);

#if (GET_SD_RESULT)
    if (FEATURE_MASK_IS_SMILE_SHOT_ENABLED(mFeatureOnMask))
    {
        SD_Result = mpFDHalObj->halSDGetSmileResult();
        //LOG_DBG("Scenario SD Result1: %d",SD_Result );
        if(SD_Result>0)
            IsDetected_SD = MTRUE;
        else
            IsDetected_SD = MFALSE;
    }
#endif
