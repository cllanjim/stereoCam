...
OTClient(sp<IParamsManager> pParamsMgr)
    ...
    , OT_StartY(0)
    , DDPBuffer(NULL)
    , OTWorkingBuffer(NULL)
    , OTWorkingBufferSize(0)
    , Rotation_Info(0)
    , VA1(NULL)
    , VA2(NULL)
{
...
onStateChanged()
{
    ...
    if  ( isEnabledState() )
    {
        status_t status = run("OTClient::onStateChanged");
...
stopOT()
{
    ...
    if ( isEnabledState() )
    {
        ::android_atomic_write(0, &mIsOTStarted);
        ret = onStateChanged();
        // Stop OT thread and wait
        requestExit();
        status_t status = join();
        if  ( OK != status )
        {
            MY_LOGW("StopOT: Not to wait thread(tid:%d), status[%s(%d)]", getThreadId(), ::strerror(-status), -status);
        }
        MY_LOGD("StopOT: join() exit");
    }
    else
    {
