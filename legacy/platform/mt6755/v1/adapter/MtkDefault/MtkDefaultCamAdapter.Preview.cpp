...
onHandleStartPreview()
{
    ...
    else
    {
        ...
        {
            mUserName = "VDO";
            if(mpResMgrDrv && mName == MtkCameraParameters::APP_MODE_NAME_DEFAULT)
            {
                mpResMgrDrv->setItem(ResMgrDrv::ITEM_THIRD_PARTY, MTRUE);
            }
        }
        else
        {
    ...
    if(mpPass2Node)
    {
        ...
        if(pHalSensorList!=NULL)
        {
            MUINT32 sensorDev = (MUINT32)pHalSensorList->querySensorDevIdx(getOpenId());
            ...
            {
                //rwb sensor force to MDP preview
                mpPass2Node->setGPUProcType(1);
            }
        }
#endif
        //
    ...
    {
        MY_LOGD("SetEnableDisplayRotation = true");
        mpDefaultBufHdl->SetEnableDisplayRotation(MTRUE);
    }
    else
    {
        MY_LOGD("SetEnableDisplayRotation = false");
        mpDefaultBufHdl->SetEnableDisplayRotation(MFALSE);
    }
    //
    ...
    if(mbTwoRunP2)
    {
        if(mpParamsMgr->getDisplayRotationSupported())
        {
            mpDefaultBufHdl->mapPort(DefaultBufHandler::eBuf_Disp,      TRANSFORM_DST_1, 0, MTRUE);
            mpDefaultBufHdl->mapPort(DefaultBufHandler::eBuf_AP,        TRANSFORM_DST_0);
            mpDefaultBufHdl->mapPort(DefaultBufHandler::eBuf_Generic,   TRANSFORM_DST_0);
    ...
    else
    {
        if(mpParamsMgr->getDisplayRotationSupported())
        {
            mpDefaultBufHdl->mapPort(DefaultBufHandler::eBuf_Disp,      PASS2_PRV_DST_1, 0, MTRUE);
            mpDefaultBufHdl->mapPort(DefaultBufHandler::eBuf_AP,        PASS2_PRV_DST_0);
            mpDefaultBufHdl->mapPort(DefaultBufHandler::eBuf_Generic,   PASS2_PRV_DST_0);
        }
        else
        {
    ...
    if(mbTwoRunP2)
    {
        ...
        if(mpRecBufSnapshotlNode != NULL)
        {
            if(mpParamsMgr->getDisplayRotationSupported())
            {
                mpCamGraph->connectData(TRANSFORM_DST_0,    RECBUFSS_SRC,   mpImgTransformNode, mpRecBufSnapshotlNode);
            }
            else
            {
    ...
    else
    {
        if(mpRecBufSnapshotlNode != NULL)
        {
            if(mpParamsMgr->getDisplayRotationSupported())
            {
                mpCamGraph->connectData(PASS2_PRV_DST_0,    RECBUFSS_SRC,   mpPass2Node,        mpRecBufSnapshotlNode);
            }
            else
            {
...
forceStopAndCleanPreview()
{
    ...
    if ( mpPass2Node != NULL )
    {
        ...
        mpPass2Node->enableSDPreview(MFALSE);
    ...
    if(mpResMgrDrv)
    {
        ...
        mpResMgrDrv->setMode(&mode);
        mpResMgrDrv->setItem(ResMgrDrv::ITEM_THIRD_PARTY, MFALSE);
    }
    //
    MY_LOGD("-");
    //
