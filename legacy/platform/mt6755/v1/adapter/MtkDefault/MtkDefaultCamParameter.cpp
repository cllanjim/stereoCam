...
setParameters()
{
    ...
    if(mpResMgrDrv)
    {
        if(mpParamsMgr->getVHdr() == NSCam::SENSOR_VHDR_MODE_IVHDR)
        {
            mpResMgrDrv->setItem(
                            ResMgrDrv::ITEM_IVHDR,
                            MTRUE);
        }
        else
        {
        ...
        //
        if(mpResMgrDrv->queryCtrlType() == ResMgrDrv::CONTROL_BEFORE)
        {
        ...
        else
        {
            if(mpPass2Node)
            {
                mpPass2Node->waitCurEnqueFrameDone();
            }
            mpResMgrDrv->triggerCtrl();
        }
    }
    ...
    else
    {
            ...
            (mpParamsMgr->getRecordingHint() && mpParamsMgr->getVideoStabilization()))
        {
            ...
            MUINT32 previewFps = mpParamsMgr->getInt(CameraParameters::KEY_PREVIEW_FRAME_RATE);
            MUINT32 fixedFps   = ( !mpParamsMgr->getRecordingHint() && (previewFps > 30) ) ? 30 : previewFps;
            MBOOL isDynamicFps = ( ::strcmp(mpParamsMgr->getStr(MtkCameraParameters::KEY_DYNAMIC_FRAME_RATE), CameraParameters::TRUE) == 0 ) ? MTRUE : MFALSE;
            ...
            if(isSlowMotion)
            {
                cam3aParam.i4MinFps = previewFps*1000;
            ...
            else
            {
                ...
                {
                    MY_LOGD("Fixed 30 fps in EIS VR");
...
setP2outBuf()
{
    ...
    if( PARAMSMANAGER_MAP_INST(eMapAppMode)->stringFor(pParamsMgr->getHalAppMode()) != MtkCameraParameters::APP_MODE_NAME_MTK_ZSD )
    {
        freeReuseZsdP2outBuf();
        return;
    }

    if( ::strcmp(pParamsMgr->getStr(MtkCameraParameters::KEY_FACE_BEAUTY), MtkCameraParameters::TRUE) == 0 )
    {
        freeReuseZsdP2outBuf();
        return;
    }

    //capture size, orientation
    MINT32 rotation = pParamsMgr->getInt(CameraParameters::KEY_ROTATION);
    int width = 0, height = 0;
    if( rotation == 90 || rotation == 270 )
    {
        pParamsMgr->getPictureSize(&height, &width);
    }
    else
    {
        pParamsMgr->getPictureSize(&width, &height);
    }
    if( width*height > 20*1000*1000 )
    {
        MY_LOGD("picture size too large, (w,h)=(%d,%d) doesn't allocate",width,height);
        return;
    }
