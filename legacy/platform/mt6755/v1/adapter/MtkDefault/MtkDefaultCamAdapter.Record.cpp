...
onHandleStartRecording()
{
    ...
    else
    {
        if(mbTwoRunP2)
        {
            ...
            {
                mpDefaultBufHdl->mapPort(DefaultBufHandler::eBuf_Rec, TRANSFORM_DST_0, 0, MTRUE);
            }
            else
            {
        ...
        else
        {
            if(mpParamsMgr->getDisplayRotationSupported())
            {
                mpDefaultBufHdl->mapPort(DefaultBufHandler::eBuf_Rec, PASS2_PRV_DST_0, 0, MTRUE);
            }
            else
            {
...
onHandleStopRecording()
{
    ...
    if(mpDefaultCtrlNode)
    {
        if( mpDefaultCtrlNode->isRecBufSnapshot() == MFALSE &&
