...
onHandleStartPreview()
{
    ...
    property_get("debug.featurepipe.enable", value, "1");
    ...
    if( mbZsdMode )
    {
        ...
        //
        MY_LOGD("get_stereo_zsd_cap_stored_frame_cnt:%d", get_stereo_zsd_cap_stored_frame_cnt());
    ...
    //
    MY_LOGD("VHdr(%d),Name(%s),userName(%s),sensorScenario(%d),pass1RingBufNum(%d),stereo:prvFps(%d),capFps(%d)->frameRate(%d)",
            mpParamsMgr->getVHdr(),
            mName.string(),
            mUserName.string(),
            sensorScenario,
    ...
    if(mpStereoCtrlNode)
    {
        ...
        {
            // When stereoCtrlNode not connect, need to force init stereo hal setting
