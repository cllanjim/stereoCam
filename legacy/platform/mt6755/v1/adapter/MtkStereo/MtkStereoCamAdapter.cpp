...
CamAdapter(
    ...
    , mpResMgrDrv(NULL)
    //
    , mModeType(DefaultCtrlNode::MODE_IDLE)
    , mEnRaw16CB(0)
    , mpHwSyncDrv(NULL)
{
    MY_LOGD(
...
init()
{
    ...
    mpCapBufMgr         = CapBufMgr::createInstance( getOpenId_Main() );
    mpCapBufMgr_Main2   = CapBufMgr::createInstance( getOpenId_Main2() );
    ...
    if  ( mpCaptureCmdQueThread == 0 || OK != (status = mpCaptureCmdQueThread->run("SteteroCamAdapter::init") ) )
    {
        MY_LOGE(
    ...
    if(mpHwSyncDrv == NULL)
    {
        MY_LOGE("Create HwSync object fail.");
        goto lbExit;
    }
    mpHwSyncDrv->init(HW_SYNC_USER_STEREO_ADAPTER, 0x0);
    //
    ret = true;
lbExit:
    if(!ret)
...
{
    MY_LOGD("+");
    // release HwSync object
    if(mpHwSyncDrv != NULL)
    {
        mpHwSyncDrv->uninit(HW_SYNC_USER_STEREO_ADAPTER, 0x0);
