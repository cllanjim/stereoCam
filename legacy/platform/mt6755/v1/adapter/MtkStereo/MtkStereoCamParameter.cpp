...
setParameters()
{
    ...
    if ( mpStereoHal )
    {
        mpStereoHal->setParameters(mpParamsMgr);
    }
    //
    ...
    else
    {
        ...
        mbFixFps = true;
        MINT32 const stereoPrvFps = mpParamsMgr->getInt(MtkCameraParameters::KEY_STEREO_PREVIEW_FRAME_RATE);
        MINT32 const stereoCapFps = mpParamsMgr->getInt(MtkCameraParameters::KEY_STEREO_CAPTURE_FRAME_RATE);
        mbZsdMode = ( ::strcmp(mpParamsMgr->getStr(MtkCameraParameters::KEY_ZSD_MODE), MtkCameraParameters::ON) == 0 ) ? MTRUE : MFALSE;
        MINT32 const frameRate = ( mbZsdMode ) ? stereoCapFps : stereoPrvFps;
        // for mt6755 dynamic framerate is not supported. 
        //cam3aParam.i4MinFps = 10*1000;
        cam3aParam.i4MinFps = frameRate*1000;
        cam3aParam.i4MaxFps = frameRate*1000;
