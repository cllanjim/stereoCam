...
DefaultCtrlNodeImpl(const char* userName)
    ...
    , mRollbackBufNum(0)
    , mRollbackBufNumPrv(0)
    , mRollbackBufPeriod(1)
    , mRollbackBufCnt(0)
    , mRollbackBufCnt_Prv(0)
    , mPreviewMaxFps(30)
    , mShotMode(0)
    , mMode(MODE_IDLE)
...
onInit()
{
    ...
    if(mpIspSyncCtrl)
    {
            ...
            mMode == MODE_VIDEO_PREVIEW)
        {
            ...
            for(i=0; i<3; i++)
            {
                MY_LOGD("sizeList[%d]=%dx%d",
                        ...
                        sizeList[i].w,
                        sizeList[i].h);
                if( i != 2 ) //EIS size already multiplied EIS_FACTOR, no need to do twice
                {
                    sizeList[i].w = (sizeList[i].w*EIS_FACTOR)/100;
                    sizeList[i].h = (sizeList[i].h*EIS_FACTOR)/100;
                }
                //
                MRect calCrop;
                MSize calSize;
                mpIspSyncCtrl->calRrzoSize(calCrop, calSize, 100, sizeList[i]);
                MY_LOGD("eis factor(%d), new size=%dx%d, rrz size=%dx%d",
                        EIS_FACTOR, sizeList[i].w, sizeList[i].h,
                        calSize.w, calSize.h);
                //
                if(initRrzoSize.w < calSize.w)
                {
                   initRrzoSize.w = calSize.w;
                }
                if(initRrzoSize.h < calSize.h)
                {
                   initRrzoSize.h = calSize.h;
                }
            }
#endif
            //
        ...
        if( pHalSensorList != NULL )
        {
            ...
            SensorStaticInfo const*const pSensorStaticInfo = pHalSensorList->querySensorStaticInfo(getSensorIdx());

            pSensorHalObj = pHalSensorList->createSensor(
                                            LOG_TAG,
                                            getSensorIdx());
            //this should be the correct method to get if sensor support PDAF
            if ( pSensorStaticInfo->PDAF_Support == 1) //PDAF Raw ( pure raw data mode)
            {
            ...
            if( val > -1 ) {
                mbIsPureRawPDAFOn = val;
                MY_LOGD("debug.pass1.pdafon %d", mbIsPureRawPDAFOn);
            }

            MY_LOGD("isPDAFOn=%d", mbIsPureRawPDAFOn);
            // if support PDAF, get PDAF frame count from sensor HAL
            if( mbIsPureRawPDAFOn )
            {
                #if 0
    ...
    #endif
    //
#if 1    //this chip support GIS
    MBOOL isVFB = (::strcmp(mspParamsMgr->getStr(MtkCameraParameters::KEY_FACE_BEAUTY), MtkCameraParameters::TRUE) == 0) ? MTRUE : MFALSE;
    MBOOL isvHDREnabled = (mspParamsMgr->getVHdr() != SENSOR_VHDR_MODE_NONE);
    if ( (mVideoSize.w*mVideoSize.h > IMG_1080P_SIZE) || isVFB || isvHDREnabled)
    {
        mEisScenario = EIS_SCE_EIS;
    }
    else
    ...
    /* EIS scenario control for Jade Minus */
    switch (CamManager::getInstance()->getEISScenarioType())
    {
    case CamManager::EIS_SCE_TYPE_NULL:
        mEisScenario = EIS_SCE_NONE;
        break;
    case CamManager::EIS_SCE_TYPE_EIS:
        mEisScenario = EIS_SCE_EIS;
    case CamManager::EIS_SCE_TYPE_DONT_CARE:
    default: /* fallthrough */
        mEisThreadState = 0;
        if( pthread_create(&mThreadHandleEis, NULL, threadLoopEis, this) != 0 )
        {
            MY_LOGE("threadLoopEis pthread create failed");
        }
        break;
    }
#else
...
onUninit()
{
    ...
    //
    if(CamManager::getInstance()->getEISScenarioType() != CamManager::EIS_SCE_TYPE_NULL)
    {
        #if EIS_TEST
        if(mpEisHal)
        {
...
onStop()
{
    ...
    while(mpCapBufMgr->popBuf(pImgBuf, MFALSE))
    {
        handleReturnBuffer(CONTROL_RESIZEDRAW, (MUINTPTR)pImgBuf);
    }
        ...
        case PASS1_START_ISP:
        {
            if( mPreviewMaxFps <= EIS_MAX_FPS &&
                (   mMode == MODE_VIDEO_PREVIEW ||
                    ...
                    mMode == MODE_STEREO_VDO_PREVIEW ||
                    mMode == MODE_STEREO_VDO_RECORD ))
            {
                if(CamManager::getInstance()->getEISScenarioType() != CamManager::EIS_SCE_TYPE_NULL)
                {
                    #if EIS_TEST
            ...
            if(mpHal3a)
            {
                ...
                //
                MY_LOGD("mpHal3a->sendCommand");
                mpHal3a->sendCommand(cmd);
                MY_LOGD("mpHal3a->setIspProfile");
        ...
        case PASS1_CONFIG_FRAME:
        {
                ...
                mpIspSyncCtrl)
            {
                ...
                if(mpEisHal->GetEisSupportInfo(getSensorIdx()))
                {
                    ...
                    eisConfig.sensorMode = mSensorScenario;
        ...
        if (mZsdExtMode != ZSDEXT_MFLL_FAST)
        {
            ...
            if (mZsdStopPreview == MFALSE)
            {
                if (mZsdExtFramePassCounter >= mZsdExtDelayFrameCount && mDequeueMagicNum >= mZsdExtDelayBaseMagicNum)
                {
                    ...
                    // enable AE
                    if (mResume3AAterZsdExt == MTRUE)
                    {
                        MY_LOGD("@@@Enable AE and AF after ZSDEXT");
                        mpHal3a->send3ACtrl(E3ACtrl_SetAFPause, MFALSE, 0);
                        mpHal3a->send3ACtrl(E3ACtrl_EnableDisableAE, MTRUE, 0);
                        // enalbe raw output after ZSD+HDR shot
                        if(mbIsPureRawPDAFOn)
                        {
                            mpIspSyncCtrl->setPass1PureRaw(MTRUE, mPDAFFrameCount);
                        }
...
onPostBuffer(MUINT32 const data, MUINTPTR const buf, MUINT32 const ext)
{
...

    {
        //drop frame or not
        Mutex::Autolock _l(mLock);
        if( getFlag(muStateFlag, FLAG_BUF_PREVIEW_OK) && !getFlag(muStateFlag, FLAG_DISABLE_PREVIEW) )
        {
            doPost = MTRUE;
            mPostBufCount++;
            MY_LOGD("data %d   mPostBufCount %d", data, mPostBufCount);
        }
    }
    ...
    if (doPost)
    {
        ...
        //MY_LOGD("ZsdExt: doPost=%d",doPost);
    }
    ...
    //
    handlePostBuffer(dstdata, buf, ext);
    //
    if( data == CONTROL_RESIZEDRAW )
    {
...
onReturnBuffer(MUINT32 const data, MUINTPTR const buf, MUINT32 const ext)
{
    ...
    switch(data)
    {
        ...
        {
            /*
            MY_LOGD("Full Cnt(%d),Num(%d),Period(%d)",
                    mRollbackBufCnt,
                    mRollbackBufNum,
                    mRollbackBufPeriod);
            */
            ...
            {
                MUINT32 stereo_disable = (MUINT32)(-1);
                MBOOL   saveBuf = MTRUE;
                MINT32  bufAFDone = 1;
                if(ext == stereo_disable){
                    MY_LOGV("bypass capBuf logic here in stereo mode");
                    saveBuf = MFALSE;
                }
                if(mpHal3a && mShotMode != eShotMode_ContinuousShotCc)
                {
                    MUINT32     outMagicNum;
                    MBOOL       outIsRrzo;
                    ...
                    mpIspSyncCtrl->queryImgBufInfo(
                                        ...
                                        outpPrivateData,
                                        outPrivateDataSize);
                    bufAFDone = mpHal3a->send3ACtrl(
                                            E3ACtrl_QueryAFStatus,
                                            outMagicNum,
                                            0);
                }
                //
                MY_LOGV("bufAFDone(%d),mShotMode(%d)",bufAFDone,mShotMode);
                if(bufAFDone != 1)
                {
                    MY_LOGD("CAP:Buf is not AF %d !",bufAFDone);
                    saveBuf = MFALSE;
                }
                else
        ...
        {
            /*
            MY_LOGD("Prv Cnt(%d),Num(%d),Period(%d)",
                    mRollbackBufCnt_Prv,
                    mRollbackBufNumPrv,
                    mRollbackBufPeriod);
                    */
            if ( mZsdExtMode == ZSDEXT_OFF )
            {
                ...
                if(bufAFDone != 1)
                {
                    MY_LOGD("PRV:Buf is not AF %d !",bufAFDone);
                    saveBuf = MFALSE;
                }
                else
                if(mRollbackBufCnt_Prv > mRollbackBufNumPrv)
                {
                    if((mRollbackBufCnt_Prv-mRollbackBufNumPrv) % mRollbackBufPeriod != 0)
                    {
                        saveBuf = MFALSE;
                        mRollbackBufCnt_Prv++;
                    }
                ...
                //
                if( mRollbackBufNumPrv > 0 &&
                    saveBuf)
                {

                    IImageBuffer* pImgBuf = NULL;
                    pImgBuf = (IImageBuffer*)buf;
                    mpCapBufMgr->pushBuf(pImgBuf, MFALSE);
                    //
                    if(mRollbackBufCnt_Prv >= mRollbackBufNumPrv)
                    {
                        pImgBuf = NULL;
                        mpCapBufMgr->popBuf(pImgBuf, MFALSE);
                        handleReturnBuffer(CONTROL_RESIZEDRAW, (MUINTPTR)pImgBuf);
                    }
                    mRollbackBufCnt_Prv++;
                }
            ...
            else // in ZSD extension mode, directly return preview buffer
            {
                handleReturnBuffer(CONTROL_RESIZEDRAW, buf);
            }
            break;
    ...
    }
    {
        Mutex::Autolock _l(mLock);
        mRetBufCount++;
        MY_LOGD("data %d   mRetBufCount %d", data, mRetBufCount);
        if (mPostBufCount == mRetBufCount)
        {
            mCondStop.signal();
        }
    }
...
{
    MY_LOGD("RPBuf:Num RRZO(%d) IMGO(%d),RBBuf:Num(%d),RBBufPrv:Num(%d), Period(%d),PrvMaxFps(%d)",
            configInfo.replaceBufNumResize,
            configInfo.replaceBufNumFullsize,
            configInfo.rollbackBufNum,
            configInfo.rollbackBufNumPrv,
    ...
    mRollbackBufNumPrv      = configInfo.rollbackBufNumPrv;
...
updateDelay(EQueryType_T const type)
{
    ...
    if(delayCnt > 0)
    {
        ...
        if (debug != 0)
        {
            ...
            MY_LOGD("type(%d),delay(%d),drop(%d)", type, delayCnt, mDropFrameCnt);
        }
    }
    //
    return ret;
...
setMode(MODE_TYPE mode)
{
    ...
    switch(mode)
    {
        ...
        case MODE_STEREO_VDO_PREVIEW:
        {
            ...
            if(mMode == MODE_STEREO_VDO_RECORD)
            {
                mpHal3a->sendCommand(ECmd_RecordingEnd);
                //
                if(mRecordingHint != MTRUE)
                {
                    mode = MODE_STEREO_PREVIEW;
                }
            }
            break;
        }
        case MODE_STEREO_VDO_RECORD:
        {
            mpHal3a->sendCommand(ECmd_RecordingStart);
            //
            MUINT32 aaaUpdatePeriod;
...
//                            #if (MTKCAM_HAVE_ZSDMFB_SUPPORT == 1)
                            else
                            {
...
//                            #endif
                            ...
                            property_get( "debug.mtk_cam_zsdmfb_support", propertyValue, "0");
                            if( runMfb && atoi(propertyValue) == 1 )
                            {
                                mShotMode = eShotMode_ZsdMfllShot;
                                MY_LOGD("set mShotMode = eShotMode_ZsdMfllShot by setprop debug.mtk_cam_zsdmfb_support 1");
                            }
                            break;
                        }
                    ...
                    case eShotMode_FaceBeautyShot:
                        {
                            ...
                            if(!runMfb) {
                                MY_LOGD("<shot mode> %#x(%s)", shotMode, s8ShotMode.string());
                                // ZSD shot only works when normal shot with strobe off & MFLL/AIS off
                                mShotMode = eShotMode_FaceBeautyShotCc;
                            }
                            break;
                        }
                    case eShotMode_HdrShot:
                        ...
                        property_get( "ro.mtk_zsdhdr_support", propertyValue, "0");
                        if( atoi(propertyValue) == 1 )
                        {
                            mShotMode = eShotMode_ZsdHdrShot;
                            MY_LOGD("set mShotMode = eShotMode_ZsdHdrShot by ro.mtk_zsdhdr_support 1");
                        }
                        break;
                    default:
                        break;
...
#undef START_PREVIEW_TIMEOUT
    ...
    if( !ret )
    {
        ...
        aee_system_exception(LOG_TAG, NULL, DB_OPT_DEFAULT, "\nCRDISPATCH_KEY:MtkCam/DCNode:3A startpreview timeout");
    }
...
startPreview()
{
    Mutex::Autolock _l(mLock);
    MY_LOGD("+");
    disableFlag(muStateFlag, FLAG_DISABLE_PREVIEW);
    // reset buffer count
    mPostBufCount   = 0;
    mRetBufCount    = 0;
}
...
{
    {
        Mutex::Autolock _l(mLock);
        MY_LOGD("+");
        enableFlag(muStateFlag, FLAG_DISABLE_PREVIEW);
    }
    {
        Mutex::Autolock _l(mLockStop);
        while(mPostBufCount != mRetBufCount)
        {
            MY_LOGD("wait lock post %d != ret %d", mPostBufCount, mRetBufCount);
            mCondStop.wait(mLockStop);
            MY_LOGD("wait done");
        }
    }
}
...
precap()
{
    ...
    if( mShotMode == eShotMode_ContinuousShot ||
        mShotMode == eShotMode_ContinuousShotCc )
    {
        mpHal3a->send3ACtrl(E3ACtrl_KeepOnLastStatus, 0, 0);
    }
    //
    if( mbSkipPrecapture )
    {
...
startZsdExtShot(ZSD_EXT_MODE extMode)
{
    ...
    if (mZsdExtMode != ZSDEXT_MFLL_FAST)
    {
        ...
        MY_LOGD("@@@Disable AE and AF");
        mpHal3a->send3ACtrl(E3ACtrl_SetAFPause, MTRUE, 0);
        mpHal3a->send3ACtrl(E3ACtrl_EnableDisableAE, MFALSE, 0);
        // only sending process raw to capbuf manager during ZSD+HDR shot
        if(mbIsPureRawPDAFOn)
        {
            mpIspSyncCtrl->setPass1PureRaw(MFALSE, 0);
        }
...
stopZsdExtShot()
{
    ...
    if ( mZsdExtMode != ZSDEXT_OFF)
    {
        ...
        if (mResume3AAterZsdExt == MTRUE)
        {
            MY_LOGD("@@@Enable AE AF after ZSDEXT");
            mpHal3a->send3ACtrl(E3ACtrl_SetAFPause, MFALSE, 0);
            mpHal3a->send3ACtrl(E3ACtrl_EnableDisableAE, MTRUE, 0);
            // enalbe raw output after ZSD+HDR shot
            if(mbIsPureRawPDAFOn)
            ...
            // assure that AE setting to take effect before returning to preview
            mZsdExtMode = ZSDEXT_WAIT_3A;
        }
    }
    FUNC_END;
}
...
threadFunEis()
{
    MY_LOGD("+ mEisThreadState (%d)",mEisThreadState);
    if(mEisThreadState == 0)
    {
    ...
    {
        MY_LOGW("Unknown state (%d)",mEisThreadState);
    }
    MY_LOGD("- mEisThreadState (%d)",mEisThreadState);
    FUNC_END;
    return MTRUE;
...
waiThreadDoneEis()
{
    if(mThreadHandleEis != 0)
    {
        FUNC_START;
        int s = pthread_join(mThreadHandleEis, NULL);
        if( s != 0 )
        {
...
void*
DefaultCtrlNodeImpl::
threadLoopEis(void* arg)
{
    CAM_LOGD("[threadLoopEis]");
    DefaultCtrlNodeImpl* pDefaultCtrlNodeImpl = (DefaultCtrlNodeImpl*)arg;
    pDefaultCtrlNodeImpl->threadFunEis();
    pthread_exit(NULL);
    return NULL;
}
