...
class DefaultBufHandlerImpl : public DefaultBufHandler
{
    ...
    //
    virtual MVOID   SetEnableDisplayRotation(MBOOL enableFlag);
    //
    protected:
        //
        typedef struct
        ...
        //
        virtual MBOOL   allocBuffer(
                            BUF_INFO&           info,
                            IImageBuffer*&      pBuf);
        virtual MBOOL   freeBuffer(IImageBuffer*& pBuf);
        //
    private:
        mutable Mutex               mLock;
        ...
        //
        MBOOL                       mEnableDisplayRotation;
        list<MUINTPTR>              mlTempBuffer;
};

/*******************************************************************************
*
********************************************************************************/
static DefaultBufHandlerImpl::BUF_INFO gDummyBufInfo;
...
DefaultBufHandlerImpl(
    ...
    , mEnableDisplayRotation(MFALSE)
{
    FUNC_NAME;
    gDummyBufInfo.w                 = 640;
    gDummyBufInfo.h                 = 480;
    gDummyBufInfo.fmt               = eImgFmt_YV12;
    gDummyBufInfo.strideInByte[0]   = 640;
    gDummyBufInfo.strideInByte[1]   = 320;
    gDummyBufInfo.strideInByte[2]   = 320;
}
...
dequeBuffer(MUINT32 const data, ImgRequest * pImgReq)
{
            ...
            if(bufProvider != 0)
            {
                ...
                while(1)
                {
                    if(bufProvider->dequeProvider(node))
                    {
                        ...
                        //
                        if(mEnableDisplayRotation)
                        {
                            if((*iterMapPort).bufType == eBuf_Disp)
                            {
                                if(timeout == 0)    //preview
                                {
                                    NeedDisplayRotation = MTRUE;
                                }
                                else
                                {
                                    node.setOrientation(0);
                                    node.setRotation(0);
                                    NeedDisplayRotation = MFALSE;
                                }
                            }
                        }
    ...
    if(isDequeProvider||
        (isTimeout && isDisp))
    {
        sp<ImageBufferHeap> pHeap = NULL;
        size_t bufStridesInBytes[] = {0,0,0};
        if(isDequeProvider && !isTimeout)
        {
            if(NeedDisplayRotation)
            {
            ...
            int allocImgWidth = 0;
            int allocImgHeight = 0;
            if(NeedDisplayRotation)
            {
                allocImgWidth = node.getImgBuf()->getImgHeight();
                allocImgHeight = node.getImgBuf()->getImgWidth();
            }
            else
            {
                allocImgWidth = node.getImgBuf()->getImgWidth();
                allocImgHeight = node.getImgBuf()->getImgHeight();
            }
        ...
        //
        IImageBuffer* tempBuffer = NULL;
        //
        if(isDisp && isTimeout)
        {
            MY_LOGD("provide Display dummy Buffer");
            allocBuffer(
                gDummyBufInfo,
                tempBuffer);
        }
        ...
        {
            if( NeedDisplayRotation && node.getOrientation() != 0)
            {
                pImgReq->mTransform  = node.getOrientation();
            }
            else
            {
        ...
        if(isDisp && isTimeout)
        {
            mlTempBuffer.push_back(GET_IIMAGEBUFFER_BUF_VA(pImgReq->mBuffer, 0));
            MY_LOGD("TempBufSize(%d) VA(%p) isTimeout(%d)",
                mlTempBuffer.size(),
        ...
        MY_LOGD("buf(%p),heap(%p),VA(%p/%p/%p),PA(%p/%p/%p),BS(%d=%d+%d+%d),Id(%d),F(0x%08X),S(%dx%d),Str(%d,%d,%d),R(%d),U(%d)",
                ...
                pImgReq->mUsage);
        */
        MY_LOGD("buf(%p),VA(%p/%p/%p),BS(%d=%d+%d+%d),Id(%d),F(0x%08X)(%s),S(%dx%d),Str(%d,%d,%d),R(%d),U(%d),O(%d),NDR(%d)",
                pImgReq->mBuffer,
                GET_IIMAGEBUFFER_BUF_VA(pImgReq->mBuffer, 0),
                GET_IIMAGEBUFFER_BUF_VA(pImgReq->mBuffer, 1),
                GET_IIMAGEBUFFER_BUF_VA(pImgReq->mBuffer, 2),
                ...
                pImgReq->mBuffer->getFD(),
                pImgReq->mBuffer->getImgFormat(),
                (node.getImgBuf() == 0) ? "null" : node.getImgBuf()->getImgFormat().string(),
                pImgReq->mBuffer->getImgSize().w,
                pImgReq->mBuffer->getImgSize().h,
                GET_IIMAGEBUFFER_BUF_STRIDE_IN_BYTE(pImgReq->mBuffer, 0),
                GET_IIMAGEBUFFER_BUF_STRIDE_IN_BYTE(pImgReq->mBuffer, 1),
...
enqueBuffer(MUINT32 const data, IImageBuffer const * pImageBuffer)
{
    ...
    MUINTPTR keepAddr = 0;
    MBOOL isReturnTempBuffer = MFALSE;
    //
    #if 0
    MY_LOGD("data(0x%08X)",data);
    #endif
    ...
    if(bufQueIdx >= 0)
    {
        // find if return buffer is temp buffer
        if(mlTempBuffer.size() > 0)
        {
            list<MUINTPTR>::iterator iterTempBuffer;
            for(iterTempBuffer = mlTempBuffer.begin(); iterTempBuffer != mlTempBuffer.end(); iterTempBuffer++)
            {
                if(enqueAddr == *iterTempBuffer)
                {
                    isReturnTempBuffer = MTRUE;
                    gDummyBufInfo.va = *iterTempBuffer;
                    mlTempBuffer.erase(iterTempBuffer);
    ...
    if(!isReturnTempBuffer)
    {
        keepAddr = (MUINTPTR)(mvBufQueNode[bufQueIdx][0].getImgBuf()->getVirAddr());
        //
        #if 0
    ...
    else
    {
        if(isReturnTempBuffer)
        {
            MY_LOGD("gDummyBufInfo.va(%p), TempBufSize(%d)",gDummyBufInfo.va,mlTempBuffer.size());
            IImageBuffer* tempBuffer = const_cast<IImageBuffer*>(pImageBuffer);
            tempBuffer->unlockBuf(LOG_TAG);
            tempBuffer->decStrong(tempBuffer);
...
mapNode2Dst(MUINT32 nodeType)
{
    switch(nodeType)
    {
        ...
        {
            return EPass2Out_CAP_DST_2;
        }
        case PASS2_CAP_DST_3:
        {
            return EPass2Out_CAP_DST_3;
        }
        default:
        {
...
MVOID
DefaultBufHandlerImpl::
SetEnableDisplayRotation(MBOOL enableFlag)
{
    MY_LOGD("set = %s", ((enableFlag==true) ? "true" : "false") );
    mEnableDisplayRotation = enableFlag;
}
    ...
    IImageBuffer*&      pBuf)
{
    ...
    if(info.strideInByte[0] == 0)
    {
        ...
        planeCount = Format::queryPlaneCount(info.fmt);
        for (MUINT32 i = 0; i < planeCount; i++)
        {
            bufStridesInBytes[i] =
                (Format::queryPlaneWidthInPixels(info.fmt,i, info.w)*Format::queryPlaneBitsPerPixel(info.fmt,i))>>3;
        }
    }
    else
    {
