    ...
    , mi4RwbProcType(0)
{
    ...
    property_get( "debug.rwb.type", value, "-1");
    mi4RwbProcType = atoi(value);
}
...
bool
ImpShot::
makeCamExif(MUINT32 const u4CamMode, CamExif &rCamExif)
{
    CamExifParam rExifParam;
    CamDbgParam rDbgParam;
...
bool
ImpShot::makeExifHeaderEx(MUINT32 const u4CamMode,
        MUINT8* const puThumbBuf,
        MUINT32 const u4ThumbSize,
        MUINT8* puExifBuf,
        MUINT32 &u4FinalExifSize,
...
MUINT32
ImpShot::
queryGpuProcEn() const
{
    MUINT32 enGPUProc = NSCamShot::ECamShot_RWB_NONE; //default disable gpu
    //
    MUINT32 rwbProcType = (mi4RwbProcType != -1) ? mi4RwbProcType : get_capture_rwbProc_type();
    MUINT32 rawFmtType = SENSOR_RAW_Bayer;
    ...
    if(pIHalSensorList)
    {
        ...
        rawFmtType = sensorStaticInfo.rawFmtType; 
    }
	else
    {
        MY_LOGE("IHalSensorList::get() == NULL");
        return enGPUProc;
    }
    MY_LOGD("rawFmtType(%d), rwbProcType(%d), property set RWB ProcType(%d)", rawFmtType, rwbProcType, mi4RwbProcType);
    //
    if(rawFmtType == SENSOR_RAW_RWB)
    {
        switch(rwbProcType)
        {
            case 1:
                enGPUProc = NSCamShot::ECamShot_RWB_MDP;
                break;
            case 2:
                enGPUProc = NSCamShot::ECamShot_RWB_GPU;
                break;
            case 0:
            default:
                enGPUProc = NSCamShot::ECamShot_RWB_NONE;
                break;
        }
    }
    //
