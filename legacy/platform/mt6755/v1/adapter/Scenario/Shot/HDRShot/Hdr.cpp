...
static int EnablePerfService()
{
    ...
    if (PerfServiceHandle != -1) {
        PerfServiceNative_userEnable(PerfServiceHandle);
    } else {
...
static void DisablePerfService(int )
{
}
...
#endif //#ifdef HDRSHOT_USE_PERFSERVICE
//-----------------------------------------------------------------------------
...
HdrShot::onCmd_capture()
{
    ...
    pthread_create(&mMainThread, NULL, HdrShot::HDRProcessTask, this);
...
HdrShot::mainflow()
{
    ...
    do {
        ...
        if (mHDRShotMode == eShotMode_ZsdHdrShot) {
            ...
            ret = ret && mpShotCallback->onCB_P2done();
            if (!ret) break;
        }
