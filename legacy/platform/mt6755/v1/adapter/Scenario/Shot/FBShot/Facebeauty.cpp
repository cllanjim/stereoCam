...
onCmd_capture()
{
    ...
    //get sensor info
    {
        ...
        IssupportVFB = MTRUE;
#endif
        MY_LOGD("RAW sensor(CFB): sensor type = %d IssupportVFB = %d",mSensorType,IssupportVFB);// raw->1; yuv->2
    }

    sem_init(&semMemoryDone, 0, 0);
    sem_init(&semFBthread, 0, 0);
    sem_init(&semFBthread2, 0, 0);
    sem_init(&semP2Done, 0, 0);
...
doFaceDetection(IImageBuffer* Srcbufinfo, int u4SrcWidth, int u4SrcHeight, int u4PostviewWidth, int u4PostviewHeight, MBOOL bSkipDetect)
{
    ...
    if (!bSkipDetect)
    {
        ...
        MY_LOGE("[doFaceDetection] u4SrcWidth = %d u4SrcHeight = %d",u4SrcWidth,u4SrcHeight);
        ...
        do {
            ...
            {
                MY_LOGE("[doFaceDetection] do FD rotation = %d",rotation);
...
//#define _WORKAROUND_MULTI_PLANE
    ...
    if (!mSDKMode)
    {
        ...
        //if (mu4ShotMode != NSCam::eShotMode_FaceBeautyShotCc)
        {
            MY_LOGD("[createFullFrame] use MFB shot \n");
        ...
        {
            MY_LOGD("[createFullFrame] use single shot \n");
        ...
        //
        MUINT32 enGpuProc = queryGpuProcEn();
        //
        pSingleShot->init();
        ...
        if(mSensorType == SENSOR_TYPE_RAW && IssupportVFB == MTRUE && mSDKMode != MTRUE)
        ...
        //
        pSingleShot->sendCommand( NSCamShot::ECamShot_CMD_SET_RWB_PROC_TYPE, enGpuProc, 0, 0 );
        //
    ...
    CAM_TRACE_END();
        ...
        if(mSensorType == SENSOR_TYPE_RAW && IssupportVFB == MTRUE && mSDKMode != MTRUE)
        {
            MY_LOGD("RAW sensor: DO FB by HW path +");
            ...
            if (mu4ShotMode != NSCam::eShotMode_FaceBeautyShotCc)//When in non ZSD mode, need to do FD
            {
                ...
                MY_LOGD("RAW sensor:  Get FD infomation on non-ZSD mode");
                ...
                CAM_TRACE_END();
                 ...
                 MY_LOGD("RAW sensor:  Get FD infomation on ZSD mode");
                 ImgProcess(Srcbufinfo, 0, 0, eImgFmt_I422, buffer_DS_Input, 0, 0, eImgFmt_YUY2, 0);
             ...
             MY_LOGD("RAW sensor:  FDgetresult Found face index %d left %d top %d right %d button %d",mvfb_FBmetadata.number_of_faces,
...
thumbnail_encode()
{
    ...
    if(thumbBuf == NULL)
    {
        MY_LOGE("[thumbnail_encode] thumbBuf alloc fail");
        ret = MFALSE;
        return ret;
    }
...
createFBJpegImg(IImageBuffer* Srcbufinfo, int u4SrcWidth, int u4SrcHeight, MUINT32 const Mode)
{
    ...
    if (mu4ShotMode == NSCam::eShotMode_FaceBeautyShotCc)
    {
        MY_LOGD("RAW sensor: send P2done call Back");
...
doCapture()
{
    ...
    if( mSensorType == SENSOR_TYPE_RAW && IssupportVFB == MTRUE && mSDKMode != MTRUE)
    {
        ...
        if(mSDKMode == MFALSE)
        {
            createFBJpegImg(mpSource, mu4W_yuv, mu4H_yuv,
#ifndef MTK_CAM_VIDEO_FACEBEAUTY_SUPPORT
                    0);
#else
                    1);
    ...
    while(UtilityStatus)
    {
        ...
        switch(UtilityStatus)
        {
            case 3: // memory allocate
                 ...
                 if( mpFbObj->mSensorType == SENSOR_TYPE_RAW && mpFbObj->IssupportVFB == MTRUE && mpFbObj->mSDKMode != MTRUE)
                 {
                     CAM_LOGD("RAW sensor: buffer + mDSWidth = %d mDSHeight = %d",mpFbObj->mDSWidth,mpFbObj->mDSHeight);
                     //mpFbObj->ImgProcess(mpFbObj->mpSource, 0, 0, eImgFmt_YV12, mpFbObj->buffer_DS_Input, 0, 0, eImgFmt_YUY2, 0);

                     mpFbObj->mpFDWorkingBuffer = new unsigned char[FD_WORKING_BUF_SIZE];

                     mpFbObj->buffer_DS_Input  =  mpFbObj->allocMem(eImgFmt_YUY2,
                                                  mpFbObj->mDSWidth,//VFB_DS_IMAGE_MAX_WIDTH,
                                                  mpFbObj->mDSHeight);//VFB_DS_IMAGE_MAX_HEIGHT);

                     mpFbObj->buffer_DS_Output =  mpFbObj->allocMem(eImgFmt_YUY2,
                                                  mpFbObj->mDSWidth,//VFB_DS_IMAGE_MAX_WIDTH,
                                                  mpFbObj->mDSHeight);//VFB_DS_IMAGE_MAX_HEIGHT);

                     mpFbObj->buffer_ALPCL     =  mpFbObj->allocMem(eImgFmt_BAYER8,
                                                  mpFbObj->mDSWidth,//VFB_DS_IMAGE_MAX_WIDTH,
                                                  mpFbObj->mDSHeight);//VFB_DS_IMAGE_MAX_HEIGHT);

                     mpFbObj->buffer_ALPNR     =  mpFbObj->allocMem(eImgFmt_BAYER8,
                                                  mpFbObj->mDSWidth,//VFB_DS_IMAGE_MAX_WIDTH,
                                                  mpFbObj->mDSHeight);//VFB_DS_IMAGE_MAX_HEIGHT);

                     mpFbObj->buffer_PCA       =  mpFbObj->allocMem(eImgFmt_BAYER8,
                                                  160,
                                                   27);

                     mpFbObj->buffer_warp_GPU  =  mpFbObj->allocMem(eImgFmt_BAYER8,
                                                  VFB_DS_IMAGE_MAX_WIDTH * 2 * 2,
                                                  VFB_DS_IMAGE_MAX_HEIGHT * 2);

                     mpFbObj->mpOutputbuffer =    mpFbObj->allocMem(eImgFmt_NV21,
                                                  mpFbObj->mu4W_yuv,
                                                  mpFbObj->mu4H_yuv);
...
requestBufs()
{
    ...
    //if (mSDKMode || ((mu4W_yuv*mu4H_yuv) >= (mShotParam.mi4PostviewWidth * mShotParam.mi4PostviewHeight)))
    {
        ...
        {
            MY_LOGD("[requestBufs] mpSource alloc format YV12");
            mpSource = allocMem(eImgFmt_YV12, mu4W_yuv, mu4H_yuv);
        } else {
            MY_LOGD("[requestBufs] mpSource alloc format I422");
        mpSource = allocMem(eImgFmt_I422, mu4W_yuv, mu4H_yuv);
    }
    }
    //else
...
releaseBufs()
{
    if( mSensorType == SENSOR_TYPE_RAW && IssupportVFB == MTRUE && mSDKMode != MTRUE)
    {
        MY_LOGD("RAW sensor: releaseBufs +");
        deallocMem(mpSource);
...
init_VFB()
{
    MY_LOGD("RAW sensor(CFB): init_VFB +");
    IsVFB = false;
    ...
    MY_LOGD("mHalVFBInit %s full image %dx%d in %d format, down-scaled image %dx%d in %d format", (vfbInfo.RecordingHint)? "recording": "previewing", vfbInfo.SrcImgWidth, vfbInfo.SrcImgHeight, vfbInfo.SrcImgFormat, vfbInfo.DsImgWidth, vfbInfo.DsImgHeight, vfbInfo.DsImgFormat);
...
uninit_VFB()
{
    MY_LOGD("RAW sensor(CFB): uninit_VFB +");
...
do_VFB()
{
    ...
    MY_LOGE("RAW sensor(CFB): SmoothLevel = %d SkinColor = %d FD_w =%d FD_h =%d ExtermeBeauty = %d",VfbProcInfo.SmoothLevel,VfbProcInfo.SkinColor,FDWidth,FDHeight,VfbProcInfo.ExtremeBeauty);
    ...
    for (int i=0; i<mvfb_FBmetadata.number_of_faces; i++)
    {
         ...
         MY_LOGD("RAW sensor(CFB): face num %d x =%d y = %d",mvfb_FBmetadata.number_of_faces, VfbProcInfo.cfb_pos[i][0] ,VfbProcInfo.cfb_pos[i][1]);
    }
    ...
    MY_LOGD("mHalVFBProc Processing QVGA 0x%x, Warped QVGA 0x%x, AlphaCL 0x%x, AlphaNR 0x%x, PCA Table 0x%x, Warp Map 0x%x, 0x%x", VfbProcInfo.DsImgAddr, VfbProcInfo.WarpedDsImage, VfbProcInfo.AlphaCL, VfbProcInfo.AlphaNR, VfbProcInfo.PCATable, VfbProcInfo.WarpMapX, VfbProcInfo.WarpMapY);
...
initP2b()
{
    MY_LOGD("RAW sensor(CFB):  init p2b +");
    ...
    {
        CAM_LOGD("IFeatureStream create instance failed!");
        goto _Exit;
    }
    ...
    if (!mp3AHal)
    {
        MY_LOGD("IHal3A::createInstance failed!");
        goto _Exit;
    }
...
UninitP2b()
{
    MY_LOGD("RAW sensor(CFB): UninitP2b +");
...
{
    MY_LOGD("RAW sensor(CFB): Set vFB PCA@0x%x", pImageBuffer->getBufVA(0));
...
doP2b()
{
    ...
    for (int i=0; i < 2; i++)
    {
        ...
        MY_LOGD("RAW sensor(CFB): [P2B sets mCropRsInfo] groupid(%d),crop w(%d),h(%d),p_int(%d,%d),p_fra(%d,%d), resize w(%d),h(%d)",
...
querySensorInfo(NSCam::SensorStaticInfo &mSensorInfo)
{
    ...
    {
        MY_LOGD("wrong sensor idx(0x%x), sensorNum(%d)", getOpenId(), sensorNum);
        goto lbExit;
    }
    ...
    MY_LOGD("sensorDevID(0x%x), sensorType(0x%x), sensorFormatOrder(0x%x), rawSensorBit(0x%x)"
