        ...
        default:
            if  ( (u4RawImgSize > 0) && (puRawImgBuf != 0) && (ms8RawFilePath != ""))
            {
                ...
                /** Start to simulate pack... **/
                       ...
                       MyDngopImgInfo.Width = engParam.u4SensorWidth; //scrBuffer->getImgSize().w;
                       MyDngopImgInfo.Height = engParam.u4SensorHeight; //scrBuffer->getImgSize().h;
                       MyDngopImgInfo.Stride_src = engParam.u4rawStride; //scrBuffer->getBufStridesInBytes(0);
                       MyDngopImgInfo.Stride_dst = engParam.u4SensorWidth*2; //scrBuffer->getImgSize().w*2;
                       MyDngopImgInfo.BIT_NUM = engParam.u4Bitdepth; //10;//rSensorParam.u4Bitdepth , buffer bit num
                       MyDngopImgInfo.BIT_NUM_DST = 10;
                       MyDngopImgInfo.Bit_Depth = 10; //for pack

                       // query buffer size
                       MUINT32 buf_size = (engParam.u4SensorWidth*2*engParam.u4SensorHeight); //(scrBuffer->getImgSize().w*2*scrBuffer->getImgSize().h);
                       ...
                       MY_LOGD("after Unpack raw16: w (%d),h (%d),src-s (%d),dst-s (%d),buf-size(%d)",
                                engParam.u4SensorWidth, engParam.u4SensorHeight,
                                engParam.u4rawStride, engParam.u4SensorWidth*2,
                                (engParam.u4SensorWidth*2*engParam.u4SensorHeight));
                               /*
                ...
                if (p_buffer)
                {
                    bool ret = saveBufToFile(ms8RawFilePath.string(), p_buffer, buf_size); // bool ret = saveBufToFile(ms8RawFilePath.string(), const_cast<uint8_t*>(puRawImgBuf), u4RawImgSize); 
