...
updateDefaultParams1_ByQuery()
{
    ...
    property_get( "debug.displayrotation.enable", value, "-1");
    ...
    if( val > -1 )
    {
        ...
        if(val==0)
            ...
            enable = true;
        MY_LOGD("(debug.displayrotation.enable)KEY_DISPLAY_ROTATION_SUPPORTED = (%s)", (enable==false) ? "false" : "true");
...
//
    #ifdef MTK_CAM_STEREO_CAMERA_SUPPORT
    {
        ...
        MY_LOGD("ParamsManagerImp : init : SensorId_Main  =  %d ", sensorId_main);
        MY_LOGD("ParamsManagerImp : init : SensorId_Main2 =  %d ", sensorId_main2);
        
        StereoSettingProvider::CheckLogStatus();
        //
        if ( StereoSettingProvider::getStereoParams(sensorId_main, sensorId_main2, stereoParams) )
        {
            mParameters.set(MtkCameraParameters::KEY_STEREO_PICTURE_SIZE, stereoParams.jpsSize);
            mParameters.set(MtkCameraParameters::KEY_SUPPORTED_STEREO_PICTURE_SIZE, stereoParams.jpsSizesStr);
            ...
            mParameters.set(MtkCameraParameters::KEY_STEREO_SENSOR_INDEX_MAIN,  sensorId_main);
            mParameters.set(MtkCameraParameters::KEY_STEREO_SENSOR_INDEX_MAIN2, sensorId_main2);
        }
        else
        {
