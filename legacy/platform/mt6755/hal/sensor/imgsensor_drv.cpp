...
)
{
    ...
    LOG_WRN("sizeof(MUINT16):%d,sizeof(MUINT32):%d, sizeof(UCHAR):%d, sizeof(MUINT64):%d"
            "sizeof(MUINTPTR):%d, sizeof(MUINTPTR *):%d, sizeof(MUINT32 *):%d, sizeof(UCHAR *):%d\n",
            sizeof(MUINT16), sizeof(MUINT32), sizeof(UCHAR), sizeof(MUINT64), sizeof(MUINTPTR),
            sizeof(MUINTPTR *), sizeof(MUINT32 *), sizeof(UCHAR *));
...
//#define SENDCMD_LOG // Open sendcommand log, for test only
    ...
    case CMD_SENSOR_SET_SENSOR_EXP_TIME:
        ...
        if(SENSOR_MAIN == sensorDevId) {
            FeaturePara[0] = (((MUINT64)1000 * (*parg1)) / m_LineTimeInus[0]); // for supporting long exposure time control manually
...
)
{
    ...
    pSensorInfo->SensorVerFOV                            = Info2.SensorVerFOV;
    pSensorInfo->SensorHorFOV                            = Info2.SensorHorFOV;

 //  LOG_MSG("fov ver=%d, hor=%d\n", pSensorInfo->SensorVerFOV,  pSensorInfo->SensorHorFOV);
...
)
{
    ...
    if(SENSOR_MAIN == sensorDevId ) {
        ...
        else if (m_psensorInfo[0]->SensorOutputDataFormat >= SENSOR_OUTPUT_FORMAT_RAW_RWB_B &&
             m_psensorInfo[0]->SensorOutputDataFormat <= SENSOR_OUTPUT_FORMAT_RAW_RWB_R) {
            return IMAGE_SENSOR_TYPE_RAW;
        }
        else if (m_psensorInfo[0]->SensorOutputDataFormat == SENSOR_OUTPUT_FORMAT_RAW_MONO) {
            return IMAGE_SENSOR_TYPE_RAW;
        }
        else {
            return IMAGE_SENSOR_TYPE_UNKNOWN;
        ...
        else if (m_psensorInfo[1]->SensorOutputDataFormat >= SENSOR_OUTPUT_FORMAT_RAW_RWB_B &&
             m_psensorInfo[1]->SensorOutputDataFormat <= SENSOR_OUTPUT_FORMAT_RAW_RWB_R) {
            return IMAGE_SENSOR_TYPE_RAW;
        }
        else if (m_psensorInfo[1]->SensorOutputDataFormat == SENSOR_OUTPUT_FORMAT_RAW_MONO) {
            return IMAGE_SENSOR_TYPE_RAW;
        }
        else {
            return IMAGE_SENSOR_TYPE_UNKNOWN;
