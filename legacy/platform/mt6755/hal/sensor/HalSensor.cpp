...
extern int best_delay_value;//for scam
extern bool Positive_Calibration;
//hwsync drv
#include "hwsync_drv.h"
HWSyncDrv* mpHwSyncDrv=NULL;

//judge boot mode
MUINT32 get_boot_mode(void)
{
  MINT32 fd;
  size_t s;
  char boot_mode[4] = {'0'};
...
)
{
    ...
    //calibration for scam need to be done right after power on sensor in case that FPS was adjusted before start preview,
    // MY_LOGD("sensorDev %d SENSOR_DEV_MAIN %d SENSOR_DEV_SUB%d",sensorDev,SENSOR_DEV_MAIN,SENSOR_DEV_SUB);
#if 1
    if ((sensorDev == SENSOR_DEV_MAIN) &&
        (sensorDrvInfo[0].SensroInterfaceType==SENSOR_INTERFACE_TYPE_SERIAL)&&
        (sensorDrvInfo[0].SCAM_DDR_En)){
        mpSeninfDrv->setSeninf4Ctrl(PAD_10BIT, SERIAL_SENSOR);//PAD_10BIT dont care for scam
        mpSeninfDrv->setSeninf4Scam(1,sensorDrvInfo[0].SCAM_CLK_INV,sensorDrvInfo[0].SensorFullResolutionX,sensorDrvInfo[0].SensorFullResolutionY,1,sensorDrvInfo[0].SCAM_DataNumber,
                sensorDrvInfo[0].SCAM_DDR_En,sensorDrvInfo[0].SCAM_DEFAULT_DELAY, sensorDrvInfo[0].SCAM_CRC_En, sensorDrvInfo[0].SCAM_Timout_Cali, sensorDrvInfo[0].SCAM_SOF_src);
        mpSeninfDrv->setSeninf4Scam(0,sensorDrvInfo[0].SCAM_CLK_INV,sensorDrvInfo[0].SensorFullResolutionX,sensorDrvInfo[0].SensorFullResolutionY,1,sensorDrvInfo[0].SCAM_DataNumber,
                sensorDrvInfo[0].SCAM_DDR_En,sensorDrvInfo[0].SCAM_DEFAULT_DELAY, sensorDrvInfo[0].SCAM_CRC_En, sensorDrvInfo[0].SCAM_Timout_Cali, sensorDrvInfo[0].SCAM_SOF_src);

    } else if((sensorDev == SENSOR_DEV_SUB) &&
           (sensorDrvInfo[1].SensroInterfaceType==SENSOR_INTERFACE_TYPE_SERIAL)&&
           (sensorDrvInfo[1].SCAM_DDR_En)){
       mpSeninfDrv->setSeninf4Ctrl(PAD_10BIT, SERIAL_SENSOR);//PAD_10BIT dont care for scam
       mpSeninfDrv->setSeninf4Scam(1,sensorDrvInfo[1].SCAM_CLK_INV,sensorDrvInfo[1].SensorFullResolutionX,sensorDrvInfo[1].SensorFullResolutionY,1,sensorDrvInfo[1].SCAM_DataNumber,
               sensorDrvInfo[1].SCAM_DDR_En,sensorDrvInfo[1].SCAM_DEFAULT_DELAY, sensorDrvInfo[1].SCAM_CRC_En, sensorDrvInfo[1].SCAM_Timout_Cali, sensorDrvInfo[1].SCAM_SOF_src);
       mpSeninfDrv->setSeninf4Scam(0,sensorDrvInfo[1].SCAM_CLK_INV,sensorDrvInfo[1].SensorFullResolutionX,sensorDrvInfo[1].SensorFullResolutionY,1,sensorDrvInfo[1].SCAM_DataNumber,
               sensorDrvInfo[1].SCAM_DDR_En,sensorDrvInfo[1].SCAM_DEFAULT_DELAY, sensorDrvInfo[1].SCAM_CRC_En, sensorDrvInfo[1].SCAM_Timout_Cali, sensorDrvInfo[1].SCAM_SOF_src);
...
)
{
    ...
    /* close sensor interface clock source
    ret = setTgPhase(sensorDev, pcEn);
    if (ret < 0) {
        MY_LOGE("setTgPhase fail n");
        return MFALSE;
    ...
    if (mpSensorDrv) {
        ...
        if( sensorDev & SENSOR_DEV_SUB) {//sub cam power off  reset cal flag for scam
            MY_LOGD("!!!scam clear calibration flag\n");
            best_delay_value = -1;
            Positive_Calibration = false;//calibration should re-start once sensor was shut down
        }
    }
    else {
