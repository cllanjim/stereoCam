    ...
    SeninfDrv()
{
    ...
    mpCSI2RxAnalog0RegAddr = NULL;
    mpCSI2RxAnalog1RegAddr = NULL;
    mpCSI2RxAnalog2RegAddr = NULL;
    mpGpioHwRegAddr = NULL;
    mCSI1 = 0; /* initial CSI value*/
    mCSI2 = 0;
    m_pIspDrv = NULL;
    mDevice = 0;
}
...
int SeninfDrvImp::init()  //kk test
{
    ...
    MINT32 gpio_base_addr = 0x10002000; /* GPIO Baseaddress */
    ...
    if (mpCSI2RxAnalogRegStartAddrAlign == MAP_FAILED) {
     ...
     //MMAP only support Page alignment(0x1000), ReMap to CSI2 base addr 0x1021_7000
    mpCSI2RxAnalogRegStartAddr = mpCSI2RxAnalogRegStartAddrAlign;
    ...
    mpCAMIODrvRegAddr = mpGpioHwRegAddr + (0x6b0 / 4); // DRV_CFG1
    ...
    // Read CIS efuse value
    {
        ...
        unsigned int devinfo_data = 14; /* CSI section*/
        unsigned int devinfo_data1 = 15; /* CSI section*/

        mCSI1 = 0; /* initial CSI value*/
        mCSI2 = 0; /* initial CSI value*/
        ...
        else
        {
            ...
            if(ioctl(fd, READ_DEV_DATA, &devinfo_data1)<0)
            {
                LOG_ERR("Get devinfo_data1 fail");

            }
            mCSI1 = devinfo_data;
            mCSI2 = devinfo_data1;
            LOG_MSG("csi_value(0x%08x,0x%08x)", mCSI1, mCSI2);
	...
	typedef struct {
        ...
        fd = open("/sys/bus/platform/drivers/dev_info/dev_info", O_RDONLY); /* v1 device node */
        if(fd < 0)
        {
            LOG_ERR("devinfo open fail, errno(%d):%s",errno,strerror(errno));
        }
        else
        {
            ...
            {	
		mCSI1  = devinfo.data[14];
		mCSI2  = devinfo.data[15];
		LOG_MSG("csi_value(0x%08x,0x%08x)", mCSI1, mCSI2);
            }

        #if 0
            /*print Devinfo*/
            LOG_ERR("Total devinfo number: %d\n", devinfo.entry_num);
            for(index = 0; index < devinfo.entry_num; index++)
            {
                LOG_ERR("Devinfo Index:%d\n", index);
                LOG_ERR("Get Devinfo Data:0x%x\n", devinfo.data[index]);
            }
        #endif
            //
            close(fd);
...
)
{
 ...
 #endif
    LOG_MSG("[setTg1PhaseCounter] pcEn(%d) clkPol(%d) mMclk1User(%d)\n",(MINT32)pcEn,(MINT32)clkPol,mMclk1User);
    //
    clkRiseEdge = 0;
...
)
{
    ...
    LOG_MSG("[setTg2PhaseCounter] pcEn(%d) clkPol(%d)  mMclk2User(%d)\n",(MINT32)pcEn,(MINT32)clkPol, mMclk2User);
    //
       clkRiseEdge = 0;
    clkFallEdge = (clkCnt > 0)? (clkCnt+1)>>1 : 0;//avoid setting larger than clkCnt
...
)
{
   ...
   if (mclkSel == CAM_PLL_48_GROUP) {
    ...
    LOG_MSG("[setTg3PhaseCounter] pcEn(%d) clkPol(%d) mMclk3User(%d)\n",(MINT32)pcEn,(MINT32)clkPol,mMclk3User);
...
#if 1
             seninf_reg_t *pSeninf = (seninf_reg_t *)mpSeninfHwRegAddr;
             unsigned int temp1 = SENINF_READ_REG(pSeninf,SENINF1_NCSI2_INT_STATUS);
             unsigned int temp2 = SENINF_READ_REG(pSeninf,SENINF2_NCSI2_INT_STATUS);
             if((temp1&0xf21)){
...
)
{
    ...
    ISP_BITS(pisp, CAM_TG2_SEN_MODE, SOF_SRC) = sof_Src;
...
)
{
        ...
        SENINF_WRITE_REG(pSeninf,SENINF2_NCSI2_DI,((vc3Id)<<24)|((vc2Id)<<16)|((vc1Id)<<8)|(vc0Id));
...
)
{
    ...
    if(ncsi2En == 1) {  // enable CSI2
        ...
        mt65xx_reg_writel(temp|0x00000103, mpCSI2RxAnalog0RegAddr + (0x20/4));
        usleep(1);
        ...
        if(mCSI1 != 0)
        {
            ...
            mt65xx_reg_writel((temp&(~0xf000))|((mCSI2&0x000000f0)<<8), mpCSI2RxAnalog0RegAddr + (0x04/4));
            temp = *(mpCSI2RxAnalog0RegAddr + (0x08/4));
            mt65xx_reg_writel((temp&(~0xf000))|((mCSI2&0x0000000f)<<12), mpCSI2RxAnalog0RegAddr + (0x08/4));
            temp = *(mpCSI2RxAnalog0RegAddr + (0x0C/4));
            mt65xx_reg_writel((temp&(~0xf000))|((mCSI1&0xf0000000)>>16), mpCSI2RxAnalog0RegAddr + (0x0C/4));
            temp = *(mpCSI2RxAnalog0RegAddr + (0x10/4));
            mt65xx_reg_writel((temp&(~0xf000))|((mCSI1&0x0f000000)>>12), mpCSI2RxAnalog0RegAddr + (0x10/4));
            temp = *(mpCSI2RxAnalog0RegAddr );
            mt65xx_reg_writel((temp&(~0xf000))|((mCSI1&0x00f00000)>>8), mpCSI2RxAnalog0RegAddr);

            LOG_MSG("CSI-0 EFUSE 0x00=0x%x, 0x04=0x%x,0x08=0x%x, 0x0c=0x%x, 0x10=0x%x\n",*(mpCSI2RxAnalog0RegAddr)
        ...
        msettleDelay = SETTLE_DELAY_COUNT;//fix count
...
)
{
    ...
    //ToDo: add GPIO config & MIPI RX config
     ...
     if(ncsi2En == 1) {  // enable CSI2
         ...
         mt65xx_reg_writel(temp|0x00000103, mpCSI2RxAnalog1RegAddr + (0x20/4));
         usleep(1);
         ...
         if(mCSI1 != 0)
         {
             ...
             mt65xx_reg_writel((temp&(~0xf000))|((mCSI1&0x000f0000)>>4), mpCSI2RxAnalog1RegAddr + (0x04/4));
             temp = *(mpCSI2RxAnalog1RegAddr + (0x08/4));
             mt65xx_reg_writel((temp&(~0xf000))|((mCSI1&0x0000f000)), mpCSI2RxAnalog1RegAddr + (0x08/4));
             temp = *(mpCSI2RxAnalog1RegAddr + (0x0C/4));
             mt65xx_reg_writel((temp&(~0xf000))|((mCSI1&0x00000f00)<<4), mpCSI2RxAnalog1RegAddr + (0x0C/4));
             temp = *(mpCSI2RxAnalog1RegAddr + (0x10/4));
             mt65xx_reg_writel((temp&(~0xf000))|((mCSI1&0x000000f0)<<8), mpCSI2RxAnalog1RegAddr + (0x10/4));
             temp = *(mpCSI2RxAnalog1RegAddr );
             mt65xx_reg_writel((temp&(~0xf000))|((mCSI1&0x0000000f)<<12), mpCSI2RxAnalog1RegAddr);

             LOG_MSG("CSI-1 EFUSE 0x00=0x%x, 0x04=0x%x,0x08=0x%x, 0x0c=0x%x, 0x10=0x%x\n",*(mpCSI2RxAnalog1RegAddr)
...
#endif
        msettleDelay = SETTLE_DELAY_COUNT;
       ...
       //DPCM Enable
        ...
        SENINF_WRITE_REG(pSeninf, SENINF2_NCSI2_DPCM, temp);//CSI2 must be slected when sensor clk output
...
)
{
    ...
    //ToDo: add GPIO config & MIPI RX config
     ...
     if(ncsi2En == 1) {  // enable CSI2
         ...
         mt65xx_reg_writel(temp|0x00000103, mpCSI2RxAnalog2RegAddr + (0x20/4));
         usleep(1);
        ...
        SENINF_WRITE_REG(pSeninf, SENINF3_NCSI2_DPCM, temp);//CSI2 must be slected when sensor clk output
...
int SeninfDrvImp::setMclk2IODrivingCurrent(unsigned long ioDrivingCurrent)
{
    ...
    if(mpCAMIODrvRegAddr != NULL) {
        *(mpCAMIODrvRegAddr) &= ~0x600;
        *(mpCAMIODrvRegAddr) |= (ioDrivingCurrent<<9); // [2:1] = 0:2mA, 1:4mA, 2:6mA, 3:8mA
        LOG_MSG("[setIODrivingCurrent]:%d 0x%08x\n", (int) ioDrivingCurrent, (int) (*(mpCAMIODrvRegAddr)));
    }
    return ret;
}
...
MUINT32 SeninfDrvImp::getISPClk(){
    int ret =0;
