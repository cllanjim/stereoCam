...
)
{
    ...
    /*support only use one back or front camera */
    if((eSensorDev & SENSOR_DEV_MAIN_2) == SENSOR_DEV_MAIN_2)
       u4sensorIndex = 2;
    else if((eSensorDev & SENSOR_DEV_SUB) == SENSOR_DEV_SUB)
       u4sensorIndex = 1;
    else
       u4sensorIndex = 0; 
    
    buildStaticInfo(Info(u4sensorIndex, eSensorType, szSensorDrvName), rEnumInfo.mMetadata);
    //
    return  &rEnumInfo;
}
