...
extern MUINT32 get_boot_mode(void);
/******************************************************************************
 *
 ******************************************************************************/
...
)
{
    ...
    property_get("camera.hwsync.enable", value, "1");
    ...
    MUINTPTR arg3)
{
   ...
   case SENSOR_CMD_GET_SENSOR_PDAF_INFO:
    ...
    case SENSOR_CMD_GET_SENSOR_ROLLING_SHUTTER:
        ...
        pSensorDrv->sendCommand((SENSOR_DEV_ENUM)sensorDevId,cmdId, arg1, arg2);
        break;
    case SENSOR_CMD_GET_SENSOR_POWER_ON_STETE:
        *((MUINT32*) arg1) = (meSensorDev & sensorDevId);
        break;
    case SENSOR_CMD_DEBUG_P1_DQ_SENINF_STATUS:
...
MVOID HalSensor::pixelModeArrange(MUINT currSensorDev)
{
    ...
    if(currSensorDev & SENSOR_DEV_MAIN ) {
        if (sensorStaticInfo[0].sensorType == SENSOR_TYPE_RAW){
            MY_LOGD("SENSOR_DEV_MAIN bandwidth %d, two pixel mode threshold %d\n",
                (sensorPara[0].cropWidth * sensorPara[0].cropHeight/1000 * (sensorPara[0].frameRate/10)), Isp_clk*TWO_PIXEL_MODE_MARGIN);
   ...
   if(currSensorDev & SENSOR_DEV_SUB ) {
        if (sensorStaticInfo[1].sensorType == SENSOR_TYPE_RAW){
            MY_LOGD("SENSOR_DEV_SUB bandwitdh %d, two pixel mode threshold %d\n",
                (sensorPara[1].cropWidth * sensorPara[1].cropHeight/1000 * (sensorPara[1].frameRate/10)), Isp_clk*TWO_PIXEL_MODE_MARGIN);
   ...
   if(currSensorDev & SENSOR_DEV_MAIN_2 ) {
        if (sensorStaticInfo[2].sensorType == SENSOR_TYPE_RAW){
             MY_LOGD("SENSOR_DEV_MAIN_2 bandwitdh %d, two pixel mode threshold %d\n",
                (sensorPara[2].cropWidth * sensorPara[2].cropHeight/1000 * (sensorPara[2].frameRate/10)), Isp_clk*TWO_PIXEL_MODE_MARGIN);
        ...
        switch(meSensorDev) {
            case SENSOR_DEV_MAIN:
                ...
                ret = pSeninfDrv->setTg1Cfg((TG_FORMAT_ENUM)sensorPara[0].inDataType, (SENSOR_DATA_BITS_ENUM)sensorPara[0].senInLsb, sensorDynamicInfo[0].pixelMode,
                    sensorPara[0].inSrcTypeSel == SERIAL_SENSOR ?1 :0);
                ...
                if (sensorPara[0].inSrcTypeSel >= VIRTUAL_CHANNEL_1) {
                    ...
                    if(sensorPara[0].mipiPad == 0) {
                        if(sensorPara[0].MIPI_OPHY_TYPE == 1) // CSI2
                        {
                            ...
                            ret = pSeninfDrv->setSeninf1NCSI2(sensorPara[0].u1MIPIDataTermDelay,sensorPara[0].u1MIPIDataSettleDelay,sensorPara[0].u1MIPIClkTermDelay,
                                sensorPara[0].u1VsyncPol,sensorPara[0].u1MIPILaneNum,1,sensorPara[0].u1MIPIPacketECCOrder,1,sensorPara[0].NCSI2_HSRXDET_MODE,sensorPara[0].DPCM_TYPE);
                        }
                        else
                        {
                    ...
                    ret = pSeninfDrv->setSeninf4Scam(1,sensorPara[0].SCAM_CLK_INV,sensorPara[0].cropWidth,sensorPara[0].cropHeight,1,sensorPara[0].SCAM_DataNumber,
						sensorPara[0].SCAM_DDR_En,sensorPara[0].SCAM_DEFAULT_DELAY, sensorPara[0].SCAM_CRC_En, sensorPara[0].SCAM_Timout_Cali, sensorPara[0].SCAM_SOF_src);
            ...
            case SENSOR_DEV_SUB:
                ...
                ret = pSeninfDrv->setTg1Cfg((TG_FORMAT_ENUM)sensorPara[1].inDataType, (SENSOR_DATA_BITS_ENUM)sensorPara[1].senInLsb, sensorDynamicInfo[1].pixelMode,
                                        sensorPara[1].inSrcTypeSel == SERIAL_SENSOR ?1 :0);
                ...
                if (sensorPara[1].inSrcTypeSel >= MIPI_SENSOR) {
                    if(sensorPara[1].mipiPad == 2) {
                        ...
                        if(sensorPara[1].MIPI_OPHY_TYPE == 1) // CSI2
                        {
                            ...
                            ret = pSeninfDrv->setSeninf3NCSI2(sensorPara[1].u1MIPIDataTermDelay,sensorPara[1].u1MIPIDataSettleDelay,sensorPara[1].u1MIPIClkTermDelay,
                                sensorPara[1].u1VsyncPol,sensorPara[1].u1MIPILaneNum,1,sensorPara[1].u1MIPIPacketECCOrder,1,sensorPara[1].NCSI2_HSRXDET_MODE,sensorPara[1].DPCM_TYPE);
                        }
                        else
                        {
                    ...
                    ret = pSeninfDrv->setSeninf4Scam(1,sensorPara[1].SCAM_CLK_INV,sensorPara[1].cropWidth,sensorPara[1].cropHeight,1,sensorPara[1].SCAM_DataNumber,
						sensorPara[1].SCAM_DDR_En,sensorPara[1].SCAM_DEFAULT_DELAY, sensorPara[1].SCAM_CRC_En, sensorPara[1].SCAM_Timout_Cali, sensorPara[1].SCAM_SOF_src);
                ...
                /// [Important] The rule of TG path for main cam2
                /// [reason] The line buffer in TG2 is limited in this platform, and TG1 is not limited.
                ///             So we need sub cam to use TG2 and main cam to use TG1 when main cam needs to output larger image size, e.x.: full size.
                /// [Rule] If the current scenario (sensor mode) is VT mode (.slim_video), let main cam2 use TG2.
                ///        Or, let sub cam use TG1.

                // 1. Main cam2 uses TG1 if not in SENSOR_SCENARIO_ID_SLIM_VIDEO2 mode (VT)
                if(SENSOR_SCENARIO_ID_SLIM_VIDEO2 != sensorPara[2].scenarioId) {
                ...
                // 2. Main cam2 uses TG2 if in SENSOR_SCENARIO_ID_SLIM_VIDEO2 mode (VT)
                else {
                ...
                if (sensorPara[2].inSrcTypeSel >= MIPI_SENSOR) {
                    if(sensorPara[2].mipiPad == 2) {
                        //sensorPara[2].MIPI_OPHY_TYPE=1; //For Test only
                        if(sensorPara[2].MIPI_OPHY_TYPE == 1) // CSI2
                        {
                            ...
                            ret = pSeninfDrv->setSeninf3NCSI2(sensorPara[2].u1MIPIDataTermDelay,sensorPara[2].u1MIPIDataSettleDelay,sensorPara[2].u1MIPIClkTermDelay,
                                sensorPara[2].u1VsyncPol,sensorPara[2].u1MIPILaneNum,1,sensorPara[2].u1MIPIPacketECCOrder,1,sensorPara[2].NCSI2_HSRXDET_MODE,sensorPara[2].DPCM_TYPE);
                        }
                        else
                        {
                    ...
                    ret = pSeninfDrv->setSeninf4Ctrl((PAD2CAM_DATA_ENUM)sensorPara[2].padSel, (SENINF_SOURCE_ENUM)sensorPara[2].inSrcTypeSel);
                     ret = pSeninfDrv->setSeninf4Scam(1,sensorPara[2].SCAM_CLK_INV,sensorPara[2].cropWidth,sensorPara[2].cropHeight,1,sensorPara[2].SCAM_DataNumber,
						sensorPara[2].SCAM_DDR_En,sensorPara[2].SCAM_DEFAULT_DELAY, sensorPara[2].SCAM_CRC_En, sensorPara[2].SCAM_Timout_Cali, sensorPara[2].SCAM_SOF_src);
            ...
            case (SENSOR_DEV_MAIN|SENSOR_DEV_MAIN_2):
                switch (currSensorDev) {
                    case (SENSOR_DEV_MAIN|SENSOR_DEV_MAIN_2):
                        //main
                        MY_LOGD("seninfControl setting main &main2 of man & main2 tginfo0, %d tginfo2 %d, mipi pad %d\n",
                        ...
                        else if((sensorPara[0].inSrcTypeSel >= MIPI_SENSOR) && (sensorPara[2].inSrcTypeSel == PARALLEL_SENSOR)) {
                            if(sensorPara[0].mipiPad == 0) {
                                ret = pSeninfDrv->setSeninf1Ctrl((PAD2CAM_DATA_ENUM)sensorPara[0].padSel, (SENINF_SOURCE_ENUM)sensorPara[0].inSrcTypeSel);
                                ret = pSeninfDrv->setSeninf1NCSI2(sensorPara[0].u1MIPIDataTermDelay,sensorPara[0].u1MIPIDataSettleDelay,sensorPara[0].u1MIPIClkTermDelay,
                                    sensorPara[0].u1VsyncPol,sensorPara[0].u1MIPILaneNum,1,sensorPara[0].u1MIPIPacketECCOrder,0,sensorPara[0].NCSI2_HSRXDET_MODE,sensorPara[0].DPCM_TYPE);
                    ...
                    case SENSOR_DEV_MAIN://SENSOR_DEV_MAIN|SENSOR_DEV_MAIN_2
                        MY_LOGD("seninfControl setting main2 of man & main2 tginfo0, %d tginfo2 %d, mipi pad %d\n",
                            sensorDynamicInfo[0].TgInfo,sensorDynamicInfo[2].TgInfo,sensorPara[0].mipiPad);
