...
MVOID HalSensorList::querySensorInfo()
{
    ...
    for(idx=0; idx<1; idx++) {
        ...
        switch (sensorRawInfo[idx].sensorType) {
         ...
         switch (sensorRawInfo[idx].u1Order) {
             ...
             case SENSOR_OUTPUT_FORMAT_RAW_RWB_B:
                sensorStaticInfo[idx].rawFmtType = SENSOR_RAW_RWB;
                sensorStaticInfo[idx].sensorFormatOrder = SENSOR_FORMAT_ORDER_RAW_B;
                break;
             ...
             case SENSOR_OUTPUT_FORMAT_RAW_MONO:
                sensorStaticInfo[idx].sensorFormatOrder = SENSOR_FORMAT_ORDER_RAW_B;
                sensorStaticInfo[idx].rawFmtType = SENSOR_RAW_MONO;
                break;
             default:
                sensorStaticInfo[idx].sensorFormatOrder = SENSOR_FORMAT_ORDER_NONE;
                sensorStaticInfo[idx].rawFmtType = SENSOR_RAW_FMT_NONE;
                break;
        ...
        sensorStaticInfo[idx].PerFrameCTL_Support = sensorDrvInfo[idx].PerFrameCTL_Support;
	if(sensorDrvInfo[idx].SensorHorFOV != 0)
		sensorStaticInfo[idx].horizontalViewAngle = sensorDrvInfo[idx].SensorHorFOV;
	if(sensorDrvInfo[idx].SensorVerFOV != 0)
		sensorStaticInfo[idx].verticalViewAngle = sensorDrvInfo[idx].SensorVerFOV;
	//MY_LOGD("fov idx %d, hor %d, ver = %d\n",idx, sensorStaticInfo[idx].horizontalViewAngle, sensorStaticInfo[idx].verticalViewAngle);
    }
