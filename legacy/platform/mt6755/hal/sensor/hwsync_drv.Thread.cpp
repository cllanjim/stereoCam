...
extern pthread_mutex_t mWaitRevertMutex;
        ...
        switch(cmd)
        {
            case ExecCmd_CHECKDIFF:
                pthread_mutex_lock(&mStateMutex);
                 if(_this->mState==HW_SYNC_STATE_IDLE || _this->mState==HW_SYNC_STATE_MODESWITCH)
                {
                    pthread_mutex_unlock(&mStateMutex);   
                    pthread_mutex_lock(&mEnterIdleMutex);
                     _this->mbPostSem4Idle=true;
                     pthread_mutex_unlock(&mEnterIdleMutex);
                     ::sem_post(&(_this->mSem2Idle));
                }
                else
                {
                     pthread_mutex_unlock(&mStateMutex);
            ...
            case ExecCmd_DOSYNC:
                ...
                else if(_this->mState==HW_SYNC_STATE_IDLE)
                {
                    pthread_mutex_unlock(&mStateMutex);
                    LOG_INF("postt1");
                    ::sem_post(&(_this->mSemJumpoutSync)); 
