...
public:
     ...
     */
    virtual MINT32   	init(HW_SYNC_USER_ENUM user, MUINT32 sensorIdx);
    /**
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     * - MFALSE indicates failure.
     */    
	virtual MINT32 		sendCommand(HW_SYNC_CMD_ENUM cmd,MUINT32 senDev,MUINT32 senScen, MUINT32 currfps, HW_SYNC_CLOCK_STAGE_ENUM clockstage=HW_SYNC_CLOCK_STAGE_0);
...
public:
    ...
    sem_t      mSem2Idle;
    bool        mNeedwaitRevertFps;
    bool        mbPostSem4Idle;
    bool        mEnableSync4Stereo;    
private:
    //[general]
    mutable  Mutex      mLock;
    ...
    MINT32              mSensorCurFpsArray[2]; //real current fps of sensor, osrted by the same orfer with  mSensorIdxList (0:mSensorDevArray[0], 1:mSensorDevArray[1], maxNum=2), UNIT: 10 base
    MUINT32             mSensorScenArray[2];//osrted by the same orfer with  mSensorIdxList (0:mSensorDevArray[0], 1:mSensorDevArray[1], maxNum=2)
    MINT32               mSensorModeToggle[2]; //used to judge that user change mode
    MINT32              mSensorUserFpsArray[2]; //sensor fps set by user when send command, osrted by the same orfer with  mSensorIdxList (0:mSensorDevArray[0], 1:mSensorDevArray[1], maxNum=2), UNIT: 10 base
    ...
    MINT32              mCurDftFps;     //default fps in current mode, UNIT: 10 base, ex 30fps->300
    MINT32              mfpsUB;
    MINT32              mfpsLB; 
