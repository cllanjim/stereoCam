...
enum ERecord_flash_state
{
    ERecord_off_flash_off = 0,
    ERecord_on,
    ERecord_on_flash_on   
};
...
Hal3AFlowCtrl::Hal3AFlowCtrl()
    ...
    , m_3ALogEnable(0)
    , m_3ACtrlEnable(0)
    , mbVideoFlashOn(ERecord_off_flash_off)
...
init(MINT32 i4SensorDevId, MINT32 i4SensorOpenIndex) //can be called only once by RAW or YUV, no user count
{
    ...
    mbMainFlashOnThisFrame = MFALSE;
    mbVideoFlashOn = ERecord_off_flash_off;
    IEventIrq::ConfigParam IrqConfig(i4SensorDevId, 0, IEventIrq::E_Event_Vsync);
...
MBOOL Hal3AFlowCtrl::postCommand(ECmd_T const eCmd, MINTPTR const i4Arg)
{
    ...
    if (fgUpdate)
    {
        ...
        if ((mbMainFlashOnThisFrame||bPre) && (u4MagicNum != 0 ||(m_rParam.u4StrobeMode == MTK_FLASH_MODE_TORCH))) 
			mLastFlashOnFrmIdx = u4MagicNum;
        if (mbMainFlashOnThisFrame) setFlashLightOnOff(1, 1);
        if (bPre) setFlashLightOnOff(1, 0);
		MY_LOG_IF(m_3ALogEnable, "mbMainFlashOnThisFrame(%d), bPre(%d), mLastFlashOnFrmIdx(%d)\n", mbMainFlashOnThisFrame, bPre, mLastFlashOnFrmIdx);
    }
...
Hal3AFlowCtrl::isMeetMainFlashOnCondition()
{
    ...
    MINT32 supportFlash = 0;
    FlashMgr::getInstance().hasFlashHw(m_i4SensorDev, supportFlash);
        
    if (!supportFlash)
    {
        if (fgFlashOn)
        {
            MY_ERR("[%s] should not turn on flash.", __FUNCTION__);
        }
        return MFALSE;
    }
    return fgFlashOn;
...
{
    if ((m_rParam.u1CaptureIntent == MTK_CONTROL_CAPTURE_INTENT_VIDEO_RECORD) || (m_rParam.u1CaptureIntent == MTK_CONTROL_CAPTURE_INTENT_VIDEO_SNAPSHOT))
    {
        if (!mbVideoFlashOn)
        {
            mbVideoFlashOn = ERecord_on;
            if ((m_rParam.u4AeMode == MTK_CONTROL_AE_MODE_ON_AUTO_FLASH) && isStrobeBVTrigger())
                mbVideoFlashOn = ERecord_on_flash_on;
        }
    }
    else
        mbVideoFlashOn = ERecord_off_flash_off;

    MBOOL fgFlashOn = (((m_rParam.u4AeMode == MTK_CONTROL_AE_MODE_OFF)||(m_rParam.u4AeMode == MTK_CONTROL_AE_MODE_ON)) && (m_rParam.u4StrobeMode == MTK_FLASH_MODE_TORCH))
        || (mbVideoFlashOn == ERecord_on_flash_on);
    MINT32 supportFlash = 0;
    FlashMgr::getInstance().hasFlashHw(m_i4SensorDev, supportFlash);
    if (!supportFlash)
