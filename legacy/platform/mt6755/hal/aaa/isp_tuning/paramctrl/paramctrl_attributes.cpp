...
{
    MY_LOG_IF(m_bDebugEnable,"[+updateShadingNVRAMdata](old, new)=(%d, %d)", m_fgShadingNVRAMdataChange, fgEnable);
    ...
    MY_LOG_IF(m_bDebugEnable,"[-updateShadingNVRAMdata] return");
    return;
}
#endif
...
setEffect(EIndex_Effect_T const eEffect)
{
    ...
    {
        MY_LOG_IF(m_bDebugEnable,"[+setEffect] effect(old, new)=(%d, %d)", m_rIspCamInfo.eIdx_Effect, eEffect);
    ...
    if (isDynamicCCM())
    {
        if (m_rIspParam.bInvokeSmoothCCM) { // smooth CCM 
            ...
            if ( checkParamChange(eIdx_CCM_old, eIdx_CCM_new) )
            {
                ...
                MY_LOG_IF(m_bDebugEnable,"[setAWBInfo][ParamChangeCount:%d]" "CCM index(old, new) =(%d, %d)", getParamChangeCount(), eIdx_CCM_old, eIdx_CCM_new);
            }
        }
    }
...
sendIspTuningIOCtrl(MUINT32 ctrl, MINTPTR arg1, MINTPTR arg2)
{
    ...
    case IspTuningMgr::E_ISPTUNING_GET_ISP_GAMMA:
         getIspGamma(reinterpret_cast<MUINT32 *>(arg1), reinterpret_cast<MBOOL*>(arg2));
        break;
