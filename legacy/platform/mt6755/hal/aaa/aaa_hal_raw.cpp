...
if (ENABLE_AE & m_3ACtrlEnable) {
    IAeMgr::getInstance().cameraPreviewInit(m_i4SensorDev, m_i4SensorOpenIdx, m_rParam);
}
if (ENABLE_AF & m_3ACtrlEnable) {
    IAfMgr::getInstance().setCallbacks(m_i4SensorDev, mpCbSet);
    IAfMgr::getInstance().init(m_i4SensorDev, m_i4SensorOpenIdx);
}
if (ENABLE_FLASH & m_3ACtrlEnable) {
    //FlashMgr::getInstance().setStrobeCallbackFunc(m_i4SensorDev, notifyStrobeMode);
    FlashMgr::getInstance().init(m_i4SensorDev, m_i4SensorOpenIdx);
}
if (ENABLE_AWB & m_3ACtrlEnable) {
   IAwbMgr::getInstance().init(m_i4SensorDev, m_i4SensorOpenIdx);
}
}
...
uninitRaw()
{
    ...
    {
        if (ENABLE_AF & m_3ACtrlEnable) {
        IAfMgr::getInstance().uninit(m_i4SensorDev);
    }
...
if (ENABLE_FLICKER & m_3ACtrlEnable) {
        ...
        if (ENABLE_AE & m_3ACtrlEnable) {
        IAeMgr::getInstance().uninit(m_i4SensorDev);
        }
        if (ENABLE_FLASH & m_3ACtrlEnable) {
        FlashMgr::getInstance().uninit(m_i4SensorDev);
        }
        if (ENABLE_AWB & m_3ACtrlEnable) {
            IAwbMgr::getInstance().uninit(m_i4SensorDev);
        }
/*
        delete mpScheduler;
        mpScheduler = NULL;
...
Hal3ARaw::notifyP1Done(MUINT32 u4MagicNum, MVOID* pvArg)
{
    ...
    if (u4MagicNum == mLastFlashOnFrmIdx && !(isMeetPreFlashOnCondition()) && !(isMeetMainFlashOnCondition()))
    {
...
MBOOL
Hal3ARaw::notifyPwrOn()
{
    return IAfMgr::getInstance().CamPwrOnState(m_i4SensorDev);
}
MBOOL
Hal3ARaw::notifyPwrOff()
{
    return IAfMgr::getInstance().CamPwrOffState(m_i4SensorDev);
}

MBOOL
Hal3ARaw::checkCapFlash()
{
...
getCurrResult(MUINT32 u4FrmId, Result_T& rResult) const
{
    ...
    if (m_rParam.u1ShadingMapMode == MTK_STATISTICS_LENS_SHADING_MAP_MODE_ON)
    {
        ...
        NSIspTuningv3::LscMgr2::getInstance(static_cast<ESensorDev_T>(m_i4SensorDev))->getGainTable(3, m_rParam.u1ShadingMapXGrid, m_rParam.u1ShadingMapYGrid, pfShadingMap);
    }
	...
	if (u4FrmId == 0xffffffff ||u4FrmId == 0x00000001)
	{
            rResult.vecExifInfo.editTop().u4FlashLightTimeus = 30000;
        }
    }
...
MBOOL Hal3ARaw::postCommandDerived(ECmd_T const eCmd, MINTPTR const i4Arg)
{
    ...
    {
        updateTGInfo();
        IspTuningMgr::getInstance().validate(m_i4SensorDev, 0, MTRUE);
    }
    ...
    mpStateMgr->updateMainFlashOnThisFrame(mbMainFlashOnThisFrame);

    MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG,"[Hal3ARaw::postCommand] before StateMgr::sendCmd(%d)\n", eCmd);
    if ((m_rParam.u1CaptureIntent == MTK_CONTROL_CAPTURE_INTENT_STILL_CAPTURE) && (eCmd == ECmd_Update) && (m_rParam.u4AeMode != MTK_CONTROL_AE_MODE_OFF))
    {
        if (mpStateMgr->getStateStatus().eCurrState == eState_Precapture)
        {
            MY_LOG("Unexpected Operation since precapture is not finished.");
            mpStateMgr->sendCmd(ECmd_PrecaptureEnd);
        }
        //else
        if(!mbMainFlashOnThisFrame && !mbIsHDRShot) //update when capture without flash, avoid override HDR updateCaptureParams if flag is raised.
        {
...
MBOOL Hal3ARaw::setParams(Param_T const &rNewParam)
{
    ...
    {
        // focus dist
        if (m_rParam.fFocusDistance != rNewParam.fFocusDistance)
...
if (ENABLE_3A_GENERAL & m_3ACtrlEnable) {
    if ((m_rParam.u4AfMode != MTK_CONTROL_AF_MODE_CONTINUOUS_PICTURE)
       && (m_rParam.u4AfMode != MTK_CONTROL_AF_MODE_CONTINUOUS_VIDEO)
       && m_rParam.u4AfMode != MTK_CONTROL_AF_MODE_OFF)   {
...
{
    MBOOL res = MTRUE;
    MY_LOG("[%s] flowtype == %d", __FUNCTION__, flowType);

    if (flowType == 0)
    {
        //processed raw
        res = validatePass2(flowType, rCamInfo, pRegBuf);
    }
    if (flowType == 1)
    {
        setPureFlowPass2();
...
MVOID Hal3ARaw::setPureFlowPass2()
{
    ...
    if (ENABLE_3A_GENERAL & m_3ACtrlEnable) {
    if (ENABLE_AAOBUF & m_3ACtrlEnable) {
        // Dequeue AAO DMA buffer
        IAAOBufMgr::getInstance().dequeueHwBuf(m_i4SensorDev, rBufInfo);
    }
    if (ENABLE_AE & m_3ACtrlEnable) {
        i4SceneLV = IAeMgr::getInstance().getLVvalue(m_i4SensorDev, MFALSE);
    ...
    if (ENABLE_AWB & m_3ACtrlEnable) {
        ...
        {
        // One-shot AWB
        IAwbMgr::getInstance().doCapAWB(m_i4SensorDev, i4AoeCompLv, reinterpret_cast<MVOID *>(rBufInfo.virtAddr));
        MY_LOG("[%s] IAwbMgr::getInstance().doCapAWB() END",__FUNCTION__);
        }
    }
    if (ENABLE_AE & m_3ACtrlEnable) {
        if (get3ACapMode() == (ECapMode_P2_Cal|ECapMode_P2_Set))
        {
            //Capture Flare compensate
            IAeMgr::getInstance().doCapFlare(m_i4SensorDev, reinterpret_cast<MVOID *>(rBufInfo.virtAddr),
                FlashMgr::getInstance().isFlashOnCapture(m_i4SensorDev) );
            MY_LOG("[%s] IAeMgr::getInstance().doCapFlare() END",__FUNCTION__);
        }
    }

    if (ENABLE_AWB & m_3ACtrlEnable) {
    if (ENABLE_LSC & m_3ACtrlEnable) {
        // TSF
        ...
        LscMgr2::getInstance(static_cast<ESensorDev_T>(m_i4SensorDev))->updateTsf(
    ...
    if (ENABLE_AAOBUF & m_3ACtrlEnable) {
        ...
        IAAOBufMgr::getInstance().enqueueHwBuf(m_i4SensorDev, rBufInfo);

        // Update AAO DMA address
        IAAOBufMgr::getInstance().updateDMABaseAddr(m_i4SensorDev);
...
send3ACtrl(E3ACtrl_T e3ACtrl, MINTPTR i4Arg1, MINTPTR i4Arg2)
{
    ...
    case E3ACtrl_GetAECapPLineTable:
        IAeMgr::getInstance().getAECapPlineTable(m_i4SensorDev, reinterpret_cast<MINT32*>(i4Arg1), *reinterpret_cast<strAETable*>(i4Arg2));
        break;
    ...
    case E3ACtrl_GetExposureParam:
        {
            CaptureParam_T* a_rCaptureInfo = reinterpret_cast<CaptureParam_T*>(i4Arg1);
    ...
    case E3ACtrl_SetHalHdr:
        {
            ...
            {
                AE_MODE_CFG_T rCaptureInfo;
                CaptureParam_T &a_rCaptureInfo = *reinterpret_cast<CaptureParam_T*>(i4Arg2);
    ...
    case E3ACtrl_GetIspGamma:
        IspTuningMgr::getInstance().sendIspTuningIOCtrl(m_i4SensorDev, IspTuningMgr::E_ISPTUNING_GET_ISP_GAMMA, i4Arg1, i4Arg2);
        break;
    case E3ACtrl_GetAsdInfo:
        i4Ret = getASDInfo(*reinterpret_cast<ASDInfo_T*>(i4Arg1));
        break;
    case E3ACtrl_DumpRegBuf:
        {
            MINT32 i4Size = sizeof(isp_reg_t);
            char* filename = (char*) i4Arg1;
            const void* pReg = (const void*) i4Arg2;
            FILE* fid = fopen(filename, "wb");
            if (fid)
            {
                fwrite(pReg, 1, i4Size, fid);
                fclose(fid);
                i4Ret = 0;
            }
            else
