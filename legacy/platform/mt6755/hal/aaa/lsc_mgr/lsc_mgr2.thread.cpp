...
updateTsfCfg(UINT32 u4W, UINT32 u4H, const ISP_NVRAM_LSC_T& rLscCfg, ELscScenario_T eLscScn, MBOOL fgForce)
{
    ...
    if (fgDumpEn)
    {
        ...
        if (dumpSdblk("/sdcard/tsf0/tsfCfgChg", rLscCfg, reinterpret_cast<MUINT32*>(m_pu4ProcTbl)) != 0)
...
#define UNIT_GAIN_FROM_NVRAM 0
    ...
    MUINT32 u4TmpPixId = MTK_BAYER_B;

    GET_PROP("debug.lsc_mgr.otpbayer", "-1", u4TmpPixId);
    if (u4TmpPixId != -1)
    {
        eBayerGain = (MTK_BAYER_ORDER_ENUM) u4TmpPixId;
    ...
    {
        case 0: eBayerGain = MTK_BAYER_B;    break;
        case 1: eBayerGain = MTK_BAYER_Gb;   break;
        case 2: eBayerGain = MTK_BAYER_Gr;   break;
        case 3: eBayerGain = MTK_BAYER_R;    break;
        }    
    }    
...
tsfInit()
{
    ...
    MINT32 i4TsfSL2En = 0;
    GET_PROP("debug.lsc_mgr.nsl2", "-1", i4TsfSL2En);
...
tsfBatchCap()
{
    ...
    if (fgDumpEn)
    {
        ...
        if (m_bDumpSdblk)
            sprintf(strTsfInput, "%s.t0.tsfInput", m_strSdblkFile.c_str());
        else
            sprintf(strTsfInput, "/sdcard/tsf0/tsfInput");
    ...
    if (m_bDumpSdblk)
        sprintf(strTsfInput, "%s.t0.tsfAwbStat.bin", m_strSdblkFile.c_str());
    else
        sprintf(strTsfInput, "/sdcard/tsf0/tsfAwbStat.bin");
