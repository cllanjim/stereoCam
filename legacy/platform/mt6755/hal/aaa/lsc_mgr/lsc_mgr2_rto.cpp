    ...
    , m_pu4RaResultTbl(NULL)
{
    ...
    if (m_pLscUtilInterface && m_rRaConfig.working_buf)
    {
...
updateCfg(const ISP_NVRAM_LSC_T& rLscCfg)
{
    ...
    u4W = ((MUINT32) rLscCfg.ctl2.bits.LSC_SDBLK_WIDTH * rLscCfg.ctl2.bits.LSC_SDBLK_XNUM + rLscCfg.lblock.bits.LSC_SDBLK_lWIDTH) * 2;
    u4H = ((MUINT32) rLscCfg.ctl3.bits.LSC_SDBLK_HEIGHT * rLscCfg.ctl3.bits.LSC_SDBLK_YNUM + rLscCfg.lblock.bits.LSC_SDBLK_lHEIGHT) * 2;
    ...
    m_rRaLscConfig.block_wd      = rLscCfg.ctl2.bits.LSC_SDBLK_WIDTH;
    m_rRaLscConfig.block_ht      = rLscCfg.ctl3.bits.LSC_SDBLK_HEIGHT;
    m_rRaLscConfig.x_grid_num    = rLscCfg.ctl2.bits.LSC_SDBLK_XNUM+2;
    m_rRaLscConfig.y_grid_num    = rLscCfg.ctl3.bits.LSC_SDBLK_YNUM+2;
    m_rRaLscConfig.block_wd_last = rLscCfg.lblock.bits.LSC_SDBLK_lWIDTH;
    m_rRaLscConfig.block_ht_last = rLscCfg.lblock.bits.LSC_SDBLK_lHEIGHT;
...
genHwTbl(MUINT32 ratio, const MUINT32* pSrc, MUINT32* pDest)
{
    ...
    else
    {
        ...
        if (ra_ret != S_LSC_CONVERT_OK)
        {
            fgRet = MFALSE;
            LSC_ERR("RA Error!, ratio(%d)", ratio);
        }
    }
lbExit:
    return fgRet;
                 ...
                 NVRAM_CAMERA_ISP_PARAM_STRUCT& rIspNvram)
{
    ...
    case ESensorDev_Main:       //  Main Sensor
        static LscMgr2Rto singleton_main(ESensorDev_Main, i4SensorIdx, eSensorTG, rIspNvram);
        pLscMgrRto = &singleton_main;
        LSC_LOG_END("RTO ESensorDev_Main(0x%08x), i4SensorIdx(%d), eSensorTG(%d)", pLscMgrRto, i4SensorIdx, eSensorTG);
        break;
    case ESensorDev_MainSecond: //  Main Second Sensor
        static LscMgr2Rto singleton_mainsecond(ESensorDev_MainSecond, i4SensorIdx, eSensorTG, rIspNvram);
        pLscMgrRto = &singleton_mainsecond;
        LSC_LOG_END("RTO ESensorDev_MainSecond(0x%08x), i4SensorIdx(%d), eSensorTG(%d)", pLscMgrRto, i4SensorIdx, eSensorTG);
        break;
    case ESensorDev_Sub:        //  Sub Sensor
        static LscMgr2Rto singleton_sub(ESensorDev_Sub, i4SensorIdx, eSensorTG, rIspNvram);
        pLscMgrRto = &singleton_sub;
        LSC_LOG_END("RTO ESensorDev_Sub(0x%08x), i4SensorIdx(%d), eSensorTG(%d)", pLscMgrRto, i4SensorIdx, eSensorTG);
        break;
    }
...
LscMgr2Rto::
LscMgr2Rto(ESensorDev_T eSensorDev, MINT32 i4SensorIdx, ESensorTG_T eSensorTG, NVRAM_CAMERA_ISP_PARAM_STRUCT& rIspNvram)
    : m_pLscRatio(NULL)
    , m_pTsfRatio(NULL)
    , m_bTSF1stGroup(MFALSE)
    , LscMgr2(eSensorDev, i4SensorIdx, eSensorTG, rIspNvram)
{
    for(MUINT32 i=0; i<4; i++)
    {
        m_pu4ResultTblQ[i] = NULL;
    }
...
setLscRatio(MINT32 i4Rto)
{
    ...
    if (!m_bTSF && m_pLscRatio)
    {
        ...
        if (!m_pLscRatio->genHwTbl(i4Rto, pSrc, m_pu4RaResultTbl))
        {
            LSC_ERR("Set LSC ratio error.");
        }
...
updateLsc()
{
    ...
    case 0:
        ...
        {
            LSC_ERR("0: SL2 idx(%d) exceeds NVRAM_NSL2_TBL_NUM", i4Idx);
            return;
        }
        break;
    case 1:
                 ...
                 NVRAM_CAMERA_ISP_PARAM_STRUCT& rIspNvram)
{
    ...
    default:
    case ESensorDev_Main:       //  Main Sensor
        static LscMgr2RtoCycle singleton_main(ESensorDev_Main, i4SensorIdx, eSensorTG, rIspNvram);
        pLscMgrRto = &singleton_main;
        LSC_LOG_END("RTOCycle ESensorDev_Main(0x%08x), i4SensorIdx(%d), eSensorTG(%d)", pLscMgrRto, i4SensorIdx, eSensorTG);
        break;
    case ESensorDev_MainSecond: //  Main Second Sensor
        static LscMgr2RtoCycle singleton_mainsecond(ESensorDev_MainSecond, i4SensorIdx, eSensorTG, rIspNvram);
        pLscMgrRto = &singleton_mainsecond;
        LSC_LOG_END("RTOCycle ESensorDev_MainSecond(0x%08x), i4SensorIdx(%d), eSensorTG(%d)", pLscMgrRto, i4SensorIdx, eSensorTG);
        break;
    case ESensorDev_Sub:        //  Sub Sensor
        static LscMgr2RtoCycle singleton_sub(ESensorDev_Sub, i4SensorIdx, eSensorTG, rIspNvram);
        pLscMgrRto = &singleton_sub;
        LSC_LOG_END("RTOCycle ESensorDev_Sub(0x%08x), i4SensorIdx(%d), eSensorTG(%d)", pLscMgrRto, i4SensorIdx, eSensorTG);
        break;
    }
...
LscMgr2RtoCycle::
LscMgr2RtoCycle(ESensorDev_T eSensorDev, MINT32 i4SensorIdx, ESensorTG_T eSensorTG, NVRAM_CAMERA_ISP_PARAM_STRUCT& rIspNvram)
    : LscMgr2Rto(eSensorDev, i4SensorIdx, eSensorTG, rIspNvram)
{
