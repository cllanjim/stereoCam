...
static MBOOL
dumpOtp(const MUINT8* ptrTbl, MUINT32 u4Size, const char* filename)
{
    MINT32 i = u4Size / 4;
    FILE* fpdebug;
    ...
    const MUINT32* pData = reinterpret_cast<const MUINT32*>(ptrTbl);
    while (i >= 4)
    {
        MUINT32 a = *pData++;
        MUINT32 b = *pData++;
...
LscMgr2*
LscMgr2::
createInstance(ESensorDev_T const eSensorDev, MINT32 i4SensorIdx, ESensorTG_T eSensorTG, NVRAM_CAMERA_ISP_PARAM_STRUCT& rIspNvram, MUINT8 fgType)
{
    ...
    case 2:
        ...
        case ESensorDev_Main:       //  Main Sensor
            _pLscMain = LscMgr2RtoCycle::createInstance(ESensorDev_Main, i4SensorIdx, eSensorTG, rIspNvram);
            LSC_LOG_END("RTOC ESensorDev_Main(0x%08x), i4SensorIdx(%d), eSensorTG(%d)", _pLscMain, i4SensorIdx, eSensorTG);
            return _pLscMain;
        case ESensorDev_MainSecond: //  Main Second Sensor
            _pLscMain2 = LscMgr2RtoCycle::createInstance(ESensorDev_MainSecond, i4SensorIdx, eSensorTG, rIspNvram);
            LSC_LOG_END("RTOC ESensorDev_MainSecond(0x%08x), i4SensorIdx(%d), eSensorTG(%d)", _pLscMain2, i4SensorIdx, eSensorTG);
            return _pLscMain2;
        case ESensorDev_Sub:        //  Sub Sensor
            _pLscSub = LscMgr2RtoCycle::createInstance(ESensorDev_Sub, i4SensorIdx, eSensorTG, rIspNvram);
            LSC_LOG_END("RTOC ESensorDev_Sub(0x%08x), i4SensorIdx(%d), eSensorTG(%d)", _pLscSub, i4SensorIdx, eSensorTG);
            return _pLscSub;
        }
        break;
    case 1:
        ...
        default:
        case ESensorDev_Main:       //  Main Sensor
            _pLscMain = LscMgr2Rto::createInstance(ESensorDev_Main, i4SensorIdx, eSensorTG, rIspNvram);
            LSC_LOG_END("RTO ESensorDev_Main(0x%08x), i4SensorIdx(%d), eSensorTG(%d)", _pLscMain, i4SensorIdx, eSensorTG);
            return _pLscMain;
        case ESensorDev_MainSecond: //  Main Second Sensor
            _pLscMain2 = LscMgr2Rto::createInstance(ESensorDev_MainSecond, i4SensorIdx, eSensorTG, rIspNvram);
            LSC_LOG_END("RTO ESensorDev_MainSecond(0x%08x), i4SensorIdx(%d), eSensorTG(%d)", _pLscMain2, i4SensorIdx, eSensorTG);
            return _pLscMain2;
        case ESensorDev_Sub:        //  Sub Sensor
            _pLscSub = LscMgr2Rto::createInstance(ESensorDev_Sub, i4SensorIdx, eSensorTG, rIspNvram);
            LSC_LOG_END("RTO ESensorDev_Sub(0x%08x), i4SensorIdx(%d), eSensorTG(%d)", _pLscSub, i4SensorIdx, eSensorTG);
            return _pLscSub;
        }
        break;
    case 0:
    ...
    , m_u4PerFrameStep(0)
{
    ...
    m_rOtp.TableRotation = 0; // default no otp orientation

    IHalSensorList* const pIHalSensorList = IHalSensorList::get();
    IHalSensor* pIHalSensor = pIHalSensorList->createSensor("lsc_mgr_otp", m_i4SensorIdx);
    ...
    LSC_LOG("Sensor(%d) power on done, count(%d)", m_eSensorDev, i);
    m_i4OtpState = importEEPromData();
    pIHalSensor->destroyInstance("lsc_mgr_otp");

    if (m_i4OtpState == E_LSC_WITH_MTK_OTP)
    {
        // write unit gain table to NVRAM
        LSC_LOG("Write Unit Gain to NVRAM buffer");
        CAM_CAL_LSC_MTK_TYPE& rMtkLsc = m_rOtp.LscTable.MtkLcsData;
        ...
        rMtkLsc.PixId = m_prShadingLut->SensorGoldenCalTable.PixId; // PixId customized in tuning file
...
uninit()
{
    ...
    // reset to prevent from EM -> normal issue
    m_bDumpSdblk = MFALSE;
    m_fgBypass1to3 = MFALSE;
...
MINT32
LscMgr2::
shadingTblCvtGainR(const ShadingTblCropCfg_T& rCropCfg, const ShadingTblInput_T& rInput, MUINT32 u4GridNumX, MUINT32 u4GridNumY, MUINT32* pu4OutputCoefTbl)
{
    MINT32 i4Ret = 0;
    SHADIND_TRFM_CONF rTrfm;
    MUINT8* gWorkinBuffer = new MUINT8[SHADIND_FUNC_WORKING_BUFFER_SIZE];
    ...
    rTrfm.output.crop_width     = rCropCfg.rCrop.u4W;      // crop
    rTrfm.output.crop_height    = rCropCfg.rCrop.u4H;      // crop
    rTrfm.output.bayer          = BAYER_R;
    rTrfm.output.grid_x         = u4GridNumX;              // output Coef (alwasy 16x16)
    rTrfm.output.grid_y         = u4GridNumY;              // output Coef
    rTrfm.output.lwidth         = 0;
    rTrfm.output.lheight        = 0;
    rTrfm.output.ratio_idx      = 0;
    rTrfm.output.grgb_same      = SHADING_GRGB_SAME_YES;
    rTrfm.output.data_type      = SHADING_TYPE_GAIN;       // gain
    rTrfm.output.table          = pu4OutputCoefTbl;        // output gain
...
RawLscTblDump(const char* filename)
{
    ...
    sprintf(strfile, "/sdcard/shading_otp/%s_%d.log", filename, m_eSensorDev);
    if (!do_mkdir("/sdcard/shading_otp", S_IRWXU | S_IRWXG | S_IRWXO))
    {
        LSC_LOG_END("Fail to mkdir (%s)", strfile);
        return;
    }
...
allocMemBuf(IMEM_BUF_INFO& rBufInfo, MUINT32 const u4Size)
{
    ...
    if (!rBufInfo.virtAddr)
    {
        ...
        if (0 == m_pIMemDrv->allocVirtBuf(LOG_TAG, &rBufInfo))
        {
...
freeMemBuf(IMEM_BUF_INFO& rBufInfo)
{
    ...
    {
        if (0 == m_pIMemDrv->freeVirtBuf(LOG_TAG, &rBufInfo))
        {
...
doShadingAlign(MVOID* pMtkLsc)
{
    ...
    GET_PROP("debug.lsc_mgr.otpbayer", "-1", u4TmpPixId);

    if (u4TmpPixId != -1)
    {
        eBayerGain = (BAYER_ID_T) u4TmpPixId;
        LSC_LOG("Debug Override bayer as (%d)", eBayerGain);
    }
    else
    {
    switch (_pMtkLsc->PixId)
    {
        case 0: eBayerGain = BAYER_B;    break;
        case 1: eBayerGain = BAYER_GB;   break;
        case 2: eBayerGain = BAYER_GR;   break;
        case 3: eBayerGain = BAYER_R;    break;
        }
    }
    ...
    rAlign.u4Width      = (u4BlkW*u4XNum + u4LastW)*2;
    rAlign.u4Height     = (u4BlkH*u4YNum + u4LastH)*2;
...
#endif
    {
        ...
        logGainTbl(pGoldenGain, u4GainTblSize, "Golden Gain Table");
        logGainTbl(pUnitGain, u4GainTblSize, "Unit Gain Table");
        char strFileName[512] = {'\0'};

        if (do_mkdir("/sdcard/shading_otp", S_IRWXU | S_IRWXG | S_IRWXO))
        {
            sprintf(strFileName, "/sdcard/shading_otp/%d_golden_%dx%d_%d.otp", m_eSensorDev, rAlign.u1GainXNum, rAlign.u1GainYNum, rAlign.u1GainBayer);
            dumpOtp(pGoldenGain, u4GainTblSize, strFileName);
            sprintf(strFileName, "/sdcard/shading_otp/%d_unit_%dx%d_%d.otp", m_eSensorDev, rAlign.u1GainXNum, rAlign.u1GainYNum, rAlign.u1GainBayer);
            dumpOtp(pUnitGain, u4GainTblSize, strFileName);
        }
        ...
        LSC_LOG("Start Align, IMG(%d x %d), Grid(%d x %d), Bayer(%d), GoldenGain(%p), UnitGain(%p)",
...
MBOOL
LscMgr2::
checkTblEffective() const
{
    MUINT32 u4HwAddr = IspDebug::getInstance().readLsciAddr(m_eSensorTG);
    LSC_LOG("[%s] currAddr(0x%08x)", __FUNCTION__, u4HwAddr);
    return (u4HwAddr == m_rBufInfo[m_u4DoubleBufIdx].phyAddr);
}
...
calLscConfig(const SensorCropInfo_T& rCrop, MUINT8 u1XNum, MUINT8 u1YNum, ISP_NVRAM_LSC_T& rLscCfg)
{
    ...
    rLscCfg.ctl1.bits.LSC_PRC_MODE = 1;
...
MBOOL
LscMgr2::
calSl2Config(const SensorCropInfo_T& rCrop, ISP_NVRAM_NSL2_T& rSl2Cfg)
{
    // default setting
    ...
    rSl2Cfg.rr_con0.bits.SL2_R_0 = u4R*3/10;
    rSl2Cfg.rr_con0.bits.SL2_R_1 = u4R*6/10;
    rSl2Cfg.rr_con1.bits.SL2_R_2 = u4R*8/10;

    //sent width and height to ISP_MGR_NSL2
    ISP_MGR_NSL2_T::getInstance(m_eSensorDev).setNSL2Size (rCrop.u4W , rCrop.u4H);
...
setCTIdx(MUINT32 const u4CTIdx)
{
    ...
    {
        _u4CtIdx = m_fgLock ? m_u4CTIdx : u4CTIdx;
    }
...
{
    if (!m_fgLock)
        m_i4Rto = (i4Rto >= 32) ? 32 : (i4Rto < 0 ? 0 : i4Rto);
    return m_i4Rto;
}

MBOOL
LscMgr2::
setLock(MBOOL fgOnOff)
{
    LSC_LOG("(%d)", fgOnOff);
    m_fgLock = fgOnOff;
    return MTRUE;
...
updateLsc()
{
    ...
    // sl2
    ISP_NVRAM_NSL2_T rSl2Cfg;
    MBOOL fgSl2OnOff = 0;
    MINT32 i4Idx = 0;
    MINT32 dbg_enable = 0;
    GET_PROP("debug.lsc_mgr.nsl2", "-1", dbg_enable);
    MINT32 i4Case = (dbg_enable != -1) ? dbg_enable : isEnableNSL2(m_eSensorDev);
    ...
    case 1:
        ...
        if (i4Idx >= NVRAM_NSL2_TBL_NUM)
        {
            LSC_ERR("1: SL2 idx(%d) exceeds NVRAM_NSL2_TBL_NUM", i4Idx);
            return;
        }
        break;
    case 2:
        ...
        if (i4Idx >= NVRAM_NSL2_TBL_NUM)
        {
            LSC_ERR("2: SL2 idx(%d) exceeds NVRAM_NSL2_TBL_NUM", i4Idx);
            return;
        }
        break;
    }
    ...
    if (getTsfOnOff()){
        ...
        ISP_NVRAM_NSL2_T rTmpGain = m_rIspNvram.ISPRegs.NSL2[i4Idx];
        rSl2Cfg = m_rSl2Cfg;
        rSl2Cfg.rr_con1.bits.SL2_GAIN_0 = rTmpGain.rr_con1.bits.SL2_GAIN_0;
        rSl2Cfg.rr_con1.bits.SL2_GAIN_1 = rTmpGain.rr_con1.bits.SL2_GAIN_1;
        rSl2Cfg.gain.bits.SL2_GAIN_2    = rTmpGain.gain.bits.SL2_GAIN_2;
        rSl2Cfg.gain.bits.SL2_GAIN_3    = rTmpGain.gain.bits.SL2_GAIN_3;
        rSl2Cfg.gain.bits.SL2_GAIN_4    = rTmpGain.gain.bits.SL2_GAIN_4;

    }
    else
        rSl2Cfg = m_rIspNvram.ISPRegs.NSL2[i4Idx];
    ...
    if (NSIspTuningv3::IspTuningMgr::getInstance().getOperMode(m_eSensorDev) != NSIspTuning::EOperMode_Meta)
    {
        ISP_MGR_NSL2_T::getInstance(m_eSensorDev).setEnable(fgSl2OnOff);
        ISP_MGR_NSL2_T::getInstance(m_eSensorDev).put(rSl2Cfg);
    }
}

MVOID
LscMgr2::
logGainTbl(const MUINT8* ptrTbl, MUINT32 u4Size, const char* str)
{
    LSC_LOG("%s", str);
    MBOOL fgLogEn = (m_u4LogEn & EN_LSC_LOG_SHOW_TABLE) ? MTRUE : MFALSE;
    const MUINT32* pu4Tbl = reinterpret_cast<const MUINT32*>(ptrTbl);
    MUINT32 u32Size = u4Size / 4;
    for (MUINT32 idx = 0; idx < u32Size; idx += 4)
    {
        MY_LOG_IF(fgLogEn, "0x%08x    0x%08x    0x%08x    0x%08x\n",
            *(pu4Tbl+idx), *(pu4Tbl+idx+1), *(pu4Tbl+idx+2), *(pu4Tbl+idx+3));
    }

    for (MUINT32 idx = 0; idx < u4Size; idx += 16)
    {
        MY_LOG_IF(fgLogEn, "0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x,\n",
