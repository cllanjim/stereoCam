...
updateTsfCfg(UINT32 u4W, UINT32 u4H, const ISP_NVRAM_LSC_T& rLscCfg, ELscScenario_T eLscScn, MBOOL fgForce)
{
    ...
    float fRto = (((float)m_rTsfLscParam.raw_wd*u4H) / ((float)m_rTsfLscParam.raw_ht*u4W));
...
tsfInit()
{
    ...
    if (i4TsfSL2En == -1)
    {
        ...
        i4TsfSL2En = isEnableNSL2(m_eSensorDev);
...
tsfCfgChg()
{
    ...
    if (!restore())
    {
        ...
        tsfSetTbl(m_pu4RaResultTbl, MTRUE);
    }
    m_bTSF1stGroup = 1;
    LSC_LOG("Start with ra(%d)", 0);
#endif
...
tsfBatch()
{
    ...
    MINT32 i4Rto = m_bTSF1stGroup ? 0 : m_i4Rto;
    LSC_LOG_BEGIN("step(0x%08x) ratio(%d) 1st(%d)", m_u4PerFrameStep, i4Rto, m_bTSF1stGroup);
    ...
    if (m_pTsfRatio)
    {
        if (!m_pTsfRatio->genHwTbl(i4Rto, m_pu4TsfRaInputTbl, m_pu4RaResultTbl))
        {
            ret = E_TSF_ERR;
            goto lbExit;
        }
...
tsfBatchCap()
{
    ...
    char strTsfInput[512] = {'\0'};
    MBOOL fgDumpEn = (m_u4LogEn & EN_LSC_LOG_TSF_DUMP) ? MTRUE : m_bDumpSdblk;
    ...
    if (m_bDumpSdblk)
        sprintf(strTsfInput, "%s.t1.tsfAwbStat.bin", m_strSdblkFile.c_str());
    else
        sprintf(strTsfInput, "/sdcard/tsf1/tsfAwbStat.bin");
    tsfSetProcInfo(m_rTsfSetProc, fgDumpEn, strTsfInput);
    ...
    if (fgDumpEn)
    {
        ...
        if (m_bDumpSdblk)
            sprintf(strTsfInput, "%s.t1.tsfInput", m_strSdblkFile.c_str());
        else
            sprintf(strTsfInput, "/sdcard/tsf1/tsfInput");

        if (dumpSdblk(strTsfInput, rLscCfg, reinterpret_cast<MUINT32*>(m_rTsfSetProc.ShadingTbl)) != 0)
            LSC_ERR("dumpSdblk error");
    }
    ...
    if (m_bDumpSdblk)
    sprintf(m_rTsfSetProc.sDumpFilePath, "%s", m_strSdblkFile.c_str());
...
backup()
{
    MUINT32 u4Size = getPerLutSize(TSF_SCN_DFT);
    m_vecBackup.resize(getPerLutSize(LscMgr2::LSC_SCENARIO_CAP));
    ::memcpy(m_vecBackup.data(), m_pu4ProcTbl, u4Size);
    LSC_LOG("from procTbl(%p) to buf(%p)", m_pu4ProcTbl, m_vecBackup.begin());
    return MTRUE;
}
...
restore()
{
    if (m_vecBackup.size())
    {
        tsfSetTbl(reinterpret_cast<const MUINT32*>(m_vecBackup.data()), MTRUE);
        LSC_LOG("from buf(%p) to procTbl(%p)", m_vecBackup.data(), m_pu4ProcTbl);
        m_vecBackup.clear();
        return MTRUE;
    }
    else
...
{
    MRESULT ret = S_TSF_OK;
    MINT32 i4Rto = m_i4Rto;//m_bTSF1stGroup ? 0 : m_i4Rto;

    LSC_LOG_BEGIN("i4Rto(%d), 1st(%d)", i4Rto, m_bTSF1stGroup);
    MBOOL fgDumpEn = (m_u4LogEn & EN_LSC_LOG_TSF_DUMP) ? MTRUE : MFALSE;
    ...
    {
        if (fgDumpEn) {
            if (dumpSdblk("/sdcard/tsf2/tsfBaseInput", rLscCfg, reinterpret_cast<MUINT32*>(m_pu4TsfRaInputTbl)) != 0)
                LSC_ERR("dumpSdblk error");
        }
        // do ratio
        if (!m_pTsfRatio->genHwTbl(i4Rto, m_pu4TsfRaInputTbl, m_pu4RaResultTbl))
        {
            ret = E_TSF_ERR;
            LSC_LOG("RA NG (%d)", i4Rto);
            if (fgDumpEn) {
                if (dumpSdblk("/sdcard/tsf2/tsfErrRaInput", rLscCfg, reinterpret_cast<MUINT32*>(m_pu4RaResultTbl)) != 0)
                    LSC_ERR("dumpSdblk error");
            }
            goto lbExit;
        }
        ...
        {
            LSC_LOG("RA OK (%d)", i4Rto);
            if (fgDumpEn) {
                if (dumpSdblk("/sdcard/tsf2/tsfRaInput", rLscCfg, reinterpret_cast<MUINT32*>(m_pu4RaResultTbl)) != 0)
                    LSC_ERR("dumpSdblk error");
            }
        }
    ...
    tsfSetProcInfo(m_rTsfSetProc, fgDumpEn, "/sdcard/tsf2/tsfBatchAwbStat.bin");
    m_rTsfSetProc.ShadingTbl = (MINT32*) m_pu4ProcTbl;
    if (fgDumpEn) {
        if (dumpSdblk("/sdcard/tsf2/tsfBatchInput", rLscCfg, reinterpret_cast<MUINT32*>(m_rTsfSetProc.ShadingTbl)) != 0)
            LSC_ERR("dumpSdblk error");
    }
    ...
    if (ret != S_TSF_OK)
    {
        ...
        m_prTsf->TsfReset();
        if (fgDumpEn) {
            if (dumpSdblk("/sdcard/tsf2/tsfBatchErrOutput", rLscCfg, reinterpret_cast<MUINT32*>(m_rTsfResult.ShadingTbl)) != 0)
                LSC_ERR("dumpSdblk error");
        }
    }
    else
    {
        ...
        tsfSetSL2(m_rTsfResult.SL2Para);
        if (fgDumpEn) {
            if (dumpSdblk("/sdcard/tsf2/tsfBatchOutput", rLscCfg, reinterpret_cast<MUINT32*>(m_rTsfResult.ShadingTbl)) != 0)
                LSC_ERR("dumpSdblk error");
        }
    }
...
tsfBatchCap()
{
    ...
    LSC_LOG_BEGIN("m_i4Rto(%d)", m_i4Rto);
    ...
    if (fgDumpEn)
    {
        ...
        if (m_bDumpSdblk)
            sprintf(strTsfInput, "%s.t2.tsfInput", m_strSdblkFile.c_str());
        else
            sprintf(strTsfInput, "/sdcard/tsf2/tsfInput");
    ...
    if (m_pTsfRatio)
    {
        ...
        m_rTsfTbl.ShadingTbl = (MINT32*) m_pu4ProcTbl; //m_pu4RaResultTbl;
        ret = m_prTsf->TsfFeatureCtrl(MTKTSF_FEATURE_SET_TBL_CHANGE, &m_rTsfTbl, NULL);
        if (ret != S_TSF_OK)
        {
            LSC_ERR("Error(0x%08x): MTKTSF_FEATURE_SET_TBL_CHANGE", ret);
    ...
    if (m_bDumpSdblk)
        sprintf(strTsfInput, "%s.t2.tsfAwbStat.bin", m_strSdblkFile.c_str());
    else
        sprintf(strTsfInput, "/sdcard/tsf2/tsfAwbStat.bin");
    tsfSetProcInfo(m_rTsfSetProc, fgDumpEn, strTsfInput);
    m_rTsfSetProc.ShadingTbl = (MINT32*) m_pu4ProcTbl;
    // disable temporal smooth
    m_prTsf->TsfFeatureCtrl(MTKTSF_FEATURE_CONFIG_SMOOTH, (void*)0, 0);
    ...
    else
    {
        ...
        if (fgDumpEn)
        {
            ...
            if (m_bDumpSdblk)
                sprintf(strTsfInput, "%s.t2.tsfOutput", m_strSdblkFile.c_str());
            else
                sprintf(strTsfInput, "/sdcard/tsf2/tsfOutput");

            if (dumpSdblk(strTsfInput, rLscCfg, reinterpret_cast<MUINT32*>(m_rTsfResult.ShadingTbl)) != 0)
                LSC_ERR("dumpSdblk error");
        }
    }
