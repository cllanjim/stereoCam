...
inline static
MINT32 _convertResultToMeta(MUINT32 u4SensorDev, const Result_T& rResult, const IMetadata& staticMeta, MetaSet_T& rMetaResult, const StaticInfo_T& rstaticInfo)
{
...
uninit()
{
    ...
    if (mi4User > 0)
    {
        ...
        {
             MY_LOG("[init gone][uninit] mi4User = %d", mi4User);
...
#define OVERRIDE_AF  (1<<2)
    ...
    MUINT32 u4RequestSize = requestQ.size();
    ...
    if (requestQ.size() != getCapacity() && (mParams.u4StrobeMode != MTK_FLASH_MODE_TORCH))
		 mParams.u4StrobeMode = MTK_FLASH_MODE_OFF; // reset mParams.u4StrobeMode to avoid strobe mode remains as last request
    mLock.lock();
    ...
    for (i = 0; i < getCapacity() && it != requestQ.end(); i++, it++)
    {
        ...
        if (i == HAL3A_REQ_PROC_KEY)
        {
            ...
            {
                if(fgHQCapEnable)
                    mParams.u1HQCapEnable = MTRUE;
                MY_LOG_IF(fgLog, "[%s] MTK_HAL_REQUEST_HIGH_QUALITY_CAP (%d)", __FUNCTION__, mParams.u1HQCapEnable);
            }

            fgOK = QUERY_ENTRY_SINGLE(it->halMeta, MTK_HAL_REQUEST_REQUIRE_EXIF, mParams.u1IsGetExif);
            if (fgOK)
            {
                MY_LOG_IF(fgLog, "[%s] magic(%d) Request EXIF(%d)", __FUNCTION__, i4FrmId, mParams.u1IsGetExif);
            }else{
    ...
    if (mParams.u1CaptureIntent == MTK_CONTROL_CAPTURE_INTENT_VIDEO_RECORD && i4FrmId == 0)
        MY_LOG("[%s] CAPTURE_INTENT protection during Video record mode, FrmId(%d)",__FUNCTION__, i4FrmId);
    else if (mParams.u1CaptureIntent != MTK_CONTROL_CAPTURE_INTENT_STILL_CAPTURE && mParams.u1CaptureIntent != MTK_CONTROL_CAPTURE_INTENT_MANUAL)
        MY_LOG("[%s] CAPTURE_INTENT protection during STILL_CAPTURE / MANUAL_CAPTURE, FrmId(%d)",__FUNCTION__, i4FrmId);
    else
        mParams.u1CaptureIntent = u1CapIntent;
    ...
    if (u4RequestSize > HAL3A_REQ_PROC_KEY)
    {
    ...
    {
        MY_LOG_IF(fgLog,"u4RequestSize(%d) should be greater than HAL3A_REQ_PROC_KEY(%d)", u4RequestSize, HAL3A_REQ_PROC_KEY);
    }
...
get(MUINT32 frmId, MetaSet_T& result)
{
    ...
    for (; it != mMetaResultQueue.end(); it++)
    {
        ...
        if (frmId == u4Id)
        {
            ...
            _convertResultToMeta(mu4SensorDev, rResult, mMetaStaticInfo, result, m_rStaticInfo);
            break;
        }
    }
...
setIsp(MINT32 flowType, const MetaSet_T& control, void* pRegBuf, MetaSet_T* pResult)
{
    ...
    if (GET_ENTRY_ARRAY(control.halMeta, MTK_PROCESSOR_CAMINFO, rCamInfo.data, sizeof(NSIspTuning::RAWIspCamInfo_U)))
    {
        ...
        if (QUERY_ENTRY_SINGLE(control.halMeta, MTK_3A_PGN_ENABLE, u1PGN))
        {
            ...
            if (u1PGN)
            {
                rRawInfo.eIspProfile = NSIspTuning::EIspProfile_Capture;
            }
        }
        MINT32 i4Magic = 0;
        MUINT8 u4CapIntent = MTK_CONTROL_CAPTURE_INTENT_PREVIEW;
        QUERY_ENTRY_SINGLE(control.halMeta, MTK_P1NODE_PROCESSOR_MAGICNUM, i4Magic);
        QUERY_ENTRY_SINGLE(control.appMeta, MTK_CONTROL_CAPTURE_INTENT, u4CapIntent);

        MY_LOG_IF(fgLog,"[%s] flowtype(%d), rrz(%d), isp(%d), camInfo received: R=%d, G=%d, B=%d", __FUNCTION__, flowType
                 , rRawInfo.fgRPGEnable, rRawInfo.eIspProfile
                 , rRawInfo.rAWBInfo.rCurrentAWBGain.i4R
                 , rRawInfo.rAWBInfo.rCurrentAWBGain.i4G
                 , rRawInfo.rAWBInfo.rCurrentAWBGain.i4B);
        mpHal3aObj->setIspPass2(flowType, rRawInfo, pRegBuf);
        if (u4CapIntent == MTK_CONTROL_CAPTURE_INTENT_STILL_CAPTURE)
        {
            char filename[512] = {'\0'};
            sprintf(filename, "/sdcard/isp_dump/setisp_0x%04d_pgn%d.bin", i4Magic, u1PGN);
            send3ACtrl(E3ACtrl_DumpRegBuf, (MINTPTR)filename, (MINTPTR)pRegBuf);
        }

        if (flowType == 1)  ///for high quality capture
        {
            mpHal3aObj->getResultCur(0xffffffff, rHQResult);
            MY_LOG("rHQResult.i4AwbGain: R=%d, G=%d, B=%d", rHQResult.i4AwbGain[0], rHQResult.i4AwbGain[1], rHQResult.i4AwbGain[2]);
            if (pResult)
                _convertResultToMeta(mu4SensorDev, rHQResult, mMetaStaticInfo, *pResult, m_rStaticInfo);
        }
        else
        {
