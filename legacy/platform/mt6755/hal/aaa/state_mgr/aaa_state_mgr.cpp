...
StateMgr::StateMgr()
    ...
    , m_i4FrameCount(0)
    , mSensorDevId (0)
    , m_bMainFlashOnThisFrame(0)
    , mePrevStateForAe(static_cast<EState_T>(0))
    , m_bHasAEEverBeenStable(MFALSE)
    , m_bAFStartWait(MFALSE)
    , mbIsRecording(MFALSE)
    , m_bPreStateIsFocused(MFALSE)
{
 ...
 {
 #warning "FIXME"
     m_i4FrameCount = -4; // delay 2 frames for 3A statistics ready
