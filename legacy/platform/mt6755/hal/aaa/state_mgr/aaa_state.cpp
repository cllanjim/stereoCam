...
if (ENABLE_FLASH & m_pHal3A->m_3ACtrlEnable) {
    ...
    bRet = FlashMgr::getInstance().start(m_SensorDevId);
    if (!bRet) {
        MY_ERR("FlashMgr::getInstance().start() fail, PvMode = %d\n", m_pHal3A->get3APreviewMode());
        return E_3A_ERR;
    }
}
...
if (ENABLE_AWB & m_pHal3A->m_3ACtrlEnable) {
    // AWB start
    bRet = IAwbMgr::getInstance().start(m_SensorDevId);
    if (!bRet) {
        MY_ERR("IAwbMgr::getInstance().start() fail, PvMode = %d\n", m_pHal3A->get3APreviewMode());
        return E_3A_ERR;
    }
}
...
if (ENABLE_AE & m_pHal3A->m_3ACtrlEnable) {
	// AE init
	err = IAeMgr::getInstance().Start(m_SensorDevId);
	if (FAILED(err)) {
		MY_ERR("IAeMgr::getInstance().Start() fail, PvMode = %d\n", m_pHal3A->get3APreviewMode());
		return err;
	}
}
