...
MRESULT AfMgr::init(MINT32 i4SensorIdx, MINT32 isInitMCU)
{
    ...
    m_PWrInit = MTRUE; /*HAL3 : control by hal flow*/
...
MBOOL AfMgr::Start()
{
    ...
    if (m_i4EnableAF == -1)
    {
        ...
        rSensorStaticInfo.sensorFormatOrder = 0;
        rSensorStaticInfo.sensorDevID = 0;
        rSensorDynamicInfo.TgInfo = 0;
...
#if 0 //if it doesn't support OTP, the search table of Cam3 will be abnormal.
    {
        MINT32 i4StartIdx = m_NVRAM_LENS.rAFNVRAM.sAF_Coef.sTABLE.i4InfIdxOffset;
        MINT32 i4EndIdx = m_NVRAM_LENS.rAFNVRAM.sAF_Coef.sTABLE.i4NormalNum - 1 - m_NVRAM_LENS.rAFNVRAM.sAF_Coef.sTABLE.i4MacroIdxOffset;

        MINT32 i4InfPos = m_NVRAM_LENS.rAFNVRAM.sAF_Coef.sTABLE.i4Pos[i4StartIdx] + m_NVRAM_LENS.rAFNVRAM.sAF_Coef.sTABLE.i4Offset;
        MINT32 i4MacroPos = m_NVRAM_LENS.rAFNVRAM.sAF_Coef.sTABLE.i4Pos[i4EndIdx] + m_NVRAM_LENS.rAFNVRAM.sAF_Coef.sTABLE.i4Offset;

        MY_LOG("[First] using nvram para [Inf]%d [Macro]%d", i4InfPos, i4MacroPos);
    ...
    if( m_PWrInit==MFALSE && (m_eLIB3A_AFMode==LIB3A_AF_MODE_AFC || m_eLIB3A_AFMode==LIB3A_AF_MODE_AFC_VIDEO))
        ...
        m_pIAfAlgo->setAFMode( m_eLIB3A_AFMode);
    m_PWrInit=MFALSE; /*HAL3 power on initial sequence*/
#endif
...
MRESULT AfMgr::triggerAF()
{
    ...
    {
        /* Mode Timing
           AP    : |<---------- MTK_CONTROL_AF_MODE_AUTO ---------->|
           Algo  : |<--- WAIT --->|<-------------- AFS ------------>|
                                  ^ triggerAF */
        if( m_eLIB3A_AFMode == LIB3A_AF_MODE_WAIT )
        {
            m_eLIB3A_AFMode = LIB3A_AF_MODE_AFS;
            m_pIAfAlgo->setAFMode(m_eLIB3A_AFMode);
        }
...
{
    MY_LOG_IF( m_bDebugEnable,"cmd-%s TG(%d) : ctl_afmode(%d)\n", __FUNCTION__, m_eSensorTG, eAFMode);
    ...
    case 1: //MTK_CONTROL_AF_MODE_AUTO
        //m_eLIB3A_AFMode = LIB3A_AF_MODE_AFS;
        m_eLIB3A_AFMode = LIB3A_AF_MODE_WAIT; /* AF will wait, set LIB3A_AF_MODE_AFS until trigger AF */
        break;
    ...
    case 5 ://MTK_CONTROL_AF_MODE_EDOF
        ...
        break;
    /*
    case 6: // Focus is set at infinity
        m_eLIB3A_AFMode = LIB3A_AF_MODE_INFINITY;
        break;
    case 7: // Manual Focus Mode
        m_eLIB3A_AFMode = LIB3A_AF_MODE_MF;
        break;
    case 8: // AF Full Scan Mode
...
MRESULT AfMgr::setMFPos(MINT32 a_i4Pos)
{
    ...
    if( (m_eLIB3A_AFMode == LIB3A_AF_MODE_MF) &&
            ...
            (a_i4Pos<=1023))
    {
        MY_LOG("cmd-%s set MF pos (%d)->(%d)\n", __FUNCTION__, m_sAFOutput.i4AFPos, a_i4Pos);
        m_i4MFPos = a_i4Pos;
...
//==========
    ...
    if( m_pLaserDrv )
    {
        ...
        if( m_LastAFArea.i4Info == 1 ) // 1 : TAF
        {
    ...
    if( (m_PDBuf_Type!=EPDBuf_NotDef) && (m_PDAF_Sensor_Support_Mode!=0))
    {
        ...
        MY_LOG_IF( m_bDebugEnable,"[m_LastAFArea][W]%d [H]%d [X]%d [Y]%d\n",
    ...
    else
    {
        ...
        if (m_pMcuDrv)
        {
            MY_LOG_IF( m_bDebugEnable,"[AFStatH]%lld [AFStatV]%lld [moveMCUTG%d] %d", m_sAFInput.sAFStat.i8Stat24, m_sAFInput.sAFStat.i8StatV, m_eSensorTG,m_sAFOutput.i4AFPos);
...
MVOID AfMgr::printAFConfigLog0()
{
    ...
    MY_LOG_IF((m_bDebugEnable&1), "DoAFSW TG%d [P1/P1D]0x%x/0x%x, [P1DMA/P1DDMA]0x%x/0x%x [AFDdone]%x  [AFDdoneST]", m_eSensorTG,
    ...
    if(m_eSensorTG ==ESensorTG_1)
    {
        ...
        MY_LOG_IF( m_bDebugEnable,"DoAFTG1 [XY]%d %d [XY]%d %d [XY]%d %d [XY]%d %d [XY]%d %d [XY]%d %d [Sz]%d %d [G]%d %d %d %d %d %d %d [Hz]%d ",
...
AF_STAT_T AfMgr::Trans4WintoOneStat(MVOID *pAFStatBuf)
{
    ...
    if ((m_sAFOutput.i4FDDetect) && (m_FDArea.i4Count))
    {
        ...
        MY_LOG("FL FV %lld, %lld", statsH[MAX_AF_HW_WIN-1], statsV[MAX_AF_HW_WIN-1]);
    }
    else
    {
...
MFLOAT AfMgr::getFocusDistance   ()
{
    ...
    if (m_i4EnableAF == 0)
    {
        MY_LOG_IF( m_bDebugEnable,"cmd-%s no focus lens\n", __FUNCTION__);
        return 0.0;
    }
    ...
    MY_LOG_IF( m_bDebugEnable,"cmd-%s %f\n", __FUNCTION__, retDist);
    return  retDist;
}
