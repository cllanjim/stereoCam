...
{
 MY_LOG_IF(m_3ALogEnable ,"setAeMeterAreaEn en=%d, m_isAeMeterAreaEn=%d",en, m_isAeMeterAreaEn);
...
//    MY_LOG("%s() : AEMode:%d AEState:%d i4ActiveAEItem:%d u4AAOUpdate:%d\n", __FUNCTION__, m_eAEMode, m_eAEState, i4ActiveAEItem, u4AAOUpdate);
    ...
    if(m_pIAeAlgo != NULL) {
        ...
        if((m_bPerframeAEFlag == MTRUE) && (bAAASchedule == MFALSE) ) {
            ...
            if(m_bTouchAEAreaChage == MTRUE){
                ...
                MY_LOG_IF(m_3ALogEnable,"[%s()] This is Perframe AE flow \n",__FUNCTION__);
            }
        } else {
            MY_LOG_IF(m_bPerframeAEFlag,"[%s()] Neither perframeAE and AEschedule (%d/%d) i4SensorDev = %d \n", __FUNCTION__,m_bPerframeAEFlag ,bAAASchedule , m_eSensorDev);
        }
    // AE schedule
        if(bAAASchedule == MFALSE) {
           ...
           bCalculateAE  = MFALSE ;
           bApplyAE = MFALSE ;
           //MY_LOG("[%s()] i4SensorDev = %d, schedule Calc/Apply = %d/%d , MagicNum = %d/%d , AAOUpdate = %d , CwvY = %d \n", __FUNCTION__,
    ...
    // Log out AE schedule
    MY_LOG_IF(m_3ALogEnable,"[%s()][AEScheduler] AECalc:%d AEApply:%d AEFlare:%d VDnum:%d AvgY:%d\n", __FUNCTION__,
                            bCalculateAE     ,
                            bApplyAE    ,
                            1      ,
                            m_i4WaitVDNum                  ,
    ...
    if(m_eAEMode == LIB3A_AE_MODE_OFF) {
        ...
        if ((i4ActiveItem & E_AE_AE_CALC) || (bCalculateAE == MTRUE)) {
            ...
            } else if (bAAASchedule == MTRUE){
                rAEInput.eAeState = AE_STATE_SLOW_MOTION;
                MY_LOG("[%s()] Stereo or VHDR or Slow Motion flow\n", __FUNCTION__);
            } else if (m_bAElimitor == MTRUE) {
            ...
            if(m_eZoomWinInfo.bZoomChange == MTRUE) {
                ...
                if (m_bPerframeAEFlag){
                    ...
                    MY_LOG_IF(m_3ALogEnable,"[%s():PassToAlgo] rAEInput.PreEvSetting[N-1]:%d/%d/%d rAEInput.PreEvSetting[N-2]:%d/%d/%d rAEInput.PreEvSetting[N-3]:%d/%d/%d rAEInput.CycleInfo.m_i4FrameCnt:%d\n", __FUNCTION__,
...
MINT32 i4ActiveItem;
    ...
    if(m_bEnableAE && (m_bAdbAELock == MFALSE)) {
        ...
        m_bAEMonitorStable = m_bAEStable;
        m_bTouchAEAreaChage = MFALSE;
        MY_LOG_IF(m_bPerframeAEFlag,"[%s()][AEScheduler] AECalc:%d AEApply:%d AEFlare:%d VDnum:%d AvgY:%d\n", __FUNCTION__,
                            bCalculateAE     ,
                            bApplyAE    ,
                            1      ,
                            m_i4WaitVDNum                  ,
        ...
        if ((i4ActiveItem & E_AE_AE_CALC)  || (bCalculateAE == MTRUE)) {
            if((m_eAETargetMode == AE_MODE_AOE_TARGET) || (m_eAETargetMode == AE_MODE_MVHDR_TARGET)) {
                ...
                copyAEInfo2mgr(&m_rAEOutput.rPreviewMode, &rAEOutput);
                MY_LOG_IF(m_3ALogEnable,"[%s():AeAlgo output] Exp/Afe/Isp = %d/%d/%d",__FUNCTION__,m_rAEOutput.rPreviewMode.u4Eposuretime,m_rAEOutput.rPreviewMode.u4AfeGain,m_rAEOutput.rPreviewMode.u4IspGain);
                mPreviewMode = m_rAEOutput.rPreviewMode;
                m_i4WaitVDNum = 0; // reset the delay frame
...
MRESULT AeMgr::UpdateSensorISPParams(AE_STATE_T eNewAEState)
{
    ...
    switch(eNewAEState)
    {
        ...
        case AE_MANUAL_FRAMERATE_STATE:
            ...
            MY_LOG("[%s:s] i4SensorDev:%d VDNum %d, Prev %d/%d/%d, Output %d/%d/%d Smooth:%d m_eAEState:%d\n", __FUNCTION__, m_eSensorDev, m_i4WaitVDNum,
                    m_u4PreExposureTime,
                    m_u4PreSensorGain,
                    m_u4PreIspGain,
...
//                        m_rAEOutput.rPreviewMode.u4IspGain);
...
#endif
                MY_LOG("[%s:e] i4SensorDev:%d VDNum %d, Delay %d/%d/%d, Prev %d/%d/%d, Output %d/%d/%d State:%d FrameRate:%d %d %d Flare Offset:%d", __FUNCTION__, m_eSensorDev, m_i4WaitVDNum,
            ...
            if(u2FrameRate > m_i4AEMaxFps) {
                MY_LOG("[updateAEInfo2ISP] Frame rate too large:%d %d\n", u2FrameRate, m_i4AEMaxFps);
                u2FrameRate = m_i4AEMaxFps;
            }
        ...
        case AE_CAPTURE_STATE:
            if( (m_bEnableAE == MTRUE) && (m_bMultiCap == MFALSE) &&  (0) && (m_rAEInitInput.rAEPARAM.strAEParasetting.bEnableLongCaptureCtrl == MTRUE)) {
        ...
        if(m_bSetGainValue) {    // update sensor gain value
            if (m_rAEInitInput.rAEPARAM.bPerFrameAESmooth){
                if ((m_u4UpdateGainValue != m_PreEvSettingQueue[0].u4AfeGain)||
                    ...
                    MY_LOG_IF(m_3ALogEnable,"[%s()] Sensor gain value is repeat (skip send command)= %d\n", __FUNCTION__, m_u4UpdateGainValue);
                }
            } else {
                if (m_pSensorGainThread){
                    ThreadSensorGain::Cmd_T rCmd(0, 0, m_u4UpdateGainValue, m_eSensorTG);
    ...
    switch(eNewAEState) {
        ...
        case AE_MANUAL_FRAMERATE_STATE:
            ...
            if(u4ISPGain != 0x00) {
                ...
                MY_LOG_IF(m_3ALogEnable,"[updateAEInfo2ISP] Smooth ISP Gain:%d\n", rAEInfo2ISP.u4IspGain);
            } else {
...
MRESULT AeMgr::monitorAndReschedule( MBOOL bAAASchedule, MUINT32 u4AvgYcur, MUINT32 u4AvgYStable, MBOOL *bAECalc, MBOOL *bAEApply )
{
    ...
    MY_LOG("[%s()] Calc:%d Apply:%d Cnt:%d Frame:%d ReSchedule:%d Exit:%d Skip:%d bStatChange:%d bAAASchedule:%d frameDelay:%d Stable:%d AEMonitorStable:%d VdCnt:%d ISPdelay:%d FaceArea:%d TouchArea:%d u4AvgYcur:%d u4AvgYpre:%d\n",
...
MRESULT AeMgr::updatePreEvSettingQueue(MUINT32 newExpsuretime, MUINT32 newAfegain, MUINT32 newIspGain)
{
    ...
    MY_LOG_IF(m_3ALogEnable,"[%s()] m_PreEvSettingQueue[N-1]:%d/%d/%d m_PreEvSettingQueue[N-2]:%d/%d/%d m_PreEvSettingQueue[N-3]:%d/%d/%d FrameCnt(%d)\n", __FUNCTION__,
