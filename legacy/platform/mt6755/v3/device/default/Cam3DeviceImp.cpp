...
onInitializeLocked()
{
...
#if '1'==MTKCAM_HAVE_3A_HAL
    {
        ...
        mpHal3a = NS3Av3::IHal3A::createInstance(
                NS3Av3::IHal3A::E_Camera_3,
                getOpenId(),
                LOG_TAG);
        if  ( ! mpHal3a ) {
...
#endif // MTKCAM_ENABLE_CPU_CONTROL
    err = OK;
lbExit:
    profile.print("");
    return err;
...
flushAndWait(AppContext const& appContext)
{
    ...
    if  ( appContext.pPipeline != 0 ) {
        ...
        MY_LOGW_IF(OK!=err, "IPipelineModelMgr::flush err:%d(%s)", err, ::strerror(-err));
    }
    //
...
)
{
    ...
    //
    mAppContext.pPipeline = NULL;
    mAppContext.pPipeline = IPipelineModelMgr::create(getOpenId(), this);
    if ( mAppContext.pPipeline == 0 ) {
        MY_LOGE("IPipelineModel::create");
    }
    //
    {
        IPipelineModelMgr::ConfigurePipelineMgr::Params params;
        ...
        //
        err = mAppContext.pPipeline->configurePipeline(params);
        if  ( OK != err ) {
...
)
{
    ...
    IPipelineModelMgr* pPipeline         = appContext.pPipeline.get();
    ...
    //
    IPipelineModelMgr::AppRequest appRequestParams;
    {
...
#undef  _CLONE_
    ...
    pAppStreamManager->queryOldestRequestNumber(appRequestParams.oldestRequestNo);
    //
    ...
    //
    MY_LOGD_IF(getLogLevel() >= 2,"[end]request->frame_number: %d", request->frame_number);
    return OK;
}
...
//    MY_LOGD_IF(getLogLevel() >= 2, "frameNo:%u + userId:%#" PRIxPTR " OAppMeta#(left:%zd this:%zu)", frameNo, userId, nOutMetaLeft, vOutMeta.size());
    NSCam::Utils::CamProfile profile(__FUNCTION__, "Cam3DeviceImp");
    //
...
//    MY_LOGD_IF(1, "frameNo:%u - userId:%#" PRIxPTR " OAppMeta#(left:%zd this:%zu)", frameNo, userId, nOutMetaLeft, vOutMeta.size());
}
