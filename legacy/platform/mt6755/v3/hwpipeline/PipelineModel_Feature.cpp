...
#define LOG_TAG "MtkCam/HwPipelineF"
//
#include "MyUtils.h"
#include <mtkcam/v3/pipeline/DefaultPipelineModel/DefaultPipelineModel.h>
//
#include <mtkcam/v3/hwpipeline/PipelineModel_Feature.h>
 ...
 ******************************************************************************/
namespace {
class PipelineFeatureImp
    : public PipelineModel_Feature
...
public:     ////
    ...
    static MUINT32 const            mMaxNumNodes = 7;
...
protected:  ////                    Data Members.
    ...
    android::sp<IImageStreamInfo>   mpHalImage_NR3D_YUV;
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//  PipelineModel_Feature Interfaces.
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
public:     ////                    Operations.
...
private:    ////                    Operations.
    ...
    MERROR                          configNodeLocked_p2anode(
                                        PipeConfigParams const& rConfigParams
                                    );
    ...
    MERROR                          evaluateIOStreamsLocked_p2anode(
                                        evaluate_node_io_streams_params& rParams
                                    ) const;
 ...
 *
 ******************************************************************************/
PipelineModel_Feature*
PipelineModel_Feature::
create(MINT32 const openId)
{
    String8 const name = String8::format("%s:%d", magicName(), openId);
    PipelineModel_Feature* pPipelineModel = new PipelineFeatureImp(openId, name);
    if  ( ! pPipelineModel ) {
 ...
 ******************************************************************************/
PipelineFeatureImp::
PipelineFeatureImp(MINT32 const openId, android::String8 const& name)
...
~PipelineFeatureImp()
{
    MY_LOGD("Feature pipeline destroyed");
}
        ...
        if ( mpResourceMgr.get() ) {
            ...
            for ( size_t i=0; i<vNodeList.size(); i++ ) {
                ...
                if ( vNodeList[i] == it->id ) {
                    MBOOL bOwner = mpResourceMgr->isOwnerOfNode(it->id, this);
                    MY_LOGD("owner of node (id:%" PRIxPTR ") : %d", it->id, bOwner);
                    if  ( !bOwner ) {
...
#warning"[FIXME] Eveny node Id should belong to the global namespace..."
    ...
    mpResourceMgr->getReusableNodes(vNodeList);
    for ( size_t i=0; i<vNodeList.size(); i++ ) {
        MY_LOGD("reusable from resource mgr: Nodeid(%d)", vNodeList[i]);
    }
    //
    Vector<StreamId_T> vStreamList;
    mpResourceMgr->getReusableStreams(vStreamList);
    for ( size_t i=0; i<vStreamList.size(); i++ ) {
        MY_LOGD("reusable from resource mgr: Streamid(%d)", vStreamList[i]);
    }
    ...
    //
    if ( mConfigProfile.mbConfigP2a )
    {
        NodeId_T const nodeId = eNODEID_P2aNode;
        typedef P2aNode NodeT;
        //
        NodeT::InitParams params;
        params.openId = mOpenId;
        params.nodeId = nodeId;
        params.nodeName = "nodeP2a";
        //
        mpNode_P2a = NULL;
        sp<NodeT> pNode = mpNode_P2a = P2aNode::createInstance(P2aNode::PASS2_STREAM);
        pNode->init(params);
        //
        pPipelineDAG->addNode(nodeId);
        pPipelineNodeMap->add(nodeId, pNode);
    ...
    //david add
    if( mpNode_P1.get() && mpNode_P2a.get() )
        pPipelineDAG->addEdge(mpNode_P1->getNodeId(), mpNode_P2a->getNodeId());
    ...
    //david add
   if( mpNode_P2a.get() && mpNode_FD.get() )
        pPipelineDAG->addEdge(mpNode_P2a->getNodeId(), mpNode_FD->getNodeId());
    ...
    if ( !mConfigProfile.mbConfigP2VSS )
    {
        ...
        //david add
        if( mpNode_P2a.get() && mpNode_Jpeg.get() )
            pPipelineDAG->addEdge(mpNode_P2a->getNodeId(), mpNode_Jpeg->getNodeId());
    }
    else
    {
...
)
{
    ...
    {
        // MY_LOGD_IF( 1, "streamId:%#" PRIxPTR " %s %p type:%d MaxBufSize%d MinInitBufSize:%d",
        //             mpHalImage_P1_Raw->getStreamId(), mpHalImage_P1_Raw->getStreamName(), mpHalImage_P1_Raw.get(),
        //             mpHalImage_P1_Raw->getStreamType(), mpHalImage_P1_Raw->getMaxBufNum(), mpHalImage_P1_Raw->getMinInitBufNum() );
        ...
        if( mpHalImage_P1_ResizerRaw.get() )
            ...
            //pPoolRawResizer = mpResourceMgr->queryResizedStream(mpHalImage_P1_ResizerRaw->getStreamId(), mpHalImage_P1_ResizerRaw);
        ...
        //david create stream pool
        if( mpHalImage_NR3D_YUV.get() )
        {
            addHalStreamBufferPoolLocked(mpHalImage_NR3D_YUV);
            MY_LOGD("allocate mpHalImage_NR3D_YUV pools done");
        }
        else
        {
            MY_LOGD("didn't allocate mpHalImage_NR3D_YUV pools");
        }
    ...
    //
    if( mpNode_P2a.get() ) {
        err = configNodeLocked_p2anode(rConfigParams);
        if  ( OK != err ) {
            MY_LOGE("configNodeLocked_p2anode - err:%d(%s)", err, ::strerror(-err));
            return err;
        }
    }
    //
...
beginFlush()
{
    ...
    for (; it != rToposort.end(); it++)
    {
        ...
        // we must notify p2a node to free streams kept
        if ( eNODEID_P2aNode == pNode->getNodeId() ) {
            MERROR err = mpNode_P2a->freeAllStreams();
            MY_LOGD("mpNode_P2a->freeAllStreams done");
            if  ( OK != err ) {
                MY_LOGE("mpNode_P2a->freeAllStreams - err:%d(%s)", err, ::strerror(-err));
            }
        }
        pNode->flush();
lbSkipFlush:
...
waitDrained()
{
    ...
    MY_LOGD("wait p1 drain done %d", getOpenId());
    MERROR err = mpNode_P2a->freeAllStreams();
    MY_LOGD("mpNode_P2a->freeAllStreams done");
    if  ( OK != err ) {
        MY_LOGE("mpNode_P2a->freeAllStreams - err:%d(%s)", err, ::strerror(-err));
...
)   const
{
    ...
    switch  (nodeId)
    {
        ...
        case eNODEID_RAW16Out:
            ...
            break;
            //
        case eNODEID_P2aNode:
            err = evaluateIOStreamsLocked_p2anode(rParams);
            break;
            //
        default:
...
#warning "[FIXME] queryMinimalUsage"
    ...
    case eNODEID_P2Node_VSS:{
        ...
        if(
                ...
                isStream(mpHalImage_NR3D_YUV, streamId)
          )
        {
            //return (eBUFFER_USAGE_HW_CAMERA_WRITE);
            return (eBUFFER_USAGE_SW_READ_OFTEN | eBUFFER_USAGE_HW_CAMERA_WRITE);
...
)
{
    ...
    //david change p2 to p2a
    //mConfigProfile.mbConfigP2    = MTRUE;
    mConfigProfile.mbConfigP2a    = MTRUE;
...
)
{
    ...
    if ( mConfigProfile.mbConfigP2a )
    {
        ...
        //add eSTREAMID_IMAGE_NR3D
        MSize const& size = MSize(mConfigProfile.mResizedSize.w+30, mConfigProfile.mResizedSize.h+30);
        //MSize const& size = MSize(2752, 1536);
        MINT const format = eImgFmt_YV12;//eImgFmt_YV12;
        MUINT const usage = eBUFFER_USAGE_SW_READ_OFTEN |
            eBUFFER_USAGE_HW_CAMERA_READWRITE
            ;

        sp<ImageStreamInfo>
            pNr3dStreamInfo = createImageStreamInfo(
                "Hal:Image:NR3D",
        ...
        if( pNr3dStreamInfo == NULL ) {
            MY_LOGE("pNr3dStreamInfo address:%p", pNr3dStreamInfo.get());
            return BAD_VALUE;
        }
        //
        mpHalImage_NR3D_YUV = pNr3dStreamInfo;
        MY_LOGD("pNr3dStreamInfo address:%p, size W/H(%d,%d)",
            mpHalImage_NR3D_YUV.get(), size.w, size.h);
        pStreamInfoSet->editHalImage().addStream(pNr3dStreamInfo);
...
)
{
    ...
    ADD_IN_STREAM(App, Meta, rConfigParams.pMeta_Control);
    //add for iso
    ADD_IN_STREAM(App, Meta, mpAppMeta_DynamicP1);
    ADD_IN_STREAM(Hal, Meta, mpHalMeta_DynamicP1);
    //
    ADD_IN_STREAM(Hal, Image, mpHalImage_P1_ResizerRaw);
    ADD_IN_STREAM(Hal, Image, mpHalImage_P1_Raw);
    //
    ADD_OUT_STREAM(Hal, Image, mpHalImage_FD_YUV);
    ADD_OUT_STREAM(Hal, Image, mpHalImage_NR3D_YUV);
    ...
    // config node
    {
        P2aNode::ConfigParams cfgParams;
        //
        cfgParams.pInAppMeta    = rConfigParams.pMeta_Control;
        cfgParams.pInAppRetMeta = mpAppMeta_DynamicP1;
        cfgParams.pInHalMeta    = mpHalMeta_DynamicP1;
        cfgParams.pOutAppMeta   = mpAppMeta_DynamicP2;
        cfgParams.pOutHalMeta   = (!mConfigProfile.mbConfigP2VSS) ? mpHalMeta_DynamicP2 : NULL;
        //
        ...
        //david add 3dnr stream in config
        if( mpHalImage_NR3D_YUV.get() )
        {
            MY_LOGD("2222 mpHalImage_NR3D_YUV =%p", mpHalImage_NR3D_YUV.get());
            cfgParams.pOutNR3DImage = mpHalImage_NR3D_YUV;
            //david add 3ndr stream info
            cfgParams.vOutImage.push_back(mpHalImage_NR3D_YUV);
        }


        // capture flow also run P2aNode
        // config yuvs for jpeg if is not configured as recording
        if( ! mConfigProfile.mbConfigP2VSS ) {
            if( mpHalImage_Jpeg_YUV.get())
                cfgParams.vOutImage.push_back(mpHalImage_Jpeg_YUV);
            if( mpHalImage_Thumbnail_YUV.get() )
                cfgParams.vOutImage.push_back(mpHalImage_Thumbnail_YUV);
            MY_LOGD("vOutImage add YUV for jpg: %d ", cfgParams.vOutImage.size());
        }

        MERROR err = mpNode_P2a->config(cfgParams);
        if  ( OK != err ) {
            MY_LOGE("mpNode_P2a->config - err:%d(%s)", err, ::strerror(-err));
            return err;
        }
        MY_LOGD("vOutImage size =%d", cfgParams.vOutImage.size());
    }
...
) const
{
    ...
    for( size_t a = 0; a < rParams.vIHalImage.size(); a++) {
        MY_LOGD_IF(0, "[IMGO] [Hal] [In] streamId:%#" PRIxPTR ", Idx=%d",
            rParams.vIHalImage.keyAt(a), a);
    }
...
MERROR
PipelineFeatureImp::
evaluateIOStreamsLocked_p2anode(
    evaluate_node_io_streams_params& rParams
) const
{
    //
    ...
    rParams.vIAppMeta.add(mpAppMeta_Control->getStreamId()  , mpAppMeta_Control);
    //for iso
    rParams.vIAppMeta.add(mpAppMeta_DynamicP1->getStreamId(), mpAppMeta_DynamicP1);
    rParams.vIHalMeta.add(mpHalMeta_DynamicP1->getStreamId(), mpHalMeta_DynamicP1);
    rParams.vOAppMeta.add(mpAppMeta_DynamicP2->getStreamId(), mpAppMeta_DynamicP2);
    if ( ! mConfigProfile.mbConfigP2VSS )
        rParams.vOHalMeta.add(mpHalMeta_DynamicP2->getStreamId(), mpHalMeta_DynamicP2);
...
//  MY_LOGD("mvYuvStreams_Fullraw size=%d, mvYuvStreams_Resizedraw size=%d",
//          mvYuvStreams_Fullraw.size(), mvYuvStreams_Resizedraw.size());

    for( size_t a = 0; a < rParams.vOAppImage.size(); a++) {
        MY_LOGD_IF(0, "[IMGO] [App] streamId:%#" PRIxPTR ", Idx=%d",
            rParams.vOAppImage.keyAt(a), a);
    }
    for( size_t a = 0; a < rParams.vOHalImage.size(); a++) {
        MY_LOGD_IF(0, "[IMGO] [Hal] streamId:%#" PRIxPTR ", Idx=%d",
            rParams.vOAppImage.keyAt(a), a);
    }
    ...
    if( mvYuvStreams_Fullraw.size() > 0 )
    {
        ...
        for( size_t idx = 0; idx < mvYuvStreams_Fullraw.size(); idx++) {
            StreamId_T const streamId        = mvYuvStreams_Fullraw[idx];
            ssize_t index                    = -1;
//          MY_LOGD("mvYuvStreams_Fullraw: streamId:%#" PRIxPTR ", idx=%d",
//          streamId, idx);
            ...
            if ( mConfigProfile.mbConfigP2VSS )
            {
                ...
                {
                    MY_LOGD_IF(0, "mConfigProfile.mbConfigP2VSS");
                    continue;
                }
            }
            // app streams
            index = rParams.vOAppImage.indexOfKey(streamId);
            if  ( 0 <= index ) {
//              MY_LOGD("[IMGO] add app image to ImageInfoIOMap, streamId:%#" PRIxPTR ", AppImage Idx=%d",
//                  streamId, index);
                map.vOut.add(streamId, rParams.vOAppImage.valueAt(index));
            }
            // hal streams
            index = rParams.vOHalImage.indexOfKey(streamId);
            if  ( 0 <= index ) {
//              MY_LOGD("[IMGO] add hal image to ImageInfoIOMap, streamId:%#" PRIxPTR ", HalImage Idx=%d",
//                  streamId, index);
                map.vOut.add(streamId, rParams.vOHalImage.valueAt(index));
            }
        }
    ...
    if( mvYuvStreams_Resizedraw.size() > 0 )
    {
        ...
        for( size_t idx = 0; idx < mvYuvStreams_Resizedraw.size(); idx++) {
            StreamId_T const streamId        = mvYuvStreams_Resizedraw[idx];
            ssize_t index                    = -1;
            MY_LOGD_IF(0, "mvYuvStreams_Resizedraw: streamId:%#" PRIxPTR ", idx=%d",
            streamId, idx);
            //
            // skip jpeg related yuv streams if configured as vss flow
            if ( mConfigProfile.mbConfigP2VSS )
            ...
            index = rParams.vOAppImage.indexOfKey(streamId);
            if  ( 0 <= index ) {
//              MY_LOGD("[RRZO] add app image to ImageInfoIOMap, streamId:%#" PRIxPTR ", AppImage Idx=%d",
//                  streamId, index);
                map.vOut.add(streamId, rParams.vOAppImage.valueAt(index));
            }
            // hal streams
            index = rParams.vOHalImage.indexOfKey(streamId);
            if  ( 0 <= index ) {
//              MY_LOGD("[RRZO] add hal image to ImageInfoIOMap, streamId:%#" PRIxPTR ", HalImage Idx=%d",
//                  streamId, index);
                map.vOut.add(streamId, rParams.vOHalImage.valueAt(index));
            }
        }

        //david add 3dnr stream to InfoIOMapSet
        //todo: add mpHalImage_NR3D_YUV to mvYuvStreams_Resizedraw on sortYuvStreamsLocked called.
        //      and use above flow to add mpHalImage_NR3D_YUV to vOut
        map.vOut.add(mpHalImage_NR3D_YUV->getStreamId(), mpHalImage_NR3D_YUV);
        rParams.vOHalImage.add(mpHalImage_NR3D_YUV->getStreamId(), mpHalImage_NR3D_YUV);
...
//  MY_LOGD("mImageInfoIOMapSet size =%d",
//                  rParams.pIOMapSet->mImageInfoIOMapSet.size());
    //
    //InfoIOMapSet::MetaInfoIOMapSet
    {
        MetaInfoIOMap map;
        map.vIn.add(mpAppMeta_Control->getStreamId(), mpAppMeta_Control);
        //for iso
        map.vIn.add(mpAppMeta_DynamicP1->getStreamId(), mpAppMeta_DynamicP1);
        map.vIn.add(mpHalMeta_DynamicP1->getStreamId(), mpHalMeta_DynamicP1);
        map.vOut.add(mpAppMeta_DynamicP2->getStreamId(), mpAppMeta_DynamicP2);
        if ( ! mConfigProfile.mbConfigP2VSS )
            map.vOut.add(mpHalMeta_DynamicP2->getStreamId(), mpHalMeta_DynamicP2);
...
#undef align2
    MY_LOGD_IF(0, "thumb %dx%d, pic %dx%d -> yuv for thumb %dx%d",
