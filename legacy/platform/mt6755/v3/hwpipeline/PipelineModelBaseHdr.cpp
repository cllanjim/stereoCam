...
PipelineModelBaseHdr(MINT32 const openId, android::String8 const& name)
    : RefBase()
    , mRWLock()
    , mOpenId(openId)
    , mName(name)
 ...
 *
 ******************************************************************************/
PipelineModelBaseHdr::Context const&
PipelineModelBaseHdr::
 ...
 *
 ******************************************************************************/
sp<PipelineModelBaseHdr::HalImageStreamBufferPoolT>
PipelineModelBaseHdr::
...
MERROR
PipelineModelBaseHdr::
submitRequest(AppRequest& request, int SubRequestCnt)
{
    MERROR err = OK;
    //
    mSubRequestCnt = SubRequestCnt;
    ...
    if  ( OK != err ) {
       ...
       for ( int i=0; i<params.vFrame.size(); i++ ) {
            sp<IPipelineFrame> pFrame = params.vFrame[i];
            MY_LOGE_IF(pFrame==0, "RequestNo:%u-%d - NULL PipelineFrame", request.requestNo, i);
            MY_LOGD_IF(
                1,
                "RequestNo:%u-%d Image:I|O=%zu|%zu Meta:I|O=%zu|%zu",
                params.pAppRequest->requestNo, i,
            ...
            err = submitRequestFrame(pFrame);
            if ( OK != err )
            {
                MY_LOGE("submitRequestSubFrame err:%d(%s)", err, ::strerror(-err));
                return err;
            }
        }
