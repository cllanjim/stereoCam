...
*/
    ...
    // Do FD
    FDRet = mpFDHalObj->halFDDo(0, (MUINT8 *)mFDImgBuffer, pDstY,  mSDEnable, RotateInfo, (MUINT8 *)mDupImage.PAddrY);
    if(FDRet) {
...
)
{
    ...
    if(FDEn) {
        ...
        //MY_LOGD("old CropRegion: p.x:%d, p.y:%d, s.w:%d, s.h:%d, ",cropRegion.p.x, cropRegion.p.y, cropRegion.s.w, cropRegion.s.h);
        ...
        //MY_LOGD("new CropRegion: p.x:%d, p.y:%d, s.w:%d, s.h:%d, ",cropRegion.p.x, cropRegion.p.y, cropRegion.s.w, cropRegion.s.h);
