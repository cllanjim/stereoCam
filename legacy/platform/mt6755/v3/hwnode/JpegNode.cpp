    ...
    while( pEncodeFrame->mpJpeg_Main.get() == NULL ) {
        //[++]get out buffer and lock
        {
            ...
            if( err == NAME_NOT_FOUND ) {
                MY_LOGD("cannot find stream %#" PRIxPTR ", skip it",
                        stream_in
                        );
                break;
            } else if( err != OK ) {
...
)
{
    ...
    MY_LOGD("stream buffer: (%#" PRIxPTR ") %p, heap: %p, buffer: %p, usage: 0x%x",
...
{
    IStreamBufferSet&      rStreamBufferSet = pFrame->getStreamBufferSet();
    MY_LOGD("nodeID %#" PRIxPTR " streamID %#" PRIxPTR ,getNodeId(), streamId);
    ...
    rpMetadata = isInMetaStream(streamId) ?
        ...
        rpStreamBuffer->tryWriteLock(getNodeName());
    if( rpMetadata == NULL ) {
        MY_LOGE("[frame:%d node:%#" PRIxPTR "][stream buffer:%s] cannot get metadata",
...
)
{
    ...
    if( rpStreamBuffer.get() == NULL ) {
        MY_LOGE("StreamId %#" PRIxPTR ": rpStreamBuffer == NULL",
                streamId);
        return;
    }
    //
...
#define tryGetOrAllocBuf( vWorkingJpeg, rpBuffer, rSize )             \
    ...
    if( pBuf ) {
        MUINT const usage = eBUFFER_USAGE_SW_MASK |
            ...
            ;
        MY_LOGW("[FIXME] lock usage 0x%x", usage);
...
)
{
    ...
    //Yuv in -> Yuv Mid
    MY_LOGD("rParams.pSrc(%dx%d)",rParams.pSrc->getImgSize().w,rParams.pSrc->getImgSize().h);
...
)
{
    ...
    MY_LOGD("@@src (wxh)= (%dx%d) set defaults (wxh) = (%dx%d) stride = %zu\n",
    ...
    MY_LOGD("@@finish/destroy compress jpeg size = %ld, free size = %zu",
            jpegSize, cinfo.dest->free_in_buffer);

    if( jpegSize > rParams.pDst->getBufSizeInBytes(0) )
        MY_LOGE("Imagebuffer size = %zu not enough, bitstream size = %ld", rParams.pDst->getBufSizeInBytes(0), jpegSize);
