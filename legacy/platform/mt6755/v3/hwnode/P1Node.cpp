...
hardwareOps_deque(QBufInfo &deqBuf, MetaSet_T &result3A)
{
    ...
    {
        // deque buffer, and handle frame and metadata
        if (mvOutImage_full.size() > 0)/*(mpStreamPool_full!=NULL)*/ {
            NSCam::NSIoPipe::NSCamIOPipe::BufInfo rBufInfo;
