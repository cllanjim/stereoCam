...
#include <mtkcam/v3/hwnode/BufferReceiveNode.h>
//
...
public:
    android::String8 log() {
        return android::String8::format( "fd:%d addr:0x%x size:%d",
                                            shareFd, ionVA, buffer_size );
    }
};
      ...
      mOpened(false)
{
    ...
    if ( mLogLevel == 0 ) {
        ::property_get("debug.camera.log.BRNode", cLogLevel, "0");
        mLogLevel = atoi(cLogLevel);
    }
}
...
submitRequest(android::sp<BufferReceiveNode::IRequest> pRequest)
{
    ...
    CAM_TRACE_CALL();
    {
        ...
        MUINT8 af_trigger = MTK_CONTROL_AF_TRIGGER_IDLE;
        if( pRequest->setting ) {
            if ( !tryGetMetadata<MUINT8>(pSetting, MTK_CONTROL_CAPTURE_INTENT, capture_intent))
                MY_LOGD("cannot get MTK_CONTROL_CAPTURE_INTENT");
            if ( tryGetMetadata<MUINT8>(pSetting, MTK_CONTROL_AF_TRIGGER, af_trigger))
                MY_LOGD_IF(af_trigger != MTK_CONTROL_AF_TRIGGER_IDLE, "get MTK_CONTROL_AF_TRIGGER %d", af_trigger);
        }
        ...
        while (mRequestQueue.size() > (size_t)mRequestQueueDepth) {
            ...
            status_t status = mRequestQueueCond.waitRelative(mRequestQueueLock, ::s2ns(4));
            MY_LOGD_IF( 1, "wait-");
            if  ( OK != status ) {
                MY_LOGW(
...
#warning "[TODO] should remove blob condition may capture yuv in future"
        ...
        //Preview response: camera_meta_t + frame(from playback session)
            ...
            if ( af_trigger == MTK_CONTROL_AF_TRIGGER_IDLE ) {
                ...
                Request request;
                setImageInfo (request.info, 0, 0, 0);
                request.settings = pRequest->setting;
                request.isRepeating = false;
                mpExternal->submitRequest( &request );
            }
        ...
        //
        dequeTimestamp(pRequest->timestamp);
        //
        // only need jpeg buffer, queue to waiting queue
        if ( pRequest->bufferTypes.hasBit(BufferBit::BLOB) &&
             !pRequest->bufferTypes.hasBit(BufferBit::YUV)) {
...
MVOID
BufferReceiveNodeImp::
dequeTimestamp(MINT64& aTime)
{
    aTime = static_cast<MINT64>(::systemTime());
}
...
)
{
    ...
    if ( !mbRequestExit && !mbFlush && !mbUninit ) {
        ...
        if(result->buffers.imageInfo.format == eImgFmt_BLOB) {
            ...
            //
            if ( mNonStallBufferQueue.size() > 4 ) {
                MY_LOGW("Current buffer in camera:%d, need to return.", mNonStallBufferQueue.size());
                sp<RemoteBuffer> pBuf = *mNonStallBufferQueue.begin();
...
)
{
    ...
    // que timestamp
    IMetadata::IEntry entry = resultMeta.entryFor(MTK_SENSOR_TIMESTAMP);
...
{
    FUNCTION_IN;
    MY_LOGD("wifi %s", (pBuffer->pWifiIon->log()).string());
    //
...
)
{
        ...
        if  ( OK != status && ! mbRequestExit && !mbFlush ) {
            ...
            if ( pBuf != 0 ) {
                if ( pBuf->pBuf->lockBuf(
                    ...
                    MY_LOGW("use latest %s", (pBuf->pWifiIon->log()).string());
                    // copy meta
                    IMetadata  srcMeta;
                    cloneMetadata( pBuf->pMeta, srcMeta );
                    pBuf->pMeta = createMetaStreamBuffer(srcMeta, mpOutMeta_yuv);
                    return OK;
                } else {
                    MY_LOGE("use latest fail to lock buffer %s", (pBuf->pWifiIon->log()).string());
                    returnBuffer( mpLatestBuffer );
                    pBuf = NULL;
                    return DEAD_OBJECT;
                }
    ...
    if ( pBuf != 0 ) {
        if ( pBuf->pBuf->lockBuf(
            ...
            MY_LOGW("fail to lock buffer %s", (pBuf->pWifiIon->log()).string());
            returnBuffer( pBuf );
            return DEAD_OBJECT;
        }
    } else {
...
#warning "may return jpeg Tag twice."
        ...
        //
        MBOOL latest = false;
        if ( OK != dequeNonStallBuffer(pBuf, ::ms2ns(60), latest) && pBuf == 0) {
...
threadLoop()
{
    ...
    {
        MBOOL latest = false;
        if ( OK != mpNodeImp->dequeNonStallBuffer(pBuf, ::ms2ns(60), latest)) {
        ...
        if ( OK != mpNodeImp->dequeRequest(pRequest) || pRequest == 0) {
            ...
            MY_LOGW_IF(latest, "latest buffer no need to unmap here.");
            if ( pBuf != 0 && !latest) {
...
{
    IMetadata* srcMetadata = NULL;
    if ( OK == acquireMetadata(pStreamBuffer, srcMetadata) ) {
        rpMetadata = *srcMetadata;
        releaseMetadata(pStreamBuffer, srcMetadata);
    } else {
        MY_LOGE("acquire fail.");
        return BAD_VALUE;
    }
...
) const
{
    if( pStreamBuffer == NULL || pImageBuffer == NULL ) {
        MY_LOGE("pImageBuffer should not be NULL");
        return;
    }
    //
...
{
    FUNCTION_IN;
    MY_LOGI("request:%d %s", pRequest->requestNo, (pSrc->pWifiIon->log()).string());
    //
...
#warning "check requested size"
    ...
    if (pRequest->bufferTypes.hasBit(BufferBit::YUV)) {
        ...
        updateTimestamp(pBuf->pMeta, pRequest->timestamp);
        MY_LOGD("%d jpeg %lld ", pRequest->requestNo, pRequest->timestamp);
        result = pBuf->pMeta;
    }
...
openDevice()
{
    ...
    if ( !mOpened ) {
        ...
        //
        mOpened = MTRUE;
        mpExternal->openDevice(eImgFmt_I420, eImgFmt_BLOB);
    }
}
