...
#define DUMP_PATH                "/sdcard/p2node/"
#define DUMP_IMGI                (1)
#define DUMP_WDMAO               (2)
#define DUMP_WROTO               (4)
#define DUMP_IMG2O               (8)
    ...
    : public VirtualLightRefBase
{
    public:
                                            ...
                                            , mpFrame(pFrame)
                                        {
                                            ...
                                            MY_LOGD_IF(mbEnableLog, "frame %d +", mpFrame->getFrameNo());
                                        }
    ...
    : public VirtualLightRefBase
{
    ...
    protected:
                                        P2Processor(
                                            ...
                                            , muDequeCnt(0)
                                        {
                                            char cDumpValue[PROPERTY_VALUE_MAX];
                                            ::property_get("debug.camera.P2Node.p2dump", cDumpValue, "0");
                                            mDumpEnable = atoi(cDumpValue);
                                        }
    ...
    : public VirtualLightRefBase
{
    ...
    protected:
                                        ...
                                        {
                                            char cDumpValue[PROPERTY_VALUE_MAX];
                                            ::property_get("debug.camera.P2Node.mdpdump", cDumpValue, "0");
                                            mDumpEnable = atoi(cDumpValue);
                                        }
...
) const
{
    ...
    MY_LOGD_IF(mLogLevel >= 1, "stream %#" PRIxPTR ": buffer: %p, usage: 0x%x",
...
) const
{
    if( pStreamBuffer == NULL || pImageBuffer == NULL ) {
        MY_LOGE("pStreamBuffer %p, pImageBuffer %p should not be NULL", pStreamBuffer.get(), pImageBuffer.get());
        return;
    }
    //
...
) const
{
    ...
    if( pStreamBuffer.get() == NULL ) {
        MY_LOGE("StreamId %#" PRIxPTR ": pStreamBuffer == NULL",
                streamId);
        return;
    }
    //
...
)
{
    ...
    for( size_t run_idx = 0 ; run_idx < imageIOMapSet.size(); run_idx++ )
    {
        ...
        //
        MY_LOGD("frame %d: p2 out %zu, copy out %zu",
                pFrame->getFrameNo(), param_p2.vOut.size(), param_mdp.vOut.size() );
        // create job
...
#define PORT_IMG2O_USED  (0x4)
    ...
    MY_LOGD_IF(0, "stream id %#" PRIxPTR ", occupied 0x%x",
            streamId, rOccupied);
    return ret;
...
waitIdle()
{
    ...
    while( mvRunning.size() ) {
        MY_LOGD("wait job done %zu", mvRunning.size());
        mCondJob.wait(mLock);
    }
    //
    return OK;
...
)
{
...

    {
        ...
        while( iter != enqueParams.mvOut.end() ) {
            ...
            if( iter->mPortID == PORT_WDMAO ) {
                ...
                MY_LOGE("not supported port 0x%x", iter->mPortID.index);
                return BAD_VALUE;
            }
    ...
    //
    MY_LOGD_IF(mbEnableLog || mDumpEnable, "cnt %d, in %zu, out %zu",
...
handleDeque(QParams& rParams)
{
    ...
    if( mDumpEnable )
    {
        ...
        for( size_t i = 0; i < rParams.mvIn.size(); i++ )
        {
            const Input& in = rParams.mvIn[i];
            bool dump = false;
            String8 portName;
            if( in.mPortID.index == EPipePortIndex_IMGI ) {
                dump = (mDumpEnable & DUMP_IMGI);
                portName = "IMGI";
            }
            if( dump ) {
                IImageBuffer* pImgBuf = in.mBuffer;
                if( pImgBuf ) {
                    String8 filename = String8::format("%s%s_%dx%d_%u_%u.raw",
                            DUMP_PATH,
                            portName.string(),
                            pImgBuf->getImgSize().w, pImgBuf->getImgSize().w,
                            pImgBuf->getBufStridesInBytes(0),
                            muDequeCnt);
                    pImgBuf->saveToFile(filename.string());
                }
            }
        }
        for( size_t i = 0; i < rParams.mvOut.size(); i++ )
        {
            const Output& out = rParams.mvOut[i];
            bool dump = false;
            String8 portName;
            switch( out.mPortID.index )
            {
#define addCase(port, flag, strname)                    \
                case EPipePortIndex_##port:             \
                    flag = (mDumpEnable & DUMP_##port); \
                    strname = #port;                    \
                    break;
                addCase(WDMAO, dump, portName)
                addCase(WROTO, dump, portName)
                addCase(IMG2O, dump, portName)
#undef addCase
                default:
                    dump = false;
            }
            if( dump ) {
                IImageBuffer* pImgBuf = out.mBuffer;
                if( pImgBuf ) {
                    String8 log = String8::format("dump port %s, size %dx%d, fmt %d, stride ",
                            portName.string(),
                            pImgBuf->getImgSize().w, pImgBuf->getImgSize().w, pImgBuf->getImgFormat()
                            );
                    for( size_t i = 0; i < pImgBuf->getPlaneCount(); i++ )
                        log += String8::format("%zu,", pImgBuf->getBufStridesInBytes(i));
                    MY_LOGD("%s", log.string());

                    String8 filename = String8::format("%s%s_%dx%d_%u.yuv",
                            DUMP_PATH,
                            portName.string(), pImgBuf->getImgSize().w, pImgBuf->getImgSize().w,
                            muDequeCnt);
                    pImgBuf->saveToFile(filename.string());
                }
            }
    ...
    {
        Mutex::Autolock _l(mLock);
        MY_LOGD_IF(mbEnableLog || mDumpEnable, "p2 done %d, success %d", muDequeCnt, rParams.mDequeSuccess);
...
)
{
    ...
    if( mDumpEnable )
    {
        ...
        {
            static
            MVOID dumpImg(String8 portName, IImageBuffer* pImgBuf, MUINT32 cnt)
            {
                String8 log = String8::format("dump port %s, size %dx%d, fmt %d, stride ",
                        portName.string(),
                        pImgBuf->getImgSize().w, pImgBuf->getImgSize().w, pImgBuf->getImgFormat()
        ...
        mkdir(DUMP_PATH, S_IRWXU | S_IRWXG | S_IRWXO);
        //
        debug::dumpImg(String8("src"), pSrc, muCnt);
        for( size_t i = 0; i < vDst.size(); i++ )
        {
            debug::dumpImg(String8::format("dst_%zu", i), vDst[i], muCnt);
        }
    }
    //
    muCnt++;
