...
{
    MY_LOGD("frameNo:%d nodeId:%#" PRIxPTR " +", pFrame->getFrameNo(), getNodeId());
    //
    ...
    //  Image
    {
        ...
        for (size_t i = 0; i < rMapSet.size(); i++)
        {
            ...
            for (size_t j = 0; j < rMapSet[i].vIn.size(); j++)
            {
                ...
                MY_LOGE_IF(OK != err, "[frame:%d node:%#" PRIxPTR "][stream buffer:%s] fail to wait acquire_fence:%d[%s] err:%d[%s]", pFrame->getFrameNo(), getNodeId(), pStreamBuffer->getName(), acquire_fence->getFd(), acquire_fence->name(), err, ::strerror(-err));
                //
    ...
    ////////////////////////////////////////////////////////////////////////////
    MY_LOGD("[applyRelease] frameNo:%d nodeId:%#" PRIxPTR " +", pFrame->getFrameNo(), getNodeId());
