...
)
{
    ...
    const char * sMain2FSFilename = "/sdcard/vsdof/BID_P2AFM_IN_FSRAW2_4208x3120_5260.raw";
...
sp<IDepthMapEffectRequest> generateEffectRequest(int request_id, DepthMapPipeOpState eState)
{
    ...
    pEffectReq->pushRequestImageBuffer({BID_P2AFM_IN_RSRAW1, eBUFFER_IOTYPE_INPUT}, buf_rs_Main1);

    // prepare input frame info: RSRAW2
    pEffectReq->pushRequestImageBuffer({BID_P2AFM_IN_RSRAW2, eBUFFER_IOTYPE_INPUT}, buf_rs_Main2);

    // prepare input frame info: FSRAW1
    pEffectReq->pushRequestImageBuffer({BID_P2AFM_IN_FSRAW1, eBUFFER_IOTYPE_INPUT}, buf_fs_Main1);

    // prepare input frame info: FSRAW2
    pEffectReq->pushRequestImageBuffer({BID_P2AFM_IN_FSRAW2, eBUFFER_IOTYPE_INPUT}, buf_fs_Main2);
    ...
    // prepare output frame: Warping matrix
    ImgParam imgParam_WarpingMatrix = getImgParam_WARPING_MATRIX();

    if(eState == eSTATE_CAPTURE)
    {
        buf = createEmptyImageBuffer(imgParam_WarpingMatrix, "WarpingMatrix",  eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN | eBUFFER_USAGE_SW_WRITE_RARELY);
        pEffectReq->pushRequestImageBuffer({BID_N3D_OUT_WARPING_MATRIX, eBUFFER_IOTYPE_OUTPUT}, buf);
    }

    // prepare output frame: Disparity Left
    ImgParam imgParam_DMP = getImgParam_DMP();
    buf = createEmptyImageBuffer(imgParam_DMP, "imgParam_DMP_L", eBUFFER_USAGE_HW_CAMERA_READWRITE|eBUFFER_USAGE_SW_READ_OFTEN|eBUFFER_USAGE_SW_WRITE_RARELY);
    pEffectReq->pushRequestImageBuffer({BID_DPE_OUT_DMP_L, eBUFFER_IOTYPE_OUTPUT},buf);

    // prepare output frame: Disparity Right
    buf = createEmptyImageBuffer(imgParam_DMP, "imgParam_DMP_R", eBUFFER_USAGE_HW_CAMERA_READWRITE|eBUFFER_USAGE_SW_READ_OFTEN|eBUFFER_USAGE_SW_WRITE_RARELY);
    pEffectReq->pushRequestImageBuffer({BID_DPE_OUT_DMP_R, eBUFFER_IOTYPE_OUTPUT},buf);
...
MVOID setupDumpProperties()
{
    ...
    SET_ID2NAME_PROB(name, DPE_OUT_DISPARITY);
    // OCC Node Section
    name = "OCCNode";
    property_set(name, "1");
...
TEST(DepthMapEffectHalTest, Standard)
{
    ...
    effParam->set(EFFECTKEY_SENSOR_STATE, (int)eSENSOR_BAYER_MONO);
    ...
    {
        sp<IDepthMapEffectRequest> pEffReq = generateEffectRequest(i, eSTATE_CAPTURE);
