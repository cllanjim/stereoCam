...
MINT32 LMVHalImp::mEISInterval = GyroInterval_ms;
...
*
********************************************************************************/
void LMVSensorListener(ASensorEvent event)
{
    ...
    switch(event.type)
    {
        ...
        {
            if (UNLIKELY(LMVHalImp::mDebugDump >= 2))
            {
                LMV_LOG("[%u] Acc(%f,%f,%f,%lld)",
                        accCnt++,
        ...
        default:
        {
            LMV_WRN("unknown type(%d)",event.type);
            break;
        }
    }
...
MINT32 LMVHalImp::Init()
{
    ...
    if (MTRUE != mpSensorListener->setListener(LMVSensorListener))
    {
        LMV_ERR("setListener fail");
    }
    else
    {
        LMV_LOG("setListener success");
    }
    DP_TRACE_END();
...
MINT32 LMVHalImp::Uninit()
{
    ...
    if(mUsers == 0)    // there is no user
    {
        ...
        {
            LMV_LOG("mpSensorListener uninit");
    ...
    if (mIsEisConfig == 0)
    {
        ...
        if (UNLIKELY(mDebugDump >= 2))
        {
            ...
            else
            {
                ...
                LMV_LOG("mEisDbgBuf : size(%u),virAdd(0x%p)", eisMemSize, m_pEisDbgBuf->getBufVA(0));
        ...
        LMV_LOG("EN:(Acc,Gyro)=(%d,%d)",mAccEnable,mGyroEnable);
...
MINT32 LMVHalImp::DoLMVCalc(const EIS_PASS_ENUM &aEisPass, QBufInfo const &pBufInfo)
{
    ...
    if (UNLIKELY(mDebugDump >= 1))
    {
        LMV_LOG("aEisPass(%d),mEisPass1Enabled(%u)",aEisPass,mEisPass1Enabled);
    }

    if (aTimeStamp <= 0)
    {
        LMV_LOG("DoP1Eis aTimeStamp is not reasonable(%llu)",aTimeStamp);
    }
    else
    {
#if EIS_ALGO_READY
        ...
        LMV_LOG("EisIn(%u,%u),P1T(%u,%u)",mEisInput_W,mEisInput_H,mP1Target_W,mP1Target_H);
        ...
        if (err != S_EIS_OK)
        {
            LMV_ERR("EisAlg:EisMain fail(0x%x)",err);
            err = LMV_RETURN_API_FAIL;
            return err;
        }
...
*
********************************************************************************/
MVOID LMVHalImp::GetGmv(MINT32 &aGMV_X, MINT32 &aGMV_Y, MUINT32 *confX, MUINT32 *confY)
{
    if(mEisSupport == MFALSE)
    {
        LMV_LOG("mSensorIdx(%u) not support EIS",mSensorIdx);
...
MINT32 LMVHalImp::GetBufLMV(android::sp<IImageBuffer>& spBuf)
{
    ...
    if (!mEISOBufferList.empty())
    {
        ...
        if (UNLIKELY(mDebugDump >= 1))
        {
            LMV_LOG("GetBufEis : %lu",mEISOBufferList.size());
        }
    }else
    {
...
MSize LMVHalImp::QueryMinSize(MBOOL isEISOn, MSize sensorSize, NSCam::MSize outputSize, MSize requestSize)
{
    MSize retSize;
    MINT32 out_width;
    MINT32 out_height;
    ...
    }else
    {
        ...
        }else
        {
            ...
            mEisPlusCropRatio = eis_factor; //4Kx2K only 20% margin
            out_width  = (requestSize.w <= (VR_UHD_W*eis_factor/100.0f))?
                         (VR_UHD_W*eis_factor/100.0f) : requestSize.w;
            out_height = (requestSize.h <= (VR_UHD_H*eis_factor/100.0f))?
                         (VR_UHD_H*eis_factor/100.0f) : requestSize.h;
            out_width = (out_width <= sensorSize.w)? out_width : sensorSize.w;
            out_height = (out_height <= sensorSize.h)? out_height : sensorSize.h;
            if (((out_width>>4)*9) < out_height)
            {
                //Align video view angle
                out_height = (out_width>>4)*9;
            }
        }

        if (UNLIKELY(mDebugDump >= 1))
        {
            LMV_LOG("sensor: %d/%d, videoSize: %d/%d, ret: %d/%d, crop %d", sensorSize.w, sensorSize.h, mVideoW, mVideoH , out_width, out_height, mEisPlusCropRatio);
        }
    }
...
MINT32 LMVHalImp::NotifyLMV(QBufInfo&   pBufInfo)
{
    ...
    for (size_t i = 0; i < pBufInfo.mvOut.size(); i++)
    {
        if (pBufInfo.mvOut[i].mPortID.index == PORT_EISO.index)
        {
            ...
            if (UNLIKELY(mDebugDump >= 1))
            {
                LMV_LOG("NotifyLMV : %lu",mEISOBufferList.size());
            }
        }
    }
...
MINT32 LMVHalImp::NotifyLMV(android::sp<NSCam::IImageBuffer>& spBuf)
{
    ...
    if (spBuf != 0)
    {
        ...
        if (UNLIKELY(mDebugDump >= 1))
        {
            LMV_LOG("NotifyLMV : %lu - Drop",mEISOBufferList.size());
        }
    }
