...
#include <pipeline/hwnode/hwnode_utilities.h>
        ...
        HDROutputType type, sp<IImageBuffer>& outBuffer)
{
    ...
    {
        HDR_LOGE("[addOutputFrame] only supports JPEG_YUV and " \
                "JPEG_THUMBNAIL_YUV");
        return MFALSE;
    }
    ...
    if (outBuffer == NULL)
    {
        HDR_LOGE("[registerOutputImg] outBuffer is NULL");
        return MFALSE;
    }
